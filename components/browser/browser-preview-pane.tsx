...
export function BrowserPreviewPane({ sessionId }: { sessionId?: string }) {
  ...
  }, [selection, selections, comment, sessionId, getRect, sendComment, clearSelection, t, detailLevel])
          ...
          <div className="space-y-1">
            {annotationQueue.map((annotation, index) => (
              <div key={annotation.id} className="flex items-center gap-2 text-xs">
                ...
                <Badge variant="outline" className="text-[10px]">
                  {t(`annotation.intent.${annotation.intent}`)} · {t(`annotation.severity.${annotation.severity}`)}
                </Badge>
                <TooltipIconButton
