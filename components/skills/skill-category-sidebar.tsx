...
/**
 * Left rail under the My Skills tab — selectable buckets:
 *   - All
 *   - Per-source (built-in / custom / imported / generated / marketplace)
 *   - Per-category (8 entries)
 *
 ...
 * predictably.
 */
export function SkillCategorySidebar({ total, countsByCategory, countsBySource }: Props) {
  const t = useTranslations("skills")
  const { filters, setFilters } = useSkillsStore((s) => ({
    filters: s.filters,
  ...
  return (
    <aside className="w-52 shrink-0 border-r bg-muted/20 px-2 py-3 text-sm">
      ...
      <button
        type="button"
        onClick={() => setFilters({ category: "all", source: "all" })}
        className={cn(
      ...
      {SKILL_SOURCES.map((src) => (
        <button
          key={src.id}
          type="button"
          onClick={() => setFilters({ source: src.id as SkillSource, category: "all" })}
          className={cn(
            "flex w-full items-center justify-between rounded-md px-2 py-1.5 text-left text-xs hover:bg-accent",
      ...
      {SKILL_CATEGORIES.map((cat) => {
        ...
        return (
          <button
            key={cat.id}
            type="button"
            onClick={() => setFilters({ category: cat.id as SkillCategory, source: "all" })}
            className={cn(
              "flex w-full items-center justify-between rounded-md px-2 py-1.5 text-left text-xs hover:bg-accent",
