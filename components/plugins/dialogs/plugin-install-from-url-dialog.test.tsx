...
describe("PluginInstallFromUrlDialog", () => {
  ...
  // A long manifest-validation error under a phone keyboard could push Fetch
  // off screen. The content is capped at the dynamic viewport and the form is
  // the one scroller; the error text wraps instead of overflowing sideways.
  it("bounds DialogContent to the viewport with one scroll body", async () => {
    global.fetch = jest
      .fn()
      .mockRejectedValue(
        new Error("offline: https://example.com/a/very/long/unbroken/manifest/path.json")
      ) as unknown as typeof fetch
    render(<PluginInstallFromUrlDialog open={true} onOpenChange={jest.fn()} />)
    const dialog = screen.getByRole("dialog")
    expect(dialog).toHaveClass("flex", "flex-col", "max-h-[85dvh]")
    const body = screen.getByTestId("plugin-install-from-url-body")
    expect(body).toHaveClass("min-h-0", "flex-1", "overflow-y-auto")
    expect(body).toContainElement(screen.getByLabelText("label"))
    ...
    expect(alert.querySelector("span")).toHaveClass("break-words", "min-w-0")
  })
})
