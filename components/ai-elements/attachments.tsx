...
/**
 * Vendored from the AI Elements source (https://elements.ai-sdk.dev/components/attachments)
 * into ai-elements/ — coverage- & i18n-excluded per CLAUDE.md. Purely
 * presentational attachment primitives (grid / inline / list variants) fed by
 * `usePromptInputAttachments()`. The cognia composer's richer
 * `components/chat/composer/attachment-preview.tsx` builds on these.
 *
 * Only change from upstream: `@repo/shadcn-ui/*` import paths rewritten to the
 * cognia `@/components/ui/*` + `@/lib/utils` aliases. Tokens are already
 * semantic (bg-muted / text-muted-foreground / border-border / bg-accent).
 */
