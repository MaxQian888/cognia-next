...
function GrantSection({
  ...
  return (
    <section className="rounded-md border p-3" data-testid={`grant-${grant.id}`}>
...
export function AccessTab({ row, actions }: { row: DeviceRow; actions: DeviceGrantActions }) {
  const t = useTranslations("devices")

  if (row.kind !== "paired-device") {
    return (
      <Alert data-testid="device-access-tab">
  ...
  return (
    <div className="space-y-3" data-testid="device-access-tab">
      {revoked ? (
      ...
      <section className="rounded-md border p-3">
        <h3 className="text-sm font-medium">{t("access.lifecycle")}</h3>
