...
describe("AccessTab — grants", () => {
  ...
  it("shows a partial grant as partial, with the missing capabilities struck through", () => {
    ...
    render(<AccessTab row={row({ grants })} actions={actions()} />)
  ...
  it("routes each switch to its own action", async () => {
    ...
    render(<AccessTab row={row()} actions={handlers} />)
...
describe("AccessTab — Locked Use dormancy", () => {
  /**
...
describe("AccessTab — lifecycle", () => {
  it("offers pause for an active device and resume for a paused one", async () => {
    const handlers = actions()
    const { rerender } = render(<AccessTab row={row()} actions={handlers} />)
    ...
    rerender(<AccessTab row={row({ adminState: "paused" })} actions={handlers} />)
  ...
  it("disables every control on a revoked device and says why", () => {
    ...
    render(<AccessTab row={row({ adminState: "revoked", grants })} actions={actions()} />)
...
describe("AccessTab — devices with no grants", () => {
  it.each(["local", "remote-host", "worker"] as const)("explains why %s has none", (kind) => {
    render(<AccessTab row={row({ kind })} actions={actions()} />)
