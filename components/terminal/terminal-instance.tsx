...
function TerminalInstanceImpl(
  ...
  // Surface prompt boundary decorations from the store (Wave 3C). The store
  // is populated by spawn-orchestrator on OSC 633 A/B events. We render a
  // thin horizontal rule via `term.registerDecoration` when available;
  // unsupported xterm builds silently no-op.
  const promptBoundaries = useTerminalStore((s) => s.sessions[sessionId]?.promptBoundaries)
  useEffect(() => {
    const term = termRef.current
    if (!term || typeof term.registerDecoration !== "function") return
    if (!promptBoundaries || promptBoundaries.length === 0) return
    // For simplicity we only mark the LAST boundary — older marks
    // accumulate quickly and xterm decorations are GC'd with the line.
    const last = promptBoundaries[promptBoundaries.length - 1]
    if (last.endMs == null) return
    try {
      const marker = term.registerMarker?.()
      if (marker) {
        term.registerDecoration({ marker, width: 1, x: 0 })
      }
    } catch {
      /* noop — older xterm builds lack decoration API */
    }
  }, [promptBoundaries])
