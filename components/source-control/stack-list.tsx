...
/**
 * The stacks of the bound repository, as a list.
 *
 * Split out of `stack-panel.tsx` so the same list can be a Sheet (from the
 * toolbar) and a section of the repository navigator, without a second copy
 * of the validation, restack and publish wiring existing to drift from this
 * one. `StackPanel` is now a Sheet around this component.
 *
 * A stack already renders as a vertical parent-to-child chain, which is the
 * shape the navigator wants, so nothing about the visual needed inventing.
 */
...
export interface StackListProps {
  /**
   * Whether this list is the one on screen. Drives the reads, so a mounted
   * but hidden list costs nothing.
   */
  active: boolean
  rootDir: string
  ...
  /** Applied to the scroll container, so a host can size it. */
  className?: string
}
...
export function StackList({
  ...
  const t = useTranslations("sourceControl.stacks")
  const { rows, loading, busy, restack, setParent, undo } = useStacks(active ? rootDir : null, deps)
  const forge = useStackForge(active ? rootDir : null, forgeDeps)
  ...
  return (
    <ScrollArea className={cn("px-4 pb-4", className)}>
