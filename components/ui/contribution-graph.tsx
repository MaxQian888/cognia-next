...
/**
 * Vendored primitive — Kibo UI `contribution-graph` (MIT).
 * Installed with `pnpm dlx shadcn@latest add https://www.kibo-ui.com/r/contribution-graph.json`
 * and relocated from `components/kibo-ui/contribution-graph/index.tsx` into the
 * shadcn `ui` alias so it sits with the other vendored primitives (excluded from
 * the co-located-test and i18n gates). Only the `cn` import path was changed.
 *
 * A GitHub-style calendar heatmap: `ContributionGraph` computes the week grid
 * and shares it by context; `ContributionGraphCalendar` renders the SVG and
 * yields each cell through a render prop, so callers own the block markup
 * (tooltips, aria labels, palette).
 */
