...
describe("BalanceCard", () => {
  it("renders the empty state before any fetch", () => {
    setup()
    render(<BalanceCard provider="codex" accountId="acc-1" queryEnabled label="DeepSeek" />)
  ...
  it("renders total and used when present", () => {
    ...
    render(<BalanceCard provider="codex" accountId="acc-1" queryEnabled />)
  ...
  it("disables refresh before quota queries are enabled", () => {
    setup()
    render(<BalanceCard provider="codex" accountId="acc-1" queryEnabled={false} />)
    expect(screen.getByTestId("balance-refresh-acc-1")).toBeDisabled()
  })
})
