...
jest.mock("@/components/ui/select", () => ({
  Select: ({
    ...
    ReactForMock.createElement(
      "select",
      {
        "data-testid": "usage-display-mode-select",
        value,
...
describe("UsageDisplayCard", () => {
  it("renders the three modes and reflects the current value", () => {
    const { getByTestId } = render(<UsageDisplayCard />)
    const select = getByTestId("usage-display-mode-select") as HTMLSelectElement
  ...
  it("persists a new mode via setMode", () => {
    const { getByTestId } = render(<UsageDisplayCard />)
    fireEvent.change(getByTestId("usage-display-mode-select"), { target: { value: "detailed" } })
    expect(setMode).toHaveBeenCalledWith("detailed")
  })

  it("shows the per-mode hint", () => {
    currentMode = "simplified"
    const { container } = render(<UsageDisplayCard />)
