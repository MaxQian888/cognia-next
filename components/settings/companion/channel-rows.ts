...
export function buildChannelRows(input: BuildChannelRowsInput): ChannelRow[] {
  ...
  //
  // The switch is checked before the server, not after: `off` and `blocked`
  // answer different questions, and a channel the user turned off is `off` no
  // matter what else is wrong upstream. Testing `server.running` first reported
  // `blocked/serverStopped` for a switched-off channel, which sends the reader
  // to fix the server — and starting it would still not make mDNS advertise.
  const mdns: ChannelRow = !mdnsOn
    ? { id: "mdns", state: "off" }
    : server.running
      ? { id: "mdns", state: "live" }
