...
describe("WorkflowPublishSection", () => {
  it("publishes and shows the resulting tool name", async () => {
    publishWorkflow.mockResolvedValue({ toolName: "wf_demo", created: true, skillId: "s1" })
    wrap(<WorkflowPublishSection workflowId="wf1" />)
  ...
  it("shows the published state up front and can unpublish", async () => {
    ...
    wrap(<WorkflowPublishSection workflowId="wf1" published={{ at: 1, toolName: "wf_demo" }} />)
  ...
  it("re-publishes from the published state", async () => {
    publishWorkflow.mockResolvedValue({ toolName: "wf_demo2", created: false, skillId: "s1" })
    wrap(<WorkflowPublishSection workflowId="wf1" published={{ at: 1, toolName: "wf_demo" }} />)
