...
// Localization bundles. The workbench reads these BEFORE activating the
// extension to substitute the manifest's `%key%` placeholders, so they have to
// travel inside the package — leaving them out silently ships the raw keys to
// every non-English user. Generated by `scripts/i18n/build-vscode-nls.mjs`
// (part of `pnpm i18n:build`), and a missing one is fatal rather than skipped:
// the default bundle is what every unmatched locale falls back to.
...
for (const name of nlsFiles) {
  ...
  cpSync(source, join(stage, "extension", name))
}
