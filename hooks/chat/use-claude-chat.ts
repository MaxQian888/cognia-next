...
export function useClaudeChat() {
  ...
  const send = useCallback(
    async (content: SendContent, opts?: SendOptions) => {
      ...
      let sendOptions = opts ?? (await buildSendOptions(session))
...
async function buildSendOptions(session: ChatSession | null | undefined): Promise<SendOptions> {
  ...
  return resolveSendOptions({ session, appSettings, referencedPaths })
}
