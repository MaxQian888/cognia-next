...
| Scheduled task scheduler   | `lib/scheduler/` (independent `SchedulerDatabase`, not `lib/db/schema.ts`; host gates in `host-support.ts`, managed-schedule target in `scheduler-host-target.ts`), `types/scheduler/`, `components/scheduler/`, `app/scheduler/`, `crates/cognia-scheduling/`, backup legs `lib/data/destinations/`                                            | SchedulerDB v2                                 | 0002, 0079, 0128       |
...
| 统一全局搜索 (⌘K)          | `lib/global-search/`（types / query-parser / registry / scoring / cache / recents / engine + `providers/*` 18 个内置 provider）、`hooks/global-search/`（context 装配、搜索 hook、动作执行）、`components/global-search/`（`GlobalSearchDialog` + tabs / chips / row / footer / empty-state）；薄适配器 `components/desktop/command-palette.tsx`、`components/mobile/home/mobile-command-palette.tsx`、`components/mobile/shell/mobile-global-search-host.tsx`；打开接缝 `lib/shell/command-palette-request.ts`（`requestCommandPalette({query, scope})`）；快捷键 `app.commandPalette.toggle`（`lib/shortcuts/app-catalog.ts`）；消息引擎沿用 ADR-0099 `lib/chat/search/` | 无（localStorage recents）                    | 0129, 0099             |
...
- **Excluded from coverage**: `components/ui/` (shadcn), `components/ai-elements/` (vendored), and `web/components/ui/` (the marketing workspace's own shadcn copy) — don't add tests there. `web/app/` and `web/content/` are also uncollected: thin route glue, and data whose en/zh parity `tsc` already proves.
