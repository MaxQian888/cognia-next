...
| **Integrated terminal** — xterm.js dock + Rust `portable-pty` backend + OSC 633; VS Code-style flat split panes per tab (additive store layer, anchor groups); command markers + exit-code gutter + jump-to-command; webview-reload session restore (`SeqEvent` Channel + swappable `ChannelSlot` + `terminal_reattach` + `rehydrate`, mirrors `WsTerminalRegistry` consumer-swap); clickable `path:line:col` → read-only Monaco viewer; agent-tab → chat `setActiveSession`; 4 `terminal_dock_*` MCP tools; mobile WS transport + `seq` replay buffer                       | `lib/terminal/`, `components/terminal/`, `stores/terminal/`, `src-tauri/src/terminal/`, `src-tauri/src/companion_api/ws_terminal.rs`                                                                                                                                                                                                       | `cognia-terminal-layout` (persist)                                                                      | 0031, 0033                      |
| **Rust performance dashboard** — Task-Manager-style `/performance` panel; ref-counted ~1 Hz sampler (`perf://sample`) composing process-tree (`sysinfo`) + Tokio `RuntimeMetrics` busy-% + an opt-in span hotspot registry (`perf::guard`/`timed`/`record`, log-bucket p50/p95); no in-process profiler (surfaces the dial9 trace + span table); desktop-only                                                                                                                                                                                                                | `src-tauri/src/perf/`, `lib/perf/backend/`, `hooks/perf/use-perf-stream.ts`, `components/performance/`, `app/performance/`                                                                                                                                                                                                                 | —                                                                                                       | 0035                            |
