...
| **Platform Connectors** — Telegram/Discord/Slack/Lark/OneBot; ConnectorBus + outbound runner (quiet-hours, circuit breaker, idempotency); A2UI ⇄ IM bridge (capability-aware downgrade, inbound callback channel, computer-use isolation)                                                      | `lib/connectors/`, `lib/a2ui/connector-callback-handler.ts`, `app/inbox/`, `components/inbox/`, `src-tauri/src/connectors/`                                                                                                                                                                                                                | v38                                                                                                     | 0009, 0025                      |
...
| **Mobile sync orchestrator** (Wave 4) — 9-table Dexie-first read cache; persistent cursors; network/resume/visibility triggers; `useDexieFirstQuery` hook; `ConnectionStateBadge` dropdown (reconnect/scan/switch); vendored Capacitor mDNS (`capacitor-zeroconf`); Serwist PWA on web+Tauri   | `lib/sync/companion-sync.ts`, `lib/sync/cursor-store.ts`, `lib/sync/handlers/`, `hooks/data/use-dexie-first-query.ts`, `lib/connectivity/{mdns-discovery,mdns-permission}.ts`, `lib/capacitor/{app,app-settings}.ts`, `components/mobile/connection-state-badge.tsx`, `components/mobile/connection-state-sheets/`, `app/{sw,manifest}.ts` | v44                                                                                                     | 0027                            |

### Cross-cutting hooks (reuse, don't reinvent)

- **PII redaction**: `lib/twin/ingest/redact.ts:hasNoLeakingPii` is the gate before any LLM/embed call. Twin + Goal share the encrypted master key (`lib/twin/ingest/redaction-key.ts`). Connector auto-mode wraps `sendPrompt` via `lib/connectors/ai-loop/safe-send-prompt.ts` so the same gate fires before every IM-driven turn.
