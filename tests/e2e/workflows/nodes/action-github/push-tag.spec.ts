...
test.describe("workflow node — action.github.pushTag", () => {
  ...
  test("seeded pushTag renders + tag round-trips through save/reload", async ({ page }) => {
    ...
    await expect(page.locator("#ins-sha, [data-field=sha]").first()).toBeVisible()
    await fillInspectorField(page, "tag", "v9.9.9-e2e")
    ...
    await openNodeInspector(page, "action.github.pushTag")
    await expectInspectorFieldValue(page, "tag", "v9.9.9-e2e")
  })
  ...
  test("manual run hits POST /repos/.../git/refs with refs/tags/<tag>", async ({ page }) => {
    ...
    await expectGithubCall({
      ...
      bodyMatch: { ref: "refs/tags/v1.0.0", sha: "abcd1234" },
    })
  })
})
