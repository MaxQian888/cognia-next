...
test("the install ZIP carries every entry the manifest names", async () => {
  const outputDirectory = await mkdtemp(join(tmpdir(), "cognia-external-agent-adapter-example-"))
  try {
    ...
    const lazy = evaluate(await archive.file("dist/demo-adapter.js").async("string"), identitySdk)
    assert.equal(typeof lazy.createDemoEchoAdapter, "function")
