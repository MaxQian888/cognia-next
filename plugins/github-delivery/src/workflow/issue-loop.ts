...
export interface IssueLoopDriver {
  ...
  run(opts: {
    ...
    signal: AbortSignal
  }): Promise<{ summary: string; durationMs: number; driverId: string }>
}
...
export interface RunIssueLoopParams {
  ...
  /** Existing External Agent configuration to use instead of the Claude sidecar. */
  externalAgentId?: string
  /**
...
export interface IssueLoopHooks {
  ...
  runLongStep?: typeof runLongStep
...
export async function runIssueLoop(
  ...
  const longStepRunner = hooks.runLongStep ?? runLongStep
  ...
  const start = now()
  let actualDriverId = params.externalAgentId ?? "claude-code"
  ...
  try {
    ...
    type DriverResult = { summary: string; durationMs: number; driverId: string }
    type DriverState = { phase: "running" } | ({ phase: "completed" } & DriverResult)
    const driverHandle = longStepRunner<DriverResult, DriverState>({
      runId,
      stepId,
    ...
    actualDriverId = driverResult.driverId
    ...
    const pr = (await octokit.request("POST /repos/{owner}/{repo}/pulls", {
      ...
      body:
        ...
        `Generated by cognia GitHub Delivery. Driver: ${driverResult.driverId}.\n\n` +
    ...
    await safeUpdateWorkOrder(rt, {
      ...
      aiDriver: driverResult.driverId,
    })
