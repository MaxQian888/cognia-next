...
The DB lives next to the analyzer cache in the plugin's host-injected data
directory. A separate file (rather than reusing the analyzer's ``cache.db``)
means wiping the chat index does not cost the wiki cache.
"""
...
#
# v2: the IDF blob is JSON, not pickle. Unpickling is arbitrary code
...
def _load_json_blob(blob, default):
    """Decode a stored JSON blob, tolerating a row written by an older build.

    A v1 row held a pickle here. It is never unpickled — the schema-version
    check drops those rows before we get this far — but a hand-edited or
    truncated blob must not take the whole index down either, so a decode
    failure degrades to the default and the caller rebuilds.
    """
...
class RagStore:
    ...
    def db_path(self) -> str:
        return self._explicit_path or str(PATHS.index_db)
