...
def test_clear_wipes_every_cached_result(tmp_path):
    # Upstream also asserted that `clear()` spared a saved project. The
    # `projects` table went with the port: nothing ever read it (the server
    # kept projects in memory, the CLI never called `load_project`), and a
    # table only ever written to is a table that will drift.
    c = _make_cache(tmp_path)
    ...
    assert not hasattr(c, "save_project")
    asyncio.run(c.close())
