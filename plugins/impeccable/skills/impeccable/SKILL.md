---
name: impeccable
description: Use when the user wants to design, redesign, shape, critique, audit, polish, clarify, distill, harden, optimize, adapt, animate, colorize, extract, or otherwise improve a frontend interface. Covers websites, landing pages, dashboards, product UI, app shells, components, forms, settings, onboarding, and empty states. Handles UX review, visual hierarchy, information architecture, cognitive load, accessibility, performance, responsive behavior, theming, anti-patterns, typography, fonts, spacing, layout, alignment, color, motion, micro-interactions, UX copy, error states, edge cases, i18n, and reusable design systems or tokens. Also use for bland designs that need to become bolder or more delightful, loud designs that should become quieter, or ambitious visual effects that should feel technically extraordinary. Not for backend-only, non-UI, or live-browser-control tasks.
version: 4.0.4
---

## Cognia integration policy

This plugin packages upstream Impeccable 4.0.4 as a Cognia local-bundle skill.

- Resolve every relative `reference/`, `scripts/`, or `agents/` path against `${COGNIA_PLUGIN_ROOT}/skills/impeccable/`. Never resolve bundled resources from the user's working directory.
- `live`, hooks, and pin/unpin are unavailable in this adapter, and their implementation scripts are not shipped. The upstream 4.0.4 live channel has an unresolved instruction-injection report, and Claude/Codex hook installation has no behaviorally equivalent Cognia declarative contribution.
- Do not create, replace, or repair `PRODUCT.md`, `DESIGN.md`, or `.impeccable/` artifacts unless the user explicitly requests that persistent change. Existing product and design documents remain authoritative.
- All reads, edits, shell commands, image generation, and network access remain subject to the active Cognia session's normal permission gates. This skill grants no plugin permissions by itself.
...
1. Run `node ${COGNIA_PLUGIN_ROOT}/skills/impeccable/scripts/context.mjs` once per session; keep cwd at the user's project. Pass a named source file or route as `--target <path>`. It loads PRODUCT.md, DESIGN.md, the matching surface brief, and native-platform guidance when applicable; follow its directives and do not rerun it.
...
**Unavailable adapter operations:** If the user asks for `live`, hooks, or pin/unpin, explain that this Cognia package intentionally disables them. Do not attempt to fetch or reconstruct the omitted implementation.
