...
- `devServer.running` only fed upstream's `live` mode, which this adapter does not ship — never recommend it. **The bundled `detect.mjs` is web-only.** If `setup.platform` is `ios`, `android`, or `adaptive`, don't lead with it; the HTML rule engine doesn't apply to native app code.
