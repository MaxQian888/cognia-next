...
- **Layout**: four settings headers (Language Servers, Canvas, Scheduled Tasks, Tray) squeezed their description into a narrow column on a phone because the buttons beside them never wrapped; they now stack. The Canvas tab strip scrolls instead of clipping "Collaboration", "Accessibility" and "Keybindings" into a four-column grid. Canvas's three settings that still have nothing behind them are shown switched off and greyed with the reason, instead of looking like working switches.
- **Scheduled task form**: the webhook channel can now be picked (and its URL edited) per task, matching the set the settings defaults offer; the four channel buttons are also translated instead of showing the raw value, so the chat channel no longer reads as "Im".
- **Conversation**: an effort default carried in from an imported Codex / Claude Code / pi config is now visible and clearable. It applies to every chat that has no thinking level of its own, and until now nothing in the app named it.
