...
GitHub Delivery's Issue → PR loop blocks `git push`, `git remote` and the `gh` CLI at the permission gate rather than trusting the prompt — the issue body is written by anyone who can open an issue. Those rules only matched the literal spelling, so `git -C <path> push`, `git -c k=v push`, `git --git-dir=… push`, an extra space, or `env gh …` all slipped through to a prompt the unattended run could not answer. The gate now covers git's pre-subcommand options and the common exec wrappers, while still leaving `git commit -m "fix the push flow"` alone.
