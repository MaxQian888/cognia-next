...
export function validateInterfaceCatalog(catalog) {
  ...
  for (const namespace of catalog.apiNamespaces) {
    ...
    for (const method of namespace.methods) {
      ...
      if (
        method.resourceEffect.kind === "returned-handle" &&
        !method.resourceEffect.disposeMethod
      ) {
        throw new Error(`API method ${method.id} returned-handle requires disposeMethod`)
      }
      if (
        method.resourceEffect.kind !== "returned-handle" &&
        method.resourceEffect.disposeMethod !== undefined
      ) {
        throw new Error(
          `API method ${method.id} may only declare disposeMethod for returned-handle`
        )
      }
    }
  }
