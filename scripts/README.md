...
| Script                                 | Purpose                                                                                                                                                                                                                                      | Trigger                                                        | Test                                                                         |
| -------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------- | ---------------------------------------------------------------------------- |
| `gates/check-all.mjs`                  | Run every deterministic CI gate with ordered group lanes, bounded local concurrency, resource locks, continue-on-error, and a ✓/✗ summary. `--jobs N` overrides concurrency; `--bail` is sequential and stops at the first blocking failure. | `pnpm check:all`                                               | `gates/check-all.test.mjs` (`pnpm check:all:test`)                           |
...
| `smoke/compose-runtime-environment.mjs` | Write a smoke baseline (`baseline`), then drive a T2 stack with `docker-compose.runtime-environment.yml`: catalog, approval, placed spawn, ACP round trip and probe on a musl and a glibc image (`--expect pool-on`), or the unchanged off path (`--expect pool-off`). | `pnpm compose:smoke:runtime-environment` | `smoke/compose-runtime-environment.test.mjs` (`pnpm compose:smoke:runtime-environment:test`) |
