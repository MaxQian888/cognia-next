...
const BODY_DIR = path.join(GENERATED_DIR, "bodies")
const BODY_LOADER_FILE = path.join(GENERATED_DIR, "built-in-body-loader.generated.ts")
...
export function renderCatalogModule(entries) {
  ...
  lines.push("")
  lines.push("/**")
  lines.push(" * One skill's facts. The SKILL.md body is NOT here: it is the bulk of the")
  lines.push(" * file and only a delivered skill needs it, so it lives in its own chunk.")
  lines.push(" * Reach it with `loadBuiltInSkillBody(entry.id)`.")
  lines.push(" */")
...
/** Render one skill's SKILL.md body as its own lazily-imported chunk. */
export function renderBodyModule(entry) {
  return [
    "/* eslint-disable */",
    `// @generated SKILL.md body for ${entry.id}`,
    "// Do not edit by hand. Run `pnpm skills:build` to regenerate.",
    "",
    `export const BUILT_IN_SKILL_BODY = ${JSON.stringify(entry.content)}`,
    "",
  ].join("\n")
}

/**
 * Render the static-import switch for the bodies.
 *
 * Unlike the resource loader, an unknown id throws rather than returning a
 * neutral value. A skill with no resources is ordinary; a skill with no body is
 * not a skill, so an id that reaches here and matches nothing is a caller bug
 * and an empty string would inject silence into a prompt.
 */
export function renderBodyLoaderModule(entries) {
  const lines = [
    "/* eslint-disable */",
  ...
  for (const entry of entries) {
    lines.push(
      ...
      `      return (await import("./bodies/${entry.id}.generated")).BUILT_IN_SKILL_BODY`
    )
  }
  lines.push(
    "    default:",
...
/**
 * Every file this codegen owns, keyed by absolute path.
 *
 * One map rather than three parallel code paths: `--check` and the write both
 * need "is this file's content exactly what we would emit", and the per-skill
 * chunk directories additionally need "is there a file here we would no longer
 * emit". Both questions are answered from this map, so a fourth kind of output
 * cannot arrive with only half its bookkeeping.
 */
function plannedOutputs(entries) {
  const outputs = new Map([
    [OUT_FILE, renderCatalogModule(entries)],
    [RESOURCE_LOADER_FILE, renderResourceLoaderModule(entries)],
    [BODY_LOADER_FILE, renderBodyLoaderModule(entries)],
  ])
  for (const entry of entries) {
    outputs.set(path.join(BODY_DIR, `${entry.id}.generated.ts`), renderBodyModule(entry))
    if ((entry.resources?.length ?? 0) > 0) {
      outputs.set(path.join(RESOURCE_DIR, `${entry.id}.generated.ts`), renderResourceModule(entry))
    }
  }
  return outputs
}

/** Chunk files on disk that this run would not emit. */
function staleChunks(outputs) {
  const stale = []
  for (const dir of [BODY_DIR, RESOURCE_DIR]) {
    if (!existsSync(dir)) continue
    for (const name of globSync("*.generated.ts", { cwd: dir, nodir: true })) {
      const file = path.join(dir, name)
      if (!outputs.has(file)) stale.push(file)
    }
  }
  return stale
}
...
function main(argv) {
  ...
  const outputs = plannedOutputs(entries)
  const stale = staleChunks(outputs)

  if (options.check) {
    const drifted = [...outputs].some(
      ([file, source]) => !existsSync(file) || readFileSync(file, "utf8") !== source
    )
    if (drifted || stale.length > 0) {
      console.error(
        "built-in skills catalog/bodies/resources are out of date. Run `pnpm skills:build` and commit the generated outputs."
      )
      process.exit(1)
    }
  ...
  for (const dir of [GENERATED_DIR, BODY_DIR, RESOURCE_DIR]) {
  ...
  console.log(
    written === 0 && stale.length === 0
      ? `built-in skills catalog unchanged (${entries.length} skills).`
      : `wrote ${written} file(s), removed ${stale.length} (${entries.length} skills).`
  )
}
