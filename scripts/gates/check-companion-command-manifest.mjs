...
export function compareCommandSets(manifest, registered, remoteExecution) {
  const errors = []
  const byName = new Map(manifest.commands.map((command) => [command.name, command]))
  for (const name of remoteExecution) {
    if (!byName.has(name)) errors.push(`remote execution command missing descriptor: ${name}`)
  }
  for (const command of manifest.commands) {
    ...
    if (!isBacked(command.name, registered, remoteExecution)) {
 ...
 */
function isBacked(name, registered, remoteExecution) {
  return registered.has(name) || remoteExecution.has(name) || name.startsWith("plugin_")
}
...
function main() {
  ...
  console.log(
    `[companion-command-manifest] OK: ${manifest.commands.length} descriptors cover ` +
      ...
      `${extractRustArray(rpcSource, "KNOWN_COMMANDS").size} remote execution commands.`
  )
  return 0
}
