...
**参考 ADR**: 0035(Rust 性能面板)、0067(crate 分解与编译提速 —— 见 R1)、0068(前端包抽取)、0022(Agent Team)、拟新增 **0073**
...
1. **ADR 编号 0073** —— 现有最新为 0072。是否现在就落 `docs/content/docs/en/adr/`?是否同出 `docs/content/docs/zh/` 中文版?
2. **P1.6 feature gate 默认值** —— 开发态是否也默认关?关掉则日常开发看不到 Rust span,开着则每次 `cargo build` 都付 OTel 编译成本(R1)。
3. **P2 事件 catalog 的 owner** —— 谁把关命名与新增?Tea 体系里这是专人职责,**本仓无对应角色。没人把关的话,事件表半年就会烂掉** —— 这是 P2 的真实前置条件,不是流程装饰。
