...
Date: 2026-07-28  
Status: active  
Baseline: generated by `pnpm exec playwright test --list` for each configured project; fixed counts are intentionally not copied here because discovery is the source of truth
...
| -------- | ---------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----- | -------------------------------------------------------------------------------------------------------------------------------------------------------- |
| P0       | Chat `/`                                                               | Native streamed reply/multi-turn/interrupt/error specs exist under `tests/e2e/tauri/chat/`; active Pixel 7 standalone coverage drives send → streamed mock reply → reload persistence | ✅    | Add attachment/tool-call journeys only at their owning runtime boundaries                                                                                |
...
| P1       | Sites `/sites`                                                         | The route is tracked, but no owning create/preview/persist/publish E2E exists                                                                                                         | ❌    | Cover create → preview → persist/publish according to ADR-0084                                                                                           |
| P1       | Browser `/browser`                                                     | The route is tracked, but no owning navigation/frame/recovery product E2E exists                                                                                                      | ❌    | Open session → navigate → observe frame/state → recover from disconnect                                                                                  |
| P1       | Source control `/source-control`                                       | The route is tracked, but no owning real-repository product E2E exists                                                                                                                | ❌    | Use a temporary real repository: stage → commit or sync → observe status; do not mock the contract into itself                                           |
...
### 2026-07-28 — Trust and governance baseline

- Activated the standalone Chat acceptance path and proved streamed reply plus
  reload persistence in the default Pixel 7 project; removed its runtime-skip
  debt entry.
- Added the shared diagnostic fixture to every browser/mobile spec. Failure
  attachments collect console/page/network/server evidence after redacting
  credentials, tokens, cookies, API keys, email addresses, and sensitive URL
  parameters. A non-exempt governance rule prevents future direct
  `@playwright/test` imports from bypassing it.
- PR gates now run curated `@smoke`, `@critical`, `@a11y`, and `@visual`
  coverage against the once-built static artifact. Full suites run on pushes;
  weekly critical journeys run on Firefox and WebKit, while iPhone WebKit and
  Windows Tauri retain their dedicated jobs.
- CI fails on flaky retry-pass results and reports first-pass rate, flaky rate,
  and P95 duration against the latest successful trunk artifact.
- Added a stable anonymous-share visual baseline and an accessibility journey.
  The latter exposed and fixed invalid landmark nesting (`header`/`footer`
  inside `main`) before passing.
- Governance now has 19 exact, dated legacy debt occurrences. The current
  uncovered priorities are Sites, Browser, and Source Control; they remain
  explicit gaps rather than being inferred from route reachability.

Next audit slice: Sites create/publish, Browser navigation/recovery, and Source
Control against a temporary repository, followed by injected keyring failure
handling.
