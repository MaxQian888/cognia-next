...
This is reserved for Ed25519 signature verification when Discord sends interaction payloads to a webhook endpoint. Phase 1 uses Gateway (WebSocket) mode only, so cognia-next does **not** persist the public key yet and does not expose an Interactions endpoint URL. Keep the value in the Discord Developer Portal for a future webhook migration.
...
3. In the **Discord Configuration** dialog:
   ...
   - Leave the **Public Key** field blank unless you are preparing for a future Interactions webhook migration; current Gateway mode does not save or use it.
...
- **Voice messages**: Supported when the voice segment already has a public audio URL; cognia-next sends a voice-flagged Discord message that references that URL. Native multipart upload is still pending the Tauri upload bridge.
