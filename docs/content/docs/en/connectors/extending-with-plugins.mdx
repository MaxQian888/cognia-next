...
cognia-next ships seven built-in platform adapters: Telegram, Discord, Slack, Lark, OneBot, WeCom (企业微信), and personal WeChat. The **Plugin Connector Bridge** (`lib/plugin/bridge/connectors-bridge.ts`) lets any installed plugin contribute additional adapters — Mastodon, Bluesky, Matrix, or an internal messaging system — without touching the cognia-next source.
