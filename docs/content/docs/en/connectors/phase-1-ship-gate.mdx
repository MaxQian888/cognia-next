...
- [ ] `pnpm test` — all Jest suites pass
...
> Current note: auto mode and scheduled digests now run the shared AI turn path via
> `runConnectorDigestTurn`. Draft mode still produces a draft row first — verify the
> approval/rejection flow end-to-end.
