---
title: Storage (Dexie schema v15)
description: The complete IndexedDB schema, version-by-version, and how to add or migrate tables safely.
---

# Storage (Dexie schema v15)

<Status variant="stable">v15</Status>

<TLDR>
  All browser-side state lives in IndexedDB via Dexie under the database name
  `cognia-claude`. Schema versions are append-only — every release adds new
  `.version(n)` blocks; old ones are never edited. Migrations are idempotent
  and run on every database open.
</TLDR>

<StatGrid>
  <Stat label="Schema version" value="15" hint="2024–2026" />
  <Stat label="Tables" value="29" hint="across all versions" />
...
Cognia persists almost all browser-side state in **IndexedDB via Dexie**. The schema is defined in `lib/db/schema.ts` and is currently at **v15**. Schema versions are append-only — never mutate an existing `.version(n).stores({...})` block.
...
| `canvasComments` | `CanvasCommentRow` | `id`, `documentId`, `createdAt` | Inline comments |
...
| `pluginScheduledJobs` | `PluginScheduledJobRow` | `id`, `pluginId`, `enabled` | Plugin scheduled jobs |
...
<Callout type="info" title="Idempotent upgrades">
  Each version is idempotent — opening an old database upgrades it through every
  intermediate version automatically. Dexie applies them in order. **Never modify
  a previous `.version(n)` block.**
</Callout>
...
<Callout type="error" title="Never modify a previous version block">
  If you realise an existing index is wrong, add a **new** version that
  re-declares the table with the corrected index list. Mutating an old block
  would orphan users who already migrated past that version.
</Callout>
...
<Cards>
  ...
  <Card title="Plugin system" href="./plugin-system" description="Uses the v15 plugin tables." />
  <Card title="Employee Digital Twin" href="./employee-twin" description="Uses the v14 twin tables." />
</Cards>
