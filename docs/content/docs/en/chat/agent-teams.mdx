...
<TLDR>
Team = lead + teammates + tasks + execution report. The lead may go through a
"plan approval" step; teammates are dispatched in parallel with limited
concurrency. Runtime is driven by `runTeamLifecycle` and cancelled via
`abortTeam`. Each teammate's runtime can be Claude / Codex / a custom
external agent CLI (ACP / OpenCode), where external agents are spawned by
the Rust-side process manager. `@<name>` mentions in workspace chat are
routed through `dispatchTeamMention` to the target runtime, with messages
streamed back in place.
</TLDR>
