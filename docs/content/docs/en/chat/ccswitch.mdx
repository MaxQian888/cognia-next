...
<TLDR>
CCSwitch is a community open-source Claude multi-account switcher. Cognia opens its `~/.cc-switch/cc-switch.db` read-only, surfaces 50+ providers, MCP servers, prompts, and skills directly in Settings, and switches cognia-next itself plus any checked external agents in one action (v1 only writes `claude-code`'s `~/.claude/settings.json`). We **never write** to the CCSwitch database.
</TLDR>
