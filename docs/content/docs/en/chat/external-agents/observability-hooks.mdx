...
<TLDR>
  ...
  `@/lib/agent-trace/emitter`. `agent-hooks.ts` wires external events into **two** hook systems:
...
`@/lib/agent-trace/emitter`, imported `:16`) so the external runtime produces the same span shape as
