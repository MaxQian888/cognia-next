...
| **pi** | acp-stdio (Pi, community adapter) | executable | acp/stdio | `npx -y pi-acp` | Pi settings keys; third-party bridge | 327 |
...
| `pi` | Pi (community adapter) | acp | stdio | `npx -y pi-acp` |
...
The Pi entry is bridged by the community `pi-acp` adapter (Pi has no native ACP support yet), so it
is tagged `community-adapter` / `experimental` in the catalog.
