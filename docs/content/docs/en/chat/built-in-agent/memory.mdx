...
flowchart TB
  subgraph WRITE["WRITE — fire-and-forget per turn (use-claude-chat.ts:269 runMemoryTasks)"]
    ...
    DB["createMemory + vector upsert<br/>memories.ts:48"]
  end
...
The whole memory subsystem **reuses the shared RAG toolkit**: `BM25Index` / `normalizeScores` / `reciprocalRankFusion` from `@cognia/rag/hybrid-search`, the context manager from `@cognia/rag/context-manager`, and the LLM contract from `@/lib/twin/distill/llm`.
...
`lib/db/memories.ts` is purely mechanical (no LLM, no gating — the gate is in the callers). The retriever's candidate pool is `listActiveForReader(characterId?)` (`:155`) — global base ∪ character override via the `[scope+status]` index. `touchMemories` (`:112`) bumps recency on a hit. Only the management panel calls `hardDeleteMemory` / `clearMemories` — consolidation only ever soft-invalidates.
