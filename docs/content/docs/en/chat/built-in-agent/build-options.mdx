...
description: resolveSendOptions — the single convergence point that collapses all UI state into one SendOptions payload. The 41 ordered contributors, the precedence chains, and the system-prompt assembly.
---

# Build-options Pipeline

`lib/claude/build-options.ts` exists so that **every** capability — character, skills, Twin, memory, goal, A2UI, computer-use, sandbox, plugins, routing — influences a turn through exactly one function. `resolveSendOptions` (`:336`) mutates a single `opts: SendOptions` object across ~41 ordered sections and returns it (`:1380`). Nothing else in the codebase constructs a `SendOptions`.
...
<TLDR>
  ...
  set committed at `:864`. The workflow-editor branch (`:1285`) overwrites the prompt and tools
...
| Export | Line | Role |
| --- | --- | --- |
| **`resolveSendOptions(ctx)`** | `:336-1380` | The pipeline. Async; returns the full `SendOptions`. |
...
The function is a straight-line accretion. This diagram groups the 41 sections into phases; each node is annotated with its line.
...
flowchart TB
  ...
  subgraph APPEND["Late appenders + branches"]
    ...
    WF["workflow-editor OVERRIDE :1285"]
...
| 39 | **Workflow-editor branch** | **overwrites** `systemPrompt`/`appendSystemPrompt`/`allowedTools`/`additionalDirectories`; deletes `mcpServers` | `:1285` |
| 40 | Team branch | unions `resolveAllSubagents({context:"team"})` into `opts.agents` | `:1348` |
| 41 | Resume / fork | `forkFromSessionId` or `resumeSessionId` | `:1364` |
...
The IM channel override (`imOverrideRow`, fetched once at `:413`) sits *above* the session for **model**, **provider**, and the **computer-use** gate only — so a per-channel binding can pin a cheaper model without touching the session.
...
- The **workflow-editor branch** (`:1285`) overwrites both `systemPrompt` (`:1327`) and `appendSystemPrompt` (`:1328`) wholesale, so a new appender must run *before* it to survive in that session kind.
...
4. Add the field to `SendOptions` in `lib/claude/types.ts`, and to the Rust `SendOptions` struct (`commands.rs:17`) **only if the sidecar must deserialize it** — renderer/sidecar-JS-only fields like `permissionRuleset` are deliberately *not* mirrored in the Rust struct.
...
<Callout type="warn" title="Don't re-read the IM override">
  `imOverrideRow` is fetched once at `:413` and reused for the computer-use gate at `:991`. Re-reading
