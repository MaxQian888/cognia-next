...
<TLDR>
  ...
  default-denies on connector channels. The preset gallery (`mcp-presets.ts`, 15 presets) is just the
...
flowchart TB
  subgraph SDK["sidecar query() mcpServers"]
    ...
    UM["user MCP servers<br/>build-options.ts:958"]
  end
  ...
  subgraph CONFIG["config-only"]
    PRE["mcp-presets.ts (15) — Add-server gallery"]
...
| coreFiles | `core/core-tools.mjs` | grep/glob/read/ls/edit/multi_edit/write/bash/TodoWrite/bash_output/kill_shell/NotebookEdit/apply_patch | 13 |

The **coreFiles** suite is resolver-bound (like lsp) and **default-OFF on the Anthropic path** (the claude-agent-sdk ships its own Read/Edit/Bash); it powers the non-Anthropic ai-sdk path, where the model has no SDK-native file tools. `apply_patch` is the multi-file complement to the single-file `multi_edit`: it applies a git-style unified diff across one or more files atomically — supporting create (`--- /dev/null`) and delete (`+++ /dev/null`) — with the same read-before-edit enforcement, BOM/EOL preservation, and post-write LSP diagnostics as `edit`/`write`. Tools are emitted in the fixed `CORE_TOOL_NAMES` order for prompt-cache prefix stability; new tools are appended last.
...
The toggle source is `opts.builtinTools` (`build-options.ts:972`) — the settings JSON that enables/disables each category.

### Core `bash` — OS-aware shell

The free-form core `bash` tool (`builtin-tools/core/bash.mjs`) drives the host's preferred shell rather than a fixed binary: on Windows it probes PATH for **PowerShell 7 (`pwsh`) → Windows PowerShell (`powershell`) → `cmd.exe`**, and `/bin/sh` elsewhere (`shared/shell-detect.mjs::resolveShellDescriptor`, mirroring the integrated terminal's `resolve_shell_binary`). The resolved shell drives **both** the spawn argv **and** the tool description, so the model is told which syntax to write (PowerShell `$env:VAR` / `$null` / cmdlets vs POSIX `$VAR` / `/dev/null`). PowerShell children are spawned `-NoProfile -NonInteractive` and have `PSModulePath` / `PSExecutionPolicyPreference` stripped from their environment (sidecar-side mirror of the [ADR-0028](../../adr/0028-per-session-claude-isolation) dangerous-env denylist) so a workspace the agent just wrote to can't auto-load `.psm1` code into the next PowerShell run.

## cognia-plugin-tools — the renderer proxy

`buildPluginToolsServer` (`plugin-tools.mjs`) builds a second in-process server (`cognia-plugin-tools`) that wraps renderer-relayed plugin tools: each tool, when called, emits a `plugin_tool_exec` back to the renderer and awaits the `plugin_tool_response`. This is the server the workflow subagents' `mcp__cognia-plugin-tools__wf_*` names resolve against. The manifest is built in build-options (`buildPluginToolsManifest`, `:1009`), which also folds in the built-in-skills manifest and drops `cognia-computer-use` when computer-use is not authorized.
...
Plugin skills win on id collision; everything is fail-soft (browser / fs failure → `undefined`, never throws). See the [Skills subsystem](../../subsystems/skills/authoring-and-workbench) for authoring.
...
2. **IM blacklist short-circuit** (`:122`): `if (imSession && allowImComputerUse !== true) return {attachedCount:0}` — connector channels default-deny screen control
...
`mcp-presets.ts` is a **code-only** catalog (`MCP_PRESETS`, `:46`) of 15 one-click servers: filesystem, github, gitlab, brave-search, puppeteer, fetch, memory, sqlite, postgres, slack, time, deepwiki (http), http-generic, sse-generic, custom. `applyPresetFields(preset, values)` (`:365`) places user values into env / args / url / headers. This populates the "Add server" UI only — distinct from the in-process `cognia-tools` server.

## Cross-module Summary

| Source | Reaches SDK as | Build-options site |
| --- | --- | --- |
| cognia-tools | `query()` mcpServers (in-process) | `opts.builtinTools` `:972` |
| plugin tools | `cognia-plugin-tools` (in-process proxy) | `opts.pluginTools` `:1009` |
| user MCP | `opts.mcpServers` | `:958` |
...
<Cards>
  ...
  <Card title="Skills subsystem" href="../../subsystems/skills" description="Authoring chat skills and the SKILL.md format." />
  <Card title="MCP System" href="../subsystems/mcp" description="The full MCP client/host/bridge section — config store, presets, built-in tools, and the a2ui-bridge." />
