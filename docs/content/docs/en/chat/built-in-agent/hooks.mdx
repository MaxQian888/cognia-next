...
<TLDR>
  Runtime capabilities are evidence-based. The pinned Claude Agent SDK exposes **33 events** and
...
<StatGrid>
  <Stat label="Claude events" value="33" hint="Pinned SDK 0.3.269 surface" />
...
`PreToolUse`, `PostToolUse`, `PostToolUseFailure`, `PostToolBatch`, `Notification`, `UserPromptSubmit`, `UserPromptExpansion`, `SessionStart`, `SessionEnd`, `Stop`, `StopFailure`, `SubagentStart`, `SubagentStop`, `PreCompact`, `PostCompact`, `PermissionRequest`, `PermissionDenied`, `Setup`, `TeammateIdle`, `TaskCreated`, `TaskCompleted`, `Elicitation`, `ElicitationResult`, `ConfigChange`, `WorktreeCreate`, `WorktreeRemove`, `InstructionsLoaded`, `CwdChanged`, `FileChanged`, `DirectoryAdded`, `MessageDisplay`, `PreModelSwitch`, and `PostModelSwitch`.
...
`WorktreeCreate` and `WorktreeRemove` are produced by Cognia itself, not by the SDK (ADR-0111 decision 9): the managed-worktree Registry fires them when a worktree becomes Active and when it is discarded or pruned, and the shared git command layer fires them for the agent-team allocator and the source-control panel. The payload carries `worktree_path`, `workspace_root`, `branch`, `owner_type`, `owner_ref`, `source` and, on removal, `reason`. They are observational — a hook cannot block a worktree from being created or removed.
...
Tool-scoped payloads (`PreToolUse`, `PostToolUse`, `PostToolUseFailure`, `PermissionRequest`, `PermissionDenied`) carry `tool_provenance`: `{ kind, source, declared_by }` naming the surface and artifact that declared the tool — `builtin` (`source: "cognia"`, `declared_by: "builtin-tools-data.json"`), `plugin` (`source` = the plugin id resolved from the session's tool manifest, `declared_by` = the plugin's `plugin.json` path), `mcp` (`source` = the MCP server name, `declared_by` = the config file / `"settings"` / `plugin:<id>` locator that declares it), or `agent` (`source` = the external agent id, for bare names on an external agent's own surface; `declared_by` is absent — the agent's own config is not ours). The field is metadata only — never tool arguments or secrets — and is absent when the event has no resolvable tool name, so hook scripts must tolerate it missing. A hook can therefore match policy on a tool's origin (for example, deny every tool from a given MCP server) instead of enumerating tool names. The bundled `tool-provenance-guard` built-in hook (default off) is one such consumer: `COGNIA_DENY_TOOL_PROVENANCE` selectors `kind` / `kind:source` / `kind:source:declared` deny matching calls.
