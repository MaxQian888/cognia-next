...
`gateExternalAgentPermission` (`agent-hooks.ts:117`) fires the System-A plugin hook, the observational `PermissionRequest`, then the blocking `PreToolUse`; on block it calls `deny(...)` and returns `true` so the manager suppresses the event. The manager seams are `manager.ts:1637` (interactive, true suppression) and `:1803` (headless, deny still enforced). `run_agent_hook` (`commands.rs:55`) applies the same `resolve_trusted_cwd` gate (`:65`) before routing to `run_tool_scoped` / `run_session_scoped`.
