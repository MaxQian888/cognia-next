...
<TLDR>
  **Layer A** lives in the sidecar: the SDK's `canUseTool` callback (`anthropic.mjs:263`) consults the
  ...
  `permission_request` handler (`use-claude-chat.ts:1267`) runs Auto-mode, a precedence ladder of
...
flowchart TB
  ...
  subgraph B["Layer B — renderer (use-claude-chat.ts:1267 permission_request)"]
    AL["always-allow list :1269"]
    AM["runAutoModeForTool :1324"]
    APV["allow → approveTool :1333"]
    DNY["deny → approveTool deny :1337"]
    MODAL["ask → manual modal :1360"]
  end
...
The resolver is a deliberately **weaker** JS mirror of `ruleset.ts`: `splitBash` (`permission-resolver.mjs:100`) is a naive top-level split that ignores quotes, and there is **no** baked-in `"*": "allow"`. Any segment that denies wins; allow is returned only when *every* segment is explicitly allowed (`:143/:153`). The full compound-command parser lives in Layer B, and unmatched calls round-trip there — so weakening Layer A is safe.

### Layer B — renderer Auto-mode

The `permission_request` case (`use-claude-chat.ts:1267-1362`):

1. **Always-allow list** (`:1269`) — pre-existing per-session grants.
2. **Inactive/remote session** (`:1277`) — remote-control routing with a backstop deny; not Auto-mode.
3. **Auto-mode** (`:1316`): builds the judge client (`buildUtilityLlmClient`, featureId `command-safety`, `:1318`), calls `runAutoModeForTool` (`:1324`) with `pluginRules: getPluginCommandRulesets()`. `allow` → `approveTool(..., "allow")` (`:1333`); `deny` → `approveTool(..., "deny", reason)` (`:1337`); `try/catch` fail-open (`:1345`).
4. `ask`/`null` → the manual modal `pushApproval` (`:1360`).
...
flowchart TB
  ...
  D2["classifier decisive (allow/deny)?<br/>→ source:rule :95"] -->|ask| D3
...
The full ladder (highest → lowest): **user `commandRules` glob → plugin rules → deterministic classifier → model judge → ask**. `auto-mode-runner.ts:56` encodes it by array order — plugin rules first, then the user `{Bash: commandRules}` pushed last so the user always outranks plugins.
...
- **Fail-open on both layers** (ADR §56): Layer A catch (`anthropic.mjs:293`), Layer B catch (`use-claude-chat.ts:1345`), judge returns `null` on every failure, `evaluateAutoDecision` never throws (`auto-mode.ts:70`).
