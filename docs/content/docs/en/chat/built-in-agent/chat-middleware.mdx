...
<TLDR>
  A middleware is `(req, next) => Promise<Response>` (`plugin-chat-middleware.ts:115`). Plugins register
...
- **timeout** — `Promise.race` against `entry.timeoutMs` (`raceWithTimeout`, `:136`). Default 5s (`registry.ts:24`), cap 60s (`:25`).
...
| Types | `types/plugin/plugin-chat-middleware.ts` (`ChatMiddlewareRequest:71`, signature `:115`) |
