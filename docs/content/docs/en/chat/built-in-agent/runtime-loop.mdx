...
<TLDR>
  `send` (`use-claude-chat.ts:596`) builds options, optimistically appends the user message, and
  ...
  line out on the `claude://message` Tauri event. A single subscription (`use-claude-chat.ts:543`)
  funnels events through a per-session serialized queue into `handleEvent` (`:1183`), whose `event`
...
`send` is a `useCallback` at `hooks/chat/use-claude-chat.ts:596-894`, signature `send(content, opts?, callOptions?)`. It is **fire-and-write**: it does not consume the stream (that is push-based, Stage 5).

| Step | What happens | Line |
| --- | --- | --- |
| Validate | resolve active session id, reject empty content | `:606-612` |
| Goal pause | ADR-0019 pause-on-fresh-user-turn | `:621-624` |
| **Build options** | `opts ?? await buildSendOptions(session, text)` — wraps [`resolveSendOptions`](./build-options) | call `:634`, wrapper `:1105` |
| Command frontmatter | merge per-slash-command overrides | `:645-658` |
| Plugin `onUserPromptSubmit` | may **block** / modify / inject `additionalContext` | `:674-710` |
| **Optimistic append** | `makeUserMessage` → `replaceMessages`; status → `"streaming"` | `:728-733` |
| External-agent branch | non-SDK path (bypasses the sidecar entirely) | `:738-835` |
| Persist + title | write messages, touch session, derive title | `:838-847` |
| Trace span | `startSpan("invoke_agent")`, stamp `traceId`/`spanId` onto `opts` | `:854-865` |
| **Invoke** | `await sendPrompt(sessionId, content, opts)` | `:867` |
| Cache send | `setLastSend(sid, { options })` (fallback retry + twin/memory readback) | `:873-877` |

`sendRef.current = send` is kept fresh (`:898-903`) so module-scope event handling can fire silent goal-continuation sends.
...
sequenceDiagram
  ...
  CMD->>CMD: validate SendOptions (:183)<br/>register_session_cwd (:202)<br/>run UserPromptSubmit hooks (:209)
  CMD->>SP: write_command {type:"send", …} (:233)
  loop per stdout line
    CH-->>RD: JSON line
    alt permission_request
      RD->>RD: handle_permission_request → PreToolUse may deny (:336)
    else a2ui_dispatch
...
- **`claude_set_api_key`** (`lib.rs:71`) writes `ApiKeyState` in memory — never to disk.
- **`claude_restart_sidecar`** (`lib.rs:119`) calls `kill_sidecar` (`:593`); the next send respawns so the SDK re-reads env at init.
...
| ← | `permission_request` | model wants a tool, not statically resolved | `anthropic.mjs:298` |
| ← | `plugin_tool_exec` | proxy a plugin tool back to the renderer | `builtin-tools/plugin-tools.mjs` |
| ← | `session_ended` / `ready` / `log` / `sdk_session_id` / `a2ui_dispatch` | lifecycle + telemetry | various |

`fetch-interceptor.mjs` **must** be the first import (`claude-host.mjs:24`) so the global `fetch` patch is in place before the SDK loads. At boot the host emits `{type:"ready", …}` (`:272`).
...
flowchart TB
  ...
  MCP3["plugin-tools bridge<br/>:150 → plugin_tool_exec"]
  ...
  PUMP -->|"first evt w/ session_id"| SID["emit sdk_session_id :361"]
...
Events are not consumed inside `send`. A single subscription (`use-claude-chat.ts:543-588`) funnels every event into a **per-session serialized promise queue** (`eventQueuesRef`, `:556-566`) so out-of-order writes can't interleave, then calls `handleEvent` (`:1183`). The `switch(evt.type)` (`:1205`):

| Event | Action | Line |
| --- | --- | --- |
| `sdk_session_id` | persist the resume id | `:1210` |
| `session_ended` | on error try routing fallback, else flush + `setError` + `endSpan`; clean end → flush + idle | `:1218` |
| `permission_request` | allowlist auto-allow → Auto-mode judge → prompt user | `:1267` (see [Permissions](./permissions)) |
| **`event`** | read mirror-first base → `applySdkEvent` (`:1383`) → tool spans → on `turnComplete` merge twin (`:1445`) + memory (`:1454`) sources → write | `:1363` |
| _(within `event`)_ | when `applySdkEvent` returns a `result`: `recordResultUsage` + `endSpan(extractUsage)` (there is no separate `result` case) | `:1499-1525` |

### Why a mirror and frame coalescing

The store commit is `requestAnimationFrame`-throttled (`commit`, `:506`) and the persist is debounced 250ms (`persistDebounced`, `:516`). So the authoritative read base for the *next* event is the synchronous **mirror** (`messagesMirrorRef`, set at `:1466`), not the store — which is a frame behind. On `turnComplete` the hook uses an explicit synchronous `replaceMessages` + `persistMessages` (`:1475-1476`) rather than `commit.flush()`, because the twin/memory merge rewrote `nextMessages` *after* the last `commit.call`. Twin and memory sources come from the `lastSendBySession` cache (runtime context), **not** the SDK stream.
...
- **Permission backstop** — the sidecar `canUseTool` has no timeout; remote-routed approvals arm a backstop deny that the next SDK event cancels (`use-claude-chat.ts:1286-1294`).
