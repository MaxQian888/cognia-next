...
<TLDR>
  ...
  `hooks/chat/use-claude-chat.ts:584`, which calls `resolveSendOptions`
...
<StatGrid>
  <Stat label="Assembly contributors" value="40" hint="ordered sections in resolveSendOptions, build-options.ts:336-1381" />
...
flowchart TB
  subgraph SURFACE["1 · Surface — hooks/chat/"]
    SEND["use-claude-chat.ts:584<br/>send()"]
    SUBS["use-claude-chat.ts:530<br/>onClaudeMessage subscription"]
    HANDLE["use-claude-chat.ts:1171<br/>handleEvent dispatcher"]
  end

  subgraph ASSEMBLY["2 · Assembly — lib/claude/"]
    BUILD["build-options.ts:336<br/>resolveSendOptions — 40 contributors"]
...
<Cards>
  ...
  <Card title="Build-options pipeline" href="./build-options" description="resolveSendOptions — the single convergence point. All 40 ordered contributors, the precedence chains, and the system-prompt assembly." />
...
sequenceDiagram
  ...
  U->>H: submit composer
  H->>H: onUserPromptSubmit plugin hook (662)
  H->>B: buildSendOptions → resolveSendOptions (622→336)
  Note over B: 40 contributors mutate one opts object;<br/>system prompt joined at 695
  B-->>H: SendOptions
  H->>I: sendPrompt(sessionId, content, opts) (855)
  ...
  loop streaming
    ...
    H->>A: applySdkEvent(current, evt) (1367→219)
    A-->>H: {messages, turnComplete, result?}
    H->>H: write to mirror / store (1454)
  end
  alt model wants a tool
    Q->>Q: canUseTool — Layer A static ruleset (anthropic.mjs:280)
    ...
    H->>H: Auto-mode — Layer B judge (1304) or prompt user
    H->>I: claude_approve(decision)
  end
  Q-->>N: result (usage)
  N-->>H: turnComplete → merge twin+memory sources (1429), seal (1464)
```
...
| Invariant | Where | Why |
| --- | --- | --- |
| **One convergence point.** All context injection happens in `resolveSendOptions`; no other code mutates a `SendOptions`. | `build-options.ts:336-1381` | A new capability needs to touch exactly one file. The [build-options page](./build-options) is the contract. |
| **`systemPrompt` is sealed early; `appendSystemPrompt` accretes late.** Anything replacing the base prompt must run at/before `:695`; everything after only appends. | `build-options.ts:695` vs A2UI `:908`, goal `:1218`, sandbox `:1151` | The workflow-editor branch (`:1264`) clobbers both wholesale, so new appenders must run before it. |
...
| **Two enforcement layers, explicit-only.** Layer A (sidecar) never honors a wildcard `*:allow`; Layer B (chat hook) runs the deterministic + small-model judge. | `anthropic.mjs:280`, `use-claude-chat.ts:1304` | A compromised renderer cannot auto-approve dangerous tools. See [Permissions](./permissions). |
