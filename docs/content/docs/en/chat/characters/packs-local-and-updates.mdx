...
Signatures **are** verified, as of 2026-08-03 (ADR-0030 amendment). Unsigned files remain fully supported and are visibly labelled; a *signed* file whose signature does not verify is refused at import and skipped by the boot scan.

The signed bytes are the RFC 8785 canonical JSON of the **`pack` object alone**. `schemaVersion` and `signature` sit on the wrapper and are stripped before canonicalization — which is exactly why importing a signed v1 file and rewriting it as v2 leaves the signature valid, and why bumping `CHARACTER_PACK_FILE_SCHEMA_VERSION` needs no re-signing campaign.

Trust has two states and no third:
...
There is deliberately no `"invalid"` state: an invalid signed pack never reaches the registry, so the type cannot represent one. `resolvePackTrust` fails closed — if a signature is present and cannot be checked (`reason: "host-unavailable"`), the pack is refused rather than assumed fine.

Packs contributed by a **plugin** show no trust chip at all. Their authenticity is anchored by the plugin's install receipt, so an "Unsigned" label beside them would claim a gap that does not exist.

To sign a pack, use the CLI:
...
```bash
cognia pack sign my-pack.cognia-pack.json --key .cognia/plugin.private.b64
```

The signature is written **in-band** into the file's own `signature` object, so a pack stays a single self-contained file. `cognia pack sign` self-verifies before writing anything. `cognia pack verify <file>` reports `verified` / `unsigned` / `invalid` exactly as the host will; `--require-signature` turns unsigned into a CI failure, and `--public-key <b64>` pins an expected key, which is the only check that detects a pack re-signed by someone else.
...
```ts
const voiceField = TTS_PROVIDER_SETTINGS[profile.provider].voiceSettingKey
```
