...
The teammate bridge is a fourth, ephemeral producer: `teammateToCharacter` (`lib/ai/agent/team/teammate/teammate-character.ts:56`) synthesizes a transient Character (id `__teammate__:<id>`) so each Agent Team member rides the **same** `resolveSendOptions` path as a chat send — never persisted, never in `listCharacters`.
