...
<TLDR>
Skill = SKILL.md (YAML frontmatter + body) + scripts/ + references/ + assets/.
The browser-side source of truth is Dexie; optional bidirectional sync to `~/.claude/skills/<dir>/`.
The marketplace comes from the `skills-lock.json` registry and the SkillsMP API; installation goes through `parseSkillMarkdown → createSkill`.
All save paths are gated by `validateSkill` + Rust-side `skills_scan_security`.
</TLDR>
