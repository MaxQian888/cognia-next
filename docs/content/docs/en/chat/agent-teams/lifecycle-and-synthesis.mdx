...
<TLDR>
  The UI calls `agentTeamManager.start` (`lib/ai/agent/team/agent-team.ts:91-115`), which sets the team
  ...
  the ordered Squad bootstrap (`lib/agent-team/bootstrap.ts`, mounted by
  `SquadBootstrapInitializer`). Since ADR-0169 the UI never calls the lifecycle directly: every
  launch goes through `startSquadRun`, and every gate through the Action Review contract.
</TLDR>
...
## The manager facade — `lib/ai/agent/team/agent-team.ts`
...
```typescript
// lib/ai/agent/team/agent-team.ts:48-52
...
```typescript
// lib/ai/agent/team/agent-team.ts:91-115
start: async (id, opts) => {
  if (!configuredDeps) {
    throw new Error(
...
Deps are installed by the ordered Squad bootstrap (ADR-0169), after definitions have hydrated and before any recovery runs. A launch that arrives before the bootstrap flips ready waits on `awaitSquadRuntimeReady` and is refused with `runtime_not_ready` if it never does:

```ts
// lib/agent-team/bootstrap.ts (stages)
...
ready           the signal every startSquadRun call awaits
```
...
| 10 | `finally` — `unregisterTeamRunContext`, dispose subscriptions, settle the durable run record (unless it is parked as `needs_input` / `paused`) | `:500-523` |

Step 5 constructs the shared state objects passed to the executor through the run context:

```typescript
// lib/ai/agent/team/agent-team-runtime.ts:251-263 (per-run module construction)
...
```typescript
// lib/ai/agent/team/agent-team-runtime.ts:530-535
...
### Blocking lead review (ADR-0071)

When `team.config.taskReview.enabled`, the synthesizer emits a second node per
task — `action.team.task.review`, id `review:<taskId>` — and **repoints every
dependent edge onto it**:

```
  taskA ──▶ review:taskA ──▶ taskB ──▶ review:taskB
```

That indirection is the whole mechanism: an unapproved task's dependents are not
runnable, so the gate is enforced by the ready-set scheduler rather than by
downstream nodes remembering to check a flag. Off by default → the DAG is
exactly as it was.

| Detail | Behaviour |
| --- | --- |
| Dep satisfied outside this workflow (`satisfiedDependencyIds`) | No review node — a prior wave already ran and reviewed it |
| `params.dependencies` | Still carries **task** ids, not node ids, so the blackboard read is unchanged |
| `params.maxRevisions` | Frozen at synthesis, so a mid-run config edit can't change a budget the DAG was shaped by |
| Cycle detection | Unaffected — Kahn's runs over task ids, not node ids |

The executor asks the lead for a structured `{ verdict, feedback }`, showing it
the worker's output plus ≤64 KiB of deterministic diff evidence
(`review-evidence.ts`). `changes_requested` re-dispatches the **same** worker in
the **same** worktree with the feedback; approval sets the task `completed` (or
`review`, when the human `requireResultReview` gate also applies). Anything
unresolved — exhausted budget, missing worker, reviewer failure — fails the task
and the run. See [ADR-0071](../adr/0071-blocking-lead-task-review).
...
## Runtime deps wiring — `lib/ai/agent/team/agent-team-runtime-deps.ts`
...
```typescript
// lib/ai/agent/team/agent-team-runtime-deps.ts:107-125 (lead planning, trimmed)
...
```typescript
// lib/ai/agent/team/agent-team-runtime-deps.ts:133-165 (notifier deps, trimmed)
...
<Cards>
  ...
  <Card title="ADR-0071" href="../adr/0071-blocking-lead-task-review" description="The blocking lead review node, its diff evidence, and the revision loop." />
</Cards>
