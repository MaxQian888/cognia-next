...
- **Shared memory** — `lib/ai/agent/team/memory/shared-memory-orchestrator.ts` (207 lines): deep PII gate on every write, version-counter writes, fire-and-forget plugin-adapter mirror, local-version-wins reverse sync.
