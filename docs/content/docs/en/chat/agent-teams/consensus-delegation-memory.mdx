...
`createConsensus`, `castVote`, and `resolveConsensus` fire <Term>onConsensusOpened</Term>, <Term>onConsensusVoted</Term>, and <Term>onConsensusResolved</Term> respectively, letting plugins observe the lifecycle. The renderer consumer is the `ConsensusPanel` in the workspace Activity tab (see <LinkCard href="./workspace-ui" title="Workspace UI" description="Where ConsensusPanel renders" />).
...
<Cards>
  ...
  <Card href="./workspace-ui" title="Workspace UI" description="ConsensusPanel and the shared-memory view in the Activity tab." />
