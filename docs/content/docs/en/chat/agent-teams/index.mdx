...
Since [ADR-0022](../adr/0022-agent-team-runtime-hardening), a team does **not** have its own orchestrator. `runTeamLifecycle` (`lib/ai/agent/team/agent-team-runtime.ts:122-527`) is a ~400-line *synthesizer*: it turns the team + tasks into a `VisualWorkflow` and hands that to the one workflow orchestrator everything else already uses. There is no second engine to keep in sync, and team runs persist exactly like any other workflow run — as `workflowRuns` rows with `triggerKind: "team"`.

<TLDR>
  Clicking Run calls `agentTeamManager.start` (`lib/ai/agent/team/agent-team.ts:91`), which invokes
...
<StatGrid>
  ...
  <Stat label="Run-detail Squad tabs" value="3" hint="coordination · report · operations, on an /agent-runs team run" />
  <Stat label="Teammate runtimes" value="5" hint="claude · codex · claude-code · gemini-cli · cursor-cli" />
  <Stat label="Store persist version" value="8" hint="v8 moved squads / teammates / tasks out to Dexie" />
</StatGrid>
...
2. **Durable definitions, ephemeral run state.** From persist v8, squads, teammates and tasks are authoritative in Dexie (`agentTeams` / `agentTeammates` / `agentTeamTasks`), mirrored by `stores/agent/agent-team-store/dexie-bridge.ts`. The localStorage blob keeps only templates, the default config and a few view preferences. Messages, shared memory, consensus and delegations stay RAM-only, and run *history* persists as `agentTeamRuns` plus `workflowRuns` rows with `triggerKind: "team"`.
...
| **Durable definitions, ephemeral run state** | Squads live in Dexie from persist v8; run history persists as `workflowRuns` rows with `triggerKind` set to "team". | `stores/agent/agent-team-store/dexie-bridge.ts` · `lib/db/agent-team-definitions.ts` |
...
- [Data model](./data-model) — the `AgentTeam` / `AgentTeammate` / `AgentTeamTask` types, the Zustand store (persist v8) and the Dexie definition tables, plus what ADR-0140 removed.
...
- [Surfaces](./surfaces) — where a Squad is configured, run and watched after ADR-0140 dissolved the workspace: Settings, `/squads`, the `/agent-runs` Squad tabs, and the composer executor chip.
