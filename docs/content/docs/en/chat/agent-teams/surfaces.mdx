...
Deep links arrive as `?id=`, never `[id]`. This app ships as a static export consumed by Tauri and Capacitor, where dynamic segments do not exist at runtime. `use-squad-route-state.ts` owns four parameters: `?id=`, `?tab=` (one of `squads`, `runs`, `board`), `?q=` and `?filter=` (one of `all`, `waiting`, `live`). Defaults drop out of the URL, and every write is a `router.replace` with `scroll: false`.

The desktop console is a list pane, a centre with two tabs, and an inspector.

- **Runs** renders `AgentTeamCommandCenter`.
- **Board** renders `AgentTeamTasks` for the selected Squad, and an explicit empty state when none is selected.
- The **inspector** shows name and description, then `TeamRunControls` wired to six `agentTeamManager` calls (start, start with ultracode, pause, abort, resume, stop), then a "Configure this Squad" link back into Settings. These controls only ever existed on a tab of the retired workspace.
