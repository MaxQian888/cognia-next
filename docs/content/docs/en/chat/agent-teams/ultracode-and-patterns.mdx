...
<TLDR>
  `isUltracodeActive` (`lib/ai/agent/team/ultracode/ultracode-trigger.ts:15-31`) gates the run: override `force`/`off`
...
```typescript
// lib/ai/agent/team/ultracode/ultracode-trigger.ts:15-31
...
```typescript
// lib/ai/agent/team/ultracode/ultracode-planner.ts:34-62
...
```typescript
// lib/ai/agent/team/ultracode/synthesize-ultracode.ts:122-145
...
```typescript
// lib/ai/agent/team/agent-team-runtime.ts:413-429
...
```typescript
// lib/ai/agent/team/agent-team-runtime.ts:485-487 (ultracode only, on completion)
...
Ultracode is configured per Squad in the **Advanced governance** group of the Settings detail panel (`enabled`, `autoMode`, `skepticsPerFinding`, `judgesPerAttempt`, `maxFinderRounds`) — see [Surfaces](./surfaces). The **Run Ultracode** control in the `/squads` inspector forces a run on (`override: "force"`) regardless of `autoMode`.
