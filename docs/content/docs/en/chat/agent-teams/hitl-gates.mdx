...
<TLDR>
  ...
  (`lib/ai/agent/team/gates/squad-review-gate.ts`). That writes a run-level checkpoint, projects an
...
```ts
// lib/ai/agent/team/agent-team-runtime.ts:269-302
subs.push(
...
```ts
// lib/ai/agent/team/agent-team-runtime.ts:307-338
subs.push(
...
```ts
// lib/ai/agent/team/agent-team-runtime.ts:370-391
subs.push(
...
```ts
// lib/ai/agent/team/agent-team-runtime.ts:217-239
...
```ts
// lib/ai/agent/team/agent-team-runtime.ts:178-198
...
The dialog's handlers call `decidePendingGate(gate, decision)` (`lib/ai/agent/team/gates/decide-pending-gate.ts`), the one settle path shared with the island overlay: it resolves the bus waiter, removes the store entry and records the answer, so no surface can do one without the others:

```ts
// lib/ai/agent/team/gates/decide-pending-gate.ts
export function decidePendingGate(gate: PendingGate, decision: PendingGateDecision): boolean {
  ...
  // …plus a best-effort record of the answer in the run's conversation
  return delivered > 0
}
```
...
```ts
// lib/ai/agent/team/agent-team-runtime.ts:507-522
for (const u of subs) {
  try {
    u()
  } catch {
