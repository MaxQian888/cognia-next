...
The bundled profile is the default release profile: every app-owned JavaScript
process uses the checksum-pinned Node 26 runtime shipped with Cognia. For a
smaller installer, use `pnpm tauri:build:system-node`. That profile still ships
the sidecar code and `node_modules`, but excludes the Node executable. On first
launch it detects Node.js 26 or newer; if Node is missing or too old, Cognia
stays open and offers a direct link to the official installer. The matching
development commands are `pnpm tauri:dev:bundled` and
`pnpm tauri:dev:system-node`.

<Callout type="warn" title="Use the profile command">
  Do not invoke `tauri build --features system-node-runtime` directly. The
  package command also removes the bundled executable resource and its prepare
  step; the Cargo feature only selects runtime behavior.
</Callout>
...
The entire sidecar `node_modules/` is shipped in both profiles because the
sidecar runs as a separate Node process and needs its dependencies. The bundled
profile never falls back to a system executable; the lightweight profile never
falls back to the bundled runtime. This keeps agents, MCP helpers, webclone,
OAuth helpers, and JavaScript plugins on one validated runtime per process.
...
ghcr.io/maxqian888/cognia-web               # static export + Caddy front door (tls profile)
ghcr.io/maxqian888/cognia-workspace-runtime # ADR-0085 shared browser runtime
ghcr.io/maxqian888/cognia-collab            # ADR-0149 collaboration plane
ghcr.io/maxqian888/cognia-diagnostics       # ADR-0135 diagnostic service
ghcr.io/maxqian888/cognia-ops-controller    # ADR-0059 ops controller
ghcr.io/maxqian888/cognia-deploy-agent      # ADR-0059 deploy agent
```
...
- **Unified sign-in** — Logto brokers GitHub and Feishu as social connectors, and the collaboration server claims an empty deployment with a one-time bootstrap credential and mirrors memberships into Logto through an M2M app. Steps, the callback modes and credential rotation are in `deploy/compose/LOGTO.md` (ADR-0149).
...
- **Multiple instances** — one directory + one `.env` per instance; `COGNIA_INSTANCE` names the compose project, the T2 workspaces volume and the runner ownership label, and every host port is overridable. One live `cognia-server` per data volume: `serve` holds an exclusive lock on the volume, so scale by adding instances (or k8s tenants), never replicas. Details in the compose README.
- **Coherence gate** — `pnpm audit:deploy-suite` pins Dockerfile bases to the Rust/Node/pnpm pins, every compose default image to what `images.yml` publishes, and the multi-instance invariants above.
- Full architecture and the isolation/topology ladder: [ADR-0059](../adr/0059-cloud-deployment-headless-brain).

The headless host uses two deliberately different signaling URLs.
`COGNIA_SIGNALING_URL` is the host-internal rendezvous endpoint
(`ws://signaling:7892/signaling` in Compose and Kubernetes).
`COGNIA_PUBLIC_SIGNALING_URL` is the client-visible endpoint derived from the
deployment public URL (`https` becomes `wss`, with `/signaling`). Caddy and
the Kubernetes Ingress route that public path to the `signaling:7892` service;
all other application paths remain on `cognia-server`.

### Browser Companion security

The production Web client accepts Companion endpoints only through:

- publicly trusted HTTPS;
- HTTPS signed by an administrator-managed local CA that the browser already trusts; or
- encrypted WebRTC, direct or through TURN.

The browser cannot install or bypass a local CA automatically. Install the CA in the operating system/browser trust store first, then verify the certificate chain in the browser before pairing. Configure the Companion server with the matching certificate and private key. Plain HTTP is rejected in production; it is available only in a non-production build with `NEXT_PUBLIC_ALLOW_INSECURE_COMPANION_HTTP=1`, displays an insecure-mode warning, and must not be used for high-risk control.

Authorize Web origins by exact HTTPS origin (`scheme://host[:port]`). Do not use `*` on authenticated endpoints. Keep the allowed methods and headers narrow, handle preflight explicitly, and permit Private Network Access only for the exact administrator-approved origins. A browser connection failure should be diagnosed separately as certificate trust, origin/CORS, PNA, expired or revoked JWT, missing TURN, or an offline host.

One browser account can retain multiple encrypted Companion targets, but only one target is active. Provider keys plus device and signaling private keys live in the browser Vault; OIDC and five-minute DPoP access tokens remain memory-only. The public target book contains connection metadata only. Each target also owns a separate IndexedDB database and outbound queue scope.
...
| `images.yml`           | tag `v*` / manual / PR on container inputs      | Builds+publishes the GHCR images above; PRs get a `cargo check` compile gate for `cognia-server`     |
