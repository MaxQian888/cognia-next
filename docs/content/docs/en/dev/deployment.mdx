...
Cognia's repo runs the Tauri build matrix via `release.yml` → `build-tauri.yml` (OS runners, installers uploaded to a draft release). Updater signing is live through the `TAURI_SIGNING_*` secrets; OS code-signing (SmartScreen/notarization) remains opt-in per platform as above.
