...
One browser account can retain multiple encrypted Companion targets, but only one target is active. Provider keys, device JWTs, OIDC tokens, and signaling private material live in the browser Vault; the public target book contains connection metadata only. Each target also owns a separate IndexedDB database and outbound queue scope.
