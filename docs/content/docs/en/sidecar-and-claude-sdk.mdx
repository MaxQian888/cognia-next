...
<Callout type="warn" title="Never plaintext">
  The Rust side never reads `~/.claude/` directly — it just spawns the sidecar
  with the right env. API keys entered in the UI are stored in `ApiKeyState`
  (memory) and the OS keyring (`keyring` crate); they are passed to the sidecar
  via env at spawn time, **never written to a config file**.
</Callout>
...
<Cards>
  <Card title="Chat & Skills" href="./chat-and-skills" description="The user-facing surfaces that call claude_send." />
  <Card title="Plugin system" href="./plugin-system" description="How plugins extend the sidecar via MCP." />
