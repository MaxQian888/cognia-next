...
<TLDR>
  ...
  <Kbd>pnpm tauri dev</Kbd> for desktop. Tests live next to source with a
  ≥90% coverage gate. Pre-commit hooks (Husky + lint-staged + commitlint) keep
  the tree clean.
</TLDR>
...
<Files>
  <Folder name="cognia-next" defaultOpen>
    <Folder name="app">
      ...
      <Folder name="twin"><File name="page.tsx" /></Folder>
      <Folder name="scheduler"><File name="page.tsx" /></Folder>
...
<Callout type="warn" title="The hook rejects malformed messages">
  The commit-msg hook rejects messages that don't match Conventional Commits.
  If you need to bypass during a rebase, prefer `git commit --amend` (which
  re-runs the hook) over `--no-verify`.
</Callout>
...
<Cards>
  ...
  <Card title="Architecture" href="./architecture" description="The layered design that informs all conventions." />
</Cards>
