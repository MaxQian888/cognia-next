...
description: Capacitor 8 mobile shell reuses Next.js static export, Transport tri-state selection, JWT pairing, SecureStorage, TLS pinning, mDNS discovery, and native plugin bridge.
---

# Mobile Architecture

cognia-next's mobile client is **not** a standalone React Native app, nor is it a stripped-down desktop version. It is the **same** Next.js static export (`out/`), wrapped by **Capacitor 8** inside an iOS / Android native WebView, then connected back to the desktop's `/api/v1/*` via HTTP/WS. One JS bundle, three shells (Tauri, Capacitor, Web), one shared React codebase.
...
- Mobile client = **Capacitor 8 + the same `out/` static export**; no UI rewrite, no parallel component library.
...
| **Capacitor 8**    | Drop the existing Next.js static export into a native WebView — zero UI rewrite.                                | Adopted  |
...
`compileSdk` / `targetSdk` = 35 (Capacitor 8 default), `minSdk` = 24, debug-only `usesCleartextTraffic="true"` only in `app/src/debug/AndroidManifest.xml`; release builds use the default security policy.
...
<Cards>
  <Card title="Companion API" href="../subsystems/companion-api" description="Server side: RPC routes, WS protocol, push, JWT issuance" />
  <Card title="Platform Connectors" href="../subsystems/platform-connectors" description="Desktop-side adapters, coordinated with mobile push" />
  <Card title="Remote Control" href="../subsystems/remote-control" description="Another 'client' semantics: desktop lends a session to another endpoint" />
  <Card title="Settings UI" href="./settings-ui" description="Mobile settings panel lives under the same settings-shell framework" />
</Cards>
