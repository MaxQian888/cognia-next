...
| --- | --- | --- |
| **simplified** | One-line rows (`ToolCallRow`): icon + tool name + concise target + status glyph, expandable on click. | Collapsed by default. |
...
Runs of **≥2 consecutive tool calls** collapse into a single foldable **activity group** (`ToolActivityGroup`) — e.g. "5 tool calls ✓" — with an aggregate status glyph and an **expand-all / collapse-all** control. Segmentation is a pure function (`lib/chat/agent-flow-grouping.ts:groupAgentParts`): sub-agent parts are transparent (they render once as a dispatch tree), `TodoWrite` is excluded (it renders as a plan), and a lone tool call stays a single card (no group chrome). In simplified mode the group owns each row's open state; in standard/detailed it delegates to `renderCard` with an optional `forceOpen` override.
...
New tool cards/rows, sub-agent nodes, and dispatch banners get a one-shot entrance (fade + small translateY) and the activity group animates its height on expand/collapse, via `components/chat/motion/motion-reveal.tsx` (`MotionReveal` / `useFlowMotion`). Motion is GPU-friendly (only `opacity` + `transform`, no `backdrop-filter`) and honours the appearance `motion.reduce` flag and OS `prefers-reduced-motion`, scaling duration by `motion.speed`.
