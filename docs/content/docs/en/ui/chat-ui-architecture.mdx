...
- **Agent-flow display toggle** — `agent-flow-display-toggle.tsx`, a 3-way `ToggleGroup` (simplified / standard / detailed) driving the global agent-flow display mode (see [Agent-flow display modes](#agent-flow-display-modes)).
...
- **Two entry points, one value** — the chat-header quick toggle (`components/chat/agent-flow-display-toggle.tsx`, a 3-way `ToggleGroup`) and the appearance settings card (`components/settings/appearance/components/agent-flow-card.tsx`) both drive the same preference, so switching from either place applies everywhere.
