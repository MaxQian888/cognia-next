...
`CompanionConfig` (`companion-storage.ts`) includes `{ baseUrl, deviceJwt, deviceId, serverVersion,
serverFingerprint?, rendezvousId?, signalingRoomDescriptor?, signalingPrivateKey? }`. The `CompanionConfigStorage` interface
