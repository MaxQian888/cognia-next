...
<TLDR>
  ...
  HTTP directly. Desktop binds `TauriTransport`; web and Capacitor rebind to `CompanionTransport` when
  the account's active runtime target is a paired Host, otherwise web stays on its standalone target.
  `CompanionTransport` (`transport-companion.ts`) POSTs to `/api/_rpc/<name>` with a five-minute
  P-256 DPoP-bound access token and opens `/ws/events` with a single-use socket ticket plus `?since=`
  replay. It transparently upgrades to a **WebRTC DataChannel** sub-tier (`transport-rtc.ts`) and falls
  back to HTTPS/WS on failure. Private keys live only in OS secure storage or the Browser Vault.
</TLDR>
...
// Desktop starts on Tauri IPC. Web/Capacitor activation resolves the account's
// active RuntimeTarget and atomically rebinds the shared transport singleton.
// A companion target loads only encrypted credentials for that exact Host.
```

`RuntimeTargetRegistry` keeps targets account-scoped and gives every target a separate Dexie database.
Activation stops old subscriptions, switches the database, rebinds the transport, then refreshes the
manifest and sync cursor; failure rolls back the active pointer. WebRTC remains a sub-tier inside
`CompanionTransport`, not a separate application transport.

| Implementation | `call` | `subscribe` |
| --- | --- | --- |
| `TauriTransport` (`transport-tauri.ts`) | `@tauri-apps/api` `invoke` | `listen` → unsubscribe |
| `CompanionTransport` (`transport-companion.ts`) | `POST {baseUrl}/api/_rpc/<name>` + DPoP | `GET {baseUrl}/ws/events` + single-use ticket |
| Standalone web target | local browser-capable runtime | local event sources |
...
opens the WebSocket if none exists (`:356`). `openWebSocket` computes `since` as the highest
seq across channels, mints a socket ticket through `/api/auth/socket-ticket`, rewrites `https→wss`,
and connects to `{wsBase}/ws/events?ticket=<single-use>&since=<seq>`. `handleWsMessage` handles
...
`CompanionConfig` (`companion-storage.ts`) includes the target URL/identity, `deviceId`, server
version/fingerprint, signaling URL/ICE metadata, and runtime references to the device/signaling keys.
The storage adapters deliberately split public target metadata from secret material:

- **Browser Vault + target book** — public fields may enter the target book; device and signaling
  private keys are encrypted/non-extractable Vault material and never enter `localStorage`.
...
Reads hydrate the exact active target before binding `CompanionTransport`. DPoP access tokens are
minted for five minutes and refreshed from the stored P-256 key; they are not durable credentials.
Registration commits Vault, target book, and runtime target as one rollback-safe operation. The config is **never** written to
...
<Cards>
  ...
  <Card title="Discovery & pairing" href="./discovery-pairing" description="How canonical target metadata and encrypted device/signaling keys are registered." />
</Cards>
