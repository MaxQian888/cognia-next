...
`services/signaling-server/` is a Rust signaling relay with two interchangeable backends sharing the
...
`services/signaling-server/src/` is the equivalent single-process server for self-hosting: `router()`
