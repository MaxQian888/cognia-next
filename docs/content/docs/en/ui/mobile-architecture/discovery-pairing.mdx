...
<TLDR>
  ...
  probe, accepting a `401` from `/api/whoami` as proof-of-Cognia), while `connection-strategy.ts`
  *resolves a known* paired device's reachable URL by walking a prioritized candidate list (mDNS →
  tunnel → cached) and probing `/healthz`. Trust is the canonical pairing wizard: scan or paste a
  `cgnp3` payload, validate Host/tenant/expiry/version, register independent P-256 device and signaling
  keys with an Owner invitation or OIDC session, and atomically persist the resulting public target plus
  encrypted private keys. The local IP needed for scanning is recovered via a WebRTC ICE
...
<StatGrid>
  ...
  <Stat label="Probe target" value="GET /api/whoami → 401" hint="lan-scanner.ts:357" />
...
flowchart TB
  ...
  PROBE{"GET ip:port/api/whoami<br/>== 401 ?  :357,:416"}
...
(`evaluateProbeResponse`, `:409-432`) — an unauthenticated request to `/api/whoami` *should* be
...
<Step>
**Pair** — `pair-step.tsx`. Scan or paste the canonical payload (`barcode.ts` → strict parser). The
submit calls `pair-api.ts`, reads `GET /api/auth/config`, obtains an Owner invitation or completes the
Logto popup PKCE session, and submits independent device-auth and signaling public keys to
`POST /api/auth/device/register`. The response carries `deviceId`, `tenantId`, `role`,
`serverVersion`, `rendezvousId`, and `roomDescriptor`. Private keys enter only the OS secure store or
Browser Vault; registration failure leaves neither a target row nor plaintext key material.
</Step>
<Step>
...
The QR encodes canonical `base`, `host`, `tenant`, `exp`, `ver`, `fp`, and `mode` fields; only
`owner-invitation` carries a one-time invitation. The client rejects expired, Host-mismatched, and
older payloads and stores the fingerprint in `CompanionConfig.serverFingerprint`. From then on every request goes through
