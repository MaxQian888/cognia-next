...
code. The submit calls `pair-api.ts`, which generates the mobile P-256 role key and redeems against
`POST /api/v1/auth/pair` (QR) or `/api/v1/auth/pair/redeem-code` (code), receiving
`{ deviceJwt, deviceId, serverVersion, rendezvousId, roomDescriptor, fingerprint }`.
Validation lives in `pair-helpers.ts`; the private key is stored separately and never enters
the public pairing row.
</Step>
<Step>
