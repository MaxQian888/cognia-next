...
> `components/mobile/settings/mobile-settings-panel.tsx` is a Wave-2.7 leftover,
> not the mobile settings shell. Its pieces were extracted into
> `components/mobile/me/`, leaving the panel with zero consumers — it is tracked
> in `scripts/gates/unreachable-component-baseline.json`. Do not build on it.
