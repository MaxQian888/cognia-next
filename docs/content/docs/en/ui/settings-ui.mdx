...
Entries with `desktopOnly: true` (e.g. `subscription`, `ccswitch`, `hooks`, `tools`, `connections`, `webhooks`, `companion`, `desktop`) are automatically filtered out by the sidebar when `usePlatform() === "web" | "mobile"`. This is a single toggle; the section itself does not need to repeat the check.
