...
interface NavItem {
  ...
  requires?: readonly CapabilityId[] // capabilities the section administers
  profiles?: readonly HostProfile[] // local-shell surfaces only (mirrors CapabilityGate)
}
```

Reachability is capability-keyed, not host-keyed (ADR-0059 D7 / F5). An entry declares the capabilities it administers in `requires` (e.g. `terminal` → `pty`, `subscription` → `keyring`, `connections` → `connector-runtime`, `automation` → `uia-automation`); `isSettingsSectionReachable` checks them with `capabilityAvailable` semantics, so a browser paired to a cloud brain sees the sections whose backend runs on that brain, and a web-standalone client sees only what runs in the webview. `profiles` is reserved for surfaces bound to the local desktop shell itself (`desktop`, `sidebar`, `companion`, `remote-hosts`) and, transitionally, for sections whose renderer path still bypasses the transport seam (`ccswitch`, `hooks`, `fleet` — each pin names the host-parity Class A file it waits on). The sidebar, the ⌘K finder and the shell's `?section=` dispatch all consume the same `useSettingsSectionReachability()` answer, so a hidden section cannot be reached through a deep link; `pnpm audit:host-parity` fails on any new `desktopOnly: true`.
...
| `system`        | System-level integrations         | webhooks, external-bridge, companion, network, desktop, about       |
...
The phone does **not** render `/settings` at all. Its settings surface is `/me` — a
separate, data-driven iOS-style list:

- `components/mobile/me/me-entries.ts` is the registry: one typed entry per row
  (icon, i18n label key, href, group, search keywords), driving ~48 `/me/*`
  sub-pages plus search and favorites.
- Per ADR-0056 D3, a desktop section that is purely settings-store-driven is
  **embedded directly** into a `/me/*` `SubPageShell` rather than rewritten —
  `/me/appearance`, `/me/canvas`, and `/me/eval` all do this. Sections that need
  Tauri are marked `desktopOnly` in `SETTINGS_NAV` and filtered out by
  `settings-sidebar.tsx` off-desktop.
- Mobile chrome (status bar / safe area / tab bar) is handled by
  `mobile-shell-wrapper`; the `/me` pages do not repeat that concern.

> `components/mobile/settings/mobile-settings-panel.tsx` was a Wave-2.7
> leftover, not the mobile settings shell. Its pieces had been extracted into
> `components/mobile/me/`, leaving the panel with zero consumers, and it has now
> been deleted along with its entry in
> `scripts/gates/unreachable-component-baseline.json`. `components/mobile/me/`
> is the mobile settings surface.
...
<Cards>
  <Card title="Provider System" href="../chat/provider-system" description="70+ AI providers behind ?section=providers" />
  <Card title="Platform Connectors" href="../subsystems/platform-connectors" description="Connector adapters under ?section=connections" />
  <Card title="Backup & Data" href="../data/backup-and-data" description="Backup/restore/domain transfer/maintenance under ?section=data" />
  <Card title="Visual Workflows" href="../subsystems/visual-workflows" description="Library / runs / templates / audit under ?section=workflows" />
  <Card title="Skills Subsystem" href="../subsystems/skills" description="Skill library management under ?section=skills" />
  <Card title="Character Management" href="../chat/chat-and-skills" description="Character editing under ?section=characters" />
  <Card title="MCP Servers" href="../subsystems/misc-subsystems" description="Local and remote MCP registration under ?section=mcp" />
  <Card title="A2UI" href="../subsystems/a2ui" description="A2UI tool panel under ?section=a2ui" />
  <Card title="Plugin System" href="../subsystems/plugin-system" description="Local plugin installation under ?section=plugins" />
  <Card title="Scheduler" href="../subsystems/scheduler" description="Cron and history under ?section=scheduled-tasks" />
  <Card title="External Bridge" href="../subsystems/external-bridge" description="MCP server external exposure under ?section=external-bridge" />
  <Card title="Webhooks" href="/settings?section=webhooks" description="Standard Webhooks signing and outbound endpoints" />
  <Card title="Companion API" href="../subsystems/companion-api" description="LAN bind / pair JWT management under ?section=companion" />
  <Card title="Claude Subscription OAuth" href="../chat/claude-subscription-oauth" description="Subscription login and quotas under ?section=subscription" />
  <Card
    title="CCSwitch"
    href="../chat/ccswitch"
  ...
  <Card
    ...
    href="../chat/slash-commands"
    description="Registry under ?section=slash-commands"
  />
  <Card
    title="Speech"
    href="../subsystems/misc-subsystems"
    description="TTS / STT configuration under ?section=speech"
  />
  <Card title="Logs" href="../subsystems/observability" description="?section=logs and ?section=diagnostics" />
</Cards>
