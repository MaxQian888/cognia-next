...
<TLDR>
  ...
  table `MODULE_BRIDGE_CAPABILITIES` (`module-bridge-map.ts:146`), which gates each entry on
  `manifest[manifestField]?.length` and normalizes 20 field-driven entries behind one await loop
  (`:126-145`).
...
<StatGrid>
  <Stat label="Bridge files" value="34" hint="lib/plugin/bridge/ TS, excl. tests" />
  <Stat label="Async dispatch entries" value="20" hint="MODULE_BRIDGE_CAPABILITIES (module-bridge-map.ts:146)" />
...
`MODULE_BRIDGE_CAPABILITIES` (`module-bridge-map.ts:146`), gating each entry on
`manifest[manifestField]?.length` and normalizing all 20 field-driven entries behind a single await
loop (`:126-145`). VS Code-style bridges (grammars, icons, languages, snippets) are *not* in this table —
...
| `routing-strategies-bridge.ts` | `manifest.routingStrategies[]` | provider-routing strategy registry | `registerRoutingStrategiesForPlugin` `:46`, `unregisterRoutingStrategiesForPlugin` `:114` |
| `deployment-filters-bridge.ts` | `manifest.deploymentFilters[]` | provider-routing deployment-filter registry | `registerDeploymentFiltersForPlugin` `:47`, `unregisterDeploymentFiltersForPlugin` `:115` |
| `protocol-adapters-bridge.ts` | `manifest.protocolAdapters[]` | renderer protocol-adapter registry + code adapter executors | `registerProtocolAdaptersForPlugin` `:72`, `unregisterProtocolAdaptersForPlugin` `:133` |
| `external-agent-adapters-bridge.ts` | `manifest.externalAgentAdapters[]` | external-agent protocol adapter registry | `registerExternalAgentAdaptersForPlugin` `:55`, `unregisterExternalAgentAdaptersForPlugin` `:148` |
| `context-providers-bridge.ts` | `manifest.contextProviders[]` | agent context-provider registry | `registerContextProvidersForPlugin` `:47`, `unregisterContextProvidersForPlugin` `:112` |
| `tool-routes-bridge.ts` | `manifest.toolRoutes[]` | Dexie `toolRoutes` rows (source `manifest`) | `registerToolRoutesForPlugin` `:19`, `unregisterToolRoutesForPlugin` `:68` |
...
| `view-bridge.ts` | `manifest.views[]` | tree/custom view registry | `registerViewsForPlugin` `:44`, `unregisterViewsForPlugin` `:119` |
| `plugin-webview-bridge.ts` | `manifest.webviews[]` | sandboxed webview registry | `registerWebviewsForPlugin` `:41`, `unregisterWebviewsForPlugin` `:112` |
...
flowchart LR
  ...
  MAP["MODULE_BRIDGE_CAPABILITIES<br/>module-bridge-map.ts:146"]
  ...
  subgraph UITHEME["UI and theming"]
    ...
    VIEW["view-bridge.ts:44"]
    WEBV["plugin-webview-bridge.ts:41"]
  end
  ...
  subgraph CHATAI["Chat and AI"]
    ...
    CTP["context-providers-bridge.ts:47"]
    TLR["tool-routes-bridge.ts:19"]
  end

  subgraph PLATCON["Platform and connectors"]
    CON["connectors-bridge.ts:61"]
    WBK["workspace-backend-bridge.ts:44"]
    RST["routing-strategies-bridge.ts:46"]
    DPF["deployment-filters-bridge.ts:47"]
    PAD["protocol-adapters-bridge.ts:72"]
    EAA["external-agent-adapters-bridge.ts:55"]
  end
  ...
  RST --> RROUTE["provider routing"]
  DPF --> RROUTE
  PAD --> RPROTO["protocol adapters"]
  EAA --> REXT["external-agent protocols"]
  CTP --> RCTX["context providers"]
  TLR --> RTOOLS["tool route rows"]
  ...
  class REG src
  class MAP,KP table
  class THM,FNT,WLP,ICO,CFG,MOD,VIEW,WEBV,GRM,LNG,SNP,TCM,TLB,STB,AGI,AIP,OCR,A2U,CMW,MRR,CTP,TLR,CON,WBK,RST,DPF,PAD,EAA,SCH,TRG,WFI bridge
  class RTHEME,RFONT,RBUS,RROUTE,RPROTO,REXT,RCTX,RTOOLS,RAIP,RMCP,RHOOK,RTASK,RGRM host
```
...
- **Two dispatch tables** — `module-bridge-map.ts` (20 field-driven module bridges) versus
  `capability-bridge-map.ts` (16 synchronous inline-def overlay capabilities). The VS Code-style storage
  bridges (grammars, icons, languages, snippets) are in neither — they are driven by
  `vscode-loader`/`context.ts`.
- **Field-driven gating + canonical capability tags** — dispatch gates on
  `manifest[manifestField]?.length`; field-driven bridges such as `workspace-backend`,
  `message-renderer`, `chat-middleware`, `modal-mount`, and `density-preset` now have explicit
  capability contracts and SDK helpers (`module-bridge-map.ts:146`).
