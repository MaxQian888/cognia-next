...
- **Security.** The manager registers each plugin with `getPermissionGuard().registerPlugin`
  ...
  inside `context.ts`.
- **Bridges.** `registerPluginContributions` (`manager.ts:2404`) registers modes/commands
  (`manager.ts:2418`, `:2437`), slash commands (`manager.ts:2302`), themes via `PluginThemesBridge`
  (`manager.ts:2450`), the four `OVERLAY_REGISTRY_CAPABILITIES` (`manager.ts:2482`), the seven
  `MODULE_BRIDGE_CAPABILITIES` (`manager.ts:2511`), LSP servers (`manager.ts:2540`), and
  skill→character attach (`manager.ts:2560`).
