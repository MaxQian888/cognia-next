...
That is `sandbox.ts:223-241`. `executeAsync` (`:246`) wraps the body in an `AsyncFunction` and races it against a timeout (`:262-267`). (This page describes a `sandbox.ts` that is no longer in the tree; its `executeCode` delegate `lib/sandbox/web` has been removed. Browser code execution is `lib/native/code-execution-strategy.ts` — see [Sandbox System](../sandbox/microvm-and-web).)
...
| `UNAVAILABLE` | Host dependency not ready (encrypted secret store initializing or locked; `details.reason`) | no |
