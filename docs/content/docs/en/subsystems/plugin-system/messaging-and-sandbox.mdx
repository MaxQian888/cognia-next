...
The `IPCMessage` envelope (`ipc.ts:63`) is `{id, type, channel, senderId, targetId?, payload, timestamp, replyTo?}`, where `type` is one of four kinds: `request | response | broadcast | event`. A `pendingRequests` `Map<requestId, ResponseHandler>` tracks in-flight calls. `sendAndWait` (`:231`) registers under `requestId` with a timeout timer and an `AbortSignal` (`:257-272`); `respond` (`:298`) resolves by `requestId` (`:306`).
