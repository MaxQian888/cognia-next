...
<TLDR>
`lib/plugin/contracts/` is the declarative truth layer: `plugin-points.ts:979` and `plugin-capabilities.ts:61` catalog every extension point and capability with binding/docs/requiredTests proof fields, `capability-bridge-map.ts:156` and `module-bridge-map.ts:118` wire manifest fields to registries/bridges, and `runtime-proof-audit.ts:48` asserts every "implemented" surface has evidence. `lib/plugin/registries/` are typed overlay-registry stores (builtin ← plugin overlay, deduped by id); `lib/plugin/sdk/` are pure `define*()` type pass-throughs the host never consumes (`sdk/index.ts:8`).
</TLDR>

<StatGrid>
  <Stat label="Capability contracts" value="33" />
  <Stat label="UI slots / hooks / activations" value="49 / ~150 / 26" />
  <Stat label="Runtime points" value="11" />
  <Stat label="Sync overlay capabilities" value="9" />
  <Stat label="Async module bridges" value="13" />
  <Stat label="SDK define*() helpers" value="7" />
</StatGrid>
...
- `plugin-capabilities.ts` — the 33 `PluginCapability` contracts.
...
`lib/plugin/registries/` are typed in-memory stores built on the `createOverlayRegistry` substrate (builtin ← plugin overlay, deduped by id, plugin-tagged). `lib/plugin/sdk/` are author-facing `define*()` helpers — pure type pass-throughs (`define-skill.ts:23`, `define-subagent.ts:22`) that narrow inline manifest entries; the host never consumes them (`sdk/index.ts:8`).
...
The bridge maps are **not** keyed by point-id; they are keyed by manifest field / capability tag and reference the same registry singletons the runtime-point bindings name. `capability-bridge-map.ts` (`OVERLAY_REGISTRY_CAPABILITIES` `:156-303`) holds the 9 synchronous overlay capabilities with a uniform `register(id, def, { pluginId })` / `unregisterByPlugin(pluginId)` shape; `defineOverlayCapability<E>` is the single variance bridge (`:144-148`). `module-bridge-map.ts` (`MODULE_BRIDGE_CAPABILITIES` `:118-277`) holds the 13 asynchronous bridges, each normalized to `register(ctx) => Promise<void>` / `unregister(pluginId)` (`:97-110`). Both expose their `*_CAPABILITY_KEYS` via `Object.keys`.

## Provable contribution

`auditPluginPointContracts()` walks every contract; for `status === "implemented"` it asserts `binding`, `docs`, and `requiredTests` are non-empty, yielding a `proofStatus` of `"verified" | "missing_proof" | "not_applicable"` (`plugin-points.ts:990-1025`). `auditPluginCapabilityContracts()` does the same for the 33 capabilities, additionally requiring `builtinContributionPaths` for tools / commands / hooks (`plugin-capabilities.ts:826-905`). `runtime-proof-audit.ts:auditPluginRuntimeClaims()` composes both, plus hand-authored `RUNTIME_RISK_AUDITS`, into one `PluginRuntimeClaimsAuditReport` (`runtime-proof-audit.ts:48-54`).
...
| `shared-memory-adapter-registry.ts` | `PluginSharedMemoryAdapterDef` (plain) | (none) | `registerSharedMemoryAdapter :24`, `unregisterSharedMemoryAdaptersByPlugin :28` |
...
All helpers are pure type pass-throughs **except** `defineCharacterPack`, which validates eagerly.

| Helper | Produces | :line |
| --- | --- | --- |
...
| `define-agent-team-template.ts` | `PluginAgentTeamTemplateDef` (`requires` block) | `:27` |
| `define-workflow-template.ts` | `PluginWorkflowTemplateDef` (`requires` block) | `:24` |
| `define-character-pack.ts` | `PluginCharacterPackDef` — throws on empty / over-50 / duplicate-localId; warns on unknown TTS / oversized avatar | `:46` (limits `:35`, `:44`) |
| `index.ts` | barrel of all 7 | `:16-22` |
