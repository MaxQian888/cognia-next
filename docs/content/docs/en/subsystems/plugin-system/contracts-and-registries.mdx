...
- `plugin-capabilities.ts` — the 72 `PluginCapability` contracts.
...
`PLUGIN_CAPABILITY_CONTRACTS` (`plugin-capabilities.ts:77`) catalogs the 72 `PluginCapability` contracts — the manifest-level features a plugin may declare. Each entry carries `id`, a `support` level (`"supported" | "partial" | "experimental" | "blocked"`), the `manifestFields` it activates, a `runtimeBinding` summary, `hostBindings` / `typescriptSdk` / `pythonSdk` proof paths, `docs`, and `requiredTests` (`plugin-capabilities.ts:8-49`). 68 are `supported`; the 4 `experimental` entries are documented but exempt from the proof audit.

`command-hooks` is the newest entry (`plugin-capabilities.ts:277-301`): declaring it merges a settings-shaped `hooks` block under user hooks and above built-ins on every rail — CLI (`cli/src/hooks/plugin-hooks.ts`), desktop (`src-tauri/src/hooks/plugin.rs`), and the ecosystem converter (`lib/plugin/convert/ecosystem.ts`). SDK: `defineCommandHooks` (`packages/plugin-sdk/src/define/define-command-hooks.ts`).

## Provable contribution

`auditPluginPointContracts()` walks every contract; for `status === "implemented"` it asserts `binding`, `docs`, and `requiredTests` are non-empty, yielding a `proofStatus` of `"verified" | "missing_proof" | "not_applicable"`. `auditPluginCapabilityContracts()` does the same across all 72 capability contracts — proof is required while `support === "supported"` — additionally requiring `builtinContributionPaths` for tools / commands / hooks. `runtime-proof-audit.ts:auditPluginRuntimeClaims()` composes both, plus hand-authored `RUNTIME_RISK_AUDITS`, into one `PluginRuntimeClaimsAuditReport`.
...
- **`ctx.pet`** (capability `pet`; permissions `pet:read` / `pet:interact`, `lib/plugin/api/pet-api.ts`) — read the PII-safe pet summary, subscribe to sanitized events, emit nurture interactions, and grant XP/coin rewards. Rewards are rate-limited (token buckets `pet:interact` / `pet:emit`) and clamped against a per-plugin daily budget (50 XP / 100 coins, `lib/pet/access/reward-budget.ts`); lifecycle kinds are not emittable. Availability, the kind whitelist, the burst bucket, item ownership and the budget are all decided by the shared pet access gate (`lib/pet/access/gate.ts`), which the command registry and the agent tools call too, so `ctx.pet` is a thin adapter over the same rules rather than the only path that enforces them.
