...
<StatGrid>
  ...
  <Stat label="Async module bridges" value="20" />
...
- `plugin-capabilities.ts` — the 59 `PluginCapability` contracts.
...
`lib/plugin/registries/` are typed in-memory stores built on the `createOverlayRegistry` substrate (builtin ← plugin overlay, deduped by id, plugin-tagged). `packages/plugin-sdk/src/define/` contains author-facing `define*()` helpers that narrow inline manifest entries. The host runtime consumes manifests and registries, not the helper functions; the helpers are public authoring affordances exposed by the `@cognia/plugin-sdk` package root and `@cognia/plugin-sdk/define/<name>`. Tooling can import the read-only contract catalogs and contribution-summary helpers from `@cognia/plugin-sdk/contracts` instead of reaching into `lib/plugin/contracts/*`.
...
All helpers are pure type pass-throughs **except** `defineCharacterPack`, which validates eagerly. The package exposes every helper from the root barrel and as a direct subpath: `@cognia/plugin-sdk/define/<name>` maps to `packages/plugin-sdk/src/define/define-<name>.ts`.
