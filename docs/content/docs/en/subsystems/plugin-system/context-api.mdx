...
| `ctx.ui` | `context.ts:456` | toasts / dialogs | no | `showToast` (→ sonner), `showDialog` / `showInputDialog` / `showConfirmDialog` (→ plugin modal stack) |
...
| `ctx.logs` | `logs-api.ts` | IndexedDB log store + `agentTraces` Dexie table | `logs:read` (logs, stats, transports) / `trace:read` (spans) — read-only | `query`, `stats`, `subscribe`, `transports`, `traces.list`, `traces.timeline`, `traces.subscribe` |
