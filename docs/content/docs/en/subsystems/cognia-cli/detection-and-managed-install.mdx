...
<TLDR>
  ...
  (`src-tauri/src/cli_bridge/detect.rs:186-216`). The managed download
  (`download_cognia_cli`, `download.rs:197-310`) resolves the platform triple, fetches
...
The Rust side (`detect.rs:186-216`):
...
2. `find_in_managed_dirs(name)` — LIFO over directories registered by past downloads
   (`register_managed_dir`, `detect.rs:61-66`; the snapshot also feeds PATH injection for
...
4. cache the result; `invalidate("cognia")` drops it after a download (`download.rs:301`)
