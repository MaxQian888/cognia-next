---
title: Bridge server
description: The desktop side of the loopback bridge — axum on an ephemeral 127.0.0.1 port, per-launch 64-hex dev token with constant-time comparison, the seven routes, zip-slip-safe bundle install, symlink-skipping directory install, session handoff, and fire-and-forget Tauri events into the renderer.
---
...
<TLDR>
  ...
  **`127.0.0.1:0`** (OS-assigned ephemeral port), write `cli-endpoint.json` (`0o600` on Unix),
  ...
  exit (`lib.rs:1249-1251`). Every request passes one middleware (`server.rs:88-115`):
  ...
  directly; plugin and session handlers notify the renderer via fire-and-forget Tauri events.
</TLDR>
