...
<TLDR>
  ...
  utilities cover `cognia acp`, `cognia host`, `cognia logs`, `cognia crash`, `cognia pack`,
  `cognia status`, `cognia open`, `cognia completions`, `cognia release-key`, and `cognia release-verify`.
</TLDR>

<StatGrid>
  <Stat label="Plugin commands" value="19" hint="PluginCommand enum — crates/cognia-cli/src/cli.rs" />
  ...
  <Stat label="Bridge routes / cognia routes" value="22 / 8" hint="shared server total / Rust CLI consumer set — health overlaps cognia-agent" />
  <Stat label="Rust source" value="~23.5k lines" hint="crates/cognia-cli/src (57 files) + src-tauri/src/cli_bridge" />
...
flowchart TB
  ...
  subgraph BRIDGE["2 · loopback bridge — src-tauri/src/cli_bridge"]
    ...
    AUTH --> H[handlers.rs<br/>12 shared routes<br/>plugins · ACP · handoff · twin · teams]
  end
...
src-tauri/src/cli_bridge/           # desktop side
  ...
  handlers.rs                       # 12 shared routes: plugins/ACP/handoff/twin/teams + renderer bridging
...
<Callout type="info" title="The bridge is not the companion API">
  ...
  mobile-pairing companion API (default port 27890, JWT auth). Neither has LAN/WAN reach for plugin
