---
title: Overview
description: The cognia plugin-author CLI — a standalone Rust binary (scaffold / lint / build / sign / verify / info / status / install / list / reload / dev) that talks to a running desktop instance over a token-gated loopback axum bridge, with renderer-side detection, managed download, and a DevTools UI surface.
---
...
<TLDR>
  `cognia` is a separate Rust crate (`crates/cognia-cli/`, ~8.0k lines + UI layer) shipping **14
  subcommands** under `cognia plugin …` — scaffold (`new`), validate (`lint`, 60 diagnostic codes), build
...
<StatGrid>
  <Stat label="Subcommands" value="14" hint="PluginCommand enum — crates/cognia-cli/src/main.rs:132-279" />
  <Stat label="Lint codes" value="60" hint="distinct diagnostic codes — cmd_lint.rs:362-1306" />
  <Stat label="Permissions / capabilities" value="45 / 45" hint="hard-coded whitelists, parity-tested against TS — cmd_lint.rs:28-223" />
  ...
  <Stat label="Rust source" value="~8.0k lines" hint="crates/cognia-cli/src (26 files) + src-tauri/src/cli_bridge (6 files)" />
...
(`TopCommand::Plugin` — `crates/cognia-cli/src/main.rs:121`). Global flags apply everywhere:
`--color auto|always|never`, `--no-color`, `-q/--quiet`, `-v/--verbose` (max `-vv`),
`-y/--yes` (pre-confirm every prompt; required for CI) — `main.rs:97-114`.

| Command | What it does | Module | Talks to app? |
| --- | --- | --- | --- |
| `new <name> [--kind wasm\|ts\|python\|hybrid\|vscode-extension]` | Stamp a starter project (interactive wizard on TTY) | `cmd_new.rs` + `template.rs` | no |
| `lint [--path .] [--json]` | 60-code manifest validation; exit 1 on errors | `cmd_lint.rs` | no |
| `build [--path .] [--out] [--skip-build]` | Lint → cargo-component / esbuild / existing-entry packaging → zip | `cmd_build.rs` / `build_ts.rs` / `packaging.rs` | no |
| `info <bundle.zip\|directory> [--json] [--detailed]` | Manifest, file table, bundle signature status or directory `not-applicable`, `cognia:api-version` | `cmd_info.rs` | no |
...
| `install <bundle.zip\|directory>` | Preflight collision check → POST bundle or unpacked-directory install | `cmd_install.rs` | **yes** |
...
| `reload [--plugin-id ID] [--path P]` | POST a hot-reload request for an installed plugin id, rebuilt bundle, or unpacked directory; `--bundle` remains accepted for existing scripts | `cmd_reload.rs` | **yes** |
...
| `embed-version <wasm> <ver> [--out]` | Re-inject the `cognia:api-version` custom section | `main.rs:395` + `packaging.rs` | no |
