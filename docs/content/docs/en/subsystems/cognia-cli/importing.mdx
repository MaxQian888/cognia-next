---
title: Importing
description: cognia plugin import — convert an existing MCP server, agent skill, or CLI binary into a plugin project, without executing anything and without copying a single credential.
---
...
- **TOML-backed agents are out of scope.** Codex stores MCP servers in TOML, and its adapter
  consumes an already-decoded tree; pulling a TOML parser into the bundle buys one agent. The
  error names the limitation rather than silently returning nothing.
