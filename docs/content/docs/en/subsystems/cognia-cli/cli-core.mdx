...
<TLDR>
  `main.rs` (`crates/cognia-cli/src/main.rs`, 527 lines) parses one nested clap-derive tree —
  `Cli` → `TopCommand::Plugin` → a 14-variant `PluginCommand` enum — resolves a `ColorMode`,
  builds a `RuntimeUi` (`ui/runtime.rs:60`), and routes through `dispatch_plugin()`
  (`main.rs:326`). Every interactive touchpoint goes through the **`Prompter` trait**
  ...
  deterministic tests. Errors flow `anyhow` → `eyre` (`anyhow_to_eyre`, `main.rs:315`) so
...
The 14 `PluginCommand` variants (`main.rs:132-279`) and their dispatch sites in
`dispatch_plugin()` (`main.rs:326-392`):

| Variant | Flags | Dispatch |
| --- | --- | --- |
| `New` | `--dir` `--kind` `--author` `--author-email` `--description` `--with-keygen` | `main.rs:328` |
| `Lint` | `--path .` `--json` | `main.rs:346` (sets `ui.flags.json` first) |
| `Build` | `--path .` `--out` `--skip-build` | `main.rs:350` |
| `Info` | `--json` `--detailed` | `main.rs:355` |
| `Sign` | `--key` (required) `--out` | `main.rs:363` |
| `Verify` | `--public-key` `--signature` `--json` | `main.rs:364` |
| `Keygen` | `--out-dir .cognia` | `main.rs:373` |
| `Install` | `<path>` bundle or directory | `main.rs:374` |
| `Uninstall` | `--purge-data` | `main.rs:375` |
| `List` | `--json` | `main.rs:379` |
| `Reload` | `--path` (visible alias for `--bundle`) `--plugin-id` | `main.rs:383` |
| `Status` | `--json` | `main.rs:384` (sets `ui.flags.json` first) |
...
Commands return `anyhow::Result`; `anyhow_to_eyre()` (`main.rs:315-323`) re-wraps the chain in
...
contains errors (`cmd_lint.rs:282-303`) — warnings alone keep exit 0.

## Integration tests

`crates/cognia-cli/tests/integration.rs` spawns the compiled binary via `CARGO_BIN_EXE_cognia`
with `NO_COLOR=1`, `CI=true`, and stdin nulled. The 16 tests pin the public contract: `--help` /
`--version` shape, lint exit codes on clean vs broken manifests, `lint --json` carrying
`schemaVersion: 1`, `info --json` round-tripping with bundle and directory inputs,
...
The newest bridge-client smoke checks keep `plugin install --help`, `plugin info --help`,
`plugin list --help`, `plugin reload --help`, and `plugin status --help` wired
through clap so these non-TUI bridge surfaces cannot silently disappear.
