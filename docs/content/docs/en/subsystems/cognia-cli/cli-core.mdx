...
description: cli.rs — the clap-derive command tree, dispatch_plugin() router, and error conversion — plus the ui/ layer — RuntimeUi state, the Prompter trait with three implementations, indicatif spinners, the owo-colors palette, and color-eyre integration.
---

# CLI core & terminal UI

<TLDR>
  `main.rs` parses one clap-derive tree defined in `crates/cognia-cli/src/cli.rs` —
  `Cli` → `TopCommand` (`plugin`, `pack`, `host`, `acp`, `logs`, `crash`, `release-key`,
  `release-verify`, `status`, `open`, `completions`) with a 19-variant `PluginCommand`
  enum under `plugin` — resolves a `ColorMode`, builds a `RuntimeUi`
...
```rust
// crates/cognia-cli/src/cli.rs (abridged)
struct Cli {
...
enum TopCommand {
    Plugin { endpoint_file, #[command(subcommand)] command: PluginCommand },
    Pack { #[command(subcommand)] command: PackCommand },
    Host { server_url, data_dir, ca_cert, server_bin, #[command(subcommand)] command: HostCommand },
    Acp,
    Logs { #[command(subcommand)] command: LogsCommand },
    Crash { #[command(subcommand)] command: CrashCommand },
    ReleaseKey { json },
    ReleaseVerify { artifact, checksums, artifact_name, signature, json },
    Status { json, endpoint_file, server_url, data_dir, ca_cert, server_bin },
    Open(OpenArgs),
    Completions { shell },
}
```

The 19 `PluginCommand` variants and their dispatch sites in `dispatch_plugin()`:
