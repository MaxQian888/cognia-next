...
| `cliTools[]` | 29 `manifest.cliTools.*` codes (`lint_cli_tools`) | requires `cli:execute`, validates binary provenance, argv tokens, parameter references, cwd/stdin/env policies, numeric knobs (`timeoutMs` capped at the `plugin_cli_exec` 600s ceiling), output parsing, success codes, and version args — plus `access` (`"read"`/`"write"`) and `confinedPathParams` (declared params only, and a cwd base must exist to confine against) |
