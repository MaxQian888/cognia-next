...
- **4** run entry points (from-here / single-step / NDV CTA / whole re-run)
...
flowchart TD
  ...
  RDI --> RERUN["handleReRun<br/>run-detail.tsx:122"]
...
| `RunDetailInner` | `run-detail.tsx:98` | header pill/duration/trigger/sparkline + Re-run |
| `handleReRun` | `run-detail.tsx:122` | re-invokes `runWorkflow` with the original `triggerPayload` |
...
flowchart LR
  ...
  D["Run history Re-run"] --> RR["RunDetail.handleReRun<br/>run-detail.tsx:122 (orig payload)"]
```
...
- **Whole re-run** → `RunDetail.handleReRun` (`run-detail.tsx:122`) re-invokes `runWorkflow` with the original `triggerPayload`.
