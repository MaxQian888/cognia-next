...
description: The authoritative VisualWorkflow graph type, the 74-kind / 7-category taxonomy, the Zod plus graph-integrity validation pipeline, the 8 workflow Dexie tables, and the 22 built-in templates.
---
...
<TLDR>
A workflow is a `VisualWorkflow` graph (`types/workflow/visual.ts:242`) at `schemaVersion: 1` (`types/workflow/visual.ts:235`) made of `WorkflowNode` plus `WorkflowEdge` records. Every node `type` is one of 74 `WorkflowNodeKind` literals (`types/workflow/visual.ts:27-119`) in 7 categories, with `workflowNodeCategory()` (`types/workflow/visual.ts:130-141`) folding `pattern.*` into `action`. Persistence touches 8 Dexie tables introduced across v22, v35, v42, v52, and v56 (`lib/db/schema.ts`), with CRUD in `lib/db/workflows.ts`. Validation runs Zod (`lib/workflow/definition/validate.ts:110-129`) then graph-integrity DFS (`lib/workflow/definition/validate.ts:151-242`), and 22 built-in templates seed from `seed.ts`, `seed-github.ts`, and four `templates/*.ts`.
</TLDR>

<StatGrid>
  <Stat label="Node kinds" value="74" hint="across 7 categories" />
  <Stat label="Schema version" value="1" hint="VisualWorkflowSchemaVersion" />
  <Stat label="Dexie tables" value="8" hint="v22 to v56" />
...
## The 74-kind taxonomy

`WorkflowNodeKind` is a union of 74 string literals (`types/workflow/visual.ts:27-119`), mirrored by the `WORKFLOW_NODE_KINDS` runtime array (`types/workflow/visual.ts:147-222`). Each kind is bucketed into one of 7 `WorkflowNodeCategory` literals (`types/workflow/visual.ts:121-128`) by `workflowNodeCategory()`, which splits on the first dot and maps `pattern.*` onto `action` (a fallthrough yields `annotation`).
...
Eight workflow tables span five schema versions. v22 declares the original four; v35, v42, v52, and v56 add one each. v52 re-declares `workflows` to add `folderId` and ships an upgrade hook backfilling `folderId = "root"` for legacy rows (IndexedDB cannot index `undefined`); v22, v35, v42, and v56 are pure additive with no upgrade hook.
