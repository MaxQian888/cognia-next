...
<StatGrid>
- **74 node kinds** across 7 categories (canonical)
...
Three families use the permissive `z.object({}).passthrough()` because their shapes are owned elsewhere: all 13 `github.*` (L417-429), all 12 desktop + `trigger.desktop.event` (L432-443), and all 6 `pattern.*` (L473-478).
...
> The 13 `action.github.*` kinds are provided by the **github-delivery plugin** (`plugins/github-delivery/`), not `built-ins.ts`. The 12 desktop kinds + `trigger.desktop.event` live in `desktop.ts`; `action.system.terminal` lives in `terminal.ts`.
...
## Desktop nodes (`lib/workflow/nodes/desktop.ts`)

Registers 12 desktop executors + `trigger.desktop.event`, each wrapping a desktop client method from `@/lib/automation/client` and tagging `surface: "workflow"` so the Rust gate evaluates **workflow** policy. Helpers at L38-95; `callCtx` (L84) routes to a cua sandbox (ADR-0020) when `target.connectionId` is present, else local.
...
`trigger.desktop.event` is a passthrough at execute time; real UIA firing happens via the Rust trigger bridge.
