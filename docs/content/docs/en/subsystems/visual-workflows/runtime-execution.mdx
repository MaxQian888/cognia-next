...
`runLongStep` (`long-step-runner.ts:119-211`) piggy-backs checkpoints on `workflowRunEvents` as `step.long_running.checkpoint` / `progress` — **no new Dexie table**. `step_completed` is written only at the end, so until then the step is "not done" and is re-entered on resume via `onResume(latestCheckpoint)`. All writes are best-effort `try/catch`.
