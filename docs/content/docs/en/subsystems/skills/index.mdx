...
<TLDR>
  ...
  marketplace** (`skills-lock.json` registry + optional SkillsMP API), a **three-target native sync**
...
<StatGrid>
  ...
  <Stat label="Marketplace sources" value="2" hint="registry (skills-lock.json, 6 entries) + SkillsMP API (opt-in via skillsMpBaseUrl)" />
...
lib/skills/
  ...
  marketplace-*.ts       # types · registry adapter · SkillsMP adapter · install · teasers
...
flowchart LR
  subgraph In["Ways in"]
    ...
    MP["Marketplace<br/>registry + SkillsMP"]
...
| [Marketplace](./marketplace) | `skills-lock.json` registry vs SkillsMP API, install/uninstall idempotency by `canonicalId`, the Rust remote-fetch proxy, teasers |
