...
`components/chat/skill-picker.tsx` exports `SkillPickerContent`, a searchable multi-select list over
`listSkills()` that keeps enabled rows split into custom and built-in groups and toggles ids through
`onChange`. The composer hosts it in `components/chat/composer/skills-menu-entry.tsx`: a `CapabilityRow`
inside the `+` menu's capability group opens a right-side `Popover` flyout wrapping the list. The picks
render as removable chips above the input via `components/chat/composer/skill-chip-row.tsx`, which
resolves names through `useEffectiveSkills` and exposes a per-chip remove button.
