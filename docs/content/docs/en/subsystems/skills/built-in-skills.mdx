...
description: The ADR-0026 typed-tool tier — a code-reviewed registry of MCP tools the assistant calls directly, including desktop plugin conversion and connector operations, gated by PII, access controls, and HITL.
---

# Built-in skills

<Status variant="beta">ADR-0026 — lib/skills/built-in · typed MCP tools · desktop and connector HITL</Status>
...
<TLDR>
  ...
  Confirm. The Lark skills shell out to a user-installed `lark-cli` (`exec-lark-cli.ts:101`) with OAuth
  credentials pulled from the existing Lark connector (`auth-bridge.ts:86`). Runtime wiring lives in
  `build-options.ts` and `plugin-tool-ipc.ts`. The desktop-only `plugin.conversion` family instead reuses the
  deterministic in-app converter and confines all paths to the active workspace.
</TLDR>
...
| `lib/skills/built-in/lark/` | 43 Lark skills across 6 families + the lark-cli exec/auth bridge |
| `lib/skills/built-in/plugin-conversion/` | Desktop-only inspect/apply tools for deterministic whole-plugin conversion |
| `lib/plugin/convert/agent-service.ts` | Source snapshot, expiring plan, empty-target enforcement, and filesystem write adapter |
...
## Plugin conversion family

`plugin.conversion` couples a seeded prompt skill (`skills/built-in/plugin-conversion/SKILL.md`) with two
typed tools:

| Tool | Mutation | Contract |
| --- | --- | --- |
| `plugin_conversion_inspect` | `read` | Detect the source ecosystem, run the canonical converter, return fidelity/issues/files, and mint an expiring plan without writing |
| `plugin_conversion_apply` | `write` | Re-read the source, reject a stale plan or non-empty/overlapping target, then write only deterministic converter output after desktop approval |

Both tools declare `imAccess: "blocked"`. `resolveSendOptions` exposes them only when the prompt skill is
active (or the opt-in `Skill` self-loader can activate it), a desktop workspace cwd exists, and the workspace
filesystem backend is available. Source and output arguments are workspace-relative; Rust's workspace
filesystem commands enforce containment again at the disk boundary.

The plan binds `workspaceRoot + sourceDir + target` to a source snapshot digest and expires after 15 minutes.
`apply` consumes it after a successful write. A changed source, expired or unknown plan, non-empty output, or
source/output overlap fails closed. Unsupported capabilities remain in the structured `blocking` report; the
agent is instructed not to approximate or hand-author converted runtime files.

Whole-bundle conversion supports Claude Code, Codex, or Gemini CLI → Cognia and Cognia → any of those three.
Foreign → foreign conversion is intentionally unsupported.

## The Lark CLI bridge

The 43 v1 skills are all Lark, grouped into six families that each `registerBuiltInSkill()` at module load
(`lark/index.ts:19-24`). The count is exact — 10 + 6 + 7 + 8 + 8 + 4 = **43** registrations.

| Family | Tools | Mutation tiers | Module |
| --- | --- | --- | --- |
| `lark.calendar` | 10 | read · write · destructive | `lark/calendar.ts` |
| `lark.doc` | 6 | read · write · destructive | `lark/doc.ts` |
| `lark.sheets` | 7 | read · write | `lark/sheets.ts` |
| `lark.base` (Bitable) | 8 | read · write · destructive | `lark/base.ts` |
| `lark.task` | 8 | read · write | `lark/task.ts` |
...
`plugin.conversion` is deliberately outside that broad switch. Its entries are filtered out of the general
desktop manifest and appended separately only when the `builtin:plugin-conversion` prompt skill is active or
the `Skill` self-loader is enabled. This keeps the filesystem write tool unavailable to characters that merely
enabled unrelated built-in connector skills. It is also appended outside `disablePluginTools`, because that
flag governs third-party plugin tools rather than first-party conversion.
...
<Callout type="info" title="Gotchas">
  ...
  - Desktop writes suspend on the standard tool-approval dialog. `plugin_conversion_apply` cannot write until
    the user approves; cancelling or timing out returns a structured denial.
</Callout>
