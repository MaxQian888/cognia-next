...
| --- | --- | --- |
| `inspect_plugin_conversion` | `read` | Detect the source ecosystem, run the canonical converter, return fidelity/issues/files, and mint an expiring plan without writing |
| `apply_plugin_conversion` | `write` | Re-read the source, reject a stale plan or non-empty/overlapping target, then write only deterministic converter output after desktop approval |
...
<Callout type="info" title="Gotchas">
  ...
  - Desktop writes suspend on the standard tool-approval dialog. `apply_plugin_conversion` cannot write until
