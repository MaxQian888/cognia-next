...
`extractContainerSkillIds` (`skills-bridge.ts:133-150`) filters the resolved set down to entries that
carry a `containerSkillId` and maps them to `{ skill_id, version? }`. The Anthropic API caps a request
at `MAX_CONTAINER_SKILLS = 8` (`:126`); overflow is dropped FIFO with a `console.warn`. The result
lands on `opts.containerSkillIds` (`build-options.ts:716`) and is forwarded by the sidecar so the API
attaches the managed skill at request time — no markdown enters the prompt.
