...
Catalog-authored built-ins may declare `metadata.default-enabled: true`. Seeding applies that default only when the row is first created and preserves a later user disable. `Diagram Design` uses this path: it is discoverable for drawing intent without injecting its full body or its 27 type references into ordinary turns.
