...
components/plugins/       # Settings UI + marketplace
```
...
The marketplace UI lives in `components/plugins/`.
...
| Tweak marketplace UX    | `components/plugins/`, `lib/plugin/index.ts` (catalog client)               |

## Tests

- `lib/plugin/index.test.ts` — registry and lifecycle integration.
- `lib/plugin/utils/icon.test.ts` — icon parsing (SVG sanitiser).
...
<Cards>
  ...
  />
  <Card title="Storage" href="../data/storage" description="The plugin tables (added in v15) in detail." />
  <Card
