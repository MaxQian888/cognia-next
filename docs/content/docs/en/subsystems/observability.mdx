...
`lib/logger/index.ts` exposes three consumption patterns:

```ts title="lib/logger/index.ts"
...
`lib/logger/context.ts` maintains an implicit context:

```ts title="lib/logger/context.ts"
...
`lib/logger/runtime.ts` infers the current runtime (`browser / server / tauri / mcp / plugin / internal / unknown`) and automatically injects it into every entry, enabling origin-based coloring in the panel.
...
`transports/langfuse-transport.ts` — an observability layer purpose-built for LLM calls; maps traceId-correlated `ai / chat / agent` module logs into Langfuse trace + observation. `lib/logger/loggers.ai` is typically paired with Langfuse.
...
Full metrics / traces are submitted via `lib/logger/transports/otel-transport.ts`. AI provider calls, Twin pipeline stages, and scheduler executions all emit spans, visible in the dev console. This layer is currently placeholder-shaped, ready to expand once a full OTLP backend is connected.
...
```ts title="lib/logger/bootstrap.ts"
...
| Goal                        | File                                                                                                                            |
| --------------------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| Add a new transport         | Implement `Transport` in `lib/logger/transports/<name>-transport.ts` → export in `transports/index.ts` → add registration path in `bootstrap.ts` |
| Adjust default sampling rate| `lib/logger/sampling.ts:DEFAULT_SAMPLING_CONFIG`                                                                                |
| Add new redaction fields    | Field name list + regex list in `lib/logger/redaction.ts`                                                                       |
| Modify Rust → platform log bridge | `src-tauri/src/logging/platform/<os>.rs`                                                                                  |
| New log panel filter dimension | `components/logging/log-filter-presets.ts` + `hooks/logging/use-log-panel-filters.ts`                                       |
| OTLP collector configuration| `lib/logger/transports/otel-transport.ts` + startup env                                                                         |
