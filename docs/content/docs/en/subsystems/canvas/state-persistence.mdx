...
<StatGrid>
  ...
  <Stat label="Runtime-only stores" value="2" hint="chunked-document-store · comment-store (mirror)" />
</StatGrid>
...
The read hook `useCanvasDocuments` (`hooks/canvas/use-canvas-documents.ts:34`) is a thin facade over
this store (sorted list, create/open/rename/duplicate/search) — confirming documents come exclusively
from the artifact store.

## The Five Canvas Stores
...
flowchart LR
  ...
  subgraph CANVAS["stores/canvas (5)"]
    ...
    CH["useChunkedDocumentStore<br/>chunked-document-store.ts:46<br/>runtime-only (no persist)"]
  end
  ART -.-> CH
...
- **`useChunkedDocumentStore`** (`chunked-document-store.ts:46`) — runtime-only; chunks are re-derived
  from `CanvasDocument.content` on session start, so adding persistence here would fight the
  artifact-store contract (`:1-11`).
...
`useCanvasAutoSave` (`use-canvas-auto-save.ts:28`) tracks local content vs last-saved content and
schedules a save after `autoSaveDelay` (default `30000` ms, `:34`) of inactivity, calling the injected
`onSave(documentId, undefined, true)` (the `true` flags an auto-save). The consuming `CanvasPanel`
wires `onSave` to `saveCanvasVersion` and `onContentUpdate` to `updateCanvasDocument`. The panel also
runs a coarser interval auto-save floored at 10 s (`canvas-panel.tsx:156-168`).
