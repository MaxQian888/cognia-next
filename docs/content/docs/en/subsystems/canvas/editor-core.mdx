---
title: Editor Core
description: How Monaco is mounted, themed, and laid out — the offline loader for Tauri, the resizable 3-pane shell, the document rail, theme sync via cognia-active-theme, the symbol parser, snippet registry, plugin manager, large-file chunking, and the keybinding dispatchers.
---
...
- **Plugins** — `CanvasPluginManager` (`plugin-manager.ts:112`) is a canvas-local plugin layer that
  bridges enable/disable into the main plugin manager via `syncWithMainManager` (`:183`) and dispatches
  typed hooks (`collectCompletions` `:295`, `collectDiagnostics` `:313`, `collectActions` `:331`).

## Large-File Chunking

For very large documents the content is chunked so the main thread never blocks:

- `LargeFileOptimizer` (`large-file-optimizer.ts:30`) — `DEFAULT_CHUNK_SIZE = 10000` chars
  (`:27`), `LARGE_FILE_THRESHOLD = 50000` chars (`:28`). `chunkContent` (`:47`) builds line offsets and
  slices; `getVisibleContent` (`:182`) returns a window with a 50-line buffer.
- `useChunkLoader` (`use-chunk-loader.ts:28`) reads `useChunkedDocumentStore` (not the artifact store),
  with `BUFFER_LINES = 100` (`:25`) and `LOAD_THRESHOLD = 50` (`:26`); the initial load fetches the
  first 200 lines via `queueMicrotask` (`:119-126`).
- `getCanvasPerformanceProfile` (`utils.ts:151`) picks `standard` / `large` / `very-large` modes with
  per-mode symbol-parse debounce (500 / 900 / 1500 ms) and chunking toggles
