...
<StatGrid>
  <Stat label="Source files" value="~50" hint="lib/components/hooks/stores/types + 2 Rust" />
  ...
  <Stat label="Zustand stores" value="5" hint="1 source-of-truth + 4 canvas-specific" />
...
<Cards>
  ...
  <Card title="AI actions & suggestions" href="./ai-actions" description="The 10 action prompts, diff-review apply pipeline, the structured-suggestion stream, the context-analyzer scope block" />
...
| (implied) context-analyzer feeds the prompts | It now does, for **suggestions only** — `buildScopeBlock` appends a 400-char scope summary. Actions still send the document alone, deliberately | `lib/canvas/ai/context-analyzer.ts`, `hooks/canvas/use-canvas-suggestions.ts` |
...
| "Open in Canvas" navigates to `/canvas/<id>` | That route never existed — the app is a static export with no dynamic segments. The inline card reveals the document through the store instead | `lib/artifacts/reveal.ts:revealCanvasDocument`, `components/chat/message-parts/canvas-inline-part.tsx` |
| Documents are persisted to localStorage, Dexie is a mirror | Inverted at persist v7: Dexie owns documents and versions, and the blob keeps only the dock's preferences | [ADR-0158](../../adr/0158-artifacts-and-canvas), `lib/canvas/dexie-bridge.ts` |
| `lib/sandbox/web/` is the web execution path | It never had a caller and has been deleted; the real path is `lib/native/code-execution-strategy.ts` | ADR-0158 |
...
- **Web mode** (no Tauri) has no Python channel. JS/TS/JSX/HTML/CSS still run for real, in a
  `sandbox="allow-scripts"` srcdoc iframe (`lib/native/code-execution-strategy.ts:135`) — a genuine
  sandbox, not a simulation. Only Python is desktop-only, and correctly so.
