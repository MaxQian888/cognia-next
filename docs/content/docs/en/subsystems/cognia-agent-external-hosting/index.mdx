...
| --- | --- | --- |
| Presets and executable selection | `lib/ai/agent/external/config/presets.ts` | reused unchanged |
...
# Claude Code through the ACP registry adapter
...
`npx -y @agentclientprotocol/claude-agent-acp`. Although the flag accepts a preset id, the CLI process host
also applies an executable/package allowlist. The runner image now carries the complete built-in preset set:
Claude Code, Codex, Gemini CLI, GitHub Copilot CLI, Qwen Code, Kiro CLI, Factory Droid, Pi,
OpenCode, Cline, and the optional Cursor Agent installer. ACP-capable presets use their current native ACP
mode or registry adapter rather than historical compatibility flags.
...
The Claude preset has since migrated to the ACP registry package; the entry below intentionally
records the exact older package used by that historical smoke run.
