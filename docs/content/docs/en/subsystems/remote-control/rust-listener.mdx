...
let app = Router::new()
    ...
    // Read surface (GET) — reachable by a `read`-capability token.
    ...
    .layer(RequestBodyLimitLayer::new(BODY_LIMIT_BYTES)); // 8 * 1024
```

Capability tiering (`route_needs_write`): a `read` token may hit the health probe and every `GET`
read endpoint; mutating methods (`POST`) require `write`. The generic `POST /api/v1/commands/:target`
route additionally rejects **sensitive** targets (`chat.send`, `connector.send`, `goal.create`) with
`403 sensitive_target_disabled` unless `RemoteControlInboundConfig.allowSensitiveTargets` is on.

Every `GET` read except `/targets` (answered natively in Rust) round-trips to the renderer via a
`remote-control://query` event + a `QueryRegistry` oneshot, resolved by the
`remote_control_query_response` command; a closed/busy window returns `503 window_unavailable` after a
bounded wait. The full wire contract lives in `docs/api/remote-control.openapi.yaml`.
