...
### Virtualized transcript and adaptive terminal budget

`fullscreen` now renders through width-dependent `TerminalBlock` values and a
prefix-sum block index. Only the visible cells plus two viewport heights of
overscan are mounted. The viewport is anchored by cell id and intra-cell row, so
resize correction and streamed appends do not move a reader who has scrolled up;
`End` or `G` resumes follow-tail. `COGNIA_TUI_RENDERER=legacy` retains the former
whole-transcript renderer for one compatibility release. `/transcript` opens the
complete searchable/copyable transcript in either layout.

Terminal chrome is measured in four tiers: full (100+ columns), abbreviated
(60–99), compact (40–59), and tiny (below 40 columns or 12 rows). Tiny terminals
drop nonessential banner/mascot rows and give overlays the viewport while keeping
permission/activity state and the composer path available. In native scrollback,
`render.terminalResizeReplayMaxRows` limits resize repaint to the newest 10,000
rendered rows by default; `0` disables the limit without truncating session data.

Canonical `content-part` envelopes carry sources, file metadata, A2UI surfaces,
artifact/canvas references, and custom fallbacks without durable binary bodies.
Every known canonical event has an explicit transcript, status, overlay, or audit
classification; future events render a redacted unsupported-event notice. `/doctor`
reports renderer, cache/window, timing, and terminal capability degradation without
prompts, paths, or payloads.

### Terminal smoke matrix

- macOS and Windows: run the native node-pty geometry suite at 20×8, 40×12,
  80×24, and 160×50; verify wheel input, resize, interrupt, and alternate-screen cleanup.
- Linux CI: repeat the suite directly, inside tmux, and inside Zellij.
- SSH: verify resize propagation, `TERM` forwarding, OSC-8 fallback, and clean disconnect.
- Non-TTY and `TERM=dumb`: verify automatic scrollback output contains no alternate-screen escapes.
- Proprietary terminals: manually check wheel/select modes, CJK/emoji wrapping,
  `/transcript` search/copy, A2UI and permission focus, image fallback, and `Ctrl+C` cleanup.
...
`mention/` has a co-located `*.test.ts`; Ink components use a DOM-backed Jest mock for fast behavior
tests, plus a real Ink/Yoga child-process probe over the production `TuiViewportFrame` and an optional native PTY matrix that mounts `App` with a deterministic session. Run the suite with
`pnpm cli:test`.

## Viewport and input invariants

The root terminal size is authoritative. Fullscreen overlay height is measured
after fixed bottom chrome, panel content is windowed inside that allocation, and
the composer keeps its grapheme-safe caret visible within the density tier's row
budget. One active input provider routes critical, modal, global, and composer
handlers by priority and stops after a handler consumes the key. Real Ink/Yoga
frame probes exercise the same production frame as `App`; the PTY matrix mounts `App` itself. Both complement the DOM-backed component suite.
