...
<TLDR>
  Tauri Rust starts an axum 0.7 HTTP server bound to 127.0.0.1:&lt;port&gt; (default 47821),
  exposing 3 v1 endpoints: health / trigger task / trigger event. Bearer + IP allowlist + rate
  limiting + 8 KiB body limit. Outbound direction adds HMAC-SHA256 + custom headers to scheduler
  webhook channels. Secrets go to the OS keyring (`com.cognia.remote-control`).**Off by default**,
  opt-in to enable.
</TLDR>
...
<StatGrid>
  <Stat
    ...
    value="3"
    hint="/api/v1/health · /api/v1/tasks/:id/run · /api/v1/events"
  />
...
<Cards>
  <Card
    title="Scheduler"
    href="./scheduler"
    description="The engine that actually runs tasks and receives events"
  />
  <Card
