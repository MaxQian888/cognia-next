...
description: How an axum handler reads and writes data that lives in the WebView's Dexie — three oneshot request/response bridges, the sync-table allowlist, the TS bridge sources, and the headless DataPlane that swaps in a SQLite store when there is no WebView.
---
...
<TLDR>
  ...
  **30 seconds**; a TS listener processes the request against Dexie and replies via a
  ...
  response. For the headless `cognia-server` (no WebView), a `DataPlane` abstraction swaps the bridge
  for a direct SQLite `AppStore`.
</TLDR>
...
<StatGrid>
  ...
  <Stat label="Sync tables" value="10" hint="sync_registry allowlist" />
...
| `desktop_messages_bridge.rs` | 5 events (`:41-45`) | `companion_message_response` (`commands.rs:200`) | 30 s (`:46`) | `message_update` / `message_delete` / `session_list` / `message_get_by_session` / `message_send` |
...
The messages bridge fans out five distinct event names but shares one `register()` (`:253`) +
`await_response()` (`:266`) + `resolve()` (`:285`). The writes bridge takes the opposite tack: a
...
`with_defaults()` (`:45`). The **10** allowed tables (`sync_registry.rs:69-122`):
...
The Companion API code is reused by a headless `cognia-server` binary that has **no WebView** — so
the message/session bridges have no TS side to answer them. `DataPlane` (`data_plane.rs:54`) abstracts
that difference:
...
A process-global `HEADLESS_STORE` (`data_plane.rs:41`) is installed via `install_headless_store()`
(`:45`); `pick()` (`:64`) prefers the headless store, else the Tauri bridge, else `None`. The five
message/session methods (`list_sessions` `:77`, `get_messages_by_session` `:99`, `send_message` `:121`,
`update_message` `:145`, `delete_message` `:175`) each branch on the active variant; `rpc.rs` selects
it via `pick_data_plane()` (`rpc.rs:647`). On the desktop product, `Direct` is never installed, so the
WebView bridge is always chosen — the `AppStore` / `SqliteAppStore` in `store.rs` exists purely for the
headless path.
