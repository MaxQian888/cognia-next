...
<StatGrid>
  <Stat label="Default port" value="7890" hint="server.rs:51" />
...
sequenceDiagram
  ...
  alt jti already used
    ...
    R-->>P: { deviceId, deviceJwt, rendezvousId, rendezvousSecret }
...
`PairResponse` (`auth.rs:99`) is `{ deviceId, deviceJwt, serverVersion, rendezvousId, rendezvousSecret }`.
The `rendezvousId` (UUIDv4) and 32-byte URL-safe `rendezvousSecret` (`auth.rs:337-342`) are the seed
for the [WebRTC plane](./signaling-webrtc) — they let the phone and desktop find each other in a room
and HMAC every signaling envelope.
