...
<Cards>
  ...
  <Card title="WebRTC signaling plane" href="./signaling-webrtc" description="The v2-only desktop/headless answerer: role-authenticated encrypted signaling, bounded webrtc-rs peer, durable idempotency, event replay/resync, and recovery FSM (ADR-0021)." />
...
- **The signing key is the identity.** A single 32-byte HS256 secret in the OS keyring
  (`secret.rs:21`) signs every pair and device JWT; WebRTC uses separate per-role P-256 keys, and the
