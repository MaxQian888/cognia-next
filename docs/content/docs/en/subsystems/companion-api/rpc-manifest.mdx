...
<TLDR>
  ...
  `POST /internal/_rpc/{name}` for the local Headless Brain. `protocol/companion-commands.json` is
  the one contract: it supplies target, transport, capability, risk, approval, idempotency, and
  schema metadata, and the runtime allowlist is rendered from it into
  `generated/known_commands.rs` (ADR-0175) instead of being typed a second time. A Host lists what
  a caller may dispatch at `GET /api/catalog` and `GET /internal/catalog`.
</TLDR>

<StatGrid>
  <Stat label="Runtime allowlist" value="generated" hint="generated/known_commands.rs" />
  <Stat label="Public device subset" value="generated" hint="execution/host-admin + HTTP" />
  <Stat label="Command contract" value="v3" hint="protocol/companion-commands.json (ADR-0175)" />
...
flowchart LR
  ...
  N["companion-command-renames.json"] --> G
  D["rpc/*.rs dispatch arms"] --> G
  G --> K["generated/known_commands.rs"]
...
## Performance command boundary

Remote performance uses execution capability contracts for lease open, renew,
close, snapshot and observation reads. The authenticated caller device ID is
injected by the dispatcher and cannot be selected by request arguments. Legacy
global `perf_start_sampling`, `perf_set_interval`, `perf_stop_sampling`, hotspot
reset and trace-folder reveal stay local/internal compatibility commands and are
not remotely advertised. Plugin `ctx.perf` is read-only: it may observe an
already active stream and its source metadata, but cannot open leases, capture,
reset state, read raw attachments or control a process.
