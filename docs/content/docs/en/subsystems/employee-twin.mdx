...
**This is the privacy red-line.** `packages/redact/src/index.ts` replaces PII with `<KIND_NNN>` placeholders **before any embed or LLM call**:
...
<Steps>
  ...
  <Step>
    ...
    See [Vector store](../data/vector-store) for backend choices.
...
<Callout type="error" title="Safety-critical: redaction tests">
  `packages/redact/src/index.test.ts` is the most safety-critical test in the whole repo — it asserts
...
| Tighten redaction                 | `packages/redact/src/index.ts` (and its tests!)                                    |
...
<Cards>
  <Card title="Vector store" href="../data/vector-store" description="How RAG is actually executed." />
  <Card title="Backup & data transfer" href="../data/backup-and-data" description="Twin tables are part of the backup payload." />
  <Card title="Storage" href="../data/storage" description="The five Dexie twin tables in detail." />
  <Card title="Chat & Skills" href="../chat/chat-and-skills" description="The character / skill bindings that activate the twin." />
</Cards>
