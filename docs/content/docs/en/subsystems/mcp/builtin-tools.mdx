...
| `file_search` | read | glob/list, maxResults 200, respects `.gitignore` (root + nested; `respectGitignore:false` to opt out) |
| `content_search` | read | substring/regex, maxResults 100, respects `.gitignore` (root + nested; `respectGitignore:false` to opt out) |
