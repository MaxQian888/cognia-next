...
<TLDR>
Seven built-in adapters (Telegram / Discord / Slack / Lark / OneBot v11 / WeCom 企业微信 / personal WeChat) + a singleton ConnectorBus for fan-in / fan-out + Dexie-backed outbound FIFO (circuit breaker, token bucket, exponential backoff, dead letter, idempotent LRU). Three modes: auto (AI auto-reply) / manual (human typed) / draft (AI drafts, human reviews). Inbox UI in `app/inbox/`; plugins can contribute additional adapters.
</TLDR>

<StatGrid>
  <Stat label="Built-in Adapters" value="7" hint="telegram · discord · slack · lark · onebot · wecom · wechat-personal" />
