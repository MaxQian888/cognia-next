...
<TLDR>
  ...
  `status` is intentionally decoupled from runtime reality. OAuth completion lives in a 2-entry Map
...
The `status` here is **intentionally decoupled from runtime reality** — for example `wechat-personal`
ships a real adapter under `adapters/wechat-personal/` but is tagged `"planned"` (`:71`) because the
Discover badge ground-truth tracks the Phase-1 built-ins, not the full builder set. The Discover page
consumes this registry; it never imports the factory.
...
Discord, and OneBot use no OAuth. The `ConnectorDeepLinkRouter` calls the registered handler after
validating the OAuth `state` parameter, shaped `<platform>:<adapterId>:<nonce>`, from which the handler
decodes the `adapterId`.
...
consumes. `mentionsFromIds(selfId, ids)` (`:74`) is the convenience wrapper for parsers that already have
a complete id list. Discord, Lark, Slack, and Telegram parsers use it; OneBot does the reduction inline.
