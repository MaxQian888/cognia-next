...
<TLDR>
  ...
  is reachable from the TS side through 21 `#[tauri::command]`s (`commands.rs`) registered in `lib.rs`, and surfaces
  inbound traffic on four Tauri event-channel families.
</TLDR>

<StatGrid>
  <Stat label="Rust source files" value="~14" hint="connectors/ + sigverify/" />
  <Stat label="Tauri commands" value="21" hint="registered in lib.rs" />
...
On a miss it fetches the source with optional adapter-supplied headers, AES-256-GCM-encrypts with a 12-byte
random nonce prepended to the ciphertext, and writes both the `.enc` blob and the decrypted copy. The optional
header path is required for authenticated Matrix `mxc://` downloads. The master key is 32 bytes from `OsRng`,
auto-generated into the keyring on first use.

## Media Upload

`media_upload.rs` handles the inverse path for platforms such as Matrix: load bytes from either a remote
`sourceUrl` or a local path, apply adapter-supplied upload headers, POST the raw body to the platform media
repository, and return the response `content_uri`. Matrix uses this to upload images, files, voice, and video
segments before sending native `m.room.message` media events.

## Tauri Command Surface

`commands.rs` exposes exactly 21 `#[tauri::command]`s — the only entry points the renderer can reach. They are
registered in `src-tauri/src/lib.rs` (alongside `.manage(ConnectorsState)`), and called through the typed
...
shows up in the performance dashboard. `connectors_reset_all_ws` is called from connector bootstrap to reap
generic WS and Lark long-connection handles left behind by a webview hard reload, preventing duplicate inbound
delivery from stale sockets.
