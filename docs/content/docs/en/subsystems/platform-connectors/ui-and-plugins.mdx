...
11. **Start the execution bridge + run-presentation runner** —
    `startWorkflowExecutionBridge()` (`lib/execution/workflow-bridge.ts:143`)
    mirrors `workflowRuns` into durable `executionRuns` + per-conversation
    `executionRunBindings`, and `startExecutionRunPresentationRunner()`
    (`lib/connectors/run-presentation/runner.ts:715`) projects those bindings
    into native run cards (Lark / Slack drivers) or the capability-aware
    fallback through the outbound queue
...
The shared bootstrap body now lives in
`lib/connectors/bootstrap/install-connector-runtime.ts` (the provider calls
`installConnectorRuntime()`), and it has grown a few extra starts between steps
9 and 11 — `startLarkSurfaceSweep` / `startBindRequestExpirySweep` (`:722-723`)
and `startResumeReconnect` (`:732`) — that this list folds into the runner phase.

Steps 5, 6 and 8–11 — the runtime/callback wiring and the runner starts:
...
`cancelled = true`, calls `ac.abort()`, disposes the cleanup/heartbeat/
execution-bridge/presentation-runner handles, then walks `listRunningAdapters()` to `unregisterRunningAdapter` each
...
Neither layout mounts a palette of its own any more: ⌘/Ctrl+K is the unified
global search (ADR-0129), and platform-bound conversations are one of its
providers (`lib/global-search/providers/system.ts`), so a search from anywhere
...
Seven tabs, and the id of the Conversations tab is `overrides`, not
`conversations` — the deep link (`?connectionsTab=`) predates the rename.
There is no standalone Inbox tab: the per-adapter conversation summary that
used to live here was dropped in favour of the `/inbox` route, and the legacy
`?connectionsTab=tunnel` link now resolves to Overview, which hosts the tunnel
card. `connections-section.test.tsx` pins both absences.
...
<Cards>
  <Card title="Cross-shell relay" href="./cross-shell-relay" description="The one facade every surface here writes through, and why none of them branch on the shell." />
