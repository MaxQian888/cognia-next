...
<TLDR>
  ...
  Sources · Jobs · Drafts · Persona · Settings — and mounts the job worker via `useTwinWorker`. The Overview,
  Cron, and Inject-log surfaces are **cards inside the Settings tab**, not top-level tabs.
</TLDR>
...
flowchart TB
  ...
  Panel --> WK[useTwinWorker<br/>start/stop worker]
...
`useTwinWorker(twinId)` (`use-twin-worker.ts:138`) live-reads `TwinRuntimeSettings`, builds a
`JobWorkerConfig` (returning `null` — and thus no worker — when the worker is disabled, no twin is selected,
the vector config is incomplete, or the LLM key is missing), and starts / stops the worker on change:

```ts title="components/twin/use-twin-worker.ts:179-187"
useEffect(() => {
  if (!config || !twinId) return
  const handle = startJobWorker(config, twinId)
  handleRef.current = handle
  return () => { void handle.stop(); handleRef.current = null }
}, [config, twinId])
```

It returns a derived `{ active, reasonKey? }` status (reasons: `noTwinSelected` / `disabled` /
`incompleteConfig`) that the panel renders as the worker badge.
