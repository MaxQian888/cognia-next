...
<TLDR>
  TTS (11 selectable providers), live voice (six providers), Push (mobile push registration), QR (pairing QR + barcode
...
| ------------------- | --------------------------------------------------- | ----------------------------------------------- |
| TTS                 | Text-to-speech, 11 selectable providers             | `packages/tts/` · `crates/cognia-tts/`          |
| Live voice          | Full-duplex voice, browser + native host transports | `lib/voice/live/` · `src-tauri/src/voice_live.rs` |
...
| Public share links  | Zero-knowledge share via Cloudflare Worker          | `lib/share/` · `services/share-server/` · ADR-0037       |

## TTS

Text-to-speech is owned by the shared `@cognia/tts` orchestrator. Eleven providers are selectable; the deprecated `edge` and `openai-realtime` ids are compatibility inputs that normalize to `system` and have no runtime transport.

Provider overview (`types/media/tts.ts:TTS_PROVIDERS`):

| Provider | Notes |
| --- | --- |
| System | Web Speech API, no key |
| OpenAI | REST neural speech |
| Local OpenAI-compatible | Loopback-only endpoint; optional key, model and voice |
| Gemini | Google AI speech |
| ElevenLabs | Account voice discovery |
| LMNT · Hume · Cartesia · Deepgram | Cloud speech adapters |
| Xiaomi | Mainland-China REST provider |
| Mistral | Voxtral speech output |
...
`crates/cognia-tts/src/keyring.rs` uses the OS keyring (macOS Keychain / Windows Credential Manager / Linux Secret Service); service name is `com.cognia.tts`, account is the provider id. Its constrained proxy fetch keeps cloud credentials in the host. The UI-neutral `TTS_PROVIDER_SETTINGS` registry drives both desktop and mobile fields, so endpoint/model/voice support is not duplicated.
...
`TTS_PROVIDERS` carries compatibility metadata; `ORDERED_TTS_PROVIDERS` and `TTS_PROVIDER_SETTINGS` are the selectable runtime/settings sources:
...
Key files: `packages/tts/src/{tts-orchestrator,tts-cache,tts-text-utils,speech-settings,types}.ts` · `packages/tts/src/providers/*.ts` · `lib/tts/{facade,keyring,speak-chat-message}.ts` · `crates/cognia-tts/src/{keyring,proxy}.rs`.

## Live Voice

The same `LiveVoiceDialog`, controller, capture, playback, persistence and
transport serve all providers. OpenAI, Gemini and xAI use ephemeral browser
sessions in the `global` region. Qwen, Doubao and Baidu are Tauri-only in the
`cn` region: Rust reads their key from the existing host keyring, constructs a
fixed official endpoint and headers, then delegates to the shared proxy-aware
connector WebSocket. Qwen and Baidu use realtime JSON adapters; Doubao uses its
gzip binary framing. The native socket accepts text and binary frames and uses
a caller-preallocated handle so listeners exist before the handshake.

The composer trigger follows the master switch, configuration readiness and
text-chat busy state. Starting a session stops ordinary TTS; assistant messages
already carrying audio modality are excluded from auto-TTS. Final user speech
is persisted in the message timeline and copied to the composer exactly once.

## Push

Mobile push registration shell (M4.6). `lib/push/push-notifications.ts` wraps `@capacitor/push-notifications`:

- `registerPushNotifications({ requestPermission })` → checks permission, optionally prompts only on explicit user intent, then triggers APNs / FCM registration and returns a typed `RegistrationOutcome`.
- `reportPushTokenToDesktop(token, platform)` → maps iOS/Android to the companion RPC's `apns`/`fcm` provider contract.
- `subscribeToPushNotifications(handler)` → subscribes to inbound push events; the unified bridge records them in Notification Center and routes background taps.
...
- `buildProxyUrl(cfg)` — constructs a credential-free public endpoint.
- `shouldBypass(targetUrl, bypass[])` — checks exact hosts, `.domain` suffixes, exact IPv4/IPv6 addresses, CIDR ranges, and loopback defaults.

`proxy-fetch.ts` provides the desktop's buffered fetch subset. It preserves `Request`/`init` precedence, transports binary bodies as base64, supports redirect/timeout/cancellation, and rejects caller-supplied `Proxy-Authorization`. Routing and credentials are decided only by Rust.
...
The password field above exists only in native memory. Dexie stores the username and non-sensitive settings; the password is stored under `cognia-network-proxy/manual-password` in the OS keyring. Boot migrates legacy plaintext rows only after a write-and-read-back succeeds.

The runtime state is `Uninitialized → Ready | Blocked`; managed public traffic fails with a stable proxy error until settings and credentials have hydrated. `proxy_apply` publishes an atomic sanitized configuration. Core Rust services:
...
- `wsproxy::connect_via_proxy()` — HTTP CONNECT, TLS-to-HTTPS-proxy CONNECT, and SOCKS5 (proxy-side DNS) dialing for WSS.
- `detect.rs` — scans for local proxy software and returns only endpoints that complete an HTTP CONNECT or SOCKS5 CONNECT handshake.

An open port is not enough to become a candidate: ordinary services on 8080/8888, unknown-auth proxies, and fake SOCKS listeners are excluded. Clash controller metadata is still used for attribution and display after protocol verification.
...
Off and bypass routes call reqwest `.no_proxy()`, so ambient system proxy variables cannot override the decision. Active invalid/unreachable routes never fall back to direct. The sidecar receives a host-managed environment with inherited proxy variables cleared or replaced. Embedded WebViews support direct, HTTP, and SOCKS5 routes; Tauri cannot configure an HTTPS proxy for a WebView, so that combination returns `PROXY_TRANSPORT_UNSUPPORTED` instead of navigating directly.
...
- **Voice stack** — TTS, ordinary STT and six-provider live voice are fully wired; this can move to a standalone page when the misc page is next split.
- **Connectivity & Push** — the Companion control plane owns discovery, pairing, and push; document future cross-device expansion under `mobile-architecture` or a dedicated `discovery-and-pairing` page.
...
| Goal                        | Files                                                                                                            |
| --------------------------- | ---------------------------------------------------------------------------------------------------------------- |
| Add a new TTS provider      | `packages/tts/src/providers/<name>.ts` + selectable descriptor + provider registry                              |
