...
<Callout type="warn" title="Built & tested, not yet wired into production">
  ...
  - **Composer menu** — `ocr-menu.tsx` / `ocr-result-bubble.tsx` / `useOcr` are built and tested, but
    ...
    is dormant.
  - **Native bindings** — only the `ocr-ocrs` and `ocr-paddle` Cargo features link real crates; `ocr-tesseract`
    / `ocr-windows` / `ocr-apple` are empty feature flags resolving to a `PlaceholderBackend`. See
