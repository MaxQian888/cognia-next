...
flowchart TD
  ...
  FEAT -->|"apple-vision (macOS target)"| AV["AppleVisionBackend::extract<br/>backend/apple.rs"]
  FEAT -->|tesseract / windows<br/>no crate today| PH["PlaceholderBackend::extract<br/>backend/placeholder.rs:27"]
...
```toml
# crates/cognia-ocr/Cargo.toml — two are empty flags; only ocrs/paddle link crates
ocr-tesseract = []
ocr-windows   = []
...
`apple-vision` has no feature flag: it is compiled whenever the target is macOS
(`backend/apple.rs`), running Vision.framework's `VNRecognizeTextRequest`
in-process through the `objc2-vision` bindings — no sidecar, no model downloads.

<Callout type="warn" title="tesseract-rs / winocr are doc-only in the current tree">
  In the current manifest, `tesseract-rs` and `winocr` are **not declared dependencies** — `ocr-tesseract` and
  `ocr-windows` are empty feature flags, so those two backends compile to a `PlaceholderBackend` and return
  `MissingBinding`. **ocrs**, **paddle-ocr**, and (on macOS) **apple-vision** have real backing today. The
  `ort = "=2.0.0-rc.12"` pin is exact, matching the ADR.
</Callout>
