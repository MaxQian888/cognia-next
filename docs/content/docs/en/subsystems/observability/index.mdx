...
<TLDR>
  ...
  export over OTLP/HTTP. (3) **Tracing dashboard** — `app/observability` is a Grafana/Langfuse-style
  draggable panel grid (`lib/observability/` is a real percentile/bucketing/rollup engine) reading
  those spans. (4) **Performance** — `app/performance` (ADR-0035) is a Task-Manager-style live view of
...
<StatGrid>
  <Stat label="Routes" value="3" hint="/logs · /observability · /performance (+ crash dialog)" />
...
## The Three Routes (and one dialog)
...
| `/observability` | Tracing dashboard | Dexie `agentTraces` table | Yes (mobile too) |
...
<Cards>
  ...
  <Card title="Tracing dashboard" href="./observability/tracing-dashboard" description="app/observability: the Grafana/Langfuse-style panel grid. lib/observability is a real engine — exact percentiles, fixed-boundary bucketing, trace rollup + waterfall, group-by breakdowns, thresholds." />
