...
| **Agent Team dispatch** | `invoke_agent` / `cognia.team` / `agent-team` | `lib/ai/agent/team/teammate/dispatch-teammate.ts:246` (start), `:290` (success w/ usage) |
| **External agent bridge** | `invoke_agent`, provider via protocol, with `handoff` | `lib/ai/agent/external/session/agent-trace-bridge.ts:74` |
