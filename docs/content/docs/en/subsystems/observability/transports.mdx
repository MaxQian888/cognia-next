---
title: Transports
description: The nine logging transports, behavior-event exporters, generic OTLP and independent PostHog product/AI destinations, plus the three-state health model.
---
...
<TLDR>
  ...
  redacted entry to all registered transports. Nine implementations exist: **console**, **indexeddb**
  ...
  *durable* IndexedDB retry queue + Sentry/Loggly wire shims), **native** (Tauri `invoke` to the Rust
  logger), **observability-spool**, **breadcrumb**, **otlp-http** (exports agent-trace spans),
  **langfuse**, and **agent-trace** (persists embedded spans to Dexie). Product events have a
  separate structured-envelope exporter fan-out for OTLP Logs and managed/BYO PostHog. Health is
  exactly `healthy | degraded | offline` — there is no
  `unhealthy` state.
</TLDR>
...
<StatGrid>
  ...
  <Stat label="Span sinks" value="dynamic" hint="generic OTLP · managed/BYO PostHog · local agent-trace" />
</StatGrid>
...
## observability spool and breadcrumb

`ObservabilitySpoolTransport` writes versioned, bounded local recovery envelopes. The desktop-only
`BreadcrumbTransport` forwards redacted crash context to Rust. These are the two implementations
omitted by the former transport list; the deleted `otel-transport.ts` is not a current transport.
...
The same OTLP transport class can be registered as `agent-trace-posthog-managed` and
`agent-trace-posthog-byo`. These instances always disable content capture, use the exact
`/i/v0/ai/otel` endpoint, obtain their public project token through the typed Rust credential
seam on Tauri, and stamp `posthog.distinct_id` on every span so renderer spans resolve to the same
person the sidecar's `enrichSpan` attributes its spans to. The sidecar independently creates one
`BatchSpanProcessor` per generic OTLP or PostHog destination under a single `NodeSDK`.

Product analytics does not enter the logging pipeline: `track-event.ts` fans a structured event
envelope to OTLP Logs and to `posthog-product.ts` after behavior consent, category, sampling,
scalar, and PII gates. `posthog-product.ts` speaks PostHog's batch capture API directly —
`POST {host}/batch/` with `{ api_key, sent_at, batch }` — rather than embedding `posthog-js`,
because a browser SDK would open its own connection from the renderer and the desktop
`connect-src` policy allows none. The batch leaves through `telemetry_otlp_export` on Tauri and
`fetch` elsewhere, buffering until 20 events are queued or 2s pass; a destination whose config is
unchanged keeps its live exporter across settings saves, so a re-apply never drops a pending batch.

## langfuse

`langfuse-transport.ts` accepts only completed `AgentTraceSpan` envelopes; ordinary application
logs and metrics are ignored. It sanitizes and batches `AgentTraceBatchV1`, then calls the narrow
Host ingest command. The Host resolves the authenticated account's write-only BYO credentials,
revalidates schema, PII, consent, rate and size limits, deduplicates span IDs, and pins export to
`/api/public/otel/v1/traces` with Langfuse ingestion version 4.

The sidecar's shared `NodeSDK` also owns the official Langfuse processor for AI SDK 7 generation
and tool children. Generic OTLP and PostHog remain separate processors. Renderer roots keep the
stable `chat.turn`, `agent.run`, and `workflow.run` names; duplicate manual provider/tool children
are filtered only from remote destinations. Model and tool content have independent opt-ins,
default off, and both pass bounded field-level filtering plus the Host PII gate.
...
This is the write side of the [tracing dashboard](./tracing-dashboard) and the Traces channel of the
...
| observability-spool | bounded local recovery spool | yes | diagnostics via the logger core |
| breadcrumb | Rust crash-context ring | no | desktop best-effort |
| otlp-http | OTLP `/v1/traces` | yes (32) | `degraded`(no endpoint \|\| lastError) / `healthy` |
| langfuse | Cognia Host → Langfuse v4 OTLP traces | yes (32) | `degraded`(unconfigured \| lastError) / `healthy` |
