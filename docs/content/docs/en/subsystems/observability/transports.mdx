...
`/i/v0/ai/otel` endpoint, and obtain their public project token through the typed Rust credential
seam on Tauri. The sidecar independently creates one `BatchSpanProcessor` per generic OTLP or
PostHog destination under a single `NodeSDK`. Product analytics does not enter the logging pipeline:
`track-event.ts` fans a structured event envelope to OTLP Logs and lazily loaded named PostHog
instances after behavior consent, category, sampling, scalar, and PII gates.
