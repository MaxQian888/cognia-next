...
One Grafana-style range control (`5m…30d` + a pinned custom window,
`lib/observability/time-range.ts`) drives the list, the waterfall AND the
[tracing dashboard](./tracing-dashboard) in the channel's other sub-view, so the headline numbers and
the rows below them always describe the same set — they are folds of one `useObservabilityData` read.
Filtering happens over the whole window rather than the visible page, which is why the list reads a
window instead of paging with `queryRecentTraces`.

A span's **Show this trace in logs** action switches to the Logs channel with `?trace=<id>` and
remounts the panel so its mount-time URL hydration picks the focus up. The aggregate view of the same
spans is one click away on the same channel: the Dashboard sub-view (`?channel=traces&view=dashboard`),
which used to be the standalone `/observability` route.
