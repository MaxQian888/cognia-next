...
<Callout type="warn">
  Runtime capabilities are adapter-derived. Only Docker with the computer-server driver implements
  start, stop, health, delete, and GUI operations. Imported cua-cloud and Lume rows remain readable
  for compatibility and render their own instance/endpoint or VM/image fields, but all unsupported
  actions are disabled. No connection currently supplies the verified workspace adapter required by
  the `cua-desktop` shell/files tier.
</Callout>
