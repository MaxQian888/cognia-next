...
The shield, Devices, Settings tier picker, and connection actions consume one
`runtime-availability` projection. OS availability comes from the active confinement check, not the
cheap binary-presence probe. microVM availability comes from the currently accepting registered
adapter; the UI also explains that final eligibility requires an E2B-backed workspace. Persisted
unavailable selections remain visible and selected but disabled with an actionable reason.

## Diagnostics — shipped & wired

`components/settings/sections/diagnostics-section.tsx` mounts two ADR-0028 cards:

- **`SandboxAuditCard`** — calls `listAuditRows({ surface: "sandbox", limit: 50 })` directly and shows
  the newest 50 sandbox rows with
...
Capabilities come from the registered lifecycle adapter, not provider defaults. Docker /
computer-server exposes start, stop, health, delete, and GUI only. Existing cua-cloud and Lume rows
remain readable with their provider-specific configuration, but expose no lifecycle or GUI actions.
`cua-desktop` remains unavailable for shell/files because no verified workspace adapter exists.
...
- **`sandbox-tier-card.tsx`** — the default-tier picker (OS / microVM radio group), writing
  `AppSettings.sandboxTier`; it preserves unavailable persisted choices and displays why they cannot
  currently execute.
