...
Every call funnels through `dispatchSandbox`, which reads the active tier (stamped by
`build-options.ts` — see [Execution overview](./execution-overview)) and either calls the Rust
`sandbox_exec` command or the registered e2b exec.
