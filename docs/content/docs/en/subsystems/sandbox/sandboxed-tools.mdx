...
<TLDR>
  ...
  either the OS sandbox (the `sandbox_exec` Tauri command, T1) or the e2b microVM (T4), based on its
  immutable `SandboxRuntimeRef`. Session-bearing plugin calls recover a missing ref centrally from
  the active session binding; if none exists they fail closed. Only explicitly sessionless calls use
  the host sentinel. A microVM tier with no eligible E2B binding never falls back to OS execution.
</TLDR>
