...
| ----------------------- | -------------------------------------------------------- | ------------------------------------------------------------------------- | ---- | --------------------------------------- |
| **Execution sandbox**   | Model tool calls (`Bash` / `Edit` / `Write` / editor)    | OS-native (`bwrap`/`sandbox-exec`/runner) · microVM · per-action policy   | 0028 | Linux/macOS/Windows real                |
...
| **Plugin isolation**    | Third-party plugin code (JS / Node / WASM)               | `PluginSandbox` globals · Node 24 `--permission` · Wasmtime/WASI          | 0028 | Renderer + T2/T3 wired                  |
...
There is no fifth family. `lib/sandbox/web` used to hold a stub for a "web code-execution sandbox"
that was never shipped and had no caller; it was removed. Running JavaScript / HTML / CSS in the
browser is `lib/native/code-execution-strategy.ts` (an opaque `sandbox="allow-scripts"` `srcdoc` iframe),
which the Canvas execution panel uses on every shell; Python on that panel is desktop-only and goes
to the Rust `canvas_run_python`.
...
flowchart TB
  ...
  subgraph Plugin["Plugin isolation (ADR-0028)"]
    ...
    T3["native host (T3, Rust/Wasmtime)"]
  end
...
<Files>
  <Folder name="crates/cognia-automation/src/sandbox/ (T1 — OS execution)" defaultOpen>
  ...
  <Folder name="crates/cognia-sandbox-runner/">
    <File name="src/main.rs — Windows restricted-token + low-integrity + Job Object runner with stdout/stderr capture" />
  </Folder>
  <Folder name="src-tauri/src/bin/">
    <File name="cognia-sandbox-setup.rs — reserved for future optional per-SID firewall setup" />
  </Folder>
  ...
  <Folder name="lib/plugin/ (plugin isolation)">
    ...
    <File name="core/wasm-loader.ts — generation-bound T3 bridge" />
  </Folder>
  <Folder name="UI + data">
