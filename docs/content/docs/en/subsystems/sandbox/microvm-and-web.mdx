...
<TLDR>
  ...
  configured in that plugin's settings via the API URL field. `types/system/sandbox.ts` carries the
  cross-boundary result and consumption-metadata shapes.
</TLDR>
...
(`executeCodeWithSandboxPriority`): JS / TS / JSX / HTML / CSS run in a `sandbox="allow-scripts"`
`srcdoc` iframe on every shell, and Python is routed to the Rust `canvas_run_python` on desktop.
`hooks/canvas/use-code-execution.ts` and `components/canvas/code-execution-panel.tsx` are its
consumers.
