...
<TLDR>
  React Flow v12 editor + Zustand+zundo store + elkjs auto-layout, with 38 node kinds across 7
  categories. Runtime is split between Rust (cron daemon, webhook receiver, SQLite mirror) and TS
  (orchestrator, node execution) for hybrid execution; event logs enable crash recovery. Schema v22
  introduces 4 Dexie tables and 10 built-in templates.
</TLDR>
