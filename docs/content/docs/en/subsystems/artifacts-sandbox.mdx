...
<Callout type="warn">
  **This is currently broken in every shell.** React 19 publishes no UMD build, so
  the `unpkg.com/react@19/umd/…` tags below 404 and every React preview falls
  through to the 15-second timeout message. Serving the runtime locally is blocked
  on measuring, in a `pnpm tauri build`, whether an `about:srcdoc` subframe
  inherits the packaged shell's CSP — see the "Open" section of
  [ADR-0158](../adr/0158-artifacts-and-canvas).
</Callout>
...
- **Copy / Download / Export as** — driven by `runtime-adapters.ts:exportFormats`. `raw` is always offered; `html`/`svg` add their own source form; `png` and `pdf` are real since ADR-0158 (`lib/artifacts/export/`). `react` offers `raw` only — an off-screen capture of unexecuted JSX is a blank rectangle. Rasterising a `chart`/`mermaid`/`math` artifact needs its preview mounted, and says `ArtifactPreviewNotMountedError` when it is not, rather than returning an empty image. Everything downloads through `exportArtifact`, replacing the three paths that disagreed (one of which named a chart `chart.chart`).
