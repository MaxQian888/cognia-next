...
<TLDR>
  ...
  through a React inline renderer. `lib/sandbox/` is a standalone code-execution sandbox surface —
  currently a stub on the Web side; the desktop Python path is implemented in [./canvas](./canvas).
</TLDR>
