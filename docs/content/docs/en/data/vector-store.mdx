...
Cognia needs a vector index for the [Employee Digital Twin](../subsystems/employee-twin) RAG path. The store lives behind a thin abstraction (`packages/vector/src/store.ts`) so the rest of the app doesn't care which backend is active.
...
flowchart TB
  ...
  Caller --> Store[packages/vector/src/store.ts<br/>VectorStore interface]
...
| Add a new metadata filter | `src-tauri/src/vector/filters.rs` and `packages/vector/src/store.ts` types          |
...
<Cards>
  <Card title="Employee Digital Twin" href="../subsystems/employee-twin" description="What the vector store is for." />
