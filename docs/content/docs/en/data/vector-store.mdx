...
Cognia needs a vector index for the [Employee Digital Twin](../subsystems/employee-twin) RAG path. The store lives behind a thin abstraction (`lib/vector/store.ts`) so the rest of the app doesn't care which backend is active.
