...
<TLDR>
  All browser-side state lives in IndexedDB via Dexie under the database name `cognia-claude`.
  Schema versions are append-only — every release adds new `.version(n)` blocks; old ones are never
  edited. Migrations are idempotent and run on every database open.
</TLDR>
...
<Callout type="info" title="Idempotent upgrades">
  Each version is idempotent — opening an old database upgrades it through every intermediate
  version automatically. Dexie applies them in order. **Never modify a previous `.version(n)`
  block.**
</Callout>
