...
<Status variant="stable">v115</Status>
...
<StatGrid>
  <Stat label="Schema version" value="115" hint="current" />
...
All browser-side state is persisted in **IndexedDB via Dexie**. The schema is defined in `lib/db/schema.ts` and is currently at **v115**. Schema versions are append-only — never mutate an existing `.version(n).stores({...})` block.
