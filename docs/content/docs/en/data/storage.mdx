...
<Status variant="stable">v54</Status>
...
<StatGrid>
  <Stat label="Schema version" value="54" hint="current" />
...
All browser-side state is persisted in **IndexedDB via Dexie**. The schema is defined in `lib/db/schema.ts` and is currently at **v54**. Schema versions are append-only — never mutate an existing `.version(n).stores({...})` block.
...
| **v16–v54** | Subsequent additions: workflow tables, goal tracker, OCR results, subscription usage, sync cursors, share links, agent-team persistence, character pack overlays, GitHub delivery state, ext. agent configs, mobile sync, perf samples, terminal sessions, etc. Read `lib/db/schema.ts` for the exact diffs. |
