...
<TLDR>
  AES-GCM-encrypted, integrity-checked backups in package format **v3**.
  Auto-key per-device (no passphrase) or user passphrase for portability.
  Six per-domain transfers (skills, MCP, presets, characters, teams, theme)
  let you move just one slice. ChatGPT / Claude / Gemini imports map external
  exports into Cognia's chat shape.
</TLDR>
...
The manifest is duplicated outside the ciphertext so the user can see *what* the file contains (timestamp, schema version, flags) without entering the passphrase.
...
<Cards>
  ...
  <Card title="Scheduler" href="./scheduler" description="The scheduling primitives the backup scheduler builds on." />
  <Card title="ADR-0001" href="../adr/0001-backup-schema-v3" description="Why schema v3 looks like this." />
</Cards>
