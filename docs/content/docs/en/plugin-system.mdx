...
<TLDR>
  Sandboxed Web Worker per plugin, declared-and-consented permissions, an
  RPC bridge to a versioned API, and a marketplace for distribution.
  Plugins ship as signed `.cognia-plugin` archives and run isolated from
  the host DOM.
</TLDR>
...
<TypeTable
  type={{
    ...
    name: { description: "Display name shown in the marketplace and settings.", type: "string", required: true },
    ...
    entry: { description: "ES module entry path; loaded into the worker.", type: "string", required: true },
    permissions: {
...
<Cards>
  <Card title="Scheduler" href="./scheduler" description="The engine plugin scheduled jobs run on." />
  <Card title="Storage" href="./storage" description="The plugin tables (v15) in detail." />
  <Card title="Sidecar & Claude SDK" href="./sidecar-and-claude-sdk" description="Plugins extend the sidecar via MCP." />
</Cards>
