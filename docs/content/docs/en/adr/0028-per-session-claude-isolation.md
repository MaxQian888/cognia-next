...
- **Dangerous-env scrub.** Beyond `LD_*` / `DYLD_*` / `NODE_OPTIONS`, the denylist now drops `GCONV_PATH` (glibc iconv module injection — an `LD_PRELOAD` equivalent), the `GIT_CONFIG_*` family (arbitrary git-config / alias / pager injection), and `HOSTALIASES` / `NLSPATH` / `RESOLV_HOST_CONF` resolver redirects.
