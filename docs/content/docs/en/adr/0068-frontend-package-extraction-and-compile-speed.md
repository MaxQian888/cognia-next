...
| **C3** | Defer heavy layout initializers | Split the web-side `null`-rendering initializers into capability-scoped `next/dynamic(ssr:false)` boundaries (`core-chat`, `plugin-runtime`, `workflow-automation`, `integrations`, `knowledge-agents`, `desktop-tools`). The default `eager` profile requests all boundaries; development-only `main` requests core chat and loads optional boundaries on route, command, tool, or real background activation. | Lower main-flow dev cold-start and RSS without removing functionality | M | M — the coordinator preserves dependency and mount order; production always resolves to `eager` |
...
- **Making `turbopackFileSystemCacheForDev` the default** — blocked upstream by a single-writer corruption race (vercel/next.js#90691); Next exposes only on/off, no safe partial enable. Keep it off by default and expose it only through the explicit `dev:cached` / `tauri:dev:cached` commands. Cache-off cleanup targets only `.next/dev/cache/turbopack`, never the whole `.next` tree.
...
| **E3** | `@cognia/tts` | `lib/tts` (25 files) + `types/media` leaf | Native bridge confined to keyring and proxy-fetch seams → inject `{ fetch, getSecret }`; retired Edge/Realtime transports were removed in ADR-0075's 2026-08-28 consolidation | M | Wanted on mobile (Capacitor) + sidecar; isolates 3rd-party TTS providers |
