...
9. **`WorktreeCreate` / `WorktreeRemove` hooks activated.** The Registry state machine is the producer, firing on entry to and exit from `active`. `event-catalog.ts` removes `dormant: true` for both events, `isDormantEvent` test assertions update accordingly, and both events observe the existing Workspace Trust gate before any handler runs.
