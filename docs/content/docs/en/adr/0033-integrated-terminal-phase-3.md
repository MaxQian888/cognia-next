...
6. ~~**Session sharing**~~ — **shipped** as ADR-0131: paired devices attach to the host session and share its controller lease; the host broadcasts the participant roster and the dock gains a Share dialog. The unwired `lib/terminal/collaboration/share-manager` invite/token model was removed in the same change.
