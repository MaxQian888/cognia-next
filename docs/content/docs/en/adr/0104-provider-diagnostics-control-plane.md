...
- The clock is one `provider-diagnostics-refresh` scheduled task, seeded by `initSchedulerSystem`. Its executor is declared in `lib/scheduler/executor-owners.ts`, so a scheduler started without that boot (a lifecycle write that arrives before the scheduler initializer mounts) loads the executor when the task comes due instead of failing with `EXECUTOR_NOT_FOUND` on every tick. It runs on every host the scheduler runs on, the plain web shell included.
