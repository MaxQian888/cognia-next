...
- **Cron-driven continuation** — DONE. The scheduler `goal` executor drives `lib/scheduler/executors/goal-headless-runner.ts:runGoalLoopHeadless`. Its turns run unattended: `createUnattendedPermissionResponder` denies a permission request at once, and `handleTurnComplete` (given the turn's denied tools as `needsApproval`) pauses the goal with exit `needs_approval` wherever it would otherwise continue. The one exception is the turn that arms the completion-promise check.
