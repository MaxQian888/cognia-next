...
- **Cron-driven continuation** — DONE. The scheduler `goal` executor drives `lib/scheduler/executors/goal-headless-runner.ts:runGoalLoopHeadless`. Its turns run unattended: with no `RunGoalLoopInput.onPermissionRequest`, `createUnattendedPermissionResponder` denies a permission request at once, and `handleTurnComplete` (given the turn's denied tools as `needsApproval`) pauses the goal with exit `needs_approval` wherever it would otherwise continue. The one exception is the turn that arms the completion-promise check. A caller that has an approver passes `onPermissionRequest`. Its answers are decisions, so a human's Deny never feeds `needsApproval`. Only the requests it hands to the `unattended` fallback do.
...
- **Connector inbound goals** — DONE. `/goal` is a connector control command (`lib/connectors/commands/goal.ts`): it reuses `dispatchGoalSubcommand` for the subcommand grammar and pumps a headless driver (`runGoalLoopHeadless` + per-turn delivery + pacing gate) because an IM session has no chat hook. Guarded by the v49 `ConversationOverrideRow.allowGoalDriving` opt-in. Tool approvals use the conversation's human-in-the-loop flow. The driver passes `makeImPermissionResponder` (`lib/connectors/hitl/tool-approval.ts`) as `onPermissionRequest`, with the approval mode read from the conversation override on each request, the `/goal` sender as the actor scope, the registry TTL and `CONNECTOR_TURN_TIMEOUT_MS` as the per-turn timeout, as `runtime.ts` does for inbound turns. It passes no run interrupt, because goal turns have no durable execution run. A card that expires or cannot be delivered goes to the unattended fallback (`onUnanswered`) and pauses the goal `needs_approval`. Runs wherever the connector runtime runs — desktop (all channels) + `cli serve` (webhook channels); the Capacitor mobile shell has no connector runtime.
