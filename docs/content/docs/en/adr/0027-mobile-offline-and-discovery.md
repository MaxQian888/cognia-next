...
## PWA install surface amendment (2026-09-14)

The Wave-4 PWA layer stopped at "technically installable": the manifest and
Serwist worker were emitted, but no product surface ever captured
`beforeinstallprompt`. This amendment records the productized install flow:

- `lib/pwa/install-state.ts` captures the single-shot prompt into a
  React-free external store. `PwaLifecycleInitializer` attaches it inside
  `LocaleGate` but above `AccountGate` — the event can fire while a locked
  vault holds the gate closed, and a missed capture strands the install
  entry until reload. `hooks/use-install-prompt.ts` is the
  `useSyncExternalStore` view over it.
- The single in-app entry is Settings → About (`InstallAppCard`), gated to
  `detectPlatform() === "web"`. States: `installable` (native dialog
  button), `installed`, `ios-manual` (Share → Add to Home Screen steps —
  iOS Safari never fires `beforeinstallprompt`), and `unavailable`
  (browser-menu copy). The card copy states explicitly that the PWA is a
  client of the deployment, not the desktop host.
- `app/manifest.ts` gains `id` (explicit identity — without it Chrome
  derives identity from `start_url` and any change orphans installs),
  `lang`, `dir`, `categories`, `launch_handler: focus-existing`,
  `shortcuts`, `screenshots`, and `protocol_handlers`
  (`web+cognia` → `/deep-link?u=%s`, reusing the existing deep-link page).
  The single static manifest keeps English identity strings — a deliberate
  trade-off vs per-locale manifest routes.
- `app/sw.ts` precaches `public/offline.html` (revision = app version, so
  every release re-precaches it) and registers a Serwist `fallbacks` entry
  for `destination: "document"`. An uncached navigation while offline now
  serves the bilingual offline shell instead of a browser error page —
  which inside a `standalone` window has no way back.
- `PwaBadgeInitializer` mounts inside the gate (it needs the post-unlock
  Dexie) and mirrors the mobile unread chat count onto
  `navigator.setAppBadge` while `display-mode: standalone`.
- Telemetry: `app.pwa.install.shown|accepted|dismissed` and
  `app.pwa.installed` under the `app` category. `app/layout.tsx` metadata
  was corrected to Cognia branding plus `appleWebApp` and `themeColor`
  viewport entries.
- Screenshot assets are generated by `pnpm screenshots:pwa`
  (`scripts/screenshots/capture-pwa-screenshots.mjs`) against the
  `NEXT_PUBLIC_E2E=1` static export, seeded with the disposable E2E account
  — never real user data.

Positioning, unchanged: an installed PWA is a client of the headless
deployment (`web-standalone` host profile). It does not gain sidecars,
filesystem, MCP, or connector-runtime capability — the card copy says so.
Web Push remains out of scope (needs VAPID keys plus server-side
subscription storage). `window-controls-overlay` title-bar reuse is a
deliberate follow-up, not part of this surface. Tauri SW registration
verification stays an open item above.
