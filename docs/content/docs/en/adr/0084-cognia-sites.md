...
Desktop is the authoring and deployment host. Mobile may receive a deliberate read-only projection only after its sync table, delta reader, tombstones, and handler are added; plain web does not claim access to local Sites metadata.
