...
- As of 2026-09-22, a fresh desktop install creates a device-managed local workspace and opens it automatically. Its random credential and recovery key live in the native encrypted secret store; neither is a bundled password or browser-storage secret. Account activation still verifies the native credential and unlocks the content vault before opening the database.
- Setting a password on the default workspace reuses password rotation and vault rewrapping, then enables the existing lock and idle-lock controls. Missing native credentials or vault records fail closed instead of replacing encrypted data.
- As of 2026-09-24, any desktop password profile can opt into "Unlock automatically on this device", either from the lock screen at unlock time or from Settings → Account → Security (which asks for the password once). The password it just proved is kept in the native secret store under `desktop-local-account/<accountId>`, the slot the device-managed workspace already uses, and boot opens the profile without a prompt. The password keeps working, so an unreadable store, a missing secret or a secret rejected after a password change elsewhere all end on the ordinary lock screen with the reason shown; a missing or rejected secret also turns the option off. A password change updates the stored copy.
- A profile that opens without a prompt (device-managed, or remembered) offers no Lock button and is never idle-locked: the next reload would undo the lock, so it protects nothing. Idle auto-lock defaults to Never and applies only to profiles that ask for their password at launch.
- A browser tab remembers a successful unlock in `sessionStorage` until the tab closes, in production as well as development, so a reload does not ask again. `lock()` and idle lock erase it. The desktop never uses this; its equivalent is the native store above.
- Debug desktop builds keep the secret store's master key in a `0600` file under `<data dir>/cognia/dev/` with a store file beside it, and never touch the Keychain. A rebuilt dev binary lost Keychain access on every launch, which locked the device unlock itself. `COGNIA_DEV_KEYCHAIN=1` restores the Keychain path in a debug build.
- Switching accounts resolves the target's credential (typed password, device secret or remembered password) before locking the current account, so a switch that could only fail no longer strands the user on a lock screen.
- Development skips the cloud sign-in gate unless `NEXT_PUBLIC_ACCOUNT_GATE=1` explicitly enables authentication testing. A production install with no configured multi-tenant deployment needs no cloud login.
