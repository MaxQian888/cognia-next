...
<Callout type="info" title="ADRs">
  Architecture Decision Records live in the [ADR catalogue](./adr). They preserve decision rationale; current
  behavior is determined by the implementation and tests. When an ADR and the implementation diverge, the docs
  describe the implementation and call out the drift instead of silently treating the ADR as runtime truth.
</Callout>
