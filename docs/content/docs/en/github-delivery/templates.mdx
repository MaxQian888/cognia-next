...
<TLDR>
GitHub Delivery ships with **8 built-in workflow templates** (`lib/workflow/definition/seed-github.ts`),
all marked `isBuiltIn: true` + `isTemplate: true`. When a user clicks "Use template" in the workflow library,
a forked editable copy is created with a regenerated id, but the node / edge structure is preserved as-is.
All templates use only registered executors and run out of the box.
</TLDR>
...
<Callout type="warn">
**Long task reminder.** `runIssueLoop` may run for tens of minutes. `lib/workflow/runtime/long-step-runner.ts`
writes checkpoints to `workflowRunEvents`; after a process crash, resume continues from the latest checkpoint without
re-running the LLM.
</Callout>
