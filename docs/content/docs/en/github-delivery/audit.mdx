...
<TLDR>
GitHub Delivery writes every policy decision (allow + deny) to the namespaced `audit` table;
the table is capped at 5000 rows, and when it overflows the oldest rows are pruned in ascending
order by `at`. The Issue → PR loop state machine is reflected in the `workOrders` table,
displayed as a 6-column kanban on the standalone `/github-delivery` page.
Both tables are stored via the M0 "Plugin Dexie Tables" mechanism, so their actual full names
carry the `github-delivery:` prefix.
</TLDR>
...
<Callout type="warn">
**On plugin uninstall.** Per M0 retention rules, all data is kept by default (`removePluginTables(..., "keep")`).
If "Delete data" is selected during uninstall in Settings → Plugins, all 4 tables are dropped —
including audit. Export audit data first via "Export CSV" in the Audit Tab.
</Callout>
