...
**7. 只路由被抬起来的那几条，且只在配对档位上路由。**
`lib/connectors/device-plane.ts` 持有设备可达的 `connectors_*` 名单 —— 上面那四条，
再无其他 —— 默认 invoker 在 `detectHostProfile()` 判定 keyring 位于配对宿主上时，
把且仅把这四条送上 `transport`。其余 38 条继续走 Tauri `invoke`，因为路由它们只会把
`control-reach.ts` 那句精确的「这个控件直接对话运行时进程」换成一个 403。

判据刻意选宿主档位而不是 `!isTauri()`：`web-standalone` 浏览器没有宿主可达，
`headless` 大脑则整个替换掉 invoker，两者都保持本地行为，也都不会发出注定失败的往返。
它同时也是 `credentialLeaseRequired` 用的那个判据 —— 「往那里路由」与「它需要租约」
不可能各说各话。

**8. 每个会话一段租约，由操作者动作触发，靠时限收回。**
`lib/connectors/credential-lease.ts` 一次申请覆盖全部四种操作的租约，并缓存到过期为止。
一个 Slack 表单要读五个凭据、最多再写五个；问十次只会训练操作者不看就点同意。被拒绝的结果
会被短暂记住，这样反复挂载的对话框不会每帧排一个提示；显式重试会清掉它 —— 这正是 `stored`
字段上那个新的「解锁」入口所做的事。租约走参数而不是 header，因为 `rpc.rs` 就是从参数里读的，
也因为 WebRTC DataChannel 那条路根本没有 header。
...
- OneBot 的「验证」与「探测」按钮改为渲染并禁用，而不是消失 —— 与能力面采纳的规则一致：
  渲染、禁用、写明原因。
- 配对的浏览器或手机现在可以读回并写入连接器凭据了。连接器设置区里的其余控件仍然显示
  `runs-on-host`，这依然是实话：测试令牌、探测机器人身份走的是运行时进程，那部分没有移动。
- 一个本壳读不到的凭据不再是死路。`stored` 旁边会渲染一个「解锁」，而这个状态过去唯一提供的
  选项 —— 蒙着眼覆盖掉它 —— 不再是唯一选项。
- 从伴侣端删除适配器现在会真的销毁它的密钥，而不是把它们记进 `failedCredentials`。
  凭据清除是这条路径上唯一一个被拒绝就会留下残留的步骤。
- 预估的「约 21 个套件」代价没有发生：那是按 `!isTauri()` 路由的后果 —— 那会让每个 node
  环境的测试都看起来像伴侣端。改按档位判定后，适配器套件一个没动，只需要改一个。
...
**把全部 `connectors_*` 包装函数都改走 `transport`。** 一条规则确实比一张名单干净，
但 42 条里有 38 条会撞上一个在 RPC 层看到它们之前就 403 的传输门。名单已经与
`protocol/companion-commands.json` 做了双向钉定，将来任何放开新命令的清单改动都会让测试
变红，而不是烂在那里。

**在 invoker 里惰性铸租约。** 那样一个调用点都不用改，但也会让任何后台凭据读取 ——
运行时启动、健康探测 —— 悄悄申请一次提权。申请动作因此留在那两个「操作者正站在表单前」
的调用点上。
