...
- **引用回复与后续气泡相互独立。** 引用决定出站消息是否引用触发它的那条消息；后续气泡决定多段回答如何拆分。中继的人工回复通过 `connector_enqueue_outbound` 原样携带 `replyTo`，因此手机回复的引用行为与桌面回复完全一致。
- **入站"可通知性"现在是共享谓词。** `lib/connectors/inbound-notifiability.ts:isNotifiableInboundEvent`（从通知中心桥抽出）是桌面通知路径与中继 `connector://message-added` 推送共同查询的对象。编辑、删除、表情回应以及机器人自己的出站回声只在一个地方被排除，因此两个面永远不会对"什么算新消息"产生分歧。
