...
`language` 默认是 `auto`，扫描前用 `ctx.i18n.getCurrentLocale()` 解析；
显式设置优先——一个用中文阅读的人，完全可能要为一个说英语的团队生成英文 wiki。
未映射的 locale 回落到英文，而不是把一个 locale code 塞进提示词：
在那里查表未命中本来也会静默变成英文，而且**看起来像是生效了**。

面板自己的界面文案——七条——来自一张带英文默认值的标签表，
通过 `ctx.i18n.t` 对着插件自己的 manifest 词条翻译。回落是**逐键**的，
所以一份不完整的词条只会让其余几条留在英文，而不是把点分 key 原样画进面板。
`lint:i18n` 只扫 `.tsx`，永远抓不到这里的任何一条，
所以有一个测试断言：面板画出的每一个字符串都来自那张表。
...
**326 个测试**，上游是 164。多出来的是改写，加上三个换层与它们汇合处那条管线的新套件
