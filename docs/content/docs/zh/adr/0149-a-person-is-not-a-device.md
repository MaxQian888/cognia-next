...
## 实现更新：对话恢复（2026-09-07）

共享会话的普通发言与显式请求 AI 分离。AI 请求引用已持久化的用户消息和服务端
上下文边界，通过 `shared-chat-execution-v2` 能力协商启用 FIFO 队列与执行租约的
原子领取。正常完成后由指定执行端继续消费队列；更换执行端或恢复中断执行需要
用户明确接管，不自动重放结果不确定的工具副作用。

会话发现和活动订阅由应用生命周期管理，不依赖控制抽屉。重连时分页补齐事件，
订阅后再次追平历史；事件缓存、消息投影和已应用游标在同一事务提交。
本地投影区分服务端、组织、会话和消息身份，修订不能修改其他会话的消息。
工作区列表替换与单会话更新分离。离线输入保留为草稿，等待用户明确发送。

Schema v225 新增 `sharedRunJournals`，使用账号数据库加密存储执行恢复信息和
待确认消息身份，仅保留在当前设备，不进入可移植备份或 Companion 同步。
租约凭据作为嵌套内容加密，不进入索引明文；终态与服务端核对完成后删除日志，
删除账号时随数据库一并删除。

流式通知只标记游标需要追平，不为每条通知分别排队拉取。最多保留一个正在执行的
拉取和一个待处理标记，拉取期间收到的新通知仍会触发补齐。仅元数据事件推进游标
时不扫描消息；消息变化只在同一事务中更新自身的媒体引用，保留其他消息的引用，
存储失败时一并回滚投影写入。

确定性回归场景执行十次采样：50 条突发通知从 50 次历史拉取减少为 2 次；
修改 20 条含附件消息中的一条，从写入 20 条媒体引用减少为 1 条。
这些是操作次数约束，不代表生产环境延迟或耗电实测。
