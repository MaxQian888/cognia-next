...
持久运行时继续复用 fair scheduler、工作流合成器、Registry 工作区租约、权限级联、
evidence bundle 和统一运行日志，不新增平行的调度器、恢复存储或产物存储。

- 在创建运行的同一事务中检查团队是否已有活动运行。定时任务使用由 execution ID
  派生的稳定 run ID；替代运行记录父运行，并等待已中止运行清理结束。取消操作绑定具体运行。
- 调度准入和工作区等待共同响应取消。CAS 状态迁移保留控制操作和终态；wave 超时覆盖
  执行与 checkpoint。缺失依赖和环路直接失败，过滤后的任务仅接受已验证完成的外部依赖。
- 恢复使用启动时冻结的约束和安全 checkpoint。缺少快照、不确定副作用及未覆盖的远端事件
  必须人工介入。远端事件和游标先原子落盘再投影；steering 在完成确认之前保持可重放。
  bootstrap 启动恢复流程后即可继续，不等待整个任务执行结束。
- 初始化、执行、证据与结算失败统一清理。child 创建和初始日志在同一事务中，
  失败回滚不完整的创建并停驻 run；持久化失败会中止活动 dispatch。清理保留原始异常和资源释放异常；dispose 重试沿用首次结算意图。
- 沙箱要求根据真实运行时能力探测。子任务不能放宽继承权限；无法执行所需权限、工具或沙箱
  策略的 external/remote 协议拒绝派发。这是能力边界检查，并未为这些协议新增策略执行能力。
- 证据绑定具体 child、attempt 和工作区 revision。验证必须明确成功，后续可能修改资源的工具
  会使验证失效。源码 revision 使用 Registry 内容 hash，包含删除和文件模式变化；generated
  元数据行不纳入，也不构成生成产物的内容完整性证明。缺少源码 hash 或必需 revision 时拒绝
  验收；读取持久化内容时校验摘要和大小。
- 交付审批绑定 head 与 base。重试复用现有 PR 和已完成节点；合并前重新校验远端 head/base，
  并提交 expected head SHA。提供方不支持原子的 expected-base 条件，因此并发远端 retarget
  仍是提供方能力边界。

进一步的并发约束：同一父运行的 restart 复用持久化 replacement，包括已结束的 replacement；
child 创建原子化，重复启动不能接管活动准入，旧 attempt 句柄不能结束新的重试。
并发 wake 合并且仍需通过调度准入。写入范围在重叠检查前复制并规范化，避免可变数组与路径别名
绕过。拒绝重复打开环境，也不允许 terminated child 恢复。交付图原子创建，审批拒绝陈旧快照，
发布重试继续持久化 draft。

存在未结束工具调用时不能完成 dispatch。帮助、枚举、仅编译不执行测试、修复模式及 shell 组合
命令不计作验证；最新失败的检查必须成功重跑，不能被另一项成功检查掩盖。证据查询复用现有
child/task 索引，同一次校验只检查每个共享内容 hash 一次，不跨校验缓存完整性结果。

回归测试覆盖准入取消、停止与完成竞争、前序运行清理、不确定重放、落盘失败、缺失 revision、
验证失败、工作区清理与交付对账。这些测试不替代真实 external provider、远端主机或 GitHub 验收。
