...
本文档中的 adapter 未变，但**谁能驱动它们**变了。手机或浏览器上的操作者现在通过中继（ADR-0131）为这些连接器审批草稿、发送人工回复，中继抵达持有该 adapter 的宿主，而不是就地执行。两点值得点名：

- 操作者在手机上审批时编辑过的 segments 才是被投递的内容——`connector_approve_draft` 携带可选的 `segments` 数组，草稿自身的文本不再被静默视为权威。
- 本 ADR 的方法矩阵对薄客户端没有任何变化：不支持的方法依然不支持，中继呈现的能力错误与桌面端一致。
