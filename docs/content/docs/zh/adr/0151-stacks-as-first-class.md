...
- **源代码管理** —— 一个 Stacks 面板，列出、校验、restack，以及记录或清除父指针。
- **Agent Team** —— `githubDeliveryPolicy` 的设置开关（默认关），以及一个不再把「当前检出的
  分支」当作主干的主干读取。
...
**两种创作模型收敛到同一条分支链上。** 分支即层把父指针放在 git config 里；commit-per-PR 用提交
时写入的 `Cognia-Change-Id` trailer 标识一个变更（不安装任何 git hook）。两者产出的都是同一条
真实分支链，而那是 forge 唯一看得见的东西。
