...
> **状态**：已接受于2026-05-18。2026-05-18修订，增加了三个本地提供商——`ocrs`（通过RTen pure-Rust ONNX）、`paddle-ocr`（通过`oar-ocr`+ONNX 运行时 PP-OCRv5）和`local-http`（用于自托管服务器如Umi-OCR / PaddleOCR-Server的通用适配器）。
...
2. 平台本地偏好表——Windows + MSIX 选`windows-media-ocr`，macOS / iOS 选`apple-vision`，Android 选`mlkit-android`，浏览器选`tesseract-wasm`。
...
- `ocr_download_model(backend)` — 将上游模型文件流式传输到`<app_data>/cognia/ocr/<backend>/`，随着字节落地`ocr://download-progress`事件。完成时写入带有SHA-256s的 `manifest.json`。幂等 — 对于已匹配摘要的文件，重新运行成功下载是不可操作的。

真实绑定搭载了四个Cargo feature（全部默认关闭，以确保标准构建保持快速;发布流水线开启`ocr-ocrs`和`ocr-paddle`），此外还有一个始终为其平台开启的目标门控后端：

- `ocr-tesseract` → `tesseract-rs`（跨平台，静态链接libtesseract + leptonica）。
...
- `ocr-paddle` → `oar-ocr` + `ort`（PP-OCRv5通过ONNX 运行时;固定到`ort = "=2.0.0-rc.12"`以避免RCs间ABI流失）。
...
- PP-OCRv5_mobile型号——Apache-2.0。

下载路径通过OS的常规HTTPS栈;除了标准TLS外，没有签名或固定。随文件记录一起写入的可选`manifest.json` SHA-256s用于后续启动时的篡改检测。
