...
- 路由 = `/settings`，唯一一条路由；section 切换走 `?section=<id>` query，不刷新整页。
- 导航源点：`components/settings/settings-nav-config.ts` —— 一个 `SETTINGS_NAV[]` 静态数组，加 section 就在这里加一行。
- 每个 section 是一个独立组件，通过 `dynamic()` 异步加载，落进 `settings-shell.tsx` 的 `<SectionContent>` 大 switch。
- 二级 tab 由 section 自己维护，URL key 用 `?<sectionName>Tab=`（如 `?dataTab=overview`、`?appearanceTab=theme`），便于深链。
- 桌面与移动复用同一份 `SettingsShell`；移动壳在外面套抽屉、收紧间距。
- 设置数据落在 Dexie `settings` 表（单行 key `"singleton"`），`SettingsSyncProvider` 启动时从 Rust 同步。
...
- **Dexie 表**：`settings`，单行，主键 `"singleton"`。schema 见 [storage](./storage)。
...
<Cards>
  ...
  />
  <Card title="技能系统" href="./skills-system" description="?section=skills 的技能库管理" />
  <Card title="角色管理" href="./chat-and-skills" description="?section=characters 的角色编辑" />
  <Card
    title="MCP 服务器"
  ...
  <Card title="插件系统" href="./plugin-system" description="?section=plugins 的本地插件安装" />
  <Card title="调度器" href="./scheduler" description="?section=scheduled-tasks 的 cron 与历史" />
  <Card
    title="外部桥"
    href="./external-bridge"
    description="?section=external-bridge 的 MCP 服务器对外暴露"
  />
  <Card title="远程控制" href="./remote-control" description="?section=remote-control 的设备链接" />
  <Card
    title="伴生端 API"
    href="./companion-api"
  ...
  />
  <Card title="Slash 命令" href="./slash-commands" description="?section=slash-commands 的注册表" />
  <Card title="语音" href="./misc-subsystems" description="?section=speech 的 TTS / STT 配置" />
  <Card title="日志" href="./observability" description="?section=logs 与 ?section=diagnostics" />
</Cards>
