...
| **Workflow Refactorer** | `workflow-refactorer.ts:26` | 11（读 + 改 + `wf_batch_apply` + 布局 + group） | 15 | 是 |
...
兄弟模块 `lib/claude/agents/markdown-agents.ts`（`665a6b81` 引入）把用户撰写的 `*.md` agent 文件——YAML frontmatter + Markdown 正文——经 `buildMarkdownAgents()`（`:101`）与 `markdownAgentsToSdkMap()`（`:120`）解析成**同一个** SDK `AgentDefinition` 形状。其 docstring 意图把结果合并进 `resolveAllSubagents`（`markdown-agents.ts:13`），但目前**还没有生产调用方**——`buildMarkdownAgents` 仅被自身测试引用，所以用户 markdown agent 目前**不会**进入 `opts.agents`。这区别于下文的 `subagent-importers/` 管线，后者物化的是*模板 / 角色*，而非活跃的 `opts.agents` 条目。
