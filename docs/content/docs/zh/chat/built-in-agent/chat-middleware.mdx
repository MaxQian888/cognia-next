...
<TLDR>
  一个中间件是 `(req, next) => Promise<Response>`（`plugin-chat-middleware.ts:116`）。插件经 `ctx.chat.use`
...
- **超时**——`Promise.race` 对抗 `entry.timeoutMs`（`raceWithTimeout`，`:136`）。默认 5s，上限 60s（`registry.ts:24`）。
...
`runAndCaptureAssistantReply`（`:217`）在开关关闭（`:223`）**或**无活跃中间件（`:226`）时直接短路到核心 capture。开启时构建 `ChatMiddlewareRequest`（`:231`），以真实 capture 为 `terminal` 运行链（`:250`），并应用返回的文本变换（`:260`）。流式聊天被刻意排除——其文本已渲染，事后变换会是可见的闪烁（理由注释，`:208`）。
...
| 类型 | `types/plugin/plugin-chat-middleware.ts`（`ChatMiddlewareRequest:71`，签名 `:116`） |
