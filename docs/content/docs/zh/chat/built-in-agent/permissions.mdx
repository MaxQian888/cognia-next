...
<TLDR>
  **A 层**在 sidecar 里：SDK 的 `canUseTool` 回调（`anthropic.mjs:263`）查阅 build-options 序列化出的静态
  ...
  `permission_request` 处理器（`use-claude-chat.ts:1267`）运行 Auto-mode，一条**用户 glob 规则 → 确定性
...
flowchart TB
  ...
  subgraph B["B 层 —— renderer（use-claude-chat.ts:1267 permission_request）"]
    AL["总是放行列表 :1269"]
...
该解析器是 `ruleset.ts` 故意**更弱**的 JS 镜像：`splitBash`（`permission-resolver.mjs:100`）是忽略引号的朴素顶层切分，且**没有**内置 `"*": "allow"`。任一段 deny 即胜出；仅当*每*段都显式 allow 才返回 allow（`:143/:153`）。完整的复合命令解析器在 B 层，未命中调用会回传到那里——所以削弱 A 层是安全的。

### B 层 —— renderer Auto-mode

`permission_request` 分支（`use-claude-chat.ts:1267-1362`）：

1. **总是放行列表**（`:1269`）——已有的每会话授权。
2. **非活跃/远程会话**（`:1277`）——带兜底 deny 的远程控制路由；非 Auto-mode。
3. **Auto-mode**（`:1316`）：构建裁决客户端（`buildUtilityLlmClient`，featureId `command-safety`，`:1318`），用 `pluginRules: getPluginCommandRulesets()` 调用 `runAutoModeForTool`（`:1324`）。`allow` → `approveTool(..., "allow")`（`:1333`）；`deny` → `approveTool(..., "deny", reason)`（`:1337`）；`try/catch` fail-open（`:1345`）。
4. `ask`/`null` → 手动弹窗 `pushApproval`（`:1360`）。
...
flowchart TB
  ...
  D2["分类器决断（allow/deny）？<br/>→ source:rule :95"] -->|ask| D3
...
完整阶梯（最高 → 最低）：**用户 `commandRules` glob → 插件规则 → 确定性分类器 → 模型裁决 → ask**。`auto-mode-runner.ts:56` 用数组顺序编码——插件规则在前，用户 `{Bash: commandRules}` 最后 push，使用户总是压过插件。
...
- **两层都 fail-open**（ADR §56）：A 层 catch（`anthropic.mjs:293`）、B 层 catch（`use-claude-chat.ts:1345`）、裁决器每条失败路径返回 `null`、`evaluateAutoDecision` 绝不抛（`auto-mode.ts:70`）。
