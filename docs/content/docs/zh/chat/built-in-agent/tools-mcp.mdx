...
<TLDR>
  ...
  （`mcp-presets.ts`，15 个预设）只是“添加服务器”UI 目录。
</TLDR>
...
flowchart TB
  subgraph SDK["sidecar query() mcpServers"]
    ...
    UM["用户 MCP 服务器<br/>build-options.ts:958"]
  end
...
**coreFiles** 套件绑定解析器（同 lsp），**在 Anthropic 路径默认关闭**（claude-agent-sdk 自带 Read/Edit/Bash）；它服务于非 Anthropic 的 ai-sdk 路径——该路径模型没有 SDK 原生文件工具。`apply_patch` 是单文件 `multi_edit` 的多文件补充：原子地对一个或多个文件应用 git 风格统一 diff——支持创建（`--- /dev/null`）与删除（`+++ /dev/null`）——并具备与 `edit`/`write` 相同的读后改强制、BOM/EOL 保留及写后 LSP 诊断。工具按固定的 `CORE_TOOL_NAMES` 顺序发出以保持 prompt 缓存前缀稳定；新工具一律追加在末尾。
...
开关来源是 `opts.builtinTools`（`build-options.ts:972`）——启用/禁用各类别的设置 JSON。

### 核心 `bash` —— 随操作系统选择 shell

自由形式的核心 `bash` 工具（`builtin-tools/core/bash.mjs`）驱动的是宿主机首选 shell，而非固定二进制：Windows 上按 PATH 探测 **PowerShell 7（`pwsh`）→ Windows PowerShell（`powershell`）→ `cmd.exe`**，其它平台用 `/bin/sh`（`shared/shell-detect.mjs::resolveShellDescriptor`，与集成终端的 `resolve_shell_binary` 一致）。所选 shell **同时**决定 spawn 的 argv **和**工具描述，从而告知模型应写哪种语法（PowerShell 的 `$env:VAR` / `$null` / cmdlet 还是 POSIX 的 `$VAR` / `/dev/null`）。PowerShell 子进程以 `-NoProfile -NonInteractive` 启动，并从其环境中剥离 `PSModulePath` / `PSExecutionPolicyPreference`（[ADR-0028](../../adr/0028-per-session-claude-isolation) 危险 env 黑名单的 sidecar 侧镜像），使 agent 刚写入的工作区无法把 `.psm1` 代码自动加载进下一次 PowerShell 运行。

## cognia-plugin-tools —— renderer 代理

`buildPluginToolsServer`（`plugin-tools.mjs`）构建第二个进程内服务器（`cognia-plugin-tools`），包装 renderer 转发的插件工具：每个工具被调用时向 renderer 发出 `plugin_tool_exec` 并等待 `plugin_tool_response`。这就是工作流子代理的 `mcp__cognia-plugin-tools__wf_*` 名所解析到的服务器。清单在 build-options 构建（`buildPluginToolsManifest`，`:1009`），它还折入内置技能清单，并在 computer-use 未授权时丢弃 `cognia-computer-use`。
...
插件技能在 id 冲突时胜出；一切 fail-soft（浏览器 / fs 失败 → `undefined`，绝不抛）。撰写见[技能子系统](../../subsystems/skills/authoring-and-workbench)。
...
2. **IM 黑名单短路**（`:122`）：`if (imSession && allowImComputerUse !== true) return {attachedCount:0}` —— 连接器通道默认拒绝屏幕控制
...
`mcp-presets.ts` 是**仅代码**目录（`MCP_PRESETS`，`:46`），含 15 个一键服务器：filesystem、github、gitlab、brave-search、puppeteer、fetch、memory、sqlite、postgres、slack、time、deepwiki（http）、http-generic、sse-generic、custom。`applyPresetFields(preset, values)`（`:365`）把用户值放进 env / args / url / headers。这只填充“添加服务器”UI——区别于进程内 `cognia-tools` 服务器。

## 跨模块小结

| 来源 | 以何形式抵达 SDK | build-options 位点 |
| --- | --- | --- |
| cognia-tools | `query()` mcpServers（进程内） | `opts.builtinTools` `:972` |
| 插件工具 | `cognia-plugin-tools`（进程内代理） | `opts.pluginTools` `:1009` |
| 用户 MCP | `opts.mcpServers` | `:958` |
...
<Cards>
  ...
  <Card title="技能子系统" href="../../subsystems/skills" description="撰写聊天技能与 SKILL.md 格式。" />
  <Card title="MCP 系统" href="../subsystems/mcp" description="完整的 MCP 客户端/宿主/桥接章节——配置存储、预设、内置工具与 a2ui-bridge。" />
