...
flowchart TB
  subgraph WRITE["写 —— 每轮 fire-and-forget（use-claude-chat.ts:269 runMemoryTasks）"]
    ...
    DB["createMemory + 向量 upsert<br/>memories.ts:36"]
  end
...
整个记忆子系统**复用共享 RAG 工具箱**：`BM25Index` / `normalizeScores` / `reciprocalRankFusion` 来自 `@/lib/ai/rag/hybrid-search`，context manager 来自 `@/lib/ai/rag/context-manager`，LLM 契约来自 `@/lib/twin/distill/llm`。
...
`lib/db/memories.ts` 纯机械（无 LLM、无门控——门在调用方）。检索器的候选池是 `listActiveForReader(characterId?)`（`:143`）——经 `[scope+status]` 索引取 global base ∪ character 覆盖。`touchMemories`（`:100`）命中时抬升 recency。只有管理面板调用 `hardDeleteMemory` / `clearMemories`——整合只软失效。
