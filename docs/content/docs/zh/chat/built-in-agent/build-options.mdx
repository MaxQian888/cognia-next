...
description: resolveSendOptions —— 把所有 UI 状态坍缩成单个 SendOptions 负载的唯一汇聚点。全部 40 个有序贡献者、优先级链与系统提示词装配。
---

# Build-options 管线

`lib/claude/build-options.ts` 之所以存在，是为了让**每一个**能力——角色、技能、Twin、记忆、目标、A2UI、computer-use、sandbox、插件、路由——都通过恰好一个函数影响一次对话。`resolveSendOptions`（`:336`）在约 40 个有序段落里修改单个 `opts: SendOptions` 对象并返回它（`:1380`）。代码库中别处都不构造 `SendOptions`。
...
<TLDR>
  ...
  **优先级链**——通常是 `session > activeProject > memberOverride > agentMode > character > appSettings`。
  ...
  提交的 `allowed` 集合。工作流编辑器分支（`:1264`）整体覆写提示词与工具。每个可选注入都包在 try/catch
  里静默降级。
</TLDR>
...
| 导出 | 行号 | 角色 |
| --- | --- | --- |
| **`resolveSendOptions(ctx)`** | `:336-1381` | 管线。异步；返回完整 `SendOptions`。 |
...
函数是一条直线式累加。下图把 40 个段落归为若干阶段；每个节点标注行号。
...
| 39 | **工作流编辑器分支** | **覆写** `systemPrompt`/`appendSystemPrompt`/`allowedTools`/`additionalDirectories`；删 `mcpServers` | `:1264` |
| 39b | 团队分支 | 把 `resolveAllSubagents({context:"team"})` 并入 `opts.agents` | `:1348` |
| 40 | resume / fork | `forkFromSessionId` 或 `resumeSessionId` | `:1364` |
...
IM 通道覆盖（`imOverrideRow`，在 `:407` 一次性取得）仅在 **model**、**provider** 与 **computer-use** 门上位于 session *之上*——这样一个每通道绑定可以钉住更便宜的模型而不触碰会话。
...
- **工作流编辑器分支**（`:1264`）整体覆写 `systemPrompt`（`:1327`）与 `appendSystemPrompt`（`:1328`），所以新追加者必须在它*之前*运行才能在该会话类型里存活。
...
4. 在 `lib/claude/types.ts` 给 `SendOptions` 加字段；若 sidecar 需读取，同步加到 Rust `SendOptions` 结构（`commands.rs:15`）。
...
<Callout type="warn" title="别重复读 IM 覆盖">
  `imOverrideRow` 在 `:407` 取一次，并在 `:991` 的 computer-use 门复用。注释里明确反对为同一行重复读 Dexie
  （`:986-989`）。
</Callout>
