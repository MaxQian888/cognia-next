...
<TLDR>
  一次对话就是一条管线：输入框调用 `hooks/chat/use-claude-chat.ts:584` 的 `send`，它调用
...
<StatGrid>
  <Stat label="装配贡献者" value="40" hint="resolveSendOptions 中的有序段落，build-options.ts:336-1381" />
...
flowchart TB
  subgraph SURFACE["1 · 表层 — hooks/chat/"]
    ...
    SUBS["use-claude-chat.ts:530<br/>onClaudeMessage 订阅"]
    HANDLE["use-claude-chat.ts:1171<br/>handleEvent 分发"]
  end

  subgraph ASSEMBLY["2 · 装配 — lib/claude/"]
    BUILD["build-options.ts:336<br/>resolveSendOptions — 40 个贡献者"]
...
<Cards>
  ...
  <Card title="Build-options 管线" href="./build-options" description="resolveSendOptions —— 唯一汇聚点。全部 40 个有序贡献者、优先级链与系统提示词装配。" />
...
sequenceDiagram
  ...
  U->>H: 提交输入框
  H->>H: onUserPromptSubmit 插件 hook (662)
  H->>B: buildSendOptions → resolveSendOptions (622→336)
  Note over B: 40 个贡献者修改同一个 opts 对象;<br/>系统提示词在 695 拼接
  B-->>H: SendOptions
  ...
  loop 流式
    ...
    H->>H: 写入 mirror / store (1454)
  end
  alt 模型要调用工具
    Q->>Q: canUseTool — A 层静态规则集 (anthropic.mjs:280)
    Q-->>I: permission_request（若未解析）
    Note over I: Rust PreToolUse hook 可拒绝 (sidecar.rs:450)
    I-->>H: permission_request 事件
    H->>H: Auto-mode — B 层裁决 (1304) 或提示用户
    H->>I: claude_approve(decision)
  end
  Q-->>N: result（用量）
  N-->>H: turnComplete → 合并 twin+memory 来源 (1429)，封板 (1464)
```
...
| 不变量 | 位置 | 原因 |
| --- | --- | --- |
| **唯一汇聚点。** 所有上下文注入都在 `resolveSendOptions` 内发生；别处不构造 `SendOptions`。 | `build-options.ts:336-1381` | 新增能力只需改一个文件。[build-options 页](./build-options)就是契约。 |
| **`systemPrompt` 早早封板；`appendSystemPrompt` 后续累加。** 要替换基础提示词必须在 `:695` 处或之前运行；其后只能追加。 | `build-options.ts:695` 对比 A2UI `:908`、goal `:1218`、sandbox `:1151` | 工作流编辑器分支（`:1264`）会整体覆写两者，所以新追加者必须在它之前运行。 |
...
| **两层强制，仅显式。** A 层（sidecar）永不认 `*:allow` 通配；B 层（聊天 hook）跑确定性 + 小模型裁决。 | `anthropic.mjs:280`、`use-claude-chat.ts:1304` | 被攻陷的 renderer 也无法自动放行危险工具。见[权限](./permissions)。 |
...
<Cards>
  ...
  <Card title="技能系统" href="./skills-system" description="一个技能如何物化为提示词、MCP 服务器与工具白名单。" />
