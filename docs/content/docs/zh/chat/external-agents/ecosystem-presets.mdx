...
| **pi** | acp-stdio（Pi，社区适配器） | executable | acp/stdio | `npx -y pi-acp` | Pi 自身配置密钥；第三方桥 | 327 |
...
| `pi` | Pi（社区适配器） | acp | stdio | `npx -y pi-acp` |
| `droid` | Factory Droid | acp | stdio | `droid exec --output-format acp` |
| `custom` | — | — | — | `null`（用户自定义） |

`ExternalAgentPresetId`（`:26`）是一个封闭联合，但预设不是——见下一节。Pi 条目由社区 `pi-acp` 适配器
桥接（Pi 尚无原生 ACP 支持），在目录中打有 `community-adapter` / `experimental` 标签。
