...
技能（Skill）是 Cognia 中最小的"可携带能力"单位 —— 一个 Markdown 片段、一组允许的工具、可选的脚本/引用/资源捆绑。本页是技能子系统的深度剖析；技能在聊天里如何被启用见 [聊天、角色、技能 & 团队](./chat-and-skills)。

技能 = `SKILL.md`（YAML frontmatter + body）+ `scripts/` + `references/` + `assets/`。浏览器端真理源在 Dexie；可选双向同步到 `~/.claude/skills/<dir>/`。市场来自 `skills-lock.json` 注册表与 SkillsMP API，安装走 `parseSkillMarkdown → createSkill`。所有保存路径都强制走 `validateSkill` + Rust 端 `skills_scan_security`。
...
插件可在 manifest 里声明 `skills` 数组，安装后被插件管理器（`lib/plugin/core/manager.ts`）合并入主表。详见[插件系统](../subsystems/plugin-system)。
...
- [插件系统](../subsystems/plugin-system) —— 与插件的沙箱 + 权限模型对照；插件可贡献技能。
- [Agent 团队](./agent-teams) —— 团队成员各自的角色继承自同一 `skills` 表。
- [存储](../data/storage) —— `skills` / `skillResources` 表索引与版本演进。
- [Sidecar & Claude SDK](../core/sidecar-and-claude-sdk) —— 技能片段最终落到 SDK `system` 字段的位置。
