...
Ultracode 在工作区的 **Settings → Ultracode** 区按团队配置（`enabled`、`autoMode`、`skepticsPerFinding`、`judgesPerAttempt`、`maxFinderRounds`）—— 参见 [Workspace UI](./workspace-ui)。Overview 标签页中的 **Run Ultracode** 按钮会强制开启一次运行（`override: "force"`），无视 `autoMode`。
