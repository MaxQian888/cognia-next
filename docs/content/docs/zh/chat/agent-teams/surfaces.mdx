...
深链接以 `?id=` 到达，从来不是 `[id]`。本应用以静态导出发布，由 Tauri 和 Capacitor 消费，那里运行期根本不存在动态段。`use-squad-route-state.ts` 拥有四个参数：`?id=`、`?tab=`（`squads`、`runs`、`board` 之一）、`?q=` 和 `?filter=`（`all`、`waiting`、`live` 之一）。取默认值时参数会从 URL 中消失，每一次写入都是带 `scroll: false` 的 `router.replace`。

桌面控制台由一个列表窗格、一个带两个标签页的中栏和一个检视面板构成。

- **Runs** 渲染 `AgentTeamCommandCenter`。
- **Board** 为选中的小队渲染 `AgentTeamTasks`，未选中时给出明确的空状态。
- **检视面板**显示名称和描述，然后是接到六个 `agentTeamManager` 调用上的 `TeamRunControls`（启动、以 ultracode 启动、暂停、中止、恢复、停止），再往下是回到设置的「Configure this Squad」链接。这些控件以前只存在于已退役工作区的一个标签页上。
