...
- **共享记忆** — `lib/ai/agent/team/memory/shared-memory-orchestrator.ts`（207 行）：每次写入都做深度 PII 门控、版本计数器写入、即发即忘的插件适配器镜像、本地版本优先的反向同步。
...
`createConsensus`、`castVote` 和 `resolveConsensus` 分别触发 <Term>onConsensusOpened</Term>、<Term>onConsensusVoted</Term> 和 <Term>onConsensusResolved</Term>，让插件能观察其生命周期。渲染端消费者是 `/agent-runs` Coordination 标签页中的 `ConsensusPanel`（参见 <LinkCard href="./surfaces" title="界面" description="ConsensusPanel 在何处渲染" />）。
...
```typescript
// lib/ai/agent/team/memory/shared-memory-orchestrator.ts:70-79
...
```typescript
// lib/ai/agent/team/memory/shared-memory-orchestrator.ts:108-119
...
```typescript
// lib/ai/agent/team/memory/shared-memory-orchestrator.ts:194-204
...
<Cards>
  ...
  <Card href="./surfaces" title="界面" description="ConsensusPanel 与 DelegationsPanel 在何处渲染。" />
