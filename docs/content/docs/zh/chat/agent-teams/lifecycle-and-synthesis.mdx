...
<TLDR>
  UI 调用 `agentTeamManager.start`（`lib/ai/agent/team/agent-team.ts:91-115`），它把团队
...
## 管理器门面 —— `lib/ai/agent/team/agent-team.ts`
...
## 运行时依赖接线 —— `lib/ai/agent/team/agent-team-runtime-deps.ts`
