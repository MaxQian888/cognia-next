...
自 [ADR-0022](../adr/0022-agent-team-runtime-hardening) 起，团队**不再**拥有自己的编排器。`runTeamLifecycle`（`lib/ai/agent/team/agent-team-runtime.ts:122-527`）是一个约 400 行的*合成器*：它把团队 + 任务转化为一个 `VisualWorkflow`，再把它交给其他所有功能早已在用的那个唯一的工作流编排器。没有第二个引擎需要保持同步，团队运行的持久化方式与任何其他工作流运行完全一致 —— 作为 `triggerKind: "team"` 的 `workflowRuns` 行。

<TLDR>
  点击 Run 会调用 `agentTeamManager.start`（`lib/ai/agent/team/agent-team.ts:91`），后者再调用
...
<StatGrid>
  ...
  <Stat label="运行详情的小队标签页" value="3" hint="coordination · report · operations，仅在 /agent-runs 的团队运行行上出现" />
  <Stat label="队友运行时" value="5" hint="claude · codex · claude-code · gemini-cli · cursor-cli" />
  <Stat label="Store 持久化版本" value="8" hint="v8 把小队 / 队友 / 任务搬去了 Dexie" />
</StatGrid>
...
2. **持久的定义，临时的运行态。** 从持久化 v8 起，小队、队友和任务以 Dexie 为权威（`agentTeams` / `agentTeammates` / `agentTeamTasks`），由 `stores/agent/agent-team-store/dexie-bridge.ts` 镜像。localStorage 这一份只剩模板、默认配置和少数视图偏好。消息、共享内存、共识和委派仍仅驻留内存，运行*历史*则作为 `agentTeamRuns` 以及 `triggerKind: "team"` 的 `workflowRuns` 行持久化。
...
| **持久的定义，临时的运行态** | 从持久化 v8 起小队存放在 Dexie；运行历史作为 `triggerKind` 设为 "team" 的 `workflowRuns` 行持久化。 | `stores/agent/agent-team-store/dexie-bridge.ts` · `lib/db/agent-team-definitions.ts` |
...
- [数据模型](./data-model) —— `AgentTeam` / `AgentTeammate` / `AgentTeamTask` 类型、Zustand store（持久化 v8）与 Dexie 定义表，以及 ADR-0140 移除了什么。
...
- [界面](./surfaces) —— ADR-0140 解散工作区之后，小队在哪里配置、运行与观察：设置、`/squads`、`/agent-runs` 的小队标签页，以及组合器上的执行器芯片。
