...
picker 用共享的模糊匹配器（`lib/chat/completion/fuzzy-match.ts`，与 `@` 提及 picker 同一打分器）排序候选：优先匹配命令名，描述作为降权的次要来源，因此 `/gc` 能命中 `git/commit`，且最接近的匹配排在最前。
...
- [技能子系统](../subsystems/skills/loading-and-injection) —— `allowedTools` / `paths` 等字段在 send-time 与技能合并取并。
