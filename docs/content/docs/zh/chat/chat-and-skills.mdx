---
title: 聊天、角色、技能 & 团队
description: 会话、消息、角色、技能、团队、Agent Mode、斜杠命令、MCP 服务器如何组合成聊天工作台。
---

# 聊天、角色、技能 & 团队

聊天工作台是 Cognia 最大的用户面，围绕六个可组合概念构建。发送时 `resolveSendOptions` 把所有层（角色 + Agent Mode + 技能 + A2UI + 孪生 + 路由）压成一份 sidecar payload。
...
`Character` 定义在 `lib/claude/types.ts`。本页只给概念视图 —— 逐字段数据模型、插件角色包 overlay、编辑器以及各运行时消费面，见专门的[角色](./characters)章节。
...
内置角色现在以第一方插件角色包 `cognia-builtin-characters` 形式发布（ADR-0030）；`lib/db/characters.ts:seedBuiltInCharacters` 给 6 个历史行打上包归属标记。用户角色由用户创建。两者位于同一张 `characters` 表，读取时与插件包 overlay 取并集 —— 见[角色 · 数据模型](./characters/data-model)。

角色编辑器在 `components/settings/characters-section.tsx`。头像为 emoji + 颜色（`avatarColor` / `avatarGlyph` 辅助函数），可选包提供的 `avatarImage` —— 见[编辑器、选择器与发现页](./characters/editor-and-discover)。
...
<TypeTable
  type={{
    ...
    source: {
      description: "来源（builtin / custom / imported / generated / marketplace）。",
      type: "SkillSource",
    },
    status: {
...
发送时 `resolveSendOptions`（`lib/claude/build-options.ts`）取启用子集，经
`renderSkillsSection`（`lib/db/skills.ts:339`）逐条追加正文，并把每个技能的 `allowedTools`
与角色白名单**取并集** —— 完整机制见[加载与注入](../subsystems/skills/loading-and-injection)。

技能管理路径：

- **技能工作台**（`app/skills/`、`components/skills/`）—— 编写、编辑、校验、批量操作（[工作台](../subsystems/skills/authoring-and-workbench)）。
- **Bundle 导入**（`lib/skills/bundle/`）—— zip / 文件夹 / OpenAI-Codex 布局（[Bundle 导入](../subsystems/skills/bundle-import)）。
- **市场**（`lib/skills/marketplace-*.ts`）—— 内置 registry + 可选 SkillsMP API（[市场](../subsystems/skills/marketplace)）。
- **校验**（`lib/skills/validate.ts`）—— 保存/导入时一律校验。
- **原生同步**（`lib/skills/sync.ts`）—— 与 `~/.claude/skills/` 双向同步并镜像 Codex（[原生同步与安全](../subsystems/skills/native-sync-and-security)）。

内置提示技能在首次运行时种子化（`lib/db/skills.ts:344`）。另有一层 ADR-0026 的**类型化内置工具技能**（Lark CLI 桥），见[内置技能](../subsystems/skills/built-in-skills)。
...
<Cards>
  <Card title="Sidecar & Claude Agent SDK" href="../core/sidecar-and-claude-sdk" description="处理 claude_send 的 Node 主机。" />
  <Card title="技能子系统" href="../subsystems/skills" description="技能生命周期、marketplace、native sync 细节。" />
  <Card title="Agent Teams" href="./agent-teams" description="团队 orchestration / mention / streamer。" />
  ...
  <Card title="员工数字孪生" href="../subsystems/employee-twin" description="最深的角色 / 发送时定制层。" />
  <Card title="插件系统" href="../subsystems/plugin-system" description="插件如何扩展技能 / 角色 / MCP。" />
  <Card title="存储" href="../data/storage" description="支撑聊天 / 角色 / 技能 / 团队的 Dexie 表。" />
</Cards>
