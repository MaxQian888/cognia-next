...
CCSwitch 是社区开源的 Claude 多账号切换器。Cognia 以只读方式打开它的 `~/.cc-switch/cc-switch.db`，把 50+ 服务商、MCP server、prompt、skill 直接呈现在 Settings 里，并能在同一动作内同时切换 cognia-next 自己 + 任意一组外部 agent（目前 v1 只写 `claude-code` 的 `~/.claude/settings.json`）。我们 **从不写** CCSwitch 的数据库。
...
<Callout type="warn">
  CCSwitch 互操作与订阅账号中心有独立的所有权边界。CCSwitch 行是外部只读来源，只有用户明确
  执行受支持的导入/切换时才会产生写操作。账号中心的「停用」只改变 Cognia 当前凭据投影，
  「从 Cognia 移除」只删除 Cognia 本地 vault 条目与引用。两者都不会写 CCSwitch 数据库、编辑
  agent 登录、撤销 token，或登出 CCSwitch/Claude Code/Codex/OpenCode。「新会话默认」只是
  Cognia resolver 偏好，不等同于 CCSwitch 切换。
</Callout>
...
- **CCSwitch 重定向目录**——用户用 Dropbox / iCloud 把 `.cc-switch` 重定向到其他位置时
  ...
  文件协议。
- **OAuth 优先级**——已投影的 Claude OAuth bearer 优先于 CCSwitch provider。明确执行
  CCSwitch 切换前，请先在账号中心使用「停用」。这不会登出 Claude Code 或撤销 OAuth 凭据，
  只会清除 Cognia 的 active 投影。
