...
<TLDR>
  TTS（9 个 provider + Edge-TTS WSS 桥）、Push（移动端推送注册）、QR（配对二维码 +
  条码扫描）、Network（代理配置）、Connectivity（mDNS / LAN 扫描 /
  tunnel）、Queue（带退避的可重试发送队列）、Wallpaper（用户壁纸持久化）、Proxy Config（Rust
  侧反向代理状态）、LaTeX（KaTeX 缓存）、Monaco（编辑器选项 +
  工作台上下文）、Media（插件媒体落地）、Time（极简相对时间）。
</TLDR>
...
供应商一览（`lib/tts/types.ts:TTS_PROVIDERS`）：
...
<Cards>
  ...
  />
  <Card title="存储" href="./storage" description="壁纸 / 队列 / 媒体落地的 Dexie 表" />
  <Card
    title="运行时 & Tauri"
    href="./runtime-and-tauri"
    description="Proxy / Wallpaper / TTS 的 Tauri 命令"
  />
  <Card title="搜索系统" href="./search-system" description="Network proxy 影响所有外部 fetch" />
  <Card
    title="聊天与技能"
