...
- 视图入口 = `ChatPane`（`components/chat/chat-view.tsx`），平台无关；外面套桌面 `DesktopChatWorkspace` 或移动 `AppShellMobile`。
- 渲染核心 = `MessageList` + `MessageRenderer`，按 `UIMessage.parts[]` 派发不同 part 渲染器。
- AI 元件 = `components/ai-elements/`，vendored 自 ai-elements 库（不写测试，不算覆盖率）；包括 Message、Reasoning、Task、Tool、Artifact、Conversation、ErrorTrace 等复合组件。
- 输入器 = `Composer`，水平布局：附件按钮 → Textarea → Submit；流式期间替换为停止按钮。
- 触发器 = slash/@/!/#：`composer-trigger.ts` 检测 → 弹 popover → 走对应 registry（slash 命令、角色、技能、引用）。
- 命令面板 = `CommandPalette`（`Ctrl+K`），跨 section 跳转，挂在桌面壳层。
...
- **导出触发器** —— `single-export-trigger.tsx`（`components/chat/dialogs/`），弹 Markdown / JSON / Plain text / Beautiful HTML / Animated HTML 五选一的对话框。详见 [备份 & 数据](./backup-and-data#%E5%8D%95%E4%BC%9A%E8%AF%9D%E5%AF%BC%E5%87%BA)。
...
- **打开**：路由跳到 `/canvas` 或专用 sandbox 页 —— 详见 [Artifacts 沙箱](./artifacts-sandbox)。
...
- **AgentRuntimeSelector / AgentModeSelector / ExternalAgentSelector** —— 选哪个 agent 实现处理这条消息（详见 [Provider 系统](./provider-system)）。
...
匹配时弹 popover（`components/chat/composer-popover.tsx`），方向键导航 + Enter 选中后 `spliceToken` 替换前缀片段。
...
| `GuildRail`      | 左极窄垂直 rail，列 guild（团队）切换                        | `components/desktop/guild-rail.tsx`      |
...
| 改移动手势              | `components/mobile/interactions/*`                                                   |
...
<Cards>
  ...
  <Card title="画板（Canvas）" href="./canvas" description="Artifact 'open' 态跳转的目标" />
  <Card
    title="Artifacts 沙箱"
