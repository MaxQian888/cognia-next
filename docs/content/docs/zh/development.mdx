...
<Files>
  <Folder name="cognia-next" defaultOpen>
    ...
    <Folder name="lib">
      ...
      <Folder name="data">— 备份 schema v3</Folder>
      <Folder name="scheduler" />
      <Folder name="plugin" />
...
<Callout type="warn" title="钩子会拒绝不规范的消息">
  commit-msg 钩子拒绝不符合 Conventional Commits 的消息。
  rebase 时如需绕过，优先用 `git commit --amend`（仍会跑钩子），而非 `--no-verify`。
</Callout>
...
| 加 Tauri 命令 | 见 [运行时 & Tauri](./runtime-and-tauri) §"添加新的 IPC 命令" |
| 升级 Dexie schema | 见 [存储](./storage) §"新增表" |
| 加 executor / importer / 域传输 | 加注册项；遵循既有模式 |
| 国际化字符串 | 见 [主题 & 国际化](./theming-and-i18n) §"加新翻译 key" |
