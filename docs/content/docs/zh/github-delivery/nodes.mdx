...
<Callout type="info">
  **首选 `reviewPrInline`.** 新写的工作流应该用 `action.github.reviewPrInline` —— 它接受 `line` +
  `side`(更稳),并自带 LLM 调用以输出结构化评论。`reviewPr` 用于人工拼装的场景。
</Callout>
...
**Conventional Commits 实现.** `lib/github/changelog.ts`。grammar:`<type>(<scope>)!: <subject>` + 可选 body + `BREAKING CHANGE:` footer。`!` 或 footer 命中即 major bump;
