...
<TLDR>
GitHub Delivery 出厂带 **8 个内置工作流模板**(`lib/workflow/definition/seed-github.ts`),
都标记为 `isBuiltIn: true` + `isTemplate: true`。用户在工作流库点 "Use template" 后会 fork
出一份可编辑的副本,id 重新生成,但节点 / 边结构原样保留。所有模板都只用已注册的执行器,
开箱即跑。
</TLDR>
...
<Callout type="warn">
**长任务提醒.** `runIssueLoop` 可能跑数十分钟。`lib/workflow/runtime/long-step-runner.ts`
会写 checkpoint 到 `workflowRunEvents`;进程崩溃后 resume 会从最近 checkpoint 继续,不会
重跑 LLM。
</Callout>
