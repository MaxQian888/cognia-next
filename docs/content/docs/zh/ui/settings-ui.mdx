...
`desktopOnly: true` 的条目（如 `subscription`、`ccswitch`、`hooks`、`tools`、`connections`、`webhooks`、`companion`、`desktop`）在 `usePlatform() === "web" | "mobile"` 时被侧栏自动过滤。这是单一开关，section 内部不需要重复判断。
