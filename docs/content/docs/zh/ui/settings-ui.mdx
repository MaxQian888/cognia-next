...
interface NavItem {
  ...
  requires?: readonly CapabilityId[] // 该 section 所管理的能力
  profiles?: readonly HostProfile[] // 仅限本机 shell 表面（与 CapabilityGate 同义）
}
```

可达性按能力而非按主机判断（ADR-0059 D7 / F5）。条目在 `requires` 中声明自己管理的能力（如 `terminal` → `pty`、`subscription` → `keyring`、`connections` → `connector-runtime`、`automation` → `uia-automation`）；`isSettingsSectionReachable` 以 `capabilityAvailable` 语义检查，因此配对到云端 brain 的浏览器能看到后端跑在该 brain 上的 section，而 web 独立模式只看到 webview 内能跑的部分。`profiles` 仅用于绑定本机桌面 shell 的表面（`desktop`、`sidebar`、`companion`、`remote-hosts`），以及过渡期内渲染端仍绕过传输接缝的 section（`ccswitch`、`hooks`、`fleet`——每处 pin 都注明其等待的 host-parity Class A 文件）。侧栏、⌘K 查找器和 shell 的 `?section=` 分发共用同一个 `useSettingsSectionReachability()` 结论，深链无法绕过；`pnpm audit:host-parity` 会拦截任何新增的 `desktopOnly: true`。
...
| `system`        | 系统级集成             | webhooks、external-bridge、companion、network、desktop、about       |
...
手机端**根本不渲染** `/settings`。它的设置面是 `/me` —— 一套独立的、数据驱动的 iOS 风格列表：

- `components/mobile/me/me-entries.ts` 是注册表：每行一条带类型的条目（图标、i18n label key、href、分组、搜索关键词），驱动约 48 个 `/me/*` 子页，外加搜索与收藏。
- 按 ADR-0056 D3，纯 settings-store 驱动的桌面分区**直接内嵌**进 `/me/*` 的 `SubPageShell`，而不是重写一份 —— `/me/appearance`、`/me/canvas`、`/me/eval` 都是这么做的。需要 Tauri 的分区在 `SETTINGS_NAV` 里标 `desktopOnly`，由 `settings-sidebar.tsx` 在非桌面环境过滤掉。
- 移动 chrome（statusbar/safe-area/tab bar）由 `mobile-shell-wrapper` 顶层处理；`/me` 各页不重复关心。

> `components/mobile/settings/mobile-settings-panel.tsx` 曾是 Wave 2.7 的遗留物，**不是**移动端设置外壳。它的内容早已拆进 `components/mobile/me/`，本体零消费者，现已连同 `scripts/gates/unreachable-component-baseline.json` 里的登记项一并删除。移动端设置面就是 `components/mobile/me/`。
...
<Cards>
  ...
  />
  <Card title="Webhooks" href="/settings?section=webhooks" description="Standard Webhooks 签名与出站端点" />
  <Card
    title="伴生端 API"
