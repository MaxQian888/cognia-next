...
> `components/mobile/settings/mobile-settings-panel.tsx` 是 Wave 2.7 的遗留物，**不是**移动端设置外壳。它的内容早已拆进 `components/mobile/me/`，本体零消费者，已登记在 `scripts/gates/unreachable-component-baseline.json`。不要在它上面继续搭东西。
