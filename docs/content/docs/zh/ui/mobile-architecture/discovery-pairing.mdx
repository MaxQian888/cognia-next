...
候选主机**仅在**返回 HTTP `401` 且携带 `{code,message}` 响应体时才被接受（`evaluateProbeResponse`，`:409-432`）——对 `/api/v1/whoami` 的未认证请求*应当*被拒绝，这恰好证明该主机是 Cognia 服务器。随后通过 `healthz` 补充 TLS fingerprint 信息。默认端口为 `27890`（`:110`）；额外端口 `[27890, 7890, 7891, 7900, 8443]`（`:119`）仅对已配对设备或模拟器 IP 尝试。
...
<Step>
**配对** — `pair-step.tsx`。可以扫描 QR（`barcode.ts` → 解析载荷）或手动输入 6 位码。`pair-api.ts` 先生成 mobile P-256 角色 key，再向 `POST /api/v1/auth/pair`（QR）或 `/api/v1/auth/pair/redeem-code`（码）兑换，接收 `{ deviceJwt, deviceId, serverVersion, rendezvousId, roomDescriptor, fingerprint }`。私钥单独进入安全存储，不写入公开配对记录；校验逻辑位于 `pair-helpers.ts`。
</Step>
<Step>
