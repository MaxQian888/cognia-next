...
`CompanionConfig`（`companion-storage.ts`）包含
`{ baseUrl, deviceJwt, deviceId, serverVersion, serverFingerprint?, rendezvousId?, signalingRoomDescriptor?, signalingPrivateKey? }`。
