...
<Cards>
  ...
  <Card title="WebRTC Transport" href="./webrtc-transport" description="TS 信封双胞胎（HMAC + 规范 JSON + 重放防护）、SignalingClient、offerer 角色、移动端/桌面端控制器，以及使用 Durable-Object 房间的 Cloudflare Worker 会合服务器（ADR-0021）。" />
</Cards>

## 为什么手机是客户端而非服务器

所有有状态且需特权的内容都保留在桌面端。手机仅持有设备 JWT、固定的 TLS 指纹、
`rendezvousSecret` 和本地 Dexie 缓存——仅此而已。它无法运行 agent sidecar、向量存储、
