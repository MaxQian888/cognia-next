...
<Cards>
  ...
  <Card title="WebRTC Transport" href="./webrtc-transport" description="单一 ECDSA/ECDH/AES-GCM 信令、受界 DataChannel offerer、恢复控制，以及 Cloudflare Durable Object rendezvous（ADR-0021）。" />
</Cards>

## 为什么手机是客户端而非服务器

所有有状态且需特权的内容都保留在 Host。手机仅持有 P-256 设备 key、固定的 TLS 指纹、
room descriptor、独立 signaling key 和本地 Dexie 缓存——仅此而已。它无法运行 agent sidecar、向量存储、
