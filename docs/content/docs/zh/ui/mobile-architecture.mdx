---
title: 移动端架构
description: Capacitor 8 移动壳：复用 Next.js 静态导出、Transport 三态选择、JWT 配对、SecureStorage、TLS 指纹钉绑、mDNS 发现与原生插件桥。
---

# 移动端架构

cognia-next 的移动客户端**不是**一个独立的 React Native 应用，也**不是**桌面版的简化版。它是同一份 Next.js 静态导出（`out/`），被 **Capacitor 8** 装进 iOS / Android 的原生 WebView，再通过 HTTP/WS 连回桌面端的 `/api/v1/*`。一份 JS bundle，三种壳（Tauri、Capacitor、Web），同一套 React 代码。
...
| **Capacitor 8**    | 把现有 Next.js 静态导出装进原生 WebView，零 UI 重写。                                             | 选用 |
...
WS 走 `?token=<JWT>` 鉴权；服务端保留一个 seq 游标，断线重连时从 `?since=<seq>` 续播，避免漏事件。详见 [Companion API](../subsystems/companion-api)。
...
- **手势交互**：`components/interactions/` —— `LongPress`、`PullToRefresh`、`SwipeRow`，全部用同一套 pointer polyfill（在移动与桌面之间共享）。
...
> 推送投递与凭证存储的服务端实现见 [Companion API](../subsystems/companion-api) 的"Push 投递"小节。
...
`compileSdk` / `targetSdk` = 35（Capacitor 8 默认），`minSdk` = 24，debug-only `usesCleartextTraffic="true"` 仅在 `app/src/debug/AndroidManifest.xml`，release 构建走默认安全策略。
...
<Cards>
  ...
  <Card
    title="远程控制"
    href="../subsystems/remote-control"
