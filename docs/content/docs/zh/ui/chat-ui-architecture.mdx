...
- **CompositionChip** —— Claude SDK 运行时组合的 Agent Mode 预设（取代了 `AgentModeSelector`）；选中未绑定 Squad 的外部 Agent 时隐藏，因为外部 Agent 自带模式。
...
同一个 `+` 触发器打开 `ComposerAttachMenu` 三组弹层：ATTACH（上述各项，另有文件夹引用、云文档、记录提及）、THIS TURN（plan mode、`/goal`，以及注入的本轮能力——**WebSearchToggle**、技能选择器、回复目标选择器——均以 `CapabilityRow` 菜单行渲染）与 EXTEND（slash commands、外部服务）。移动端同一组内容以行形式呈现在 `ComposerPlusMenu` 底部抽屉里。
...
- **`ComposerPlusMenu`**（`components/mobile/chat/composer-plus-menu.tsx`）—— 原生壳上的"+"按钮抽屉（相机、相册、文件、技能、引用等入口）。窄浏览器窗口同样使用手机形态的输入框，但保留桌面的回形针菜单：浏览器里没有原生相机和相册可用。
- **`MessageActionSheet`**（`components/mobile/chat/message-action-sheet.tsx`）—— 长按消息弹底部 sheet：复制、编辑重发、引用、删除、分享。
- **全平台同一个 `@` 面板** —— 手机与桌面共用组合式 `ComposerPopover`（文件、智能体、技能，以及团队房间成员及其角色），行高按手指触控设计。旧的"仅成员"手机底部弹层已移除：它让手机上的团队房间无法引用文件和智能体。
- **指定运行时的一轮** —— 在直接对话（或新对话输入框）里，位于消息第一个词的裸 `@` 会在面板顶部列出 **运行时**（`@claude`、`@codex`）和 **Squad 成员**，每一行都标明它的运行时此处能否接手、不能的话原因是什么。发送 `@codex …` 会把这一轮交给已配置的 Codex 智能体（`@claude` 走内置通道，成员则以该成员身份回答）——开头的标签按解析出的通道着色；被拒绝的路由会保留草稿，并给出原因和"去设置"操作；回复会标注"经由 @handle"。组成部分：`lib/chat/turn-route/`（解析、解析通道、发送时快照、混合运行时历史）、`hooks/chat/use-route-targets.ts`，以及 `use-claude-chat-controller` 中的 `turnRoute`。
