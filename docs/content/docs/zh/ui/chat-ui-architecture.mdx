...
- **Agent 流程展示切换** —— `agent-flow-display-toggle.tsx`，3 档 `ToggleGroup`（简化 / 标准 / 详细），驱动全局 Agent 流程展示模式（见 [Agent 流程展示模式](#agent-流程展示模式)）。
...
| 模式 | 工具调用 | 推理 / 子代理 |
| --- | --- | --- |
| **简化** | 单行行（`ToolCallRow`）：图标 + 工具名 + 简洁目标 + 状态字形，点击展开。 | 默认折叠。 |
...
- **两个入口、同一值** —— 聊天头部快捷切换（`components/chat/agent-flow-display-toggle.tsx`，3 档 `ToggleGroup`）与外观设置卡（`components/settings/appearance/components/agent-flow-card.tsx`）驱动同一偏好，从任一处切换即全局生效。
- **单行摘要** —— `lib/chat/tool-summary.ts`（`summarizeToolCall`）按工具抽取简洁目标（文件名、命令头、模式、URL host、查询…）和图标分类。

#### 活动分组

**≥2 个连续工具调用**会聚合成一个可折叠的**活动组**（`ToolActivityGroup`）——如"5 次工具调用 ✓"——带汇总状态字形与**全部展开/折叠**控件。分段是纯函数（`lib/chat/agent-flow-grouping.ts:groupAgentParts`）：子代理 part 透明（它们作为调度树单独渲染一次），`TodoWrite` 排除（它渲染为计划），单个工具调用保持单卡（无分组装饰）。简化档由组掌管每行开合状态；标准/详细档委托 `renderCard` 并可选 `forceOpen` 覆盖。
...
新工具卡/行、子代理节点与调度横幅有一次性入场（淡入 + 轻微 translateY），活动组展开/折叠时做高度过渡——经 `components/chat/motion/motion-reveal.tsx`（`MotionReveal` / `useFlowMotion`）。动效对 GPU 友好（仅 `opacity` + `transform`，不引 `backdrop-filter`），尊重外观 `motion.reduce` 与系统 `prefers-reduced-motion`，并按 `motion.speed` 缩放时长。
...
- **AgentRuntimeSelector / AgentModeSelector / ExternalAgentSelector** —— 选哪个 agent 实现处理这条消息（详见 [Provider 系统](../chat/provider-system)）。
...
composer 外层声明了 `@container/composer`，文本域行在容器达到 `@sm` 时回流为单行 `[附件 | 文本域 | 发送]`。**底部工具栏**会测量自身宽度（`hooks/use-element-width.ts`）：Tier 1（模型 / 权限 / 沙箱 / 上下文）始终内联，而 Tier 2（增强 / 联网 / 技能）与 Tier 3（Agent 运行时 / 模式 / 外部 Agent / 插件槽）在宽度低于约 448px 时折叠进单一的 `⋯ 更多` popover。每个控件只挂载在一处——把这些 popover 触发器重挂进 `DropdownMenuItem` 会让其开合状态错乱——当折叠的控件处于激活状态时「更多」按钮会显示一个圆点。这样工作流编辑器聊天侧栏（可缩到视口 20%）里所有控件依然可达。
