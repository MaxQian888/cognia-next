...
<TLDR>
  Cognia 用一份静态 catalog 维护 70+ 内置服务商（OpenAI / Anthropic / Google / DeepSeek / 本地
  Ollama …）， 按 6 个 protocol family 把它们映射到 5 个
  adapter；插件可以动态注册自己的服务商；最后由一个 pure-function 投影层把内置 + 自定义 + 本地 +
  插件四条路径合并成 UI 用的扁平数组。
</TLDR>

<StatGrid>
  <Stat label="内置服务商" value="70+" hint="cloud + local 合计" />
  <Stat
    label="Adapter id"
    value="5"
...
embedding-only 服务商通过 [向量存储](./vector-store) 消费。
