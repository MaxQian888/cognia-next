...
飞书把跟随气泡列为在开发者后台配置的 AI 机器人展示能力，目前没有文档化的服务端 API 可
用于注入每次运行动态生成的控制项。它只能作为可选的临时增强：点击或收到下一条消息后可能
消失。`Stop`、查看状态与审批等持久控制必须保留在 CardKit 卡片上；在目标租户和客户端完成
发布验证前，不得把跟随气泡标记为有效运行能力。
...
阶段明确支持安全注入时实时 steering；否则消息和附件仍持久化，并在最早安全边界逐条执行。
