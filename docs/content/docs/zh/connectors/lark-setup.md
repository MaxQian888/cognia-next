...
机器人菜单是开发者后台中的静态配置，且主要用于机器人单聊。只应放置 `status`、`new`、
`help` 等固定命令，不能把它描述为话题级动态进度面；话题中的持久进度由 CardKit 卡片承载。

飞书提供 `POST /im/v1/messages/:message_id/push_follow_up` 跟随气泡 API。cognia-next 会在
**机器人与用户单聊**中，把最近进度卡的停止、查看状态和待审批操作投射为 1–3 个临时气泡；
点击后仍按真实发送者身份经过运行控制授权。该能力要求飞书客户端 7.20+，目标必须是机器人
...
仅在后台勾选 `im:message.group_msg` 不会立即生效；权限和事件订阅变化都必须发布新版本。

1. 授予 `im:message.group_msg`，保留 `im.message.receive_v1` 订阅，并发布应用。
2. 把已发布机器人加入测试群，创建两个不同话题。
3. 在 cognia-next 打开 **设置 → 连接 → 飞书 → 准入**，点击 **开始投递探测**；操作员也可运行
   `/agent verify`。
4. 十分钟内发送一条不 @ 机器人的普通群/话题消息。
5. 确认 readiness 变为 **已验证全部消息**。探测消息只记录审计，不会执行 Agent turn。
6. 在话题 A 首次 @ Agent 后继续免 @ 交流；话题 B 和父群必须保持逐次 @，直到各自被激活。

话题控制命令：

- `/agent status`：显示准入策略、过期时间、派发模式、队列深度、当前运行、readiness 和恢复数。
- `/agent off`：由激活者或配置的操作员关闭话题激活。
- `/agent verify`：由配置的操作员启动十分钟免 @ 探测。

选择 **Queue** 时，每条新消息都作为独立 FIFO turn 保留。选择 **Steer** 时，运行时仅在当前
阶段明确支持安全注入时实时 steering。内置 Anthropic sidecar 使用 SDK streaming input：先
持久化真实发送者消息，再通过带回执的 `steer` 控制把消息加入当前 query；短暂合并窗口结束后
关闭本次输入流，使 query 正常结算。回执缺失、输入已关闭或使用非 Anthropic provider 时，
消息和附件仍保持 durable，并在最早安全边界逐条执行。该语义保证“加入活动输入队列”，不宣称
能改写已经发出的单次模型 HTTP 请求。
