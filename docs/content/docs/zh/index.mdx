...
**Cognia**（代码库 `cognia-next`）是一个面向 [Claude Code](https://claude.com/claude-code) 的桌面客户端。它以 Tauri 2.9 应用形式运行在 Windows、macOS 与 Linux 上；前端是 Next.js 16 + React 19，后端是 Rust 实现的若干原生子系统（调度器、向量库、文件 IO、TTS、深链、MCP HTTP 服务器、工作流 cron 守护进程、Companion 配对服务器……）。一个 Node sidecar 打包了官方 `@anthropic-ai/claude-agent-sdk`，并通过 stdio 与父进程通信。此外还支持通过 Capacitor 在移动端运行（详见 [移动架构](./ui/mobile-architecture)）。
...
- **次级二进制 `cognia-server`**：`src-tauri/Cargo.toml` 注册了一个独立 `[[bin]]` 入口（详见 [Companion API](./subsystems/companion-api)），允许把 Cognia 当作 headless 配对服务器跑在远端机器。
...
- **移动模式**（`pnpm mobile:sync`）— `mobile/` 包通过 Capacitor 把同一份静态导出包到 iOS/Android。`isCapacitor()` 守卫触发后，UI 用 LAN 或 Tunnel 连回桌面侧的 Companion 服务器。详见 [移动架构](./ui/mobile-architecture)。

## 仓库里有什么

| 子系统                                                   | 位置                                                                                                                                                            | 文档                                                                                                                                           |
| -------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- |
| 多角色 / 技能 / 团队 聊天                                | `app/`、`components/chat/`、`lib/chat/`、`lib/character/`（经由 `lib/claude/types.ts`）、`lib/skills/`、`lib/db/{characters,skills,teams,sessions,messages}.ts` | [聊天 & 技能](./chat/chat-and-skills) · [Agent 团队](./chat/agent-teams)                                                                                 |
| Claude Agent SDK + Node sidecar                          | `sidecar/claude-host.mjs`、`lib/claude/`、`src-tauri/src/claude/`                                                                                               | [Sidecar & Claude SDK](./core/sidecar-and-claude-sdk)                                                                                               |
| Claude 订阅 OAuth                                        | `lib/anthropic-subscription/`、`src-tauri/src/anthropic_subscription/`、`components/settings/subscription/`                                                     | [Claude 订阅 OAuth](./chat/claude-subscription-oauth)                                                                                               |
| CCSwitch 提供商切换                                      | `src-tauri/src/ccswitch/`、`lib/ccswitch/`                                                                                                                      | [CCSwitch](./chat/ccswitch) · [提供商系统](./chat/provider-system)                                                                                       |
| 员工数字孪生（RAG + 风格少样本）                         | `lib/twin/{ingest,distill,runtime}/`、`components/twin/`、`app/twin/`、`types/twin/`                                                                            | [员工数字孪生](./subsystems/employee-twin)                                                                                                                |
| 原生向量库（sqlite-vec）                                 | `src-tauri/src/vector/`、`lib/vector/`                                                                                                                          | [向量库](./data/vector-store)                                                                                                                       |
| 加密备份 / 导入（schema v3）                             | `lib/data/`、`components/settings/data/`                                                                                                                        | [备份 & 数据传输](./data/backup-and-data) · [导出系统](./data/export-system)                                                                             |
| 跨平台调度器                                             | `src-tauri/src/scheduler/`、`lib/scheduler/`、`components/scheduler/`                                                                                           | [调度器](./subsystems/scheduler)                                                                                                                          |
| 插件系统                                                 | `lib/plugin/`、`lib/db/plugins.ts`、`src-tauri/src/plugin_api/`、`components/plugins/`                                                                          | [插件系统](./subsystems/plugin-system)                                                                                                                    |
| A2UI / 画板 / Artifacts 输出层                           | `lib/a2ui/`、`lib/canvas/`、`lib/artifacts/`、`components/{a2ui,canvas,artifacts}/`                                                                             | [A2UI](./subsystems/a2ui-system) · [画板](./subsystems/canvas) · [Artifacts](./subsystems/artifacts-sandbox)                                                                    |
| TTS（Edge / OpenAI / Azure / ElevenLabs / 原生）         | `src-tauri/src/tts/`、`lib/tts/`                                                                                                                                | [杂项子系统](./subsystems/misc-subsystems)                                                                                                                |
| MCP 服务器 + 预设                                        | `lib/claude/mcp-presets.ts`、`lib/db/mcp-servers.ts`                                                                                                            | [Sidecar & Claude SDK](./core/sidecar-and-claude-sdk)                                                                                               |
| **External Bridge（Wiki + MCP）**                        | `lib/wiki/`、`lib/external-bridge/`、`src-tauri/src/mcp_server/`                                                                                                | [External Bridge](./subsystems/external-bridge) · [Wiki 生成器](./subsystems/wiki-generator) · [Tauri MCP](./subsystems/mcp-server-tauri) · [Sidecar MCP](./subsystems/mcp-server-sidecar) |
| **可视化工作流**                                         | `lib/workflow/`、`components/workflow/`、`src-tauri/src/workflow/`                                                                                              | [可视化工作流](./subsystems/visual-workflows)                                                                                                             |
| **平台连接器（Telegram / Discord / Slack / Lark / QQ）** | `lib/connectors/`、`components/connectors/`、`src-tauri/src/connectors/`、`app/inbox/`                                                                          | [平台连接器](./subsystems/platform-connectors)                                                                                                            |
| **Companion API（移动配对）**                            | `src-tauri/src/companion_api/`、`mobile/`                                                                                                                       | [Companion API](./subsystems/companion-api) · [移动架构](./ui/mobile-architecture)                                                                           |
| **远程控制**                                             | `src-tauri/src/remote_control/`                                                                                                                                 | [远程控制](./subsystems/remote-control)                                                                                                                   |
| **桌面同步 / 多会话源**                                  | `components/providers/desktop-sync-source-provider.tsx`、`components/providers/desktop-message-source-provider.tsx`                                             | [同步系统](./data/sync-system)                                                                                                                      |
| Skills / Slash commands                                  | `lib/skills/`、`src-tauri/src/skills/`、`lib/commands/`                                                                                                         | [技能系统](./chat/skills-system) · [Slash commands](./chat/slash-commands)                                                                               |
| 主题（oklch + 自定义主题）                               | `lib/themes/`、`lib/theme/`、`app/globals.css`                                                                                                                  | [主题 & 国际化](./ui/theming-and-i18n) · [shadcn/ui 配置](./ui/shadcn-ui-setup)                                                                      |
| 国际化（next-intl）                                      | `lib/i18n/`、`i18n/messages/{en,zh-CN}.json`                                                                                                                    | [主题 & 国际化](./ui/theming-and-i18n)                                                                                                            |
| 设置 UI（多 section 嵌套面板）                           | `app/settings/`、`components/settings/`                                                                                                                         | [设置 UI](./ui/settings-ui)                                                                                                                       |

各层如何衔接，参见 [架构](./core/architecture)。
