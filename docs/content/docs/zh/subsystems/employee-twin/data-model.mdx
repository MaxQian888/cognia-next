...
| --- | --- | --- |
| **Sources** | `TwinSource`（L64）、`TwinSourceKind`（L28）、`TwinSourceFormat`（L34）、`TwinSourceStatus`（L62） | 每个被 ingest 的原始素材的注册表。横跨文档 / 聊天 / 邮件 / 代码四族共 21 种格式。 |
...
- **文档** → `markdown · pdf · docx · pptx · odt · odp · html · csv · epub · rtf · code`（路由到 `lib/document/document-processor`）。
