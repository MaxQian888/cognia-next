...
<TLDR>
  ...
  并经 `useTwinWorker` 挂载任务 worker。Overview、Cron 与 Inject-log 界面是 **Settings tab 内的卡**，不是顶级 tab。
</TLDR>
...
flowchart TB
  ...
  Panel --> WK[useTwinWorker<br/>启停 worker]
...
`useTwinWorker(twinId)`（`use-twin-worker.ts:138`）实时读取 `TwinRuntimeSettings`，构建一个 `JobWorkerConfig`
（当 worker 被禁用、未选孪生、向量配置不全或缺 LLM key 时返回 `null`，即无 worker），并在变更时启停 worker：
...
它返回一个派生的 `{ active, reasonKey? }` 状态（原因：`noTwinSelected` / `disabled` / `incompleteConfig`），由面板渲染成 worker 徽标。
