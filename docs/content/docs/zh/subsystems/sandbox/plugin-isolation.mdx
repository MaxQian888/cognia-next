...
<TLDR>
  ...
  Node 目标插件。**T3** 是原生 Rust/Wasmtime 宿主。T2 已经通过 `PluginLoader` / `PluginManager`
  触达 Node-target frontend 插件；T3 会在原生 load 前对账 manifest preopens，并把宿主返回的
  runtime generation 绑定到每个工具与 workflow node。
</TLDR>
...
权限审查 UI 会如实投影这个边界。scoped filesystem grant 可用；`network:fetch`、
`network:upload`、`network:websocket`、`shell:execute` 与 `process:spawn` 为兼容 manifest
仍然可见，但 grant/tier 控件会按缺失 broker 原因禁用，不再显示为已成功落实。
...
`PluginLoader` 保存 `plugin_wasm_load` 返回的 generation。缺失 generation 时，`PluginManager`
拒绝 activation，并把 generation 传入 `buildWasmToolDefinitions` 与 `buildWasmNodeDefs`。
因此每次 execute 都点名确切的 loaded generation；原生宿主拒绝陈旧的 activate/call/deactivate/unload。
未使用的浏览器 WASM runtime 与未实现的 JCO 路径已移除，Rust/Wasmtime 是唯一生产权威。
...
`wasm-loader.test.ts` + `manager.test.ts` + 原生 Wasmtime 测试（generation 绑定与 stale
generation 拒绝）。
