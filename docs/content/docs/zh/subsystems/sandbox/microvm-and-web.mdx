...
JS / TS / JSX / HTML / CSS 在每个壳上都跑在 `sandbox="allow-scripts"` 的 `srcdoc` iframe 里，
Python 在桌面端被路由到 Rust 的 `canvas_run_python`。消费方是
`hooks/canvas/use-code-execution.ts` 与 `components/canvas/code-execution-panel.tsx`。
