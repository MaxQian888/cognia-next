...
<Callout type="warn">
  runtime capability 来自实际 adapter。只有 Docker + computer-server 实现 start、stop、health、
  delete 与 GUI。导入的 cua-cloud 和 Lume 行为兼容性继续可读，并分别渲染自己的 instance/endpoint
  或 VM/image 字段，但所有未支持操作都会禁用。当前没有任何连接提供 `cua-desktop` shell/files
  tier 所要求的已验证 workspace adapter。
</Callout>
...
<Cards>
  ...
  <Card title="Computer Use" href="../computer-use" description="更广义的 computer-use 子系统" />
</Cards>
