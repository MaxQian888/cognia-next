...
层级则单独解析。注意这里**没有 `session.sandboxTier`**——该链从
character 开始：
...
`setActiveSandboxTier`（在 `build-options.ts:13` 处从 `@/lib/sandbox/microvm-bridge` 导入）会写入
该层级，以便 `cognia-sandboxed-tools` 插件能在其 `execute()` 内部读取它——见
...
当沙盒被**禁用**时，else 分支会重置该层级，以免同一 session id 上一次发送
留下的过期值泄漏到下一次调用：
