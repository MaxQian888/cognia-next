...
<TLDR>
  ...
  选择各平台后端，并在该后端下运行命令。**Linux (`bwrap`) 与
  macOS (`sandbox-exec`) 是真实可用的后端**；**Windows 仍是部分实现**（一个基于
  `runas` 的双二进制 runner，更深层的受限令牌 vendoring 仍待完成）。两个 Tauri 命令
  均已注册（`lib.rs:349-350`）。
</TLDR>
...
<Callout type="info">
  `types.rs:191` 处的文档注释仍写着 Windows 后端字符串是 `"windows-codex"`。Windows 后端
  **实际**发出的字符串是 `"windows-cognia-sandbox"`（`windows.rs:191`）——
  该注释已经过时。
</Callout>
...
<Callout type="warn">
  `mod.rs:11-15` 处的文件头注释声称 `sandbox_exec` “在 Phase 4.2+ 之前一直保持 UNREGISTERED”。
  这条注释已经**过时**—— `sandbox::sandbox_health_probe` 与 `sandbox::sandbox_exec` 如今
  都已在 `lib.rs:349-350` 注册。
</Callout>
...
- Edit/Write/TextEditor 会对**每个目标文件单独** `--bind`（而非父目录），并强制
  `NetworkPolicy::Off`。
- **注意事项：** `max_cpu_seconds` / `max_memory_mb` 会被接受，但目前**尚未**翻译为
  cgroup/rlimit 标志。
...
- **注意事项：** 在 SBPL 层 allowlist 会降级为完全网络（渲染端才是真正的主机
  门禁）；资源上限不通过 SBPL 强制执行。

### Windows —— 双二进制（部分实现 / vendoring 待完成）

`WindowsSandboxBackend` 组合两个随附的二进制：`cognia-sandbox-setup.exe`（需 UAC 提权、
幂等 —— 创建合成用户 `CogniaSandboxOffline` / `CogniaSandboxOnline` 以及
按 SID 的防火墙规则，然后写入一个标记文件）与 `cognia-sandbox-runner.exe`（无需提权 ——
在合成用户下重新启动 argv）。`run()` 受标记文件门控；新机器在 setup 运行之前会报告
`SetupRequired`。
...
<Callout type="warn">
  Windows 是完成度最低的后端。已知缺口：`health().backend == "windows-cognia-sandbox"`，
  且 `is_available()` 纯粹只看标记文件是否存在；runner 使用的是浅层的
  `runas /trustlevel:0x20000` 机制（来自 `codex-rs/windows-sandbox-rs` 的更深层
  受限令牌 + `CreateProcessAsUser` vendoring 仍待完成）；runner **不会捕获子进程的
  stdout/stderr**（`cognia-sandbox-runner.rs:114-116`）；setup 二进制中的
  `grant_logon_as_batch` 只是一条 no-op 备注（`cognia-sandbox-setup.rs:133-145`）。
</Callout>
...
| ----------- | ------------------------- | ---------------------- | ---------------------------------------------------------------------------- |
| Linux       | `linux-bwrap`             | 真实                   | 是 —— 解析并在 namespace/超时下执行 `bwrap`。上限未强制执行。  |
| macOS       | `macos-sandbox-exec`      | 真实                   | 是 —— 以渲染出的 SBPL 执行 `/usr/bin/sandbox-exec`。allowlist → On。      |
| Windows     | `windows-cognia-sandbox`  | 部分实现 / vendoring 待完成 | 是但浅层 —— `runas /trustlevel`；无 stdout 捕获；受标记文件门控。     |
...
按文件 bind、share-net、后端字符串）、`macos.rs:299-374`（SBPL 渲染、literal vs subpath、
转义）、`windows.rs:240-386`（标记文件门控、offline/online 用户选择、payload 形状）、
`uninstalled.rs:85-159`、`mock.rs:126-201`。真实沙盒集成测试在 Linux + macOS 上通过
`cargo test --test sandbox_integration` 运行（Windows runner 在 CI 下被跳过 ——
UAC）。
