...
| 体系                    | 隔离对象                                                 | 机制                                                                       | ADR  | 状态                                    |
| ----------------------- | -------------------------------------------------------- | ------------------------------------------------------------------------- | ---- | --------------------------------------- |
| **执行沙盒**            | 模型工具调用（`Bash` / `Edit` / `Write` / editor）       | OS 原生（`bwrap`/`sandbox-exec`/runner）· microVM · 按动作策略             | 0028 | Linux/macOS/Windows 已落地              |
| **CUA 远程沙盒**        | 远程桌面上的 computer-use（GUI 操控）                     | 通过 WebSocket 驱动的 Docker `cua-xfce` 容器                               | 0020 | 已完全接通                              |
| **插件隔离**            | 第三方插件代码（JS / Node / WASM）                       | `PluginSandbox` globals · Node 24 `--permission` · Wasmtime/WASI          | 0028 | Renderer + T2/T3 已接线                 |
| **渲染沙盒**            | 一次性模型制品（HTML / SVG / React）                     | iframe + CSP + DOMPurify（见 [制品 / 沙盒](../artifacts-sandbox)）         | —    | 稳定                                    |

不存在第五个家族。`lib/sandbox/web` 曾放着一个「web 代码执行沙盒」的桩，从未发布、也没有任何调用方，
现已删除。浏览器里跑 JavaScript / HTML / CSS 的是 `lib/native/code-execution-strategy.ts`
（opaque `sandbox="allow-scripts"` 的 `srcdoc` iframe），Canvas 执行面板在每个壳上都走它；该面板上的 Python
仅限桌面，走 Rust 的 `canvas_run_python`。
...
<Files>
  <Folder name="crates/cognia-automation/src/sandbox/ (T1 — OS 执行)" defaultOpen>
  ...
  <Folder name="crates/cognia-sandbox-runner/">
    <File name="src/main.rs — Windows restricted-token + low-integrity + Job Object runner，带 stdout/stderr capture" />
  </Folder>
  <Folder name="src-tauri/src/bin/">
    <File name="cognia-sandbox-setup.rs — 保留给未来可选 per-SID firewall setup" />
  </Folder>
  ...
  <Folder name="UI + 数据">
    ...
    <File name="components/settings/sandbox/* — 设置区块（已接入：导航 + shell）" />
...
<Cards>
  ...
  <Card
    ...
    description="输入框 shield、诊断审计卡片、已接入的设置区块、引导流程"
  />
  <Card
    title="制品 / 沙盒（渲染）"
