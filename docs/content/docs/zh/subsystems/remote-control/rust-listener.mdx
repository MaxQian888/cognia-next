...
能力分档（`route_needs_write`）：`read` 令牌可访问健康探针与全部 `GET` 读取端点；变更类方法（`POST`）需要
`write`。通用的 `POST /api/v1/commands/:target` 路由还会对**敏感**目标（`chat.send`、`connector.send`、
`goal.create`）额外拦截，除非开启 `RemoteControlInboundConfig.allowSensitiveTargets`，否则返回
`403 sensitive_target_disabled`。

除 `/targets`（由 Rust 原生应答）外，每个 `GET` 读取都通过 `remote-control://query` 事件 + `QueryRegistry`
oneshot 往返渲染进程，由 `remote_control_query_response` 命令解析；窗口关闭/繁忙时在有界等待后返回
`503 window_unavailable`。完整线缆契约见 `docs/api/remote-control.openapi.yaml`。
