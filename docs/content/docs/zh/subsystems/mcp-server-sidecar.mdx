...
<Status variant="stable">A2UI v0.9 · cognia-tools 内建工具</Status>

<TLDR>
  `sidecar/` 目录里有两类完全不同的 MCP 服务：**in-process** 的 SDK MCP server（由 `claude-host.mjs`
  在每个 Claude Agent SDK 会话里挂起，承载 `builtin-tools/` 的五个分类工具与 `a2ui-tools/` 的 A2UI
  桥），以及 **standalone stdio** 的 `a2ui-mcp.mjs`（用于让 Cognia 进程**之外**的外部 Agent ——
  Claude Code CLI / Cursor / Codex —— 也能创建 A2UI 表面）。两者共用一套工具
  schema，但通信方式不同：前者通过 SDK 的 `createSdkMcpServer` 走进程内消息总线，后者用 JSON-RPC
  over newline-delimited stdin/stdout，并通过 `COGNIA_BRIDGE_SOCKET` 的 Unix domain socket 把
  dispatch 事件转回 Cognia 渲染进程的 Zustand store。
</TLDR>
