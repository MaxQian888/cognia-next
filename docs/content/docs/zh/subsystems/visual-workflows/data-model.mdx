---
title: 数据模型与定义
description: 权威的 VisualWorkflow 图类型、74 种节点 / 7 大类的分类体系、Zod 加图完整性的校验流水线、8 张工作流 Dexie 表，以及 22 个内置模板。
---
...
<TLDR>
一个工作流就是一张 `VisualWorkflow` 图（`types/workflow/visual.ts:242`），其 `schemaVersion: 1`（`types/workflow/visual.ts:235`），由 `WorkflowNode` 与 `WorkflowEdge` 记录组成。每个节点的 `type` 都是 74 个 `WorkflowNodeKind` 字面量之一（`types/workflow/visual.ts:27-119`），归入 7 大类，其中 `workflowNodeCategory()`（`types/workflow/visual.ts:130-141`）把 `pattern.*` 折叠进 `action`。持久化涉及 8 张 Dexie 表，跨 v22、v35、v42、v52、v56 引入（`lib/db/schema.ts`），CRUD 位于 `lib/db/workflows.ts`。校验先跑 Zod（`lib/workflow/definition/validate.ts:110-129`）再跑图完整性 DFS（`lib/workflow/definition/validate.ts:151-242`），22 个内置模板来自 `seed.ts`、`seed-github.ts` 与四个 `templates/*.ts`。
</TLDR>

<StatGrid>
  <Stat label="节点种类" value="74" hint="across 7 categories" />
  <Stat label="Schema 版本" value="1" hint="VisualWorkflowSchemaVersion" />
  <Stat label="Dexie 表" value="8" hint="v22 to v56" />
...
`WorkflowNodeKind` 是 74 个字符串字面量的联合类型（`types/workflow/visual.ts:27-119`），由运行期数组 `WORKFLOW_NODE_KINDS`（`types/workflow/visual.ts:147-222`）镜像。每个种类由 `workflowNodeCategory()` 归入 7 个 `WorkflowNodeCategory` 字面量之一（`types/workflow/visual.ts:121-128`）：它按第一个点切分，并把 `pattern.*` 映射到 `action`（兜底回退则得到 `annotation`）。
...
8 张工作流表跨越五个 schema 版本。v22 声明了最初的四张；v35、v42、v52、v56 各新增一张。v52 重新声明 `workflows` 以加入 `folderId`，并附带一个升级钩子，为旧行回填 `folderId = "root"`（IndexedDB 无法索引 `undefined`）；v22、v35、v42、v56 为纯增量，无升级钩子。
