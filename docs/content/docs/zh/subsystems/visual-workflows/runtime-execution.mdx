...
密钥：`SecretResolver`（`secret-resolver.ts:13-15`）；`NoopSecretResolver`（`:22-24`）是 web / Phase-4 默认，返回 `undefined`；`createInMemoryResolver`（`:30-34`）。keyring resolver `createKeyringSecretResolver()`（`secret-resolver-keyring.ts:17-26`）从 `@/lib/keyring/index` 读取；`parseRef`（`:33-49`）接受 `keyring:ns:key` 与 `ns/key` 简写，不匹配时返回 `null`（→ `undefined`）。
