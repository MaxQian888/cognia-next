...
`runLongStep`（`long-step-runner.ts:119-211`）把检查点搭载在 `workflowRunEvents` 上，作为 `step.long_running.checkpoint` / `progress`——**没有新的 Dexie 表**。`step_completed` 仅在最后写入，因此在此之前该步骤为"未完成"，恢复时通过 `onResume(latestCheckpoint)` 重新进入。所有写入都是 best-effort `try/catch`。
