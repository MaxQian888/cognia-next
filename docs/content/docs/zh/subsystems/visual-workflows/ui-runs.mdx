...
<TLDR>
用户可触达的一切都位于 `components/workflow/` 之下：三栏编辑器外壳在 `canvas.tsx:894` 挂载“每工作流一份 store”，外层包裹 `ReactFlowProvider` + `EditorStoreProvider`，而 `FlowCanvas`（`flow-canvas.tsx:117`）是**唯一**逐帧订阅者，外壳的其余部分（chrome）通过 `getState()` 读取。选中节点驱动 Config/Input/Output 三段式 NDV 检查器（`inspector-panel.tsx:351` + `inspector/data/data-tabs.tsx:20`），其各标签由 `resolveNodeIo` / `useLatestRunOutputs` 供给；6 标签右侧栏（`right-sidebar/index.tsx:272`）在选中时自动切到检查器。文件夹感知的库（`workflow-library.tsx:56`）叠加 `filterWorkflows`/`sortWorkflows`、`@dnd-kit` 文件夹拖放、虚拟化列表行与自动隐藏的统计条。运行历史经纯函数 `buildSpans`（`run-timeline.tsx:172`）渲染甘特图，并提供 `run-detail.tsx:31` 重跑，由四个入口触达（从此处运行 / 运行此步 / NDV 空态 CTA / 整体重跑）。路由位于 `app/workflows/`，并通过 `generateStaticParams [{id:"_"}]` 输出静态导出的 `[id]` 占位页。
</TLDR>
...
| `RunDetailInner` | `run-detail.tsx:98` | 头部 pill/时长/触发器/sparkline + 重跑 |
| `handleReRun` | `run-detail.tsx:122` | 以原始 `triggerPayload` 重新调用 `runWorkflow` |
...
flowchart LR
  ...
  D["运行历史 重跑"] --> RR["RunDetail.handleReRun<br/>run-detail.tsx:122（原 payload）"]
```
...
- **整体重跑** → `RunDetail.handleReRun`（`run-detail.tsx:122`）以原始 `triggerPayload` 重新调用 `runWorkflow`。
