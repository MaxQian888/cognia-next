...
三个家族使用宽松的 `z.object({}).passthrough()`，因为它们的形状由别处拥有：全部 13 个 `github.*`（L417-429）、全部 12 个 desktop + `trigger.desktop.event`（L432-443）、以及全部 6 个 `pattern.*`（L473-478）。
...
> 13 个 `action.github.*` kind 由 **github-delivery 插件**（`plugins/github-delivery/`）提供，而非 `built-ins.ts`。12 个 desktop kind + `trigger.desktop.event` 位于 `desktop.ts`；`action.system.terminal` 位于 `terminal.ts`。
...
注册 12 个 desktop 执行器 + `trigger.desktop.event`，每个都包裹来自 `@/lib/automation/client` 的一个 desktop 客户端方法，并打上 `surface: "workflow"`，从而让 Rust gate 评估 **workflow** 策略。辅助位于 L38-95；`callCtx`（L84）在 `target.connectionId` 存在时路由到 cua 沙箱（ADR-0020），否则本地。
...
`trigger.desktop.event` 在 execute 时为透传；真正的 UIA 触发由 Rust trigger 桥完成。
