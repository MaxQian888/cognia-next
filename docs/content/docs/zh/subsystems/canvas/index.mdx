...
<StatGrid>
  <Stat label="源文件" value="约 60" hint="lib/components/hooks/stores/types + 2 个 Rust" />
  ...
  <Stat label="Zustand store" value="6" hint="1 个唯一真源 + 5 个 canvas 专用" />
...
将 artifact「升级」进工作区的入口是 `revealArtifactInWorkspace`（`lib/artifacts/reveal.ts:4`），由聊天
