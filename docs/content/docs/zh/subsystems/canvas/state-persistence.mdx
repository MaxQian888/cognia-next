...
<StatGrid>
  ...
  <Stat label="仅运行时 store" value="2" hint="chunked-document-store · comment-store（镜像）" />
</StatGrid>
...
持久化：name `"cognia-artifacts"`，version 3（`:1455-1456`）；`partialize` **整体**持久化
`canvasDocuments`，无 LRU/截断（`:1511`）—— 内容截断 + LRU 淘汰只对一次性 artifact 生效，不影响 canvas
文档。自动保存版本保留至多 `MAX_CANVAS_AUTOSAVE_VERSIONS = 30`（`:51`），裁剪最旧的自动保存而保留手动版本
（`applyCanvasVersionRetention`，`:254`）。

读取 hook `useCanvasDocuments`（`hooks/canvas/use-canvas-documents.ts:34`）是该 store 之上的薄外观（排序
列表、create/open/rename/duplicate/search）—— 印证文档完全来自 artifact store。
...
flowchart LR
  ...
  subgraph CANVAS["stores/canvas（5 个）"]
    ...
    CH["useChunkedDocumentStore<br/>chunked-document-store.ts:46<br/>仅运行时（不持久化）"]
  end
  ART -.-> CH
...
- **`useChunkedDocumentStore`**（`chunked-document-store.ts:46`）—— 仅运行时；分块在会话启动时从
  `CanvasDocument.content` 重新推导，因此在此加持久化会与 artifact-store 契约冲突（`:1-11`）。
...
1. UI 编辑调用 `useArtifactStore.updateCanvasDocument`（`artifact-store.ts:922`）—— 内存映射改变，Zustand
   将其持久化到 `localStorage["cognia-artifacts"]`（权威副本）。
...
`useCanvasAutoSave`（`use-canvas-auto-save.ts:28`）跟踪本地内容与上次保存的内容，在静默 `autoSaveDelay`
（默认 `30000` ms，`:34`）后调度一次保存，调用注入的 `onSave(documentId, undefined, true)`（`true` 标记
为自动保存）。消费方 `CanvasPanel` 把 `onSave` 接到 `saveCanvasVersion`、`onContentUpdate` 接到
`updateCanvasDocument`。面板还跑一个下限 10 秒的更粗粒度间隔自动保存（`canvas-panel.tsx:156-168`）。
