---
title: 插件系统
description: Cognia 的一等插件运行时——惰性加载的 PluginManager 负责发现、校验、签名、加载与激活插件；约 48 个权限受控的 context API 命名空间；34 个把插件贡献投射进宿主子系统的桥接器文件；声明式的 contracts/registries/SDK 层；按调用粒度的同意 + Ed25519 签名；WASM 与 VS Code 扩展运行时；以及带备份/回滚生命周期的四来源市场。
---
...
<TLDR>
  ...
  **能力受限的 API**（`ctx`）。中枢编排器是 `PluginManager`（`lib/plugin/core/manager.ts:364`）——一个
  ...
  `lib/plugin/core/context.ts:233` 组装）；**声明贡献**，由 34 个*桥接器文件*（`lib/plugin/bridge/`）投射进
...
<StatGrid>
  ...
  <Stat label="贡献桥接器" value="34" hint="lib/plugin/bridge/ TS 文件，不含测试" />
...
一等插件在仓库内 `plugins/` 目录下（`computer-use`、`clipboard-history`、`screenshot`、
...
lib/plugin/
  ...
  bridge/        # 34 个把插件贡献投射进宿主子系统的桥接器文件
...
<Cards>
  ...
  <Card title="贡献桥接器" href="./bridges" description="34 个把 manifest/SDK 贡献投射进 chat、主题、字体、Monaco、连接器、工作流的桥接器文件；module-bridge-map 分发循环；kind-prefix 命名空间化" />
