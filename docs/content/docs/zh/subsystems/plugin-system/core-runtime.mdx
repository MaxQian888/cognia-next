...
- **桥接。** `registerPluginContributions`（`manager.ts:2404`）注册 modes/commands
  （`manager.ts:2418`、`:2437`）、斜杠命令（`manager.ts:2302`）、经由 `PluginThemesBridge` 的主题
  （`manager.ts:2450`）、四个 `OVERLAY_REGISTRY_CAPABILITIES`（`manager.ts:2482`）、七个
  `MODULE_BRIDGE_CAPABILITIES`（`manager.ts:2511`）、LSP 服务器（`manager.ts:2540`），以及 skill→character
  附着（`manager.ts:2560`）。
