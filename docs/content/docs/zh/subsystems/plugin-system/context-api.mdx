...
| `ctx.ui` | `context.ts:456` | toasts / dialogs | 否 | `showToast`（→ sonner）、`showDialog` / `showInputDialog` / `showConfirmDialog`（→ 插件 modal 栈） |
...
| `ctx.logs` | `logs-api.ts` | IndexedDB 日志库 + `agentTraces` Dexie 表 | `logs:read`（日志、统计、传输健康）/ `trace:read`（span）—— 只读 | `query`、`stats`、`subscribe`、`transports`、`traces.list`、`traces.timeline`、`traces.subscribe` |
