...
`network:fetch` 是用户同意的能力权限，`manifest.networkAccess` 则是资源策略。
旧版 `allowedDomains` 用于限制主机；新的 `rules` 会同时校验 HTTP Method 与 URL
路径：
...
Web/移动端由 renderer 执行策略，桌面/headless 端会把同一策略同步到 Rust host
做纵深防御。`get`、`fetch`、`download`、`upload` 都经过同一策略。出站 URL、
非凭证 Header 与请求体还会经过宿主 PII gate：默认自动脱敏；需要 fail-closed
时设置 `piiPolicy: "block"`，并用 `dataClassification` 标记审计记录。
