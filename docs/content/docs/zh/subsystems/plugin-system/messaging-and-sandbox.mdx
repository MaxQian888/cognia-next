...
即 `sandbox.ts:223-241`。`executeAsync`（`:246`）把函数体包进 `AsyncFunction` 并与超时竞速（`:262-267`）。（本页描述的 `sandbox.ts` 已不在树内；其 `executeCode` 的委托目标 `lib/sandbox/web` 也已删除。浏览器里的代码执行是 `lib/native/code-execution-strategy.ts`，见[沙盒系统](../sandbox/microvm-and-web)。）
...
| `CONFLICT` | 状态冲突 | 否 |
| `TIMEOUT` | 网关超时 | 是 |
| `UNAVAILABLE` | 宿主依赖未就绪（加密密钥存储初始化中或已锁定；见 `details.reason`） | 否 |
| `INTERNAL` | 未知网关错误 | 是 |
...
`IPCMessage` 信封（`ipc.ts:63`）是 `{id, type, channel, senderId, targetId?, payload, timestamp, replyTo?}`，其中 `type` 是四种之一：`request | response | broadcast | event`。请求/响应式 RPC 完全由 `call()` / `expose()`（见下）承担——一个单向的 `send()` 与一个发完即忘的 `broadcast()` 构成其余表面。（此处曾有一对 `sendAndWait`/`respond` 请求-应答孪生，但已被移除：其应答路径结构性失效——`respond` 从未出现在面向插件的 `PluginIPCAPI` 上，接收方也拿不到 `requestId` 来关联回复，于是任何 façade 级 `sendAndWait` 都会挂到超时。`call()` + `expose()` 是受支持的请求-响应路径。）
