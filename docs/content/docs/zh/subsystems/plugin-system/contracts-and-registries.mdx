...
- `plugin-capabilities.ts` —— 59 个 `PluginCapability` 契约。
...
`auditPluginPointContracts()` 遍历每个契约；对于 `status === "implemented"`，它断言 `binding`、`docs` 与 `requiredTests` 非空，产出 `"verified" | "missing_proof" | "not_applicable"` 的 `proofStatus`。`auditPluginCapabilityContracts()` 对 59 个能力做同样的事，并额外要求工具 / 命令 / 钩子提供 `builtinContributionPaths`。`runtime-proof-audit.ts:auditPluginRuntimeClaims()` 把两者加上手写的 `RUNTIME_RISK_AUDITS` 组合成一个 `PluginRuntimeClaimsAuditReport`。
