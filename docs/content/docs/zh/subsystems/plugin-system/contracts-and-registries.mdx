...
- **`ctx.pet`**（capability `pet`；权限 `pet:read` / `pet:interact`，`lib/plugin/api/pet-api.ts`）—— 读取去 PII 的宠物摘要、订阅净化后的事件、发起养育互动、发放 XP/金币奖励。奖励受令牌桶限流（`pet:interact` / `pet:emit`）并被每插件每日预算钳制（50 XP / 100 金币，`lib/plugin/api/pet-budget.ts`）；生命周期事件不可由插件发出。
...
所有辅助函数都是纯类型透传，**除了** `defineCharacterPack`，它会立即校验。包根会导出全部 helper，并提供直接子路径：`@cognia/plugin-sdk/define/<name>` 映射到 `packages/plugin-sdk/src/define/define-<name>.ts`。
