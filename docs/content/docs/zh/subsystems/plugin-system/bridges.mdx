...
<TLDR>
  ...
  （`module-bridge-map.ts:146`）通用派发：它依据 `manifest[manifestField]?.length` 对每个条目做门控，
  并在单个 await 循环里归一化 20 个字段驱动条目（`:126-145`）。VS Code 风格的桥接器（语法、图标、语言、片段）是
...
<StatGrid>
  <Stat label="桥接器文件" value="34" hint="lib/plugin/bridge/ TS，不含测试" />
  <Stat label="异步派发条目" value="20" hint="MODULE_BRIDGE_CAPABILITIES (module-bridge-map.ts:146)" />
...
（`module-bridge-map.ts:146`），依据 `manifest[manifestField]?.length` 对每个条目做门控，并在单个
await 循环里归一化全部 20 个字段驱动条目（`:126-145`）。VS Code 风格的桥接器（语法、图标、语言、片段）*不在*
...
- **两张派发表**——`module-bridge-map.ts`（20 个字段驱动模块桥）对
  `capability-bridge-map.ts`（16 个同步的内联定义叠加能力）。VS Code 风格的存储桥接器（语法、图标、语言、
  片段）两者皆不在——它们由 `vscode-loader`/`context.ts` 驱动。
- **字段驱动门控 + 规范能力标签**——派发依据 `manifest[manifestField]?.length` 门控；
  `workspace-backend`、`message-renderer`、`chat-middleware`、`modal-mount`、`density-preset`
  等字段驱动桥现在都有显式能力契约和 SDK helper（`module-bridge-map.ts:146`）。
