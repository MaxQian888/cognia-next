...
`conversational-task-intent.ts` 从用户正在输入的一行里读出调度意图（"每天早上提醒我看 PR"、"every weekday at 9 send a stand-up reminder"）；`conversational-task-authoring.ts` 把它变成一份带类型的 `CreateScheduledTaskInput`，payload 形状与 executor 读的完全一致。这是模式匹配而非 LLM 调用 —— 它给出的节奏（每天 09:00 的 cron）是起点，不是对真实节奏的猜测。

落点：聊天输入框下方的一行安静建议（`components/chat/composer/schedule-suggestion.tsx`）。它从不接管这一轮 —— 回车照常发送 —— 并且可以按行忽略。接受后草稿存进 `lib/scheduler/task-draft-handoff.ts` 并跳转 `/scheduler`，页面下次挂载时以预填状态打开创建面板。草稿只被消费一次，五分钟后过期，因此过期的交接不会在事后弹出面板。

## 内置 executor

`lib/scheduler/executors/index.ts` 中的 `registerBuiltInExecutors()` 注册全部内置 executor。`initSchedulerSystem` 在启动时调用它。在那之前就被启动的调度器（插件的生命周期写入早于初始化组件挂载）会在内置任务到期时经由 `lib/scheduler/executor-owners.ts` 按需加载它，因此内置类型在那里以 `BUILT_IN_EXECUTOR_TASK_TYPES` 列出。所有 chat 类（chat / agent / skill / goal / plan / agent-team）都通过同一份 `resolveSendOptions` 流水线，复用交互式 Composer 使用的角色 / agent mode / 技能 / 工具 / MCP / 内置工具配置 —— 调度任务跑出的提示词与人工发送的语义完全一致。
...
失败的运行会在 `retryAttempt < config.maxRetries` 时按指数退避重试。例外是执行器标记为“重试只会重演”的失败（见 `types/scheduler/index.ts` 中的 `isRetryableTerminalReason`），目前只有 `needs-approval`。

`chat`、`agent`、`skill` 和 `goal` 运行无人值守。需要权限的工具会被 `lib/claude/unattended-permission-responder.ts` 立即拒绝；未受信任工作区中的运行只拿到受限模式（Restricted Mode）的工具。两种情况下，运行都以 `failed` 结束，`terminalReason` 为 `"needs-approval"`，`output` 中带有 `status: "needs_approval"`、被拒绝的工具（`needsApproval`）以及可能的 `workspaceTrust` 限制。调度器随后：

- **不**安排重试，因为同一轮会停在同一处，并再次消耗 token；
- 发出 `needs-approval` 通知（warning 级别，由 `notification.onError` 控制），取代 `error` 通知；
- 通过 `onScheduledTaskError` 交给插件一个 `code: "NEEDS_APPROVAL"` 的 `SchedulerError`，让插件区分“在等人”和“出故障”；
- 仍把这次运行计为一次失败，计入 `pauseAfterConsecutiveFailures`，因此一个每次都需要批准的计划会被暂停，而不是无限计费。

工作台以琥珀色的 **需要批准** 显示这类运行，在运行详情里列出被拒绝的工具和未受信任的根目录，并产生 `needs-approval` 关注信号。Bot 的 `agent-turn` 运行遵循同样的规则：`lib/bot/runtime/run.ts` 把 `needs_approval` 结果记为 `failed`，并撤销投递，而不是退避重试。
...
这道闸**执行两次**：每个 executor 先调 `assertTaskTypeSupportedOnHost`，`task-scheduler.ts` 在派发前再查一次同一张表。第二次不是为冗余而冗余 —— 一个声明了需求但 executor 忘记设闸的类型，过去会抛出与真 bug 无法区分的裸 `Error`。
...
这个选择约束**整页**。统一列表的 `app` 与 `plugin` 两个 source 每次调用都经 `getSchedulerDataSource()` 解析后端，并在目标切换时重新订阅，因此侧栏、日历、时间线与分面计数展示的主机，与详情面板、暂停、立即运行作用的主机是同一台。远端日程在伴侣通道上没有变更流，所以这两个 source 采用轮询（`REMOTE_TASK_POLL_INTERVAL_MS`，30 秒），并在自己写入后立即重读。其余四种 kind —— 备份、工作流、系统、连接器 —— 没有跨主机 RPC，永远属于本机；管理配对日程时主机栏会明说这一点。
...
- 菜单 / 系统托盘有未读失败时显示徽章。
- 运行进行中的**进度**通知：任务设置了 `notification.onProgress` 时触发。只有会上报中途进度的 executor 才会产生它 —— 目前是 `plugin` 任务，经 `PluginTaskContext.reportProgress`。上报同时作为 `info` 条目写入执行记录。二者都由 `lib/scheduler/execution-progress.ts` 统一处理：写入会合并、通知按执行限流、保留的进度条目有上限，一个高频上报的 handler 无法把记录撑大。
...
| 目标                | 文件                                                                            |
| ------------------- | ------------------------------------------------------------------------------- |
| 加内置 executor     | 在 `executors/` 写一个并注册，在 `executor-owners.ts` 列出其类型                     |
