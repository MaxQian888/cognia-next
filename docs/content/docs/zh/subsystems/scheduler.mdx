...
<TLDR>
  双层调度器 —— 应用内 cron 循环负责"只在 Cognia
  打开时才需要跑"的任务，原生提升路径把选中的任务镜像到 Windows Task Scheduler / launchd /
  systemd，应用关闭时也能触发。 内置 9 个 executor，并支持运行时注册自定义 executor。
</TLDR>
...
<StatGrid>
  ...
  <Stat
    label="内置 executor"
    value="9"
    hint="chat · agent · skill · external-agent · script · plugin · backup · twin · custom"
  />
