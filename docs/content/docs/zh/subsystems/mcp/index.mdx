...
| Agent 镜像 + 健康 | `components/settings/mcp-*`, `mcp_test.rs`, `mcp-audit-log.ts` | [Agent 同步与健康](./agent-sync-and-health) |
...
每一个 MCP 服务器——无论是手工填写、从预设安装、从 CLI 配置导入，还是
由插件播种——最终都落入 **同一个** `McpServer` 行结构（`lib/claude/types.ts:1969`）：
...
`enabled` 控制该服务器是否抵达 Cognia 自己的 chat；`appsEnabled[agent]` 控制它是否
被镜像进那个外部 agent 的配置文件。两者相互独立——一个服务器可以对
Claude Code 的 CLI 开启而在 Cognia 中关闭，反之亦然。
