...
<TLDR>
  ...
  两个纯投射把行转换成其他形态——`buildMcpServerMap`（供 Agent SDK 的
  `mcpServers` 选项）和 `serverToTestRequest`（供 Rust 连接探测）。
</TLDR>
...
### → Rust 探测（`serverToTestRequest`）

`components/settings/mcp/mcp-server-utils.ts:50` 把一个松散类型的 `config` blob 强制成严格的
`McpTestRequest` IPC 形态，做显式的 `typeof` 检查，使一个格式错误的行无法抵达 Rust：
