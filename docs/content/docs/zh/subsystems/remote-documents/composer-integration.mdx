...
命名空间前缀是按模式区分的。所有聊天输入框（`files` 与 `combined`）都拿到上面这组前缀；工作流编辑器拿到的
是 `@node:` / `@edge:`。在直接对话里，位于消息最开头的裸 `@` 还会列出路由目标（`@claude`、`@codex`、Squad
成员），它们决定由哪个运行时来回答这一轮（`lib/chat/turn-route/`）。
