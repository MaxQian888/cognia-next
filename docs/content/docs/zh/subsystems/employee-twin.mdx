...
`html`、`pdf`、`office`。代码：`dispatch.ts`。详见 [document-processing](../data/document-processing)。
...
**这是隐私红线。** `packages/redact/src/index.ts` 在**任何 embed 或 LLM 调用之前**把 PII 替换为 `<KIND_NNN>` 占位符：
...
- 远程 —— 当前向量后端（`lib/vector/`，详见 [vector-store](../data/vector-store)），collection 名 `cognia_twin_{twinId}`。
...
<Steps>
  ...
  <Step>
    ...
    `sourceTitle`。后端选择见 [vector-store](../data/vector-store)。
...
`resolveSendOptions`，runtime 内部跳过 `generateEmbedding`。详见 [agent-teams](../chat/agent-teams)。
...
<Callout type="error" title="安全关键：脱敏测试">
  `packages/redact/src/index.test.ts` 是整个仓库最安全关键的测试 —— 在合成 PII 串语料上断言
...
| 加强脱敏                 | `packages/redact/src/index.ts`（连同测试！）                                         |
...
<Cards>
  <Card title="向量库" href="../data/vector-store" description="RAG 的真正执行层 + collection 设计。" />
  <Card title="文档处理" href="../data/document-processing" description="parse 阶段委派的 document-processor。" />
  <Card title="聊天 & 技能" href="../chat/chat-and-skills" description="启用孪生的角色 / 技能绑定。" />
  <Card title="Agent Teams" href="../chat/agent-teams" description="team chat embedding 共享路径。" />
  <Card title="备份 & 数据传输" href="../data/backup-and-data" description="孪生表是备份载荷的一部分。" />
  <Card title="存储" href="../data/storage" description="孪生 Dexie 表细节。" />
</Cards>
