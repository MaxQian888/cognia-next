---
title: Rust 层
description: 平台连接器子系统的原生 Tauri 后端——一个带逐平台签名验证的 axum webhook 接收器、OS keyring 凭证托管、OneBot 反向 WS 桥接、通用出站 WS 客户端、飞书 protobuf 长连接、限速 HTTP 客户端，以及 AES-256-GCM 附件缓存，通过 18 个 Tauri 命令和四类事件通道暴露给渲染端。
---
...
<TLDR>
  ...
  的报文体。这一切都可由 TS 侧通过 `lib/connectors/tauri/commands.ts` 中的类型化封装抵达，它们 `invoke` 下列 18 个
  `#[tauri::command]`（`commands.rs`，注册于 `lib.rs`），并将入站流量呈现在四类 Tauri 事件通道上。
</TLDR>

<StatGrid>
  <Stat label="Rust 源文件" value="~14" hint="connectors/ + sigverify/" />
  <Stat label="Tauri 命令" value="18" hint="注册于 lib.rs:579-596" />
...
未命中时它 `reqwest::get` 源、用 12 字节随机 nonce（前置于密文）做 AES-256-GCM 加密，并同时写入 `.enc` 密文 blob 与解密副本。
主密钥是来自 `OsRng` 的 32 字节，首次使用时自动生成进 keyring。

## Tauri 命令面

`commands.rs` 恰好暴露 18 个 `#[tauri::command]`——渲染端唯一能抵达的入口点。它们注册于 `src-tauri/src/lib.rs:579-596`
