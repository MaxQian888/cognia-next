...
11. **启动工作流进度 runner** ——
    `startWorkflowProgressRunner()` 将 IM 触发的工作流运行经由出站队列扇出回去
    （`connector-bus-provider.tsx:241-242`）。
12. **订阅凭据轮换 → 生命周期** ——
    `subscribeCredentialsRotatedToLifecycle()`，经由同一个 `AbortController` 解绑
    （`connector-bus-provider.tsx:250-255`）。

第 5、6 与 8–11 步 —— 运行时/回调接入与四个 runner 的启动：
...
调用 `ac.abort()`、释放 cleanup/heartbeat/workflow 句柄，然后遍历
...
`InboxCommandPalette`（cmdk）在桌面与平板/移动两种布局中**都**挂载。
