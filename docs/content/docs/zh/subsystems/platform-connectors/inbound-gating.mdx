...
| `reply-to-bot`      | `event.replyTo?.messageId` 存在                                    | `policy-eval.ts:43` |
...
有了解析出的 `mode`、`evaluatePolicy` 结果以及 `storeUnmatchedInDraftMode` 标志，`routeInbound`（`mode-router.ts:21`）把入站映射到五个 `RouteDecision` 中恰好一个（`mode-router.ts:11`）。
