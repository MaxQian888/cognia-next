...
- **`daily-schedule.ts`** —— 通用的 `startDailySchedule(options)`（`lib/connectors/daily-schedule.ts:50`）：一个 `setTimeout` 初始延迟（默认 60 s，使其不与应用启动争抢）交接给一个 `setInterval`（默认 24 h）。返回一个 `{ dispose, runNow }` 句柄；抛出的任务会被捕获并记录，因此一次失败绝不会终结整个计划。被跨适配器扫描共享。
