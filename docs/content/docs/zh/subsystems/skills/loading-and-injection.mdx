...
<Callout type="info">
  **仅名称模式（`ctx.skillRenderMode: "name"`）。** 调用方（独立 CLI）可选择*渐进式披露*：
  `resolveSendOptions` 不再用 `renderSkillsSection`（完整正文），而是调用 `renderSkillsCatalog`
  （`lib/db/skills.ts`），仅在「Available skills」标题下输出名称 + 描述的**目录**，并指示模型调用
  `load_skill` 工具按需拉取技能完整指令。这样在启用大量技能时仍能保持提示精简。该字段在桌面聊天路径上缺省，
  故其行为（完整正文）不变。加载工具本身由调用方负责——见 CLI 的 `agent/skill-load-tool.ts`。
</Callout>
...
`extractContainerSkillIds`（`skills-bridge.ts`）将已解析的集合过滤为携带 `containerSkillId` 的条目，
并把它们映射为 `{ skill_id, version? }`，上限 `MAX_CONTAINER_SKILLS = 8`。

> **当前无法投递。** Claude Agent SDK **没有任何 `query()` 选项可以附加已上传（托管）的 skill_id**
> ——已对 sdk `0.3.x` 核实：没有任何运行时读取 `containerSkillIds`，`SdkBeta` 不含 skills 值，也不会
> 构造 `container.skills` 请求。因此 sidecar dispatcher 不再转发 `containerSkillIds`（且移除了无效的
> `skills-2025-10-02` beta 头），`resolveSendOptions` 在选中托管技能时改为**告警**而非静默丢弃。
> `inline` / `local-folder` 插件技能不受影响——它们仍以 markdown 折入系统提示。`extractContainerSkillIds`
> 与 `SendOptions.containerSkillIds` 类型保留（`@deprecated`），仅为将来 SDK 支持该能力时可无 schema
> 变更地重新接线。如今要真正运行已上传技能，需要 SDK 新特性或直连 Messages API 旁路（超出本范围）。
