...
<TLDR>
  ...
  市场**（`skills-lock.json` 注册表 + 可选的 SkillsMP API）；一个**三目标原生同步**
...
<StatGrid>
  ...
  <Stat label="市场来源" value="2" hint="注册表（skills-lock.json，6 条目）+ SkillsMP API（通过 skillsMpBaseUrl 选择性启用）" />
...
lib/skills/
  ...
  marketplace-*.ts       # 类型 · 注册表适配器 · SkillsMP 适配器 · 安装 · 预告
...
| [市场](./marketplace) | `skills-lock.json` 注册表 vs SkillsMP API、按 `canonicalId` 的安装/卸载幂等性、Rust 远程拉取代理、预告 |
