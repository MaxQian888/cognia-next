---
title: 内置技能
description: ADR-0026 的类型化工具层级——一个经过代码审查的 MCP 工具注册表（40 个 Lark 技能），助手可直接调用，由 PII/白名单/IM 访问/HITL 分发器把守，并在确认卡片获批后通过连接器总线重新触发。
---

# 内置技能

<Status variant="beta">ADR-0026 — lib/skills/built-in · 40 个 Lark 技能 · 经 A2UI 确认卡片实现 HITL</Status>
...
<TLDR>
  ...
  `build-options.ts:895` 和 `plugin-tool-ipc.ts:144`。
</TLDR>
...
`lib/twin/ingest/redact.ts` 中的 `hasNoLeakingPii`（`dispatcher.ts:36`），即保护 Twin、
Goal 和连接器自动模式的同一条红线。
...
<Callout type="info" title="坑点">
  ...
  - 审计是尽力而为：`safeAppendAudit`（`dispatcher.ts:387`）吞掉 Dexie 失败，因此缺失的审计
    存储绝不会破坏技能。
  - 桌面应用内聊天目前运行写技能时**不**弹出确认卡片（`dispatcher.ts:345`）——用户
    已经在盯着聊天；桌面同意覆盖层是已记录的未来钩子。
</Callout>
