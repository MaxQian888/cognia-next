...
<TLDR>
  `/logs`（`app/logs/page.tsx`）渲染 `<LogPanel>`（`components/logging/log-panel.tsx:140`），即
...
`LogsPage`（`app/logs/page.tsx:39`）渲染 `<LogPanel showStats showTimeline
includeAgentTrace={false} … headerSlot={<LogsPageHeader/>}>`。页头通过
`useLogPanelHeader()`（面板发布的一个 React context）读取面板 API，使面包屑计数徽标和预设
下拉菜单存在于 page chrome 中，而状态则存在于面板内。
...
| 新增一个设置字段 | `components/logging/log-settings.tsx`（+ 为新传输选项改 `bootstrap.ts`） |
