...
同一个时间窗口控件（`今天 / 7 天 / 30 天 / 全部`，`lib/observability/trace-window.ts`）同时驱动
列表和 `AgentTraceStatsBar`，因此头部数字与下方行永远描述同一批数据。筛选作用于整个窗口而非当前
页，这也是列表读取窗口而不是用 `queryRecentTraces` 分页的原因。

span 详情的**在日志中查看这条追踪**会切到日志通道并写入 `?trace=<id>`，同时重挂面板，让它的挂载期
URL 水合读到这个焦点。`/observability` 依旧是同一批 span 的聚合视图；两者互相跳转。
