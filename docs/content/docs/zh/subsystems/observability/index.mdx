---
title: 可观测性
description: cognia-next 的完整可观测性面 —— 一个统一的日志管道（浏览器 fan-out + Rust fern 分发 + 原生 OS 日志桥接）、一条 agent-trace span 管道、一个 Langfuse 风格的追踪仪表盘、Rust 性能仪表盘（ADR-0035）、原生崩溃上报，以及 token/成本用量追踪。两条路由（/logs、/performance）外加一个下次启动时的崩溃对话框。
---
...
<TLDR>
  ...
  OTLP/HTTP 导出。(3) **追踪仪表盘（Tracing dashboard）** —— `/logs` → 追踪频道的「仪表盘」子视图是一个
  Grafana/Langfuse 风格的可拖拽面板网格（`lib/observability/` 是一个真实的百分位/分桶/汇总引擎），
  与旁边的 trace 列表读取同一个时间窗口、同一套筛选下的同一批 span。(4) **性能（Performance）** —— `app/performance`（ADR-0035）是一个任务管理器风格的实时视图，展示
...
<StatGrid>
  <Stat label="路由" value="2" hint="/logs（日志 · 追踪 · 事故）· /performance（+ 崩溃对话框）" />
...
flowchart TB
  subgraph Browser["Browser runtime (static export)"]
    ...
    Dexie --> Obs["/logs?channel=traces<br/>探索 + 仪表盘"]
...
| `/logs?channel=traces` | 追踪浏览 + 追踪仪表盘 | Dexie `agentTraces` 表 | 是（移动端也可） |
...
<Cards>
  ...
  <Card title="追踪仪表盘" href="./observability/tracing-dashboard" description="/logs 追踪频道：Grafana/Langfuse 风格的面板网格。lib/observability 是一个真实引擎 —— 精确百分位、固定边界分桶、trace 汇总 + 瀑布、group-by 分解、阈值。" />
