...
`agent-trace-posthog-byo`。这些实例始终禁用内容采集，固定使用 `/i/v0/ai/otel`，并在 Tauri
中通过 Rust 类型化凭据 seam 注入公开项目 token。Sidecar 在单一 `NodeSDK` 下，为每个通用
OTLP 或 PostHog 目的地创建独立 `BatchSpanProcessor`。产品分析不进入日志管道：
`track-event.ts` 只有在通过行为授权、分类、采样、标量和 PII 闸门后，才把结构化事件 envelope
fan-out 到 OTLP Logs 与按需加载的具名 PostHog 实例。
