...
`langfuse-transport.ts` 只接受已完成的 `AgentTraceSpan` envelope；普通应用日志与 metrics 会被
忽略。它清洗并批量提交 `AgentTraceBatchV1` 到窄 Host ingest command。Host 根据认证账户解析
只写的 BYO credential，再次校验 schema、PII、授权、速率与大小限制，按 span ID 去重，并固定
导出到 `/api/public/otel/v1/traces`，Langfuse ingestion version 为 4。

Sidecar 的共享 `NodeSDK` 同时挂载官方 Langfuse processor，生成 AI SDK 7 的 generation/tool
子 observation；通用 OTLP 与 PostHog 仍是独立 processor。Renderer 根节点保留稳定的
`chat.turn`、`agent.run`、`workflow.run` 名称；重复的手写 provider/tool 子节点只在远程目的地
过滤。模型内容与工具内容分别授权、默认关闭，并都经过有界字段级过滤与 Host PII gate。
...
| langfuse | Cognia Host → Langfuse v4 OTLP traces | 是（32） | `degraded`(未配置 \| lastError) / `healthy` |
