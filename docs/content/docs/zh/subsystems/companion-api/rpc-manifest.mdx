...
<TLDR>
  ...
  Brain 使用 `POST /internal/_rpc/{name}`。`protocol/companion-commands.json` 是唯一的契约：
  它提供 target、transport、capability、risk、approval、idempotency 与 schema 元数据，运行时
  allowlist 由它渲染成 `generated/known_commands.rs`（ADR-0175），不再手写第二份。宿主在
  `GET /api/catalog` 与 `GET /internal/catalog` 列出调用方可以派发的命令。
</TLDR>

<StatGrid>
  <Stat label="运行时 allowlist" value="自动生成" hint="generated/known_commands.rs" />
  <Stat label="公开设备子集" value="自动生成" hint="execution/host-admin + HTTP" />
  <Stat label="命令契约" value="v3" hint="protocol/companion-commands.json（ADR-0175）" />
