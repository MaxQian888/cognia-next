...
<StatGrid>
  <Stat label="默认端口" value="7890" hint="server.rs:51" />
...
`PairResponse`（`auth.rs:99`）为 `{ deviceId, deviceJwt, serverVersion, rendezvousId, rendezvousSecret }`。
`rendezvousId`（UUIDv4）和 32 字节 URL-safe `rendezvousSecret`（`auth.rs:337-342`）是
[WebRTC 平面](./signaling-webrtc)的种子——它们让手机和桌面端在同一房间中找到彼此，并对每个信令
信封做 HMAC 校验。
