...
<StatGrid>
  ...
  <Stat label="默认 companion 端口" value="7890" hint="server.rs:51 — 可调整" />
...
| --- | --- | --- | --- | --- | --- |
| **Companion API** | `src-tauri/src/companion_api/` | `127.0.0.1` **或** `0.0.0.0`（可切换）+ TLS | 默认 `7890` | **设备 JWT**（HS256） | 移动端 companion 应用 |
...
<Cards>
  ...
  <Card title="WebRTC 信令面" href="./signaling-webrtc" description="桌面端应答方：SignalingHub、rendezvous WSS 客户端、webrtc-rs peer、DataChannel→dispatch 泵、HMAC 签名的标准信封，以及每设备连接层级 FSM（ADR-0021）。" />
...
- **签名密钥即身份。** OS keyring 中一个 32 字节的 HS256 密钥（`secret.rs:21`）签发所有
  pair JWT 和设备 JWT，并作为 WebRTC rendezvous HMAC 的种子；`healthz` 发布的 `server_id`
