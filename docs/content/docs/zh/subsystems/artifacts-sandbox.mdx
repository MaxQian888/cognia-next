...
<Callout type="warn">
  **目前在每个壳里都是坏的。** React 19 不再发布 UMD 构建，下面那些
  `unpkg.com/react@19/umd/…` 标签都是 404，每次 React 预览都落到 15 秒超时提示。
  把运行时改成本地提供，卡在一次实测上：需要在 `pnpm tauri build` 里确认
  `about:srcdoc` 子框架是否继承打包壳的 CSP——见
  [ADR-0158](../adr/0158-artifacts-and-canvas) 的「未决」一节。
</Callout>
...
- **复制 / 下载 / 导出为** —— 由 `runtime-adapters.ts:exportFormats` 驱动。`raw`
  ...
  （`lib/artifacts/export/`）。`react` 只提供 `raw`——对未执行的 JSX 做离屏截图就是
  一个空白矩形。栅格化 `chart`/`mermaid`/`math` 需要其预览已挂载，取不到时抛
