...
<TLDR>
  ...
  （`src-tauri/src/cli_bridge/detect.rs:230-265`）。托管下载
  （`download_cognia_cli`，`download.rs:200-309`）解析平台三元组，从 GitHub Releases
...
- **签名**（`:256-275`）—— `.sig` 接受原始 64 字节或 base64
  ...
  真实密钥 ⇒ 失败即中止安装
- **离线校验** —— `cognia release-verify <artifact> --checksums <checksums.txt> [--signature <sig>] [--json]`
  在不下载、不安装的情况下复用同一套 checksum 与 release-key 策略。发布密钥是真实密钥时，
  默认读取 `<artifact>.sig`；`--signature` 只是覆盖这个 sidecar 路径。JSON 会报告两个摘要、
  `checksumVerified`、`signatureStatus` 和 release-key fingerprint。
