---
title: 导入转换
description: 转换 MCP 条目、Skill、CLI 骨架，或完整的 Claude Code、Codex、Gemini CLI 与 Cognia 插件包；不执行源代码，也不静默丢失行为。
---
...
整插件转换使用 `lib/plugin/convert/ecosystem.ts`，与应用内 GitHub 预览及 Agent 转换技能共用同一个
规范转换器：

```bash
# 受支持的 Agent 插件 → Cognia
...
# 写入导出（只有审阅并接受警告后才添加 --accept-warnings）
cognia plugin export --input ./cognia-plugin --to codex --dir ./codex-plugin
```

目标包括 `cognia`、`claude-code`、`codex`、`gemini-cli`、`agent-plugins`、`cursor`、`copilot`、
`kimi`、`devin`、`opencode`、`pi`。转换器识别受支持的清单，保留 Skill 资源树与二进制文件，
返回保真度及逐能力处理报告。外部生态之间也通过同一规范表示进行转换。各目标支持的能力不同：
例如 Pi 插件包不能直接携带 MCP 服务器，OpenCode 插件包需要可移植的本地 MCP 命令。

调用控制、Hook、Agent、命令式运行时行为、权限或配置字段，如果没有实现对应的目标映射，
会阻止输出。存在警告时，写入前必须明确接受。Gemini Skill 保留原生 Skill 形态，不降级为命令提示。
已识别的凭证配置会被清理，需要补充的设置在报告中说明。

`--surface cli|desktop|cloud` 指定预期运行端。生成插件包不代表已经验证安装、认证或真实运行。
云端导出在交付路径确认前会被阻止。无法原生导出的运行时工具可能提供 Cognia 托管选项：
在受支持的 Cognia 外部 Agent 会话中启用插件；其界面、存储、Hook 和生命周期仍由 Cognia 承载。

### Cognia 中由 Agent 驱动转换

种子 `plugin-conversion` 提示技能会在桌面工作区聊天中向 Cognia Agent 暴露
`plugin_conversion_inspect` 与 `plugin_conversion_apply`。检查阶段只读，并返回绑定源快照的
限时计划；应用阶段会重新校验快照、拒绝非空或重叠输出目录、要求桌面审批，并且只写确定性转换产物。
这些工具不会暴露给 IM、Web 或移动端会话。桌面插件工具栏也提供转换对话框，共用检查报告、
警告确认和绑定源快照的应用计划。
...
- **单条 MCP 导入与整插件转换的输入不同。** `--from mcp` 仍消费受支持的 Agent 配置格式；
  Codex 插件 TOML 属于整插件路径（`--from plugin`）。
