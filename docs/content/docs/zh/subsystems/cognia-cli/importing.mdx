...
```bash
# Claude Code、Codex 或 Gemini CLI 插件 → Cognia
...
转换器会检测 `.claude-plugin/plugin.json`、`.codex-plugin/plugin.json`、`gemini-extension.json`
或 Cognia `plugin.json`；转换结构化 Skill、Agent、MCP 定义和受支持元数据；逐字节保留二进制资源；
并返回显式保真度报告。支持 Claude Code、Codex 或 Gemini CLI → Cognia，以及 Cognia → 这三种生态
中的任意一种；外部生态 → 外部生态会被拒绝。

转换采用关闭式失败。Hook、命令式运行时行为、权限或缺少安全目标契约的贡献类型会成为 blocking
问题，而不会产出一个残缺插件。

### Cognia 中由 Agent 驱动转换

种子 `plugin-conversion` 提示技能会在桌面工作区聊天中向 Cognia Agent 暴露
`inspect_plugin_conversion` 与 `apply_plugin_conversion`。检查阶段只读，并返回绑定源快照的
限时计划；应用阶段会重新校验快照、拒绝非空或重叠输出目录、要求桌面审批，并且只写确定性转换产物。
这些工具不会暴露给 IM、Web 或移动端会话。
