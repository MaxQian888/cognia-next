---
title: Bridge 服务端
description: 共享回环 bridge 的桌面端 —— 临时 127.0.0.1 端口、每次启动认证、供两个本地 CLI 使用的 12 条路由、插件操作、会话事件与 renderer-backed twin/team request。
---
...
<TLDR>
  ...
  **`127.0.0.1:0`**（OS 分配的临时端口）的 axum router，写入 `cli-endpoint.json`（Unix 上 `0o600`），
  ...
  插件与会话 handler 发出 event，twin/team handler 则使用 pending renderer request/response
  bridge。本页聚焦插件作者 CLI 子集；完整共享拓扑见 [CLI ↔ App 桥接](../cli-app-bridge)。
</TLDR>
...
带着一个失效的端口 + token。当前原生 startup block 没有显式的移动端编译门控；renderer listener
与同步控制受 desktop/Tauri gate 保护。
...
插件与会话通知是 **fire-and-forget** 的。Twin context 与三条 team 路由会创建 Rust pending
oneshot、emit `cli-bridge://renderer-request`，再由 `cli_bridge_renderer_response` Tauri command
resolve。Rust 在 30 秒后过期 request；Agent CLI 在 10 秒后 abort fetch。Renderer event hook
处理异步插件/会话 channel：
