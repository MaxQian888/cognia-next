...
<TLDR>
  `main.rs`（`crates/cognia-cli/src/main.rs`，465 行）解析一棵嵌套的 clap-derive 树 ——
  `Cli` → `TopCommand::Plugin` → 一个 11 变体的 `PluginCommand` enum —— 解析出一个 `ColorMode`，
  构建一个 `RuntimeUi`（`ui/runtime.rs:60`），并通过 `dispatch_plugin()`
  （`main.rs:288`）路由。每个交互触点都经过 **`Prompter` trait**
  ...
  `eyre`（`anyhow_to_eyre`，`main.rs:277`），让 color-eyre 渲染完整的成因链；exit code 为
...
11 个 `PluginCommand` 变体（`main.rs:117-241`）及其在
`dispatch_plugin()`（`main.rs:288-346`）中的分发点：

| 变体 | Flags | 分发 |
| --- | --- | --- |
| `New` | `--dir` `--kind` `--author` `--author-email` `--description` `--with-keygen` | `main.rs:298` |
| `Lint` | `--path .` `--json` | `main.rs:310`（先设置 `ui.flags.json`） |
| `Build` | `--path .` `--out` `--skip-build` | `main.rs:316` |
| `Info` | `--json` `--detailed` | `main.rs:323` |
| `Sign` | `--key`（必需） `--out` | `main.rs:325` |
...
命令返回 `anyhow::Result`；`anyhow_to_eyre()`（`main.rs:277-286`）将链反向重新包装，
...
（`cmd_lint.rs:226-230`）—— 仅有 warning 则保持 exit 0。

## 集成测试

`crates/cognia-cli/tests/integration.rs` 以 `NO_COLOR=1`、`CI=true` 并清空 stdin，通过
`CARGO_BIN_EXE_cognia` 启动已编译的二进制。这 10 个测试钉住公开契约：`--help` /
`--version` 的形态、对干净 vs 损坏 manifest 的 lint exit code、`lint --json` 携带
`schemaVersion: 1`、`info --json` 以 `signature.status = "no-sidecar"` 往返、
