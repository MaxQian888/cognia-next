---
title: 状态与持久化
description: Zustand store 是 A2UI 唯一的消息入口——它把 surface 元数据持久化到 localStorage（LRU 20，已剥离），而独立的 Dexie 层（apps · surfaces · templates · event-history）保存可持久化的行。
---
...
<TLDR>
  ...
  (`stores/a2ui/a2ui-store.ts`) 是所有 server message 的唯一摄入入口，也是运行时的事实来源——但它**只把元数据**
  持久化到 `localStorage`（key 为 `cognia-a2ui-surfaces`，按 LRU 上限保留 20 个 surface，并剥离 `components` 与
  `dataModel`）。四张 Dexie 表（`a2uiApps`、`a2uiSurfaces`、`a2uiTemplates`、`a2uiEventHistory`）是一个**独立的可持久化层**，
  由 renderer/hook 层调用，store 从不调用它。undo/redo 仅存在于内存中（50 条栈，300 ms 防抖）。
</TLDR>

<StatGrid>
  <Stat label="持久化层" value="2" hint="zustand→localStorage（元数据） · Dexie（可持久化行）" />
...
<Callout type="warn">
  Zustand store 与 Dexie `a2ui-*` 表是**两个相互独立的持久化层**。store **只把元数据**持久化到 `localStorage`
  （`partialize` 在 `a2ui-store.ts:680`，按 LRU 把 ready surface 上限定为 20，并在第 692 行**剥离 `components` 与
  `dataModel`**——它们"从模板重新生成"）。Dexie 表（`lib/db/a2ui-*.ts`）是一个**独立的可持久化层**，store **从不
  import** 它；把 surface/app/template 写入 Dexie 是由 renderer/hook 层发起的。不要以为写 store 就会把行持久化到
  Dexie，反之亦然。
</Callout>
...
| 保存内容 | surface **仅元数据**（无 components/dataModel） | 完整行：components、dataModel、rootId、审计 |
...
flowchart LR
  ...
  STORE -.->|"partialize 680<br/>剥离 components+dataModel"| LS["localStorage<br/>cognia-a2ui-surfaces (v3)"]
  REND["renderer / hook 层"] -->|"upsertSurface · upsertApp<br/>upsertTemplate · appendEvent"| DEXIE[("Dexie a2ui-* 表")]
...
| 项 | 行号 | 行为 |
| --- | --- | --- |
| `name` | 642 | `cognia-a2ui-surfaces` —— **localStorage**，不是 Dexie |
| `version` | 643 | `3` |
| `migrate` | 644 | → `normalizePersistedSurfaces` (653) |
| `onRehydrateStorage` | 666 | 重新规范化 surface，重新推导 `activeSurfaceId` |
| `partialize` | 680 | LRU 上限 + 剥离（见下） |

`partialize` (`680`) 是承重部分：

1. 按 LRU 上限取 `MAX_PERSISTED_SURFACES = 20` (`682`)。
2. **只保留 `ready` surface** (`685`)，按 `updatedAt` 降序。
3. **剥离 `components` + `dataModel`** (`692`)——注释说明它们"从模板重新生成"。
4. 持久化 `{ surfaces: metadataOnly, activeSurfaceId }`；undo/redo 栈被排除。

这就是为什么仅靠 store 层无法重建完整 surface——重负载被刻意丢弃，必须重新生成（从模板）或从 Dexie 层重新拉取。

## 四张 Dexie 表

可持久化层位于 `lib/db/`。这些表最早在 Dexie `version(13)` 引入（`schema.ts:553` 块，定义 `573-576`）；
**当前**的 schema 版本是 `version(65)` (`schema.ts:1692`)。表类声明在 `schema.ts:114-117`。
...
| | `getApp` | 11 | |
| | `upsertApp` | 15 | |
| | `bulkUpsertApps` | 19 | 空守卫 |
| | `deleteApp` | 24 | 若 `isBuiltIn` 则 **抛出** `"Cannot delete built-in app"` (26-28) |
| | `deleteAllUserApps` | 32 | 内置保留（过滤 `!isBuiltIn`，35） |
| | `touchApp` | 41 | 更新 `lastUsedAt` + `usageCount` |
...
- **内置保护** —— `deleteApp` (`a2ui-apps.ts:24`) 在 `isBuiltIn` 置位时抛出 `"Cannot delete built-in app"`
  (26-28)；`deleteAllUserApps` (32) 与 `deleteAllUserTemplates` (`a2ui-templates.ts:33`) 过滤掉内置，使其在
  批量清除时保留。
...
<Callout type="info">
  a2ui 表**在 Dexie `version(13)` 引入**（`lib/db/schema.ts:553` 块，定义 `573-576`），其索引字符串自此未变。
  **当前/最新**的 schema 版本是 `version(65)` (`schema.ts:1692`)。旧文档的"Dexie v13"把*引入*版本 (13) 与*当前*
  版本 (65) 混为一谈——它们是不同的数字。引用 v13 表示"a2ui 表何时出现"，引用 v65 表示"线上 schema"。
</Callout>
...
lib/db/
  schema.ts              # 表声明 114-117；v13 块 553（定义 573-576）；当前 v65 @1692
  a2ui-types.ts          # A2UIAppRow 35 · A2UISurfaceRow 65 · A2UITemplateRow 86 · A2UIEventHistoryRow 105
  a2ui-apps.ts           # listApps 7 … deleteApp 24（内置抛错）… touchApp 41
