...
<Callout type="warn" title="已构建并测试，但尚未接入生产">
  ...
  - **原生绑定** —— `ocr-ocrs` 和 `ocr-paddle` 这两个 Cargo feature 链接了真实 crate，且 `apple-vision`
    在 macOS 目标上进程内编译启用；`ocr-tesseract` / `ocr-windows` 是解析为 `PlaceholderBackend` 的
    空 feature flag。见[原生后端](./native-backends)。
</Callout>
