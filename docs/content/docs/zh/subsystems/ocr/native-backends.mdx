...
flowchart TD
  ...
  FEAT -->|"apple-vision（macOS 目标）"| AV["AppleVisionBackend::extract<br/>backend/apple.rs"]
...
`apple-vision` 没有 feature 标志：只要目标平台是 macOS 就会编译（`backend/apple.rs`），
通过 `objc2-vision` 绑定在进程内运行 Vision.framework 的 `VNRecognizeTextRequest`——
无 sidecar、无模型下载。

<Callout type="warn" title="tesseract-rs / winocr 在当前代码树中仅停留在文档层面">
  在当前 manifest 中，`tesseract-rs` 与 `winocr` **并未被声明为依赖**——`ocr-tesseract` 与
  `ocr-windows` 是空 feature 标志，因此这两个后端会编译为 `PlaceholderBackend` 并返回
  `MissingBinding`。如今 **ocrs**、**paddle-ocr** 以及（macOS 上的）**apple-vision** 拥有真实实现。
  `ort = "=2.0.0-rc.12"` 这一钉定是精确版本，与 ADR 一致。
</Callout>
