...
<TLDR>
  统一入口 `processDocument()`（同步，仅文本格式）与 `processDocumentAsync()`（异步，含 PDF / DOCX /
  XLSX / PPTX / EPUB / RTF）。类型识别按文件名后缀（`support-matrix.ts`），异步路径按 type 动态
  import 具体 parser（PDF 走 pdfjs-dist；Word 走 mammoth；Excel 走
  xlsx；其余各自的库），把结果归一化到 `ProcessedDocument`。可选生成 chunks 喂给向量库；可选导出
  `parseSummary` 做面包屑/目录展示。
</TLDR>
...
Twin ingest 不直接调 `processDocument` 生成 chunks —— 它有自己的 `prepareChunks` 流程（按 markdown / code / chat 三种 chunker）；但底层 parser 完全复用 `lib/document/`。详见 [员工数字孪生](./employee-twin)。
...
实际生效的 RAG 路径是 Twin runtime（见 [员工数字孪生](./employee-twin)）。
...
UI 入口：`components/document/document-format-toolbar.tsx` 是 markdown 编辑工具栏，绑定到画板上的活跃编辑器（详见 [画板](./canvas)，若已存在）。
...
<Cards>
  ...
  />
  <Card title="artifacts 沙箱" href="./artifacts-sandbox" description="生成的文档落地" />
