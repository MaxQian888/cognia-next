...
<TLDR>
  AES-GCM 加密、含 SHA-256 完整性校验的备份，包格式为 **v3**。
  每设备自动密钥（无需密码）或用户密码（可移植）。 按域传输覆盖 20
  个域（技能、MCP、预设、角色、团队、主题、画板、A2UI、外部 agent、插件……）， 只搬一个切片。ChatGPT
  / Claude / Gemini 三个导入器把外部导出映射到 Cognia 的聊天形态。
</TLDR>

<StatGrid>
  <Stat label="Schema" value="v3" hint="v1 导入时迁移" />
  <Stat label="KDF 迭代" value="60 万" hint="PBKDF2-SHA256" />
  <Stat label="按域传输" value="20" hint="见下表" />
...
<Cards>
  <Card title="存储" href="./storage" description="底层 Dexie schema v26" />
