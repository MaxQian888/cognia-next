...
设置 UI 在 Settings → 搜索 provider 下：每个 provider 一张卡片（启用 / 优先级 / 每 provider `defaultOptions` 编辑器 / 测试连接按钮——会校验整个 key 轮换池并逐个报告备用 key），另有「性能」面板调熔断器参数、「诊断」面板展示各 provider 的实时熔断状态、成功率、平均延迟以及单个/全局重置。
