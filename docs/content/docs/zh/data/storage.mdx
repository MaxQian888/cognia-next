---
title: 存储（Dexie schema）
description: 完整 IndexedDB schema、按版本演进、表索引清单与新增表 / 迁移流程。
---

# 存储（Dexie schema）

Cognia 几乎所有浏览器侧状态都通过 **Dexie 存在 IndexedDB**。Schema 定义在 `lib/db/schema.ts`，当前版本 **v150**，共 **234** 张受治理静态表。Schema 版本只增不改 —— 永远不要修改既有的 `.version(n).stores({...})` 块。

机器可读的治理权威是 `lib/data-governance/table-catalog.ts`，生成摘要位于 `docs/data-governance.generated.json`。`pnpm audit:data-governance` 会阻断 schema/catalog、TypeScript/Rust 同步表、版本顺序或生成文档的漂移。
...
| `characters`    | `id, name, updatedAt, isBuiltIn`                                | 角色人格（[详见](../chat/chat-and-skills)） |
...
| `canvasComments`  | `id, documentId, [documentId+createdAt], parentId, resolvedAt`（旧 Canvas 兼容投影） |
| `canvasSessions`  | `id, documentId, ownerId, createdAt`                           |

详见 [Canvas 页](../subsystems/canvas)。
...
详见 [员工数字孪生](../subsystems/employee-twin)。
...
详见 [插件系统](../subsystems/plugin-system)。
...
详见 [External Bridge](../subsystems/external-bridge) 与 [Wiki 生成器](../subsystems/external-bridge/wiki-generator)。
...
详见 [平台连接器](../subsystems/platform-connectors)。
...
详见 [Claude 订阅 OAuth](../chat/claude-subscription-oauth)。
...
详见 [移动端架构](../ui/mobile-architecture)。
...
timeline
  ...
  v26  : + chatDrafts
  v27+ : ocr / goal / agent-team / overlay / share-links / ... (见 schema.ts)
```
...
| **v113** | 调度清理              | 删除未使用的 `pluginScheduledJobs`；插件计划任务统一存储为 SchedulerDB 中 `type: "plugin"` 的任务。                                                                  |
...
| **v27 – v112** | 后续追加              | OCR 结果、Goal 跟踪、Agent Team 持久化、角色 pack overlay、GitHub Delivery 状态、external agent 配置、移动同步游标、性能采样、终端会话、WeChat/WeCom 适配、公共分享链等。具体差异以 `lib/db/schema.ts` 为准。 |
| **v115** | 资源评论 | 新增 `contextComments` 作为跨资源评论的唯一可写表，并幂等回填 Canvas 评论。所有历史版本块保持不变。 |
| **v116 – v150** | 后续治理与运行时能力 | Sites、统一执行日志、记忆治理、账户 / RuntimeTarget 隔离、模板平台、Knowledge Base、单 Agent 任务、不可变工作流部署、可回放事件序列与 Agent Trace 全局时间索引。精确策略与数量由 catalog 生成。 |

### 资源评论（v115）

`contextComments` 按资源类型/id、父评论、解决状态、创建时间和 project scope 建索引，保存资源级、文本/行区间、Workflow 节点/边锚点，以及 revision 与 stale-anchor 元数据。Canvas store、插件 API、旧 Canvas UI 与备份/导入兼容均通过 adapter 访问该表，因此只有一个可写事实来源。
...
<Cards>
  ...
  <Card title="插件系统" href="../subsystems/plugin-system" description="使用 v15 插件 5 张表" />
  <Card title="员工数字孪生" href="../subsystems/employee-twin" description="使用 v14 孪生 5 张表" />
  <Card
    title="平台连接器"
    href="../subsystems/platform-connectors"
    description="使用 v18 / v19 / v21 连接器 8 张表"
  />
  <Card title="可视化工作流" href="../subsystems/visual-workflows" description="使用 v22 工作流 4 张表" />
  <Card
    title="External Bridge"
    href="../subsystems/external-bridge"
    description="使用 v17 Wiki + 审计 4 张表"
  />
  <Card title="移动端架构" href="../ui/mobile-architecture" description="使用 v23 / v25 移动 2 张表" />
</Cards>
