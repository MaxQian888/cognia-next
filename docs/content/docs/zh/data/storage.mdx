...
Cognia 几乎所有浏览器侧状态都通过 **Dexie 存在 IndexedDB**。Schema 定义在 `lib/db/schema.ts`，当前版本 **v54**。Schema 版本只增不改 —— 永远不要修改既有的 `.version(n).stores({...})` 块。
...
| **v27 – v54** | 后续追加              | OCR 结果、Goal 跟踪、Agent Team 持久化、角色 pack overlay、GitHub Delivery 状态、external agent 配置、移动同步游标、性能采样、终端会话、WeChat/WeCom 适配、公共分享链等。具体差异以 `lib/db/schema.ts` 为准。 |
