---
title: Sidecar 与多供应商 SDK
description: 内嵌 Node sidecar 如何把 Claude Agent SDK 与 Vercel AI SDK 桥接到 Tauri 父进程，包含 JSON-lines 协议、生命周期与 A2UI MCP。
---
...
<TLDR>
  Cognia 内嵌一个小型 Node 主机（**sidecar**），它在 stdio 上通过 **JSON-lines 协议** 与 Tauri
  父进程通信。 默认 dispatcher 包装官方
  [`@anthropic-ai/claude-agent-sdk`](https://www.npmjs.com/package/@anthropic-ai/claude-agent-sdk)，
  非 Anthropic 供应商则走 **Vercel AI SDK**（`ai` 包 + `@ai-sdk/*` 家族）。 每条协议消息都携带
  `sessionId`， 支持并行会话；权限请求 / 响应、用量头采集、OAuth bearer 注入都在这一层处理。
</TLDR>

<StatGrid>
  <Stat label="协议消息类型" value="9" hint="入站 + 出站" />
  <Stat label="供应商 dispatcher" value="2" hint="anthropic / ai-sdk" />
  <Stat label="内置 MCP 服务器" value="2" hint="cognia-tools + a2ui-bridge" />
  <Stat label="支持的协议族" value="5" hint="openai/anthropic/google/mistral/cohere" />
</StatGrid>
...
详见 [provider-system](./provider-system) 与 [chat-and-skills](./chat-and-skills)。
...
<Callout type="warn" title="工具能力差异">
  ...
  按钮以避免静默掉能力。详见 [provider-system](./provider-system)。
</Callout>
...
1. **采集 `anthropic-ratelimit-*` 响应头** —— 所有命中 `api.anthropic.com` 的响应都触发
   ...
   见 [claude-subscription-oauth](./claude-subscription-oauth)。
...
- **`claude_set_api_key` / `claude_set_oauth_bearer`** —— 把新凭证写入 `ApiKeyState`（仅内存，永不落盘）。
  下次 spawn 时通过 env 传入。详见 [claude-subscription-oauth](./claude-subscription-oauth)。
...
3. **技能解析** —— `lib/skills/executor.ts` 把每个技能的 markdown、工具白名单实例化（详见
...
外部 MCP 服务器（非 sidecar 内嵌）参考 [mcp-server-sidecar](./mcp-server-sidecar)。
...
<Callout type="warn" title="永不明文">
  ...
  **永不**写入配置文件。OAuth bearer 路径见
  [claude-subscription-oauth](./claude-subscription-oauth)。
</Callout>
...
<Cards>
  ...
  <Card title="Provider 系统" href="./provider-system" description="多供应商路由 + alias 解析" />
  <Card
    title="Claude 订阅 OAuth"
    href="./claude-subscription-oauth"
    description="OAuth bearer 注入与用量头采集"
  />
  <Card
    title="MCP 服务器（sidecar）"
    href="./mcp-server-sidecar"
    description="A2UI MCP 与外部 MCP 客户端"
  />
