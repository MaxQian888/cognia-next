...
详见 [运行时 & Tauri](./runtime-and-tauri) 与 [移动架构](./mobile-architecture)。封装统一在 `lib/tauri/index.ts` 集中导出，按职责分组（clipboard、dialog、fs、notification、opener、os、process、store、canvas、cli、deep-link、events、autostart、webview-zoom、remote-control、companion-storage、pinned-fetch……）。
...
| `ccswitch`               | 一键切换 Claude Code 提供商档案 + MCP 服务器 + 提示词；详见 [CCSwitch](./ccswitch)。                                                                   |
| `anthropic_subscription` | Claude Pro/Max OAuth 凭证持久化（OS keyring）；详见 [Claude 订阅 OAuth](./claude-subscription-oauth)。                                                 |

### 数据与基础设施

| 模块              | 用途                                                                                                                               |
| ----------------- | ---------------------------------------------------------------------------------------------------------------------------------- |
| `vector`          | sqlite-vec 后端；在任何连接打开前加载 FFI 扩展；详见 [向量库](./vector-store)。                                                    |
...
| `logging`         | `fern` 分发链：tauri-plugin-log（文件 + webview）+ 平台原生日志（EventLog / OSLog / journald）。详见 [可观测性](./observability)。 |

### 输出 / UI

| 模块              | 用途                                                                     |
| ----------------- | ------------------------------------------------------------------------ |
| `canvas`          | 画板文档落盘 + 原生 Python 执行；详见 [画板](./canvas)。                 |
| `skills`          | 技能资源解析 + 沙箱准备 + 远程注册表；详见 [技能系统](./skills-system)。 |
...
| 模块             | 用途                                                                                                                                   |
| ---------------- | -------------------------------------------------------------------------------------------------------------------------------------- |
| `mcp_server`     | Rust 侧 HTTP MCP 服务器，暴露 Wiki + RAG 给外部代理；详见 [Tauri MCP 服务器](./mcp-server-tauri)。                                     |
| `companion_api`  | 移动配对 API（axum + axum-server TLS）、JWT 配对、mDNS 广播、cloudflared 隧道、推送配置；详见 [Companion API](./companion-api)。       |
| `connectors`     | 平台连接器底层：HTTP / WebSocket / keyring / 适配器服务器；详见 [平台连接器](./platform-connectors)。                                  |
| `workflow`       | 工作流 cron 守护进程 + webhook 路由（axum）+ SQLite 运行态镜像；详见 [可视化工作流](./visual-workflows)。                              |
| `remote_control` | 远程控制入站监听器 + HMAC 签名；详见 [远程控制](./remote-control)。                                                                    |
| `proxy_config`   | 系统代理探测、HTTP 测试、统一 HTTP 请求函数。                                                                                          |
| `plugin_api`     | 插件生命周期、权限、API 桥、文件系统监视、窗口操作、快捷键、上下文菜单、签名、备份、市场、devtools；详见 [插件系统](./plugin-system)。 |
...
- `fetch-interceptor.mjs` —— 在 Claude Agent SDK 加载**之前** monkey-patch `globalThis.fetch`，监听 `api.anthropic.com` 的响应头并以 `usage_headers` 事件回传，驱动 [Claude 订阅 OAuth](./claude-subscription-oauth) 的 5h/7d 用量可视化。
...
| 向量嵌入                                                | `<app_data>/cognia/vectors.sqlite`（桌面）或远程（Web）               | 原生定长向量索引需要 sqlite-vec；Web 退回云端后端。详见 [向量库](./vector-store)。          |
| 备份快照                                                | 用户通过 Tauri 对话框选择的文件夹                                     | 默认 AES-GCM 加密。详见 [备份 & 数据传输](./backup-and-data)。                              |
...
| 推送凭证（FCM / APNs）                                  | OS keyring + Dexie 持久化的 dispatcher 配置                           | 详见 [Companion API](./companion-api)。                                                     |
...
也就是说，React 中的一次 `logger.error(...)` 直接出现在 OS 原生日志查看器里，无需额外接线。详见 [可观测性](./observability)。
...
对应的 Provider 也会感知这三态：例如 `DesktopSyncSourceProvider` 在桌面上接 Tauri 事件、在移动上接 Companion 客户端拉取、在 Web 上禁用。详见 [移动架构](./mobile-architecture) 与 [同步系统](./sync-system)。
