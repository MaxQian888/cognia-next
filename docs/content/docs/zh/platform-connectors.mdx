---
title: 平台连接器
description: 让 AI 角色变成 Telegram / Discord / Slack / Lark / OneBot 上的真实机器人 —— ConnectorBus、三种模式、FIFO 出站队列。
---
...
<TLDR>
  五个内置适配器（Telegram / Discord / Slack / Lark / OneBot v11）+ ConnectorBus 单例 做扇入扇出 +
  Dexie-backed 出站 FIFO（断路器、令牌桶、指数退避、死信、幂等 LRU）。 三种模式：auto（AI 自动回）/
  manual（人手敲）/ draft（AI 起草、人审核）。 Dexie v18 新增 8 张表；Inbox UI 在
  `app/inbox/`；插件可贡献额外适配器。
</TLDR>

<StatGrid>
  <Stat label="内置适配器" value="5" hint="telegram · discord · slack · lark · onebot" />
  <Stat label="Dexie 表" value="8" hint="v18 schema" />
  <Stat label="运行模式" value="3" hint="auto · manual · draft" />
  <Stat label="审计上限" value="5000" hint="connectorAudit capped ring" />
</StatGrid>

本页是**架构总览**。平台一键接入指南请见兄弟文档：
...
`lib/plugin/connectors-bridge.ts:registerPluginAdapters(pluginId, manifest, exports)` 在插件启用时注册到 ConnectorBus；禁用时 `unregisterPluginAdapters` 清理。详见 [插件系统](./plugin-system) 与 `docs/content/docs/connectors/extending-with-plugins.md`。
...
<Cards>
  ...
  />
  <Card title="移动端架构" href="./mobile-architecture" description="手机也是连接器的接收端之一" />
...
- **审计上限 5 000 条**：FIFO ring buffer；旧条目自动滚出。需要长期保留请走 [备份](./backup-and-data)。
