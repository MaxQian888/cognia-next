...
仓库通过 `release.yml` → `build-tauri.yml` 在多 OS 矩阵上跑 Tauri 构建并把安装包上传到 draft release。updater 签名已通过 `TAURI_SIGNING_*` secrets 生效；操作系统层的代码签名（SmartScreen / 公证）仍按上文各平台自行接入。
