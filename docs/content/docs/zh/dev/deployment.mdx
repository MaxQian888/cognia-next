...
同一浏览器账号可以保存多个加密 Companion 目标，但一次只激活一个。Provider key、device JWT、OIDC token 与 signaling 私钥只进入浏览器 Vault，公开目标簿只保存连接元数据；每个目标还拥有独立 IndexedDB 数据库和 outbound queue 作用域。
