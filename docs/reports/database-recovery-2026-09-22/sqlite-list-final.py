...
current_source=(ROOT/'crates/cognia-agent-state/src/agent_session_store/mod.rs').read_text()
candidate=re.search(r'let heads = query\(\s*"(.*?)"',current_source.split('pub fn list_sessions(',1)[1],re.S).group(1)
(ART/'sqlite-list-baseline.sql').write_text(baseline)
(ART/'sqlite-list-current.sql').write_text(candidate)

def run_candidate(conn,query):
    rows=conn.execute(query,('t','w','p')).fetchall()
    return conn.execute(baseline,('t','w','p')).fetchall() if len(rows)==129 else rows

def measure_pair(conn,baseline,candidate):
    queries={'baseline':baseline,'candidate':candidate};samples={k:[] for k in queries}
    for name,query in queries.items():
        if name=='candidate':run_candidate(conn,query)
        else:conn.execute(query,('t','w','p')).fetchall()
    for i in range(10):
        for name in (['baseline','candidate'] if i%2==0 else ['candidate','baseline']):
            start=time.perf_counter_ns()
            if name=='candidate':run_candidate(conn,queries[name])
            else:conn.execute(queries[name],('t','w','p')).fetchall()
...
for n,sessions in [(100000,100),(100000,1000),(10000,10000),(100000,100000)]:
    ...
    conn.commit()
    expected=conn.execute(baseline,('t','w','p')).fetchall(); actual=run_candidate(conn,candidate);assert actual==expected
    ...
    conn.close()
(ART/'sqlite-list-final.json').write_text(json.dumps(report,indent=2)+'\n')
