 ...
 * `scripts/gates/gen-settings-sync.mjs`; CI runs it with `--check`.
 */
...
export const SETTINGS_SYNC = {
  ...
  evalSettings: { category: "desktop-only" },
  ...
  turnProvider: {
    category: "server-authoritative",
    rationale:
      "Dynamic TURN credential provider (Cloudflare Calls / Twilio) is configured with the host's account. The phone consumes provisioned credentials; it must never define the provider.",
  },
