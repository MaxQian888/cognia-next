...
describe("evaluation generation seam", () => {
  const docs: RerankResult[] = [{ id: "d1", content: "alpha beta", rerankScore: 0.9 }]
  ...
  it("calls the judge directly when no seam is injected", async () => {
    const result = await evaluateRAG("q", "alpha beta", "alpha", docs, {
      useLLM: true,
      model: seamModel,
    })
    expect(result.faithfulness).toBeCloseTo(0.9)
  ...
  it("runs the judge through an injected seam, which bounds the call", async () => {
    ...
    await evaluateRAG("q", "alpha beta", "alpha", docs, {
      useLLM: true,
      model: seamModel,
      generate: seam,
    })
    expect(requests[0]).toMatchObject({ stage: "rag.evaluate", modelId: "m-1" })
