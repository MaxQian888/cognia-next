...
describe("generateDefaultMappings", () => {
  it("returns no mappings when no providers are enabled", () => {
    expect(generateDefaultMappings([])).toEqual([])
    expect(generateDefaultMappings(new Set<string>())).toEqual([])
  })

  it("returns one mapping per tier that has at least one enabled provider", () => {
    const result = generateDefaultMappings(["groq"])
    // groq lives only under the 'fast' tier in the catalog
    expect(result.map((m) => m.alias)).toEqual(["fast"])
    const fast = result[0]
  ...
  it("filters entries within each tier to only enabled providers", () => {
    const result = generateDefaultMappings(["openai", "anthropic"])
  ...
  it("seeds well-formed ModelMapping rows", () => {
    const result = generateDefaultMappings(["openai"])
  ...
  it("accepts both array and Set inputs", () => {
    const arr = generateDefaultMappings(["openai", "groq"])
    const set = generateDefaultMappings(new Set(["openai", "groq"]))
  ...
  it("omits tiers where no enabled provider matches", () => {
    ...
    expect(generateDefaultMappings(["cohere"])).toEqual([])
  })

  it("issues unique ids across mappings", () => {
    const result = generateDefaultMappings(["openai", "anthropic", "google", "deepseek"])
...
describe("getDefaultTierAliases", () => {
  it("returns the list of tier aliases supported by the seed catalog", () => {
    expect(getDefaultTierAliases()).toEqual(["fast", "balanced", "powerful", "reasoning"])
  })
})
