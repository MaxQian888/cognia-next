...
function DropdownMenuItem({
  ...
  return (
    <DropdownMenuPrimitive.Item
      ...
      // `inset` / `variant` land as data attributes rather than classes so the
      // host stylesheet can restyle a plugin's menu wholesale from outside the
      // plugin's `@scope` block — a plugin cannot ship its own `:root` tokens.
      data-inset={inset}
...
function DropdownMenuCheckboxItem({
  ...
  return (
    <DropdownMenuPrimitive.CheckboxItem
      ...
      {...props}
    >
      {/* The indicator is absolutely positioned into the `pl-8` gutter so a
          checked and an unchecked row keep identical text metrics. */}
...
function DropdownMenuSubTrigger({
  ...
  return (
    ...
    >
      {children}
      {/* Affordance only — the chevron is decorative and must not join the
          trigger's accessible name, hence no title/aria-label. */}
