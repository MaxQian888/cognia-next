...
## Running a slideshow standalone

Prefer `hyperframes present`, which serves the composition through the shared player with presenter/audience synchronization. Use the standalone harness only when the user explicitly needs a self-contained wrapper outside that player.

Standalone demos (a composition opened via the bare player bundle in a browser, without the engine) require workarounds for three gaps: the composition must expose a seekable root timeline, the island must be duplicated into the wrapper, and wrapper-owned SFX/global audio should live in the parent frame. These patterns are documented in:

```
skills/slideshow/references/standalone-harness.md
```

Do not treat those patterns as the default architecture; they are compatibility guidance for standalone delivery.
