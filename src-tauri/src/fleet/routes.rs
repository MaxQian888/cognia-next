...
async fn opencode_command_ack_handler(
    ...
    match super::runtime().ack_opencode_command(&body.id, body.result) {
...
async fn opencode_command_nack_handler(
    ...
    match super::runtime().nack_opencode_command(&body.id, body.error) {
        Ok(true) => StatusCode::NO_CONTENT.into_response(),
        Ok(false) => StatusCode::NOT_FOUND.into_response(),
        Err(error) => (StatusCode::INTERNAL_SERVER_ERROR, error).into_response(),
    }
...
mod tests {
    ...
    async fn opencode_commands_poll_returns_queued_command() {
        ...
        assert!(rt
            .take_opencode_commands(&["route-cmd-sess".into()])
            .unwrap()
            .is_empty());
    }
    ...
    async fn opencode_command_settlement_requires_auth_and_nack_redelivers() {
        ...
        let id = rt
            ...
            .unwrap();
        assert_eq!(
            rt.take_opencode_commands(&["route-nack-sess".into()])
                .unwrap()
                .len(),
            1
        );
        ...
        let redelivered = rt
            .take_opencode_commands(&["route-nack-sess".into()])
            .unwrap();
