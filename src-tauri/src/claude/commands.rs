...
pub async fn claude_send_with_host(
    ...
    // Resolve trusted settings once and hand the complete hook map to the
    // sidecar. The SDK callback is the sole owner for built-in Claude hooks,
    // including UserPromptSubmit; running it here too caused double execution.
    let cwd = opts_value
        .get("cwd")
        .and_then(|v| v.as_str())
        .map(String::from);
    ...
    // Injected here, after the Rust trust gate, so a compromised renderer
    // cannot smuggle project/local hooks from an untrusted workspace.
