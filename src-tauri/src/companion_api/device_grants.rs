...
pub fn seed_allow_lists(store: &dyn DeviceGrantStore) -> Result<(usize, usize), String> {
    ...
    super::control_allow_list::global().reseed(control);
    super::control_allow_list::agent_control_global().reseed(agent);
    Ok(counts)
}
...
mod tests {
    ...
    fn seeding_replaces_rather_than_unions_so_revocations_survive_a_restart() {
        ...
        assert!(super::super::control_allow_list::global().is_allowed("dev-1"));
        assert!(super::super::control_allow_list::agent_control_global().is_allowed("dev-2"));
        // A device granted control is NOT thereby allowed to run agents.
        assert!(!super::super::control_allow_list::agent_control_global().is_allowed("dev-1"));

        revoke(&store, "dev-1", GrantKind::Control).unwrap();
        assert_eq!(seed_allow_lists(&store).unwrap(), (0, 1));
        assert!(!super::super::control_allow_list::global().is_allowed("dev-1"));
