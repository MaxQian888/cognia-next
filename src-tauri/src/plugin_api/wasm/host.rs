...
/// The cognia API surface for WASM plugins. Owns the compiled `Component`s
/// (via [`WasmPluginState::loaded`]); `activate` stashes the live post-`init`
/// [`ActivatedPlugin`] (Store + Instance) in [`WasmPluginState::activated`] so
/// re-entry from TS reuses the initialised instance instead of rebuilding it.
