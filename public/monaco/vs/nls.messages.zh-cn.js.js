define("vs/nls.messages.zh-cn.js", function () {
  "use strict"
  /*!-----------------------------------------------------------
   ...
   *-----------------------------------------------------------*/ ;((globalThis._VSCODE_NLS_MESSAGES =
    [
      ...
      "控制是否应启用树 sitter 分析和收集遥测数据。设置 `#editor.experimental.preferTreeSitter#` 以优先使用特定语言。",
      "控制是否应为 css 启用 Tree Sitter 分析。对于 css，此操作将优先于 `#editor.experimental.treeSitterTelemetry#`。",
      "控制是否应为 Typescript 启用 tree sitter 分析。对于 Typescript，此操作将优先于 `#editor.experimental.treeSitterTelemetry#`。",
      "控制是否应为 ini 启用 tree sitter 分析。对于 ini，此操作将优先于 `#editor.experimental.treeSitterTelemetry#`。",
      "控制是否应为正则表达式启用 tree sitter 分析。对于正则表达式，此操作将优先于 `#editor.experimental.treeSitterTelemetry#`。",
      "定义增加和减少缩进的括号。",
      "左方括号字符或字符串序列。",
      "右方括号字符或字符串序列。",
      ...
      "控制隐藏悬停提示前的延迟时间(毫秒)。需要启用 `#editor.hover.sticky#`。",
      "如果有空间，首选在线条上方显示悬停。",
 ...
 - 大于或等于 8 的值将用作有效值。`,
      ...
      "定义用于在注释中查找节标头的正则表达式。正则表达式必须包含命名匹配组“label”，(写为 “(？<label>.+)”) 封装节标头，否则它将不起作用。可以选择包括另一个名为“separator”的匹配组。使用模式中的 \\n 匹配多行标头。",
      "控制缩略图中节标题的字号。",
      ...
      "控制是否可以并排显示较大的建议。",
      "如果有足够的空间，较大的建议将并排显示，否则将显示在下面。",
      "较大的建议从不并排显示，而是始终显示在下面。",
      "控制在跳到建议之前，建议是否显示为折叠。",
      ...
      "控制编辑器是否应自动缩进粘贴的内容。",
      "控制编辑器是否应在粘贴到字符串中时自动缩进粘贴的内容。当 autoIndentOnPaste 为 true 时，此操作将会生效。",
      "使用语言配置确定何时自动包住所选内容。",
      "使用引号而非括号来包住所选内容。",
      "使用括号而非引号来包住所选内容。",
      ...
      "当 `#editor.cursorStyle#` 设置为 `line` 时，控制光标的高度。光标的最大高度取决于行高。",
      "控制在编辑器中是否允许通过拖放来移动选中内容。",
      "设置是否应使用 EditContext API 而不是文本区域来支持在编辑器中输入。",
      "启用 `#editor.editContext#` 设置时是否呈现丰富的屏幕阅读器内容。",
      "使用基于 DOM 的常规呈现。",
      "使用 GPU 加速。",
      "控制是否使用实验性 GPU 加速来呈现编辑器。",
      "将新的呈现方法与 svg 配合使用。",
      "使用包含字体字符的新呈现方法。",
      "使用稳定呈现方法。",
      "控制是否使用新的实验性方法呈现空格。",
      ...
      "控制在不突出显示类似匹配项之前，可以选择的字符数。设置为零表示无限制。",
      "控制编辑器是否应突出显示跨多行的选择匹配项。",
      "始终显示折叠控件。",
      "切勿显示折叠控件并减小装订线大小。",
      "仅在鼠标位于装订线上方时显示折叠控件。",
      "控制何时显示行号槽上的折叠控件。",
      ...
      '控制在启用 `#editor.wordWrap#` 时文本 `\\n` 是否应触发 wordWrap。\r\n\r\n例如:\r\n```c\r\nchar* str="hello\\nworld"\r\n```\r\n将显示为\r\n```c\r\nchar* str="hello\\n\r\n           world"\r\n```',
      "仅当没有扩展提供颜色修饰器时才显示默认颜色修饰器。",
      "始终显示默认颜色修饰器。",
      "从不显示默认颜色修饰器。",
      ...
      "显示或聚焦独立颜色选取器(&&S)",
      "显示或聚焦独立颜色选取器",
      "显示或聚焦使用默认颜色提供程序的独立颜色选取器。它会显示 rgb/hsl/hex 颜色。",
      "隐藏颜色选取器",
      "隐藏独立颜色选取器。",
      "使用独立颜色选取器插入颜色",
      "使用聚焦的独立颜色选取器插入 hex/rgb/hsl 颜色。",
      "切换行注释(&&T)",
      "切换块注释(&&B)",
      "切换行注释",
      "添加行注释",
...
{1}`,
      ...
      "键入要转到的字符位置(从 0 到 {0})。",
      "键入要转到的字符位置(从 1 到 {0})。",
      '按 "Enter" 键转到第 {0} 行，第 {1} 列。',
      "键入要转到的行号(从 1 到 {0})。",
      '按 "Enter" 键转到第 {0} 行，或输入冒号 : 以添加列号。',
      '按 "Enter" 键跳转到第 {0} 行，或输入列号(从 1 到 {1})。',
      '按 "Enter" 键转到第 {0} 行，第 {1} 列。',
      ...
      "建议小组件中的详细信息的图标。",
      "了解详细信息",
      "在单词结尾时为 true 的上下文键。请注意，仅在启用制表符完成时才定义此项",
...
收到的: "{1}"。`,
      ...
      "在任何快速输入的上下文中使用。如果为此命令更改一个键绑定，则还应更改此命令的所有其他键绑定(修饰符变体)。",
      ...
      "在某些快速输入的上下文中使用。如果为此命令更改一个键绑定，则还应更改此命令的所有其他键绑定(修饰符变体)。",
      "切换所有复选框",
      "{0} 个结果",
      "已选 {0} 项",
      "确定",
