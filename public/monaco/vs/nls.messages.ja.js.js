define("vs/nls.messages.ja.js", function () {
   ...
   *-----------------------------------------------------------*/ ;((globalThis._VSCODE_NLS_MESSAGES =
    [
      ...
      "css に対してツリー シッター解析を有効にするかどうかを制御します。これは、css に対する `#editor.experimental.treeSitterTelemetry#` よりも優先されます。",
      "TypeScript に対してツリー シッター解析を有効にするかどうかを制御します。これは、TypeScript に対する `#editor.experimental.treeSitterTelemetry#` よりも優先されます。",
      "ini に対してツリー シッター解析を有効にするかどうかを制御します。これは、ini に対する `#editor.experimental.treeSitterTelemetry#` よりも優先されます。",
      "RegEx に対してツリー シッター解析を有効にするかどうかを制御します。これは、RegEx に対する `#editor.experimental.treeSitterTelemetry#` よりも優先されます。",
      ...
      "検索ウィジェットからの検索履歴を保存しません。",
      "アクティブなワークスペース全体に検索履歴を保存する",
      "検索ウィジェットの履歴を保存する方法を制御する",
      "置換ウィジェットからの履歴を保存しません。",
      "アクティブなワークスペース全体に置換履歴を保存する",
      "置換ウィジェットの履歴を保存する方法を制御する",
      "入力中に検索ウィジェットが検索を行うかどうかを制御します。",
      ...
      "ホバーが非表示になるまでの延期期間をミリ秒単位で制御します。`#editor.hover.sticky#` を有効にする必要があります。",
 ...
 - 8 以上の値は有効値として使用されます。`,
      ...
      "ミニマップは常に表示されます。",
      "ミニマップは、マウスがミニマップ上にない場合に非表示になり、マウスがミニマップの上にあるときに表示されます。",
      "ミニマップは、エディターがスクロールされているときにのみ表示されます",
      ...
      "コメント内のセクション ヘッダーを検索するために使用する正規表現を定義します。正規表現には、セクション ヘッダーをカプセル化する名前付き一致グループ 'label' ('(?<label>.+)' と記述) が含まれている必要があります。それ以外の場合は機能しません。必要に応じて、'separator' という名前の別の一致グループを含めることができます。パターン内の \\n を使用して、複数行のヘッダーを照合します。",
      ...
      "入力中に候補を自動的に表示するかどうかを制御します。これは、コメント、文字列、およびその他のコードを入力するために制御できます。クイック提案は、ゴースト テキストとして表示するか、提案ウィジェットで表示するように構成できます。また、提案が特殊文字によってトリガーされるかどうかを制御する {0}-設定にも注意してください。",
      "行番号は表示されません。",
      "行番号は、絶対値として表示されます。",
      ...
      "十分なスペースがある場合に、大きな候補は横並びに表示されますが、そうでない場合は下側に表示されます。",
      "大きな候補は横並びに表示されることはなく、常に下側に表示されます。",
      "候補が折りたたまれた状態で表示されるかどうかを、その候補にジャンプするまで制御します。",
      ...
      "ウィジェットを表示する際に候補を選択するかどうかを制御します。これは、自動的にトリガーされる提案 ({0} および {1}) にのみ適用され、明示的に呼び出された場合 (例: `Ctrl + Space`) で提案が常に選択されることに注意してください。",
      ...
      "単語に関連するナビゲーションまたは操作を行うときに単語のセグメント化に使用されるロケール。認識する単語の BCP 47 言語タグを指定します (例: ja、zh-CN、zh-Hant-TW など)。",
      ...
      "エディターでの可変フォントの使用を許可するかどうかを制御します。",
      "アクセシビリティ モードのエディターで可変フォントの使用を許可するかどうかを制御します。",
      "エディターのコンテンツ",
      ...
      "エディターが貼り付けたコンテンツを自動的にインデントするかどうかを制御します。",
      "文字列内に貼り付けたときに、エディターが貼り付けたコンテンツを自動的にインデントするかどうかを制御します。これは autoIndentOnPaste が true の場合に有効になります。",
      ...
      "挿入入力モードのカーソル スタイルを制御します。",
      "上書き入力モードでのカーソル スタイルを制御します。",
      ...
      "`#editor.cursorStyle#` が `line` に設定されている場合、カーソルの高さを制御します。カーソルの最大高さは、行の高さによって異なります。",
      "ドラッグ アンド ドロップによる選択範囲の移動をエディターが許可するかどうかを制御します。",
      "エディターでの入力を強化するために、テキスト ボックスの代わりに EditContext API を使用するかどうかを設定します。",
      "`#editor.editContext#` 設定が有効な場合にリッチ スクリーン リーダー コンテンツをレンダリングするかどうか。",
      "通常の DOM ベースのレンダリングを使用します。",
      "GPU アクセラレーションを使用します。",
      "試験的な GPU アクセラレータを使用してエディターをレンダリングするかどうかを制御します。",
      ...
      "スクロールを慣性にする - Linux 上のタッチパッドでほとんど役に立ちます。",
      "文字間隔 (ピクセル単位) を制御します。",
      ...
      "エディターでマウスの中央ボタンをクリックしたときの動作を制御します。",
      ...
      "発生回数が強調表示されるまでの遅延をミリ秒単位で制御します。",
      "貼り付けで上書きするかどうかを制御します。",
      ...
      "中央ボタンが押されたときにエディターがスクロールするかどうかを制御します。",
      ...
      "同様の一致が強調表示されない前に、選択範囲に含めることができる文字の数を制御します。無制限の場合は 0 に設定します。",
      "エディターで複数の行にまたがる選択範囲と一致するものを強調表示するかどうかを制御します。",
      "常に折りたたみコントロールを表示します。",
      ...
      "タブ補完を有効にします。",
      "改行を削除するときに、エディターで次の行のインデントの空白も削除するかどうかを制御します。",
      ...
      '`#editor.wordWrap#` が有効になっている場合に、リテラルの `\\n` によって行の折り返しをトリガーするかどうかを制御します。\r\n\r\n例:\r\n```c\r\nchar* str="hello\\nworld"\r\n```\r\nは次のように表示されます\r\n```c\r\nchar* str="hello\\n\r\n           world"\r\n```',
      "拡張機能が色デコレーターを提供しない場合にのみ、既定のカラー デコレーターを表示します。",
      "常に既定のカラー デコレーターを表示します。",
      "既定のカラー デコレーターを表示しません。",
      ...
      "複数のカーソルが存在する場合のプライマリ エディター カーソルの色。",
      "複数のカーソルが存在する場合のプライマリ エディター カーソルの背景色。ブロック カーソルにより重なった文字の色をカスタマイズできます。",
      "複数のカーソルが存在する場合のセカンダリ エディター カーソルの色。",
      "複数のカーソルが存在する場合のセカンダリ エディター カーソルの背景色。ブロック カーソルにより重なった文字の色をカスタマイズできます。",
      "エディターのスペース文字の色。",
      "エディターの行番号の色。",
      "エディター インデント ガイドの色。",
      ...
      "ソース アクション...",
      "インポートを整理",
      '現在のファイル内のインポートを整理します。一部のツールでは "インポートの最適化" とも呼ばれます',
      "すべて修正",
      "自動修正...",
      "コード アクション メニューでのグループ ヘッダーの表示の有効/無効を切り替えます。",
      "現在診断を行っていないときに、行内の最も近い クイック修正 を表示する機能を有効または無効にします。",
      "{1} が {2} に設定されている場合に、{0} のトリガーを有効にします。ウィンドウとフォーカスの変更に対してトリガーされるコード アクションを {3} に設定する必要があります。",
      "コンテキスト: {1} 行 {2} 列 の {0}。",
      "無効なものを非表示",
      "無効を表示",
      "その他の操作...",
      ...
      "ブロックの挿入",
      "ソース アクション...",
      "エディターにスペースがない場合にとじしろからコード アクション メニューを生成するアイコン。",
      "エディターにスペースがなく、クイック修正が利用可能な場合にとじしろからコード アクション メニューを生成するアイコン。",
      "エディターにスペースがなく、AI 修正が利用可能な場合にとじしろからコード アクション メニューを生成するアイコン。",
      "エディターにスペースがなく、AI 修正とクイック修正が利用可能な場合にとじしろからコード アクション メニューを生成するアイコン。",
      "エディターにスペースがなく、AI 修正とクイック修正が利用可能な場合にとじしろからコード アクション メニューを生成するアイコン。",
      "実行: {0}",
      ...
      "コード アクションの表示",
      "コマンドの選択",
      "現在の行の CodeLens コマンドを表示する",
      "カラー ピッカーを閉じるアイコン",
      "クリックして色オプションを切り替えます (rgb/hsl/hex)",
      "ここにカラー ピッカーがあります。",
      "スタンドアロン カラー ピッカーの表示またはフォーカス(&&S)",
      "スタンドアロン カラー ピッカーの表示またはフォーカス",
      "既定のカラー プロバイダーを使用するスタンドアロン カラー ピッカーを表示またはフォーカスします。16 進/RGB/HSL の色が表示されます。",
      "カラー ピッカーを非表示にする",
      "スタンドアロン カラー ピッカーを非表示にします。",
      "スタンドアロン カラー ピッカーで色を挿入",
      "フォーカスされたスタンドアロン カラー ピッカーを使用して、16 進/RGB/HSL 色を挿入します。",
      "行コメントの切り替え(&&T)",
      "ブロック コメントの切り替え(&&B)",
      "行コメントの切り替え",
      "行コメントの追加",
...
ユーザー設定に一致する最初の編集が適用されます。`,
      ...
      "'{0}' の貼り付けの編集を解決しています。クリックしてキャンセル",
      "貼り付けハンドラーを実行しています。クリックしてキャンセルし、基本的な貼り付けを行います",
      "既定の貼り付けアクションを構成する",
      "貼り付け操作の選択",
      "貼り付けハンドラーを実行しています...",
      "プレーンテキストの挿入",
      ...
      `編集 '{0}' の解決中にエラーが発生しました:\r
{1}`,
      `編集 '{0}' の適用中にエラーが発生しました:\r
{1}`,
      ...
      "折りたたまれた範囲の最初の行の後で折りたたまれたテキストの色。",
      ...
      "[下にスクロール] ホバー",
      "エディターのホバーを下にスクロールします。",
      "[左にスクロール] ホバー",
      "エディターのホバーを左にスクロールします。",
      "[右にスクロール] ホバー",
      "エディターのホバーを右にスクロールします。",
      "[ページを上に] ホバー",
      "エディターのホバーをページの上に移動します。",
      "[ページを下に] ホバー",
      "エディターのホバーをページの下に移動します。",
      "[上に移動] ホバー",
      "エディターのホバーを上部に移動します。",
      "[下に移動] ホバー",
      "エディターのホバーを下部に移動にします。",
      ...
      "スペースを含むインデントを使用します。",
      "タブの表示サイズの変更",
      "タブと同等のスペース サイズを変更します。",
      "内容からインデントを検出",
      "コンテンツからのインデントを検出します。",
      "行の再インデント",
      "エディターの行のインデントを元に戻します。",
      "選択行を再インデント",
      "エディターの選択した行のインデントを元に戻します。",
      "ダブルクリックして挿入する",
      "cmd キーを押しながらクリック",
      "ctrl キーを押しながら クリック",
      "option キーを押しながらクリック",
      ...
      "次のインライン編集にジャンプ",
      "インライン候補を非表示にする",
      "インライン修正候補の折りたたみ表示の切り替え",
      "インライン候補を表示するかどうか",
      "インライン候補がスペースで始まるかどうか",
      "インライン候補が、タブで挿入されるものよりも小さいスペースで始まるかどうか",
      "現在の候補について候補表示を止めるかどうか",
      "カーソルがゴースト テキストにあるかどうか",
      "カーソルがインデント内にあるかどうか",
      "エディターに選択範囲があるかどうか",
      "カーソルがインライン編集中かどうか",
      "インライン編集が表示されるかどうか",
      "タブをインライン編集にジャンプするかどうか。",
      "タブでインライン編集を受け入れるかどうか。",
      "現在のコード エディターがインライン編集プレビューを表示しているかどうか",
      ...
      "インライン編集の元のテキストで変更されたテキストのオーバーレイの色。",
      "インライン編集の変更されたテキスト内の変更された行の背景色。",
      "インライン編集の変更されたテキスト内の変更されたテキストのオーバーレイの色。",
      "プライマリ インライン編集余白インジケーターの前景色。",
      "プライマリ インライン編集とじしろインジケーターの境界線の色。",
      "プライマリ インライン編集余白インジケーターの背景色。",
      "セカンダリ インライン編集余白インジケーターの前景色。",
      "セカンダリ インライン編集とじしろインジケーターの境界線の色。",
      "セカンダリ インライン編集余白インジケーターの背景色。",
      "成功したインライン編集余白インジケーターの前景色。",
      "インライン編集とじしろインジケーターの境界線の色。",
      "成功したインライン編集余白インジケーターの背景色。",
      "インライン編集余白インジケーターの背景色。",
      "インライン編集での元のテキストの境界線の色。",
      "インライン編集で変更されたテキストの境界線の色。",
      "タブで受け入れられるときのインライン編集ウィジェットの変更された境界線の色。",
      "タブで受け入れられるときの元のテキストに対するインライン編集ウィジェットの元の境界線の色。",
      "前の値に置換",
      "次の値に置換",
      "新しい行を末尾に挿入",
      "行全体を選択する",
      ...
      "提案ウィジェットの詳細情報のアイコン。",
      "詳細を参照",
      "単語の末尾にある場合に true であるコンテキスト キー。これはタブ入力候補が有効な場合にのみ定義されることに注意してください",
      ...
      "タブ キーがワークベンチの周囲にフォーカスを移動するか、現在のエディターにタブ文字を挿入するかを決定します。これは、タブ トラップ、タブ ナビゲーション、またはタブ フォーカス モードとも呼ばれます。",
      "開発者: トークン再作成の強制",
      "バナーを閉じる",
      ...
      "普通ではない行終端記号",
      "普通ではない行終端記号が検出されました",
      `このファイル '{0}' には、行区切り文字 (LS) や段落区切り記号 (PS) などの特殊な行の終端文字が 1 つ以上含まれています。\r
\r
...
受取済み: '{1}'。`,
      ...
      "警告",
      "情報",
      '次の理由により、問題は一時停止しています: "{0}"',
      '次の理由により、問題は一時停止しています: "{0}" および {1} その他',
      "最近使用したもの",
      "同様のコマンド",
      "よく使用するもの",
      "その他のコマンド",
      ...
      "入力すると結果が絞り込まれます。",
      "OK",
      "{0} ('Enter' を押して確認するか 'Escape' を押して取り消します)",
      "あらゆる種類のクイック入力のコンテキスト内で使用されます。このコマンドのキー バインドを 1 つ変更する場合は、このコマンドの他のすべてのキー バインド (修飾子のバリアント) も変更する必要があります。",
      ...
      "クイック入力のコンテキストで使用されます。このコマンドのキー バインドを 1 つ変更する場合は、このコマンドの他のすべてのキー バインド (修飾子のバリアント) も変更する必要があります。",
      "すべてのチェック ボックスを切り替える",
      "{0} 件の結果",
      "{0} 個選択済み",
      "OK",
      ...
      "アクティブなラジオ オプションの前景色。",
      "アクティブなラジオ オプションの背景色。",
      "アクティブなラジオ オプションの境界線の色。",
      "非アクティブなラジオ オプションの前景色。",
      "非アクティブなラジオ オプションの背景色。",
      "非アクティブなラジオ オプションの境界線の色。",
      "非アクティブなラジオ オプションのホバー時の背景色。",
      "チェックボックス ウィジェットの背景色。",
      ...
      "操作の一覧の背景色。",
      "操作の一覧の前景色。",
      "フォーカスされた項目の操作の一覧の前景色。",
      "フォーカスされた項目の操作の一覧の背景色。",
      "メニューの境界線色。",
      "メニュー項目の前景色。",
      "メニュー項目の背景色。",
      ...
      "この色は透明である必要があります。そうでないと内容が見えにくくなります",
      "既定の色を使用します。",
      "フォント ID に使用できるのは、文字、数字、アンダースコア、ダッシュのみです。",
