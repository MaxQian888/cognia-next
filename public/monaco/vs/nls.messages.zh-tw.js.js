define("vs/nls.messages.zh-tw.js", function () {
   ...
   *-----------------------------------------------------------*/ ;((globalThis._VSCODE_NLS_MESSAGES =
    [
      ...
      "控制是否應該開啟 css 的樹狀座標 (tree sitter) 剖析。針對 css，這將優先於 `#editor.experimental.treeSitterTelemetry#`。",
      "控制是否應該開啟 TypeScript 的樹狀座標 (tree sitter) 剖析。針對 TypeScript，這將優先於 `#editor.experimental.treeSitterTelemetry#`。",
      "控制是否應該開啟 ini 的樹狀座標 (tree sitter) 剖析。針對 ini，這將優先於 `#editor.experimental.treeSitterTelemetry#`。",
      "控制是否應該開啟 regex 的樹狀座標 (tree sitter) 剖析。針對 Regex，這將優先於 `#editor.experimental.treeSitterTelemetry#`。",
      "定義增加或減少縮排的括弧符號。",
      "左括弧字元或字串順序。",
      "右括弧字元或字串順序。",
      ...
      "啟用編輯器中的嵌入式提示。",
      "已啟用嵌入式提示",
      "預設會顯示嵌入式提示，並在按住 {0} 時隱藏",
      "預設會隱藏嵌入式提示，並在按住 {0} 時顯示",
      "已停用嵌入式提示",
      "控制編輯器中嵌入式提示的字型大小。當設定的值小於 {1} 或大於編輯器字型大小時，則會使用{0} 預設值。",
      "控制編輯器中，嵌入式提示的字型家族。設定為空白時，則會使用 {0}。",
      "在編輯器中啟用的嵌入式提示周圍的填補。",
      "在編輯器截斷之前，單行嵌入式提示的整體長度上限。設定為 `0` 以永不截斷",
      `控制行高。\r
 - 使用 0 從字型大小自動計算行高。\r
 ...
 - 大於或等於 8 的值將用來作為有效值。`,
      "控制是否會顯示縮圖",
      "一律顯示檔案縮圖。",
      "當滑鼠不在檔案縮圖上時，檔案縮圖會隱藏；當滑鼠在檔案縮圖上時，檔案縮圖會顯示。",
      "只有在捲動編輯器時，才會顯示檔案縮圖",
      "控制是否會自動隱藏縮圖。",
      "縮圖大小與編輯器內容相同 (且可能會捲動)。",
      ...
      "定義用來在批註中尋找區段標頭的正則表達式。regex 必須包含具名的相符群組 'label' (寫成 '(？<label>.+)') 封裝區段標頭，否則將無法運作。您可以選擇性地加入另一個名為 「分隔符」的相符群組。使用模式中的 \\n 來比對多行標頭。",
      "控制縮圖中區段標頭的字型大小。",
      ...
      "永不顯示內嵌建議工具列。",
      "控制何時顯示內嵌建議工具列。",
      "控制是否要在編輯器中顯示內嵌建議的語法強調顯示。",
      "控制內嵌建議如何與建議小工具互動。如果啟用，有可用的內嵌建議時，不會自動顯示建議小工具。",
      "控制使用程式碼片段模式時，是否抑制內嵌建議。",
      "控制輸入後顯示內嵌建議的最小延遲時間 (以毫秒為單位)。",
      "抑制指定擴充功能的內嵌完成 -- 以逗號分隔。",
      "控制是否從內嵌建議提供者傳送要求資訊。",
      "控制是否在內嵌建議提供者變更時觸發命令。",
      "控制在建議衝突時是否顯示內嵌建議。",
      "控制內嵌建議的字體系列。",
      "控制顯示建議是否會移動程式碼，以便騰出空間讓建議內嵌。",
      "控制是否可以並排顯示較大的建議。",
      "如果空間足夠，較大的建議將會並排顯示，否則會顯示在下方。",
      "較大的建議永遠不會並排顯示，且一定會顯示在下方。",
      "控制在跳到建議之前，是否將建議顯示為折疊。",
      ...
      "控制編輯器是否應自動將貼上的內容自動縮排。",
      "控制在字串中貼上內容時，編輯器是否應自動將貼上的內容自動縮排。當 autoIndentOnPaste 為 True 時，此功能會生效。",
      "使用語言組態來決定何時自動環繞選取項目。",
      "用引號括住，而非使用括弧。",
      "用括弧括住，而非使用引號。 ",
      ...
      "控制游標高度，當`#editor.cursorStyle#` 設定為 `line` 時。游標的最大高度取決於行高。",
      "控制編輯器是否允許透過拖放來移動選取項目。",
      "設定是否應使用 EditContext API，而不是文字區域來支援編輯器中的輸入。",
      "啟用 `#editor.editContext#` 設定時，是否要轉譯豐富的螢幕閱讀程式內容。",
      "使用一般 DOM 型轉譯。",
      "使用 GPU 加速。",
      "控制是否使用實驗性 GPU 加速來轉譯編輯器。",
      "使用新的 svg 轉譯方法。",
      "使用具有字型字元的新轉譯方法。",
      "使用穩定轉譯方法。",
      ...
      "控制在未醒目提示類似符合項目之前，選取範圍中可包含的字元數。設定為零以表示無限制。",
      "控制編輯器是否應醒目提示跨越多行的選取範圍符合項目。",
      "一律顯示摺疊控制項。",
      "永不顯示摺疊控制項與減少裝訂邊大小。",
      ...
      '控制啟用 `#editor.wordWrap#` 時，常值 `\\n` 是否應該觸發 wordWrap。\r\n\r\n例如:\r\n```c\r\nchar* str="hello\\nworld"\r\n```\r\n會顯示為\r\n```c\r\nchar* str="hello\\n\r\n           world"\r\n```',
      "只有當沒有延伸模組提供色彩裝飾時，才會顯示預設色彩裝飾。",
      "永遠顯示預設色彩裝飾。",
      "永不顯示預設色彩裝飾。",
      ...
      "&&顯示或聚焦獨立的顏色選擇器",
      "顯示或聚焦獨立的顏色選擇器",
      "顯示或聚焦使用預設色彩提供者的獨立色彩選擇器。它會顯示十六進位/RGB/HSL 色彩。",
      "隱藏顏色選擇器",
      "隱藏獨立色彩選擇器。",
      "使用獨立的顏色選擇器插入顏色",
      "使用焦點獨立色彩選擇器插入十六進位/RGB/HSL 色彩。",
      "切換行註解(&&T)",
      "切換區塊註解(&&B)",
      "切換行註解",
      "加入行註解",
...
{1}`,
      ...
      "原始文字的框線色彩，內嵌編輯。",
      "內嵌編輯中修改文字的框線色彩。",
      "當 Tab 要接受時，內嵌編輯 Widget 的修改後框線色彩。",
      "當 Tab 要接受時，原始文字上內嵌編輯 Widget 的原始框線色彩。",
      "以上一個值取代",
      "以下一個值取代",
      "插入最後一個新行",
      "展開線條選取範圍",
      ...
      "輸入要前往的字元位置 (從 0 到 {0})。",
      "輸入要前往的字元位置 (從 1 到 {0})。",
      "按 'Enter' 以前往資料行 {1} 的第 {0} 行。",
      "輸入要前往的行號 (從 1 到 {0})。",
      "按 'Enter' 以前往行 {0} 或輸入冒號 : 以新增資料行號碼。",
      "按 'Enter' 以前往第 {0} 行或輸入資料行號碼 (從 1 到 {1})。",
      "按 'Enter' 以前往資料行 {1} 的第 {0} 行。",
      ...
      "建議小工具中 [更多詳細資訊] 的圖示。",
      "閱讀更多",
      "文字結尾時為 true 的內容索引鍵。請注意，只有在啟用索引標籤完成時，才會定義",
...
收到: '{1}'。`,
      ...
      "用於任何類型的快速輸入內容。如果您變更此命令的一個按鍵繫結關係，則也應該變更此命令的所有其他按鍵繫結關係 (修飾元變體)。",
      ...
      "用於一些快速輸入內容。如果您變更此命令的一個按鍵繫結關係，則也應該變更此命令的所有其他按鍵繫結關係 (修飾元變體)。",
      "切換所有核取方塊",
      "{0} 個結果",
      "已選擇 {0}",
      "確定",
