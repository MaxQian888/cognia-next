...
interface AnimatedThemeTogglerProps extends React.ComponentPropsWithoutRef<"button"> {
   ...
   */
  theme?: "light" | "dark"
  /** Explicit visual theme to apply. Defaults to toggling the current theme. */
  targetTheme?: "light" | "dark"
  /** Applies the theme immediately while preserving the same controlled callback. */
...
export const AnimatedThemeToggler = ({
  ...
  const toggleTheme = useCallback(() => {
    ...
    const applyTheme = () => {
      const newTheme = targetTheme ?? (isDark ? "light" : "dark")
    ...
    if (disableTransition || typeof document.startViewTransition !== "function") {
      applyTheme()
      return
    }
  ...
  return (
    <button
      ...
      {...props}
    >
      {children ?? <>{isDark ? <Sun aria-hidden /> : <Moon aria-hidden />}</>}
    </button>
  )
}
