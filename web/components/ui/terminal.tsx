...
export const Terminal = ({
  ...
  const content = (
    <div
      ref={containerRef}
      className={cn(
        "border-hairline bg-graphite text-on-stage h-full max-h-100 w-full max-w-lg border-y md:border-x",
        className
      )}
    >
      <div className="border-hairline flex items-center gap-2 border-b px-4 py-3">
        ...
        {title ? (
          <span className="font-mono text-[10px] uppercase tracking-widest text-on-stage-muted">
            {title}
          </span>
        ) : null}
      </div>
