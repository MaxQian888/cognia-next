...
export function Hero({ locale, copy, releaseState, docsOrigin }: HeroProps) {
  ...
  return (
    <section className="relative border-b border-hairline bg-paper">
      ...
      <div className="relative mx-auto max-w-shell px-5 pb-16 pt-20 md:pt-28 lg:px-8 lg:pb-20 lg:pt-32">
        ...
        <ul className="mt-16 grid grid-cols-2 gap-px border-t border-hairline bg-hairline md:grid-cols-4">
          {hero.trustRail.map((item) => (
            <li key={item.label} className="bg-paper px-1 pt-5 md:px-0 md:pr-6">
              <p className="font-mono text-xs uppercase tracking-widest text-ink">{item.label}</p>
