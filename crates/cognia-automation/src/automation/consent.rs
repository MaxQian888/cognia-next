...
impl ConsentBroker {
    ...
    pub async fn request_with_thumbnail(
        ...
        self.emit_and_wait(app, id, event, rx, timeout_ms).await
    }
        ...
        if allow && persist {
            if let Some(p) = prompt {
                ...
                self.inner
                    .lock()
                    .session_grants
                    .insert(GrantKey::from_prompt(p), expiry);
            }
        }
    }
...
mod tests {
    ...
    fn resolve_persists_a_bounded_grant() {
        let b = ConsentBroker::new();
        let p = prompt("click", Surface::ComputerUse);
        b.resolve("unknown-id", true, true, Some(15 * 60 * 1000), Some(&p));
        assert!(b.has_session_grant(&p));
    }
    ...
    fn resolve_never_persists_a_rejection() {
        let b = ConsentBroker::new();
        let p = prompt("click", Surface::ComputerUse);
        b.resolve("unknown-id", false, true, Some(15 * 60 * 1000), Some(&p));
        assert!(
            !b.has_session_grant(&p),
    ...
    fn resolve_unknown_id_is_silent_noop() {
        let b = ConsentBroker::new();
        b.resolve("nonexistent", true, false, None, None);
        // No panic = pass.
    }
