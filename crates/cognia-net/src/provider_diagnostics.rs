...
async fn execute_template(
    ...
    let secret = cognia_secrets::secret_store::get(SECRET_NAMESPACE, &template.credential_ref)?
...
pub fn migrate_balance_token(
    ...
    cognia_secrets::secret_store::set(SECRET_NAMESPACE, &source_id, &token)?;
    Ok(source_id)
}

pub fn clear_balance_token(source_id: String) -> Result<(), String> {
    validate_source_id(&source_id)?;
    cognia_secrets::secret_store::delete(SECRET_NAMESPACE, &source_id)
}

pub async fn run_balance_script_timed(
