...
| `cognia plugin list [--json] [--query PATH]`                                                                                                                                                | List plugins currently known to the running desktop bridge.                                                                                                   |
...
| `cognia plugin status [--json] [--query PATH]`                                                                                                                                              | Probe the running desktop bridge without exposing the per-launch dev token.                                                                                   |
| `cognia plugin dev [--path .] [--reload-url LOOPBACK-URL] [--once] [--json]`                                                                                                                | Watch the crate, rebuild + hot-reload on changes; `--once` builds/reloads once and exits, with optional JSON for CI/editor smoke checks.                      |
...
| `cognia host categories [--format json\|table] [--query PATH]`                                                                                                                              | Summarize the stable Headless command domains, risk/operation counts, and matching embedded skills.                                                           |
| `cognia host resources [--category CATEGORY] [--format json\|table] [--query PATH]`                                                                                                         | Browse stable resource groups within a domain before selecting an exact RPC.                                                                                  |
| `cognia host commands [--search TEXT] [--target T] [--operation O] [--risk R] [--approval A] [--capability C] [--category C] [--resource R] [--format json\|table] [--query PATH]`          | Browse the embedded loopback Headless RPC catalog without connecting to a server.                                                                             |
| `cognia host schema <command> [--format json\|human] [--query PATH]`                                                                                                                        | Inspect the concrete input schema and opaque-output marker for one Headless RPC.                                                                              |
| `cognia host call <command> [--data JSON\|-\|@FILE] [-f KEY=VALUE] [-F KEY=JSON] [--dry-run] [--paginate] [--query PATH]`                                                                   | Validate and invoke one named Headless RPC with durable idempotency and risk confirmation. `--paginate` follows pageToken/cursor chains into one merged list. |
| `cognia host doctor [--offline] [--format json\|human] [--query PATH]`                                                                                                                      | Diagnose Headless URL, TLS, data-directory, credential, readiness, and safe-RPC access.                                                                       |
| `cognia host events [--since SEQ] [--event TYPE] [--query PATH]`                                                                                                                            | Stream replayable Headless events as NDJSON; `--query` projects each frame (e.g. `.payload`).                                                                 |
...
| `cognia status [--json] [--query PATH] [--endpoint-file PATH]`                                                                                                                              | Probe every local control plane at once — the desktop CLI bridge and the Headless service. Exits zero when at least one plane responds.                       |
| `cognia open [PATH\|cognia://URL] [--session ID] [--settings[=TAB]] [--workflow-run WF/RUN] [--scheduler-task ID] [--im[=KEY]] [--pair PAYLOAD] [--share --text T --url U] [--print]`       | Launch the desktop at a specific surface via `cognia://` deep links (`cognia open .` opens a workspace like `code .`). `--print` emits the URL only.          |
| `cognia completions <bash\|zsh\|fish\|powershell\|elvish>`                                                                                                                                  | Print a shell completion script to stdout.                                                                                                                    |
| `cognia pack <sign\|verify>`                                                                                                                                                                | Character Pack authoring: sign/verify `.cognia-pack.json` files (no code; in-band signature).                                                                 |
| `cognia logs <tail\|query\|doctor\|export>`                                                                                                                                                 | Inspect, query, follow, diagnose, and export local structured logs.                                                                                           |
| `cognia crash <list\|show\|package\|submit\|status\|withdraw\|delete>`                                                                                                                      | Inspect, package, submit, track, and delete crash reports.                                                                                                    |
...
kubectl exec <pod> -- cognia host commands --search session
...
`--query <path>` (alias `--select`) projects any JSON output through a small dot-path
grammar: `.data.items[0].id` picks fields and indices, `.plugins[].id` (or `[*]`) maps the
rest of the path over every element, and a bare `.` returns the document. It is traversal
only — for filters, pipes, or construction, pipe the emitted JSON into `jq`. The same flag
works on `host` JSON outputs, `plugin list|status|doctor --json`, `cognia status --json`,
and per-frame on `host events` (a frame that fails projection aborts the stream).

`host call --paginate` follows a command's declared page-token convention
(`pageToken`→`nextPageToken` or `cursor`→`nextCursor`) until the list is exhausted,
concatenating the result's list field into one response (`meta.pages` records the fetch
count). Commands without a followable token pair reject the flag up front; repeated tokens,
non-object pages, and `--timeout-seconds` bound the loop.
