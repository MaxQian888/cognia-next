//! RFC 8785 (JSON Canonicalization Scheme) — the Rust twin of
//! `lib/plugin/character-pack/canonical-json.ts`.
//!
//! Consumers: `cognia-cli` (Character Pack signatures) and
//! `cognia-environment` (EnvironmentSpec digests, ADR-0182).
//!
...
pub enum CanonicalJsonError {
    /// NaN or an infinity: JSON has no spelling for either.
    #[error("non-finite number cannot be canonicalized")]
    NonFinite,
    /// A `serde_json::Number` with no `f64` view (arbitrary precision).
    #[error("number {0} cannot be represented as a double")]
    UnrepresentableNumber(String),
    /// Rust's float formatter produced something the placement tree cannot read.
    #[error("unexpected float formatting: {0}")]
    Formatting(String),
}
...
fn format_number(x: f64) -> Result<String, CanonicalJsonError> {
    if !x.is_finite() {
    ...
    let Some((mantissa, exponent)) = scientific.split_once('e') else {
        return Err(CanonicalJsonError::Formatting(scientific));
    };
    let digits: String = mantissa.chars().filter(|c| *c != '.').collect();
    let exponent: i32 = exponent
        .parse()
        .map_err(|_| CanonicalJsonError::Formatting(scientific.clone()))?;
