...
- **Axis labels:** one word per spoke (Jobs-minimal). system sans sans 11px weight 600. Place 16px outside the outer ring along the axis vector. Top/bottom = `text-anchor="middle"`; right side = `start`; left side = `end`.
- **Scale ticks** (e.g. `2 4 6 8 10`) only on the **first (top) axis** — putting numbers on every spoke clutters the chart fast. system mono 8px, `muted`, anchored end at `cx − 6`.
...
- **Mono-font axis labels.** Names go in system sans sans (the global rule). Mono is for technical sublabels only.
