...
- Each row contains (left→right):
  1. **Index tag** on the far left (`L3`, `07`, `APPLICATION`) — system mono 8–9px eyebrow.
  2. **Layer name** slightly right of center-left — system sans 14–16px 600.
  3. **Sublabel / note** on the far right — system mono 9–10px muted.
...
- Direction indicator on the LEFT margin (outside the stack): small up/down arrow + system mono label (`abstraction ↑`, `packets ↓`).
