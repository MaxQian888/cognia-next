...
> Cognia distribution note: the upstream icon gallery is intentionally not bundled. The attributions below are retained for the icon definitions included in `references/primitive-icons.md`.

The diagram-design skill itself is MIT-licensed (see `references/UPSTREAM_LICENSE.txt`). It bundles content from the following third-party sources, redistributed under their respective licenses.

## Tabler Icons

- **License:** MIT
- **Upstream:** https://github.com/tabler/tabler-icons
- **Used in:** stroked icons in `references/primitive-icons.md` (categories: Compute, People, Network, Data, Kubernetes, Action, DevOps, plus the stroked Brand outlines for Docker, Terraform, AWS, Azure, GitHub).
...
- **Used in:** filled brand silhouettes in `references/primitive-icons.md` (Kubernetes, Google Cloud, PostgreSQL, Nginx, Gitea, Keycloak, MinIO, Apache NiFi, Apache Airflow, Trino, Apache Superset, Jupyter, Python, R).
