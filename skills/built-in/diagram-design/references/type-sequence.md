...
Use a rectangular **frame** that spans only the lifelines participating in the branch. Operator label is system mono, uppercase, in a small tab at the top-left of the frame. Time still flows top→down inside the frame.
...
|---|---|---|---|
| `opt` | 1 | none | `[if condition]` under the tab (system mono 8px) |
...
<text x="X+12" y="GUARD_Y" fill="#4f5d75" font-size="8"
      font-family="ui-monospace, monospace" letter-spacing="0.04em">[token valid]</text>
...
- Fragment operator label in system sans sans — must be mono: `ALT` / `OPT` / `LOOP`.
