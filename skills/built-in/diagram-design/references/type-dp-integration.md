...
- **Labels:** every `primary`, `secondary`, `federated`, `auth` edge gets a protocol label (system mono 8 px, paper-filled rect mask with 6–10 px clear gap above the stroke). `trigger` edges are unlabelled.
...
If you need more icons, load `references/primitive-icons.md` and define matching inline `<symbol>` blocks.
