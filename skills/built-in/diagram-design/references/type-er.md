...
- Each entity is a two-section box:
  - **Header**: type tag (`ENTITY`) + entity name in system sans.
  - **Body**: field list in system mono, one per line. PK prefixed with `#`, FK prefixed with `→`.
- Relationships: lines between entities with cardinality at each end:
  - `1`, `N`, `0..1`, `1..*` in system mono, 8px, placed 10–12px from the entity edge.
