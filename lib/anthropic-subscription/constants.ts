...
//
// Sources (see docs/content/docs/adr/0010-claude-subscription-oauth.md):
...
// not an Anthropic-sanctioned third-party OAuth client; if the upstream
// flow changes, update this file + the ADR.
...
/**
 * Per-mode OAuth wiring. The `subscription` mode targets claude.ai (Pro/Max
 * accounts); the `console` mode targets console.anthropic.com (API-billing
 * accounts that still want OAuth instead of pasting an API key).
 */
...
export const OAUTH_ENDPOINTS: Record<OAuthMode, OAuthEndpointConfig> = {
  subscription: {
    authorizeUrl: "https://claude.ai/oauth/authorize",
    redirectUri: "https://platform.claude.com/oauth/code/callback",
    // Subscription-flow scopes captured from real Claude Code OAuth requests.
    scopes: "user:profile user:inference user:sessions:claude_code",
  },
  console: {
    authorizeUrl: "https://console.anthropic.com/oauth/authorize",
    redirectUri: "https://console.anthropic.com/oauth/code/callback",
    // Console flow needs `org:create_api_key` to mint API keys server-side.
    scopes: "org:create_api_key user:profile user:inference",
  },
}

/**
 * Headers a sidecar request must carry when authenticating with an OAuth
 * bearer (any mode). Without `oauth-2025-04-20` the server returns "OAuth
 * authentication is currently not supported"; without `claude-code-20250219`
 * + `x-app: cli` the Sonnet/Opus 4 series 429s OAuth callers (see
 * [hermes-agent#17169](https://github.com/NousResearch/hermes-agent/issues/17169)).
 *
 * `user-agent` is set on the SDK client config rather than here because
 * `customFetch` doesn't always have the privilege to override it on Node's
 * undici fetch.
 */
