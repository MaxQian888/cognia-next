...
const isCredentialFreshMock = jest.fn()
jest.mock("./credential-store", () => ({
  isCredentialFresh: (...args: unknown[]) => isCredentialFreshMock(...args),
}))
...
beforeEach(() => {
  jest.useFakeTimers()
  jest.resetAllMocks()
  isCredentialFreshMock.mockReturnValue(true)
})
...
describe("startUsageScheduler", () => {
  ...
  it("retries once on auth failure after refresh", async () => {
    ...
    const refreshed: SubscriptionCredential = { ...credential, accessToken: "oat-2" }
  ...
  it("stop() cancels future ticks", async () => {
    ...
    // Only the initial tick — no follow-ups after stop.
    expect(probeOnceMock).toHaveBeenCalledTimes(1)
  })
