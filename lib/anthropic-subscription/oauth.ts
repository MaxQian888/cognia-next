// PKCE OAuth flow for Claude (subscription / console). Three pure-ish
// entry points:
//
...
import type { OAuthMode, SubscriptionCredential } from "./types"

/**
 * Inputs the dialog needs to remember between "open authorize URL" and
 * "user pasted the code back". The dialog can keep this in component state
 * — there's no need for a global store.
 */
 ...
 */
export async function buildAuthorizeUrl(input: { mode: OAuthMode }): Promise<AuthorizeUrlResult> {
...
export function extractAuthorizationCode(input: string): { code: string; state?: string } | null {
  ...
  // Full URL form: parse with URL and pull `code` from the query.
  ...
  // The `code#state` shape some Anthropic pages display verbatim.
  if (trimmed.includes("#")) {
...
function buildCredential(
  ...
  // expires_in absent → assume a conservative 8h so the UI eventually retries.
...
/**
 * Swap the long-lived refresh token for a fresh access (and possibly fresh
 * refresh) token. The returned credential carries whichever refresh_token
 * the server sends back — fall through to the input value when the server
 * doesn't rotate.
 */
