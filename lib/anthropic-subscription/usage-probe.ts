...
//
// The user opts in (Settings → Subscription → Settings: probeEnabled). When
// enabled, this module sends a near-empty `/v1/messages` request and parses
// `anthropic-ratelimit-*` from the response headers, persisting one
...
import type { SubscriptionCredential, UsageSnapshot } from "./types"
...
export interface ProbeFailure {
  ok: false
  /** "auth" → 401/403; caller should refresh the token and retry once.
   *  "rate-limited" → 429; caller should back off.
   *  "no-headers" → 200 OK but missing unified-* headers (API-key fallback path).
   *  "transport" → fetch threw.
   *  "http" → other non-2xx. */
...
/**
 * Send one probe request. Returns either a parsed snapshot or a structured
 * failure — never throws. Persisting the snapshot is the caller's job
 * (`recordUsageSnapshot` is wired in `runOnceAndPersist` below).
 */
export async function probeOnce(
  ...
  if (response.status === 429) {
    // 429 still carries the unified-* headers — capture them as evidence.
...
/**
 * Convenience: probe + persist on success. Returns the persisted row (or
 * `null` when the snapshot was suppressed by the collector's debounce) on
 * success, or the structured failure.
 */
