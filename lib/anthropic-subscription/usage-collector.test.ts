...
beforeEach(async () => {
  jest.resetAllMocks()
  _resetPersistDebounce()
  // Wipe Dexie between tests by deleting the underlying database.
...
describe("recordUsageSnapshot", () => {
  ...
  it("trims oldest rows when over the cap", async () => {
    // Lower the cap by stuffing 1001 rows in via direct Dexie writes — the
    // collector's own writes would be debounced. We then call
    // recordUsageSnapshot once more so the trimming branch fires.
    const db = getDb()
    ...
    // Cap is 1000 — adding one took us to 1002, then we trimmed back to 1000.
    expect(await db.subscriptionUsage.count()).toBe(1000)
    // Earliest remaining row should NOT be the very first one we inserted.
...
describe("subscribeToUsageHeaders", () => {
  it("forwards usage_headers events through the parser into Dexie", async () => {
    ...
    // recordUsageSnapshot is async; flush the microtask queue.
