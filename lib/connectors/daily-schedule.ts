 ...
 *
 * Extracted so cross-adapter cleanup tasks share one timer mechanism instead
 * of each re-implementing the setTimeout → setInterval handoff. The first
 * run fires after `initialDelayMs` (so it doesn't compete with app boot);
 * subsequent runs fire every `intervalMs`. Errors thrown by the task are
 * caught and logged so a single failure never kills the schedule.
 *
 * The outbound terminal-row retention sweep is NOT scheduled here: it is a
 * managed scheduler task (`housekeeping-scheduler.ts`
 * `OUTBOUND_RETENTION_TASK_TYPE`), so it survives restarts and shows up in
 * the scheduler UI like every other housekeeping job. `startCallbackBinding-
 * CleanupSchedule` predates this and keeps its own equivalent loop; it is
