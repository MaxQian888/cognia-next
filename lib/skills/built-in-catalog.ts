...
import { loadBuiltInSkillBody } from "@/generated/built-in-skills/built-in-body-loader.generated"
...
/**
 * The SKILL.md body behind a catalog entry.
 *
 * Bodies are three quarters of the generated bytes and only a *delivered* skill
 * needs one, so they live in per-skill chunks rather than in the catalog every
 * importer pulls in. Accepts any alias `resolveBuiltinSkillIdentity` accepts,
 * and throws on an id that names no built-in: an empty body would inject
 * silence into a prompt, which is worse than the error.
 */
export async function loadBuiltInSkillContent(alias: string): Promise<string> {
  const identity = resolveBuiltinSkillIdentity(alias)
  if (!identity) throw new Error(`Unknown built-in skill: ${alias}`)
  return loadBuiltInSkillBody(identity.bundleId)
}
