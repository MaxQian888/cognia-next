 ...
 *
 * Overrides the bounded process port so the wrapper can be exercised without
 * an actual lark-cli binary on PATH. Auth bridge is bypassed via the
...
const mockProcessRunner = jest.fn()
...
beforeEach(() => {
  mockProcessRunner.mockReset()
  __setLarkCliProcessRunnerForTests(mockProcessRunner)
...
describe("execLarkCli — happy path", () => {
  it("parses JSON stdout and returns ok", async () => {
    mockProcessRunner.mockResolvedValue({
  ...
  it("returns raw string when stdout is not JSON", async () => {
    mockProcessRunner.mockResolvedValue({ stdout: "hello world\n", stderr: "" })
  ...
  it("returns null data for empty stdout", async () => {
    mockProcessRunner.mockResolvedValue({ stdout: "  \n  ", stderr: "" })
...
describe("execLarkCli — argv composition", () => {
  it("prepends --as <identity> when caller omitted it", async () => {
    mockProcessRunner.mockResolvedValue({ stdout: "{}", stderr: "" })
    await execLarkCli({
      args: ["calendar", "+list-events"],
      __authOverride: okAuth,
    })
    const argv = mockProcessRunner.mock.calls[0][1] as string[]
...
describe("execLarkCli — env injection", () => {
  it("merges auth env onto process.env", async () => {
    ...
    const opts = mockProcessRunner.mock.calls[0][2] as { env: Record<string, string> }
    expect(opts.env.LARK_APP_ID).toBe("cli_x")
    expect(opts.env.LARK_USER_ACCESS_TOKEN).toBe("uat-x")
    expect(opts.env.LARK_APP_SECRET).toBe("s-x")
  })
})
...
describe("execLarkCli — error paths", () => {
  ...
  it("binary_not_found when the process port reports a missing binary", async () => {
    mockProcessRunner.mockResolvedValue({ stdout: "", stderr: "no such file", notFound: true })
  ...
  it("timeout when killed/SIGTERM", async () => {
    mockProcessRunner.mockResolvedValue({ stdout: "", stderr: "", timedOut: true })
    const r = await execLarkCli({
      args: ["slow"],
      timeoutMs: 1000,
      __authOverride: okAuth,
...
describe("execLarkCli — timeout clamping", () => {
  it("clamps timeoutMs above 5min to 5min", async () => {
    ...
    const opts = mockProcessRunner.mock.calls[0][2] as { timeoutMs: number }
