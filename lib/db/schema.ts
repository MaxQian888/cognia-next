...
/** v161 makes legacy uncertainty and procedural review state explicit. */
export function backfillMemoryGovernanceV161(memory: Memory): Memory {
...
export function backfillMemoryJobV161(job: MemoryJob): MemoryJob {
...
export class CogniaDB extends Dexie {
  ...
  constructor(name = LEGACY_COGNIA_DB_NAME, connectionOwner = "unspecified") {
    ...
    // v160 — Shared Memory/RAG control plane. Domain rows remain authoritative;
    ...
    // v161 — Memory governance metadata and the shared eight-state job model.
    ...
    this.version(161)
      ...
      .upgrade(async (tx) => {
        await tx
          ...
          .toCollection()
          .modify((memory) => {
            backfillMemoryGovernanceV161(memory)
          })
        await tx
          .table<MemoryJob, string>("memoryJobs")
          .toCollection()
          .modify((job) => {
            backfillMemoryJobV161(job)
          })
      })
  ...
  // v160 — Shared Memory/RAG control plane.
