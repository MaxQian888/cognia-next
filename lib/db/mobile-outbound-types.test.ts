 ...
 *      appear in `KNOWN_COMMANDS` so the runner's drain doesn't 404 — and
 *      have a `match` arm in one of the per-domain dispatch submodules under
 *      `src-tauri/src/companion_api/rpc/` (`rpc.rs::dispatch` is only a
 *      thin gate that forwards to `<domain>::dispatch` when the name is in
 *      that domain's `COMMANDS` slice).
 *
 *   2. `tests/e2e/mobile/outbound-queue.spec.ts` — the COMMAND_KINDS
 *      list MUST match the typed enum so the e2e doesn't fall behind. The
 *      only tolerated gaps are the ones enumerated (with a reason) in
 *      `E2E_GENERIC_DRAIN_EXCLUSIONS` below.
 *
...
describe("MOBILE_OUTBOUND_COMMANDS spec parity", () => {
  ...
  it("every command has a dispatch arm in rpc.rs or a rpc/ dispatch submodule", async () => {
