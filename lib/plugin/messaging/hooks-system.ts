...
// =============================================================================
// Plugin hook failure telemetry — re-exported from the leaf module
// =============================================================================
...
export class PluginEventHooks {
  ...
  /**
   * `onChatRequest` — the `model.request.prepare` interceptor point (ADR-0189).
   *
   * The old body ran EVERY plugin against the same `messages` array and then
   * scanned the results backwards for the last successful one, so with two
   * plugins installed exactly one of them mattered and which one depended on
   * registration order. The interceptor chain is serial: each plugin sees its
   * predecessor's output, which is what a "pipeline" was always meant to be.
   */
  async dispatchChatRequest(messages: PluginMessage[], model: string): Promise<PluginMessage[]> {
    const { value } = await dispatchTransform<{ messages: PluginMessage[]; model: string }>(
      "model.request.prepare",
      { messages, model },
      { operationId: `chat-request:${model}` }
    )
  ...
  async dispatchBuildOptions(
    ...
    const { value } = await dispatchTransform<
      import("@/types/plugin/plugin-hooks").BuildOptionsHookInput
    >("agent.context.prepare", options, {
      operationId: `build-options:${options.sessionId}`,
      sessionId: options.sessionId,
      // A rewrite that redirects a turn to a different conversation is an
      // authorization change wearing a transform's clothes.
      invariantFields: ["sessionId"],
    })
    return value
  }
  ...
  async dispatchPostToolUse(
    ...
    const { value } = await dispatchTransform<ToolResultProjectValue>(
      "tool.result.project",
      { toolName, toolArgs, toolResult, sessionId, projection: {} },
      {
        operationId: `tool-result:${sessionId}:${toolName}`,
        sessionId,
        invariantFields: ["toolName", "sessionId"],
      }
    )
