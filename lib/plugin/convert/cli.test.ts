...
describe("runEcosystemConvertCli", () => {
  it("inspects without writes and requires review of contextual conversions", () => {
    const io = makeIo({
      "/plugins/gemini-extension.json": JSON.stringify({ name: "review", version: "1.0.0" }),
      "/plugins/commands/review.toml": 'prompt = "Review {{args}}"',
    })
    const argv = ["--from", "plugin", "--input", "/plugins"]
    const before = new Map(io.files)
    expect(runEcosystemConvertCli([...argv, "--dry-run"], io)).toMatchObject({
      mode: "inspect",
      report: { fidelity: "contextual" },
    })
    expect(io.files).toEqual(before)
    expect(() => runEcosystemConvertCli(argv, io)).toThrow(/accept-warnings/)
    expect(runEcosystemConvertCli([...argv, "--accept-warnings"], io)).toMatchObject({
      ok: true,
      pluginId: "review",
    })
  })

  it("returns machine-readable blockers for a read-only inspection", () => {
    const io = makeIo({
      "/plugins/plugin.json": JSON.stringify({
        id: "runtime",
        name: "Runtime",
        version: "1.0.0",
        type: "frontend",
    ...
    const argv = ["--operation", "export", "--input", "/plugins", "--to", "codex"]
    expect(runEcosystemConvertCli([...argv, "--dry-run"], io)).toMatchObject({
      ok: true,
      mode: "inspect",
      report: {
        ...
        delivery: {
          target: "codex",
          native: "blocked",
          hosted: { status: "requires-cognia", capabilities: ["tools"] },
          capabilities: expect.arrayContaining([{ capability: "tools", status: "hosted" }]),
        },
      },
    })
    const failure = runMain(argv, io)
    expect(JSON.parse(failure.output)).toMatchObject({
      ok: false,
      report: {
        blocking: expect.any(Array),
        delivery: { hosted: { status: "requires-cognia" } },
      },
    })
    expect(failure.exitCode).toBe(1)
  })

  it.each(["/plugins", "/plugins/output", "/"])(
    "rejects overlapping output %s before writes or a successful inspection",
    (output) => {
      const io = makeIo({
        "/plugins/plugin.json": JSON.stringify({
          id: "empty",
      ...
      expect(() => runEcosystemConvertCli(argv, io)).toThrow(/must not overlap/)
      expect(() => runEcosystemConvertCli([...argv, "--dry-run"], io)).toThrow(/must not overlap/)
      expect(io.files).toEqual(before)
    }
  )

  it("separates cloud verification and rejects unknown surfaces", () => {
    const io = makeIo({
      "/plugins/plugin.json": JSON.stringify({
        id: "empty",
        name: "Empty",
    ...
    const argv = ["--operation", "export", "--input", "/plugins", "--to", "codex"]
    expect(runEcosystemConvertCli([...argv, "--surface", "cloud", "--dry-run"], io)).toMatchObject({
      mode: "inspect",
      report: {
        blocking: [expect.objectContaining({ capability: "surface" })],
        delivery: { surface: "cloud", native: "blocked" },
      },
    })
    expect(() => runEcosystemConvertCli([...argv, "--surface", "phone"], io)).toThrow(/--surface/)
  })
  it.each([
  ...
  it("exports a Cognia bundle to a target ecosystem", () => {
    ...
    io.files.set(
      "/plugins/cognia/dist/index.js",
      renderDist(JSON.parse(io.files.get("/plugins/cognia/plugin.json")!))
    )
    const out = runEcosystemConvertCli(
      [
        "--operation",
