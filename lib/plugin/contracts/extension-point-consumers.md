...
| -------------------------------- | -------------- | --------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `sidebar.left.top`               | implemented    | `components/shell/guild-rail.tsx` (icon column) + `components/shell/sidebar-nav-section.tsx` (expanded sidebar) | Always-mounted. Icon form factor in both — the expanded sidebar draws an icon strip above its nav rows.                                                                                              |
| `sidebar.left.bottom`            | implemented    | `components/shell/guild-rail.tsx` (footer) + `components/shell/sidebar-footer.tsx` (expanded sidebar)           | Always-mounted. Icon form factor in both.                                                                                                                                                            |
...
| `chat.input.effort`              | implemented    | `components/chat/composer/bottom-toolbar.tsx` (effort chip)                                                     | Replacing slot: the host `EffortChip` is the fallback; limit 1.                                                                                                                                      |
...
| `agent.team.panel`               | implemented    | `components/agent/workspace/overview.tsx` (team overview tab)                                                   | Context: `{ teamId, status, teammateCount, taskCount, completedTaskCount }` (ids + aggregates).                                                                                                      |
