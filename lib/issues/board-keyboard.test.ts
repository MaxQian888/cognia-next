...
describe("nextBoardColumnCoordinates", () => {
  it("steps to the column next door, not to whichever corner is nearest", () => {
    const next = nextBoardColumnCoordinates("right", card(300), ALL, ISSUE_STATUSES)
    expect(next?.x).toBe(ALL.get("in_progress")!.left + 8)
  })

  it("steps left", () => {
    const next = nextBoardColumnCoordinates("left", card(850), ALL, ISSUE_STATUSES)
    expect(next?.x).toBe(ALL.get("in_progress")!.left + 8)
  })

  it("stops at the right edge rather than wrapping to the start", () => {
    expect(nextBoardColumnCoordinates("right", card(1400), ALL, ISSUE_STATUSES)).toBeNull()
  })

  it("stops at the left edge", () => {
    expect(nextBoardColumnCoordinates("left", card(20), ALL, ISSUE_STATUSES)).toBeNull()
  })

  it("skips a column that is not rendered", () => {
    const sparse = new Map(ALL)
    sparse.delete("in_progress")
    const next = nextBoardColumnCoordinates("right", card(300), sparse, ISSUE_STATUSES)
  ...
  it("carries the vertical position across, so crossing the board is not also a jump", () => {
    const next = nextBoardColumnCoordinates("right", card(300, 220), ALL, ISSUE_STATUSES)
    expect(next?.y).toBe(220)
  })

  it("never lands on top of the target column's header", () => {
    const next = nextBoardColumnCoordinates("right", card(300, 0), ALL, ISSUE_STATUSES)
  ...
  it("clamps a card held below a short column back inside it", () => {
    ...
    const next = nextBoardColumnCoordinates("right", card(300, 500), short, ISSUE_STATUSES)
