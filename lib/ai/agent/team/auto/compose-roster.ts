...
import type { CapabilityCatalog, ProposedTeammate } from "./types"
...
{"teammates":[{"name":"...","role":"lead|teammate","specialization":"...","description":"...","capabilities":{"skillIds":["..."],"subagentIds":["..."]}}]}`
...
export async function composeRoster(input: ComposeRosterInput): Promise<ProposedTeammate[]> {
  ...
  try {
    raw = await input.client.complete(
      renderUserPrompt(input.objective, input.assessment, input.catalog, maxRoster),
      {
  ...
  return normalizeRoster(parsed, input.catalog, maxRoster) ?? heuristicRoster()
}
