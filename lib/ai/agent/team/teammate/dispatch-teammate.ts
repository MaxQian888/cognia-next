...
import { agendaFingerprint, parseRateLimitCooldown } from "../gates/nudge-guard"
...
import { workspaceEvidenceRevision } from "../ledger/evidence-bundle"
...
async function runExternalBacked(
  ...
  const [
    ...
    import("@/lib/ai/agent/external/runtimes/acp/resolve-acp-mcp-servers"),
    import("@/lib/ai/agent/external/session/external-event-progress"),
  ])
...
export async function dispatchTeammate(
  ...
  let durableDispatch:
    | Awaited<ReturnType<typeof import("../durable/durable-dispatch").beginDurableDispatch>>
    | undefined
  ...
  try {
    ...
    durableDispatch = await (async () => {
      const [{ beginDurableDispatch }, { getDurableTeamCoordinator }] = await Promise.all([
        import("../durable/durable-dispatch"),
    ...
    const executeTurn = async (): Promise<{ text: string; usage?: TokenUsage }> => {
      ...
      if (
        ...
        if (recoverySessionId) {
          ...
          const { isDurableChildReplaySafe } = await import("../durable/durable-runtime")
      ...
      if (channel === "external" && externalAgentId) {
        ...
        const { parseGatewaySessionId } =
          await import("@/lib/ai/agent/external/config/gateway-task")
