...
import { isTerminalSquadRunStatus } from "../squad/squad-run-records"
...
async function defaultArm(input: {
  ...
  const { armSquadReview } = await import("../gates/squad-review-gate")
...
async function defaultAssessReplay(runId: string) {
  const { assessSquadRunReplay } = await import("../squad/squad-control")
...
export async function applyTeamRecoveryDecision(
  ...
  const { recordSquadRecoveryOutcome } = await import("../squad/squad-telemetry")
