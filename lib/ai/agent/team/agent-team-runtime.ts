...
import { openSquadReview, type SquadReviewOutcome } from "./gates/squad-review-gate"
import {
...
import { createTeammatePool } from "./teammate/teammate-pool"
import { resolveTeamTwinRuntime } from "./teammate/twin-context"
import { createTeamNotifier, type TeamNotifierDeps } from "./team-notifier"
import {
...
import { applyGateBehavior, resolveGatePolicy, type TeamRunOrigin } from "./gates/gate-policy"
...
import { createDeadlockHandler } from "./gates/deadlock-gate"
import { runTeamWaves } from "./team-wave-runner"
import { createLedgerCheckpoint } from "./ledger/progress-ledger-checkpoint"
import {
  RateLimitResumeController,
  createRealResumeDeps,
  NUDGE_CONTINUE_PROMPT,
} from "./durable/rate-limit-resume"
import { isUltracodeActive, type UltracodeOverride } from "./ultracode/ultracode-trigger"
...
export interface RunTeamLifecycleDeps {
  ...
  runLeadReview?: (params: {
    team: AgentTeam
    lead: AgentTeammate
    task: import("./squad/lead-review").LeadReviewTask
    workerName?: string
    workerOutput: string
    evidence: import("./ledger/review-evidence").ReviewEvidence
    revision: number
    previousFeedback?: string
    signal: AbortSignal
  }) => Promise<import("./squad/lead-review").LeadReviewVerdict>
...
export async function buildRunPrFeedback(opts: {
  ...
  notify: (n: import("./pr-feedback/runtime").PrFeedbackNotifyInput) => void
  addMessage: (m: import("./pr-feedback/runtime").PrFeedbackMailboxInput) => void
  onWarn: (message: string) => void
  loadRuntime?: () => Promise<Pick<typeof import("./pr-feedback/runtime"), "buildTeamPrFeedback">>
  loadGit?: () => Promise<Pick<typeof import("@/lib/git/commands"), "gitPush">>
}): Promise<import("./pr-feedback/runtime").TeamPrFeedback | undefined> {
  const loadRuntime = opts.loadRuntime ?? (() => import("./pr-feedback/runtime"))
...
export async function runTeamLifecycle(
  ...
  try {
    ...
    let workspaceController:
      import("./workspace/registry-controller").AgentTeamRegistryWorkspaceController | undefined
    if (configuredRoots.length > 0) {
      const { AgentTeamRegistryWorkspaceController } =
        await import("./workspace/registry-controller")
    ...
    let durableEnvironment:
      NonNullable<import("./team-run-context").TeamRunContext["durableEnvironment"]> | undefined
    {
      try {
        const { getDurableTeamCoordinator } = await import("./durable/durable-runtime")
    ...
    const { resolveTeamMemoryRuntime } = await import("./memory/memory-context")
    ...
    if (ultracodeActive) {
      ...
      await import("./patterns/index")
      const [{ planUltracodeWorkflow }, { synthesizeUltracodeWorkflow }] = await Promise.all([
        import("./ultracode/ultracode-planner"),
        import("./ultracode/synthesize-ultracode"),
      ])
