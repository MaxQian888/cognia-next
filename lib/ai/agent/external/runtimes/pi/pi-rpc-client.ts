...
interface PiProcess {
  ...
  lastUsedAt: number
  exited: boolean
  /** The host's exit code, once `external-agent://exit` arrived. */
...
export class PiRpcClientAdapter extends BaseProtocolAdapter {
  ...
  private async startSession(
    ...
    try {
      ...
      // The Rust process manager outlives this realm. A reload (watchdog, Fast
      // Refresh, a restarted renderer) forgets the Pi process it started while
      // the host keeps it registered, and every later resume of this
      // conversation was refused with "Agent <id> is already running" — a
      // lease held by a process with no owner left. The other adapters reclaim
      // that orphan already; Pi never did. It is safe for the same reason:
      // `resumeSession` retires any process THIS adapter still owns for the id
      // before it gets here, so whatever the host still has registered is dead
      // weight. A second collision after the reclaim is a live holder and
...
/**
 * The Pi process exited before the bundled extension reported ready — the
 * session never started, so no turn ran. Typed (and given the lifecycle reason
 * code) so the chat can mark the turn as failed-to-start instead of printing
 * this sentence in a transient toast.
 */
export class PiProcessExitedError extends Error {
  readonly reasonCode = "initialization_failed"
  readonly exitCode: number | null
  constructor(exitCode: number | null | undefined) {
    super(
      typeof exitCode === "number"
        ? `Pi process exited (code ${exitCode}) before the Cognia extension was ready`
        : "Pi process exited before the Cognia extension was ready"
    )
    this.name = "PiProcessExitedError"
