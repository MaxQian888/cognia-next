...
describe("PiRpcClientAdapter — teardown", () => {
  ...
  it("reports a startup process exit immediately instead of waiting for a handshake timeout", async () => {
    ...
    expect(failure).toBeInstanceOf(PiProcessExitedError)
    // Typed with the lifecycle reason code, and the exit code is kept.
    expect(failure).toMatchObject({ reasonCode: "initialization_failed", exitCode: 1 })
    expect((failure as Error).message).toMatch(/exited \(code 1\) before/)
    await adapter.disconnect()
  })

  it("reclaims a process id an earlier realm left registered, then starts normally", async () => {
    const host = createFakeHost()
    const adapter = await connected(host)
    const invoke = host.invoke.bind(host)
    let collisions = 1
    host.invoke = async <T>(name: string, args: Record<string, unknown>): Promise<T> => {
      if (name === "spawn_external_agent" && collisions > 0) {
        collisions -= 1
        const id = (args.config as { id: string }).id
        // The Tauri command rejects with a bare string, not an Error.
        throw `Agent ${id} is already running`
      }
      return invoke<T>(name, args)
    }
    const session = await adapter.createSession({ cwd: "/w" })
  ...
  it("reports a process id still held after the reclaim as a typed lease conflict", async () => {
    const host = createFakeHost()
    const adapter = await connected(host)
    const invoke = host.invoke.bind(host)
    host.invoke = async <T>(name: string, args: Record<string, unknown>): Promise<T> => {
      if (name === "spawn_external_agent" && (args.config as { id: string }).id.includes("sess-")) {
        throw new Error(`Agent ${(args.config as { id: string }).id} is already running`)
      }
      return invoke<T>(name, args)
    }
    const failure = await adapter.createSession({ cwd: "/w" }).catch((error: unknown) => error)
