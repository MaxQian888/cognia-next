...
describe("withTimeout", () => {
  ...
  it("preserves TimeoutError.label and TimeoutError.timeoutMs fields", async () => {
    const slow = new Promise((resolve) => setTimeout(resolve, 5000))
    const wrapped = withTimeout(slow, 500, "loader.unloadWasmPlugin")
    ...
    try {
      ...
      expect((err as TimeoutError).label).toBe("loader.unloadWasmPlugin")
