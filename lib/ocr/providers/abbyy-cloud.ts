/**
 * ABBYY Cloud OCR SDK provider.
 *
 * Two-stage workflow:
 *   1. POST <region>.ocrsdk.com/processImage?language=<...>&exportFormat=<...>
 *      Basic auth (application_id : password); body is the image bytes.
 *      Response (XML or JSON): { taskId, status: "Submitted" }.
 *   2. Poll GET <region>.ocrsdk.com/getTaskStatus?taskId=<id> until
 *      status === "Completed", then GET the resultUrl which serves the result
 *      text or XML.
 */
...
export async function abbyyCloudExtract(
  ...
  const submit = await cloudFetch({
    ...
    url: `${baseUrl}/processImage?language=${encodeURIComponent(
      language
  ...
  for (let i = 0; i < maxPolls; i++) {
    ...
    if (task.status === "ProcessingFailed" || task.status === "NotEnoughCredits") {
    ...
    const poll = await cloudFetch({
      providerId: "abbyy-cloud",
      method: "GET",
      url: `${baseUrl}/getTaskStatus?taskId=${encodeURIComponent(task.id)}`,
...
function parseAbbyyTask(body: string): AbbyyTask {
  ...
  if (trimmed.startsWith("{")) {
    try {
      const parsed = JSON.parse(trimmed) as Partial<AbbyyTask>
      if (parsed.id && parsed.status) return parsed as AbbyyTask
    } catch {
      /* fall through to XML */
    }
  }
