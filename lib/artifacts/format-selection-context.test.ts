...
describe("an excerpt selected inside a message", () => {
  const excerpt = (over = {}) => ({
    kind: "entity" as const,
    entityKind: "message" as const,
    entityId: "s1#m1",
    title: "run /stack restack",
    snapshot: "run /stack restack",
    comment: "",
    capturedAt: 1_000,
    href: "/?session=s1&message=m1",
    sourceSessionId: "s1",
    excerpt: { derivation: "quote" as const, quote: "run /stack restack" },
    ...over,
  })

  it("says the body is what the user selected, and where", () => {
    const out = formatContextSelectionsForLLM([excerpt()], { sessionId: "s1" })
    expect(out).toContain(
      "Text the user selected from a message earlier in this conversation — /?session=s1&message=m1:"
    )
  })

  it("says a generated body was generated, and never calls it the conversation's words", () => {
    const out = formatContextSelectionsForLLM(
      [
        excerpt({
          snapshot: "A short summary.",
          excerpt: { derivation: "summary", quote: "run /stack restack" },
        }),
      ],
      { sessionId: "other" }
    )
    expect(out).toContain(
      "A summary, generated by the app, of text the user selected from a message in another conversation"
    )
    expect(out).not.toContain("Text the user selected")
  })

  it("names the target language of a translation", () => {
    const out = formatContextSelectionsForLLM([
      excerpt({ excerpt: { derivation: "translation", quote: "hello", language: "fr" } }),
    ])
    expect(out).toContain("A translation, generated by the app, of text the user selected from")
    expect(out).toContain(", into fr — ")
  })

  // One selection that runs across turns is one excerpt, not a list of whole
  // messages — the combined-reference heading would say the wrong thing.
  it("counts the messages a selection runs across without calling them whole messages", () => {
    const out = formatContextSelectionsForLLM(
      [
        excerpt({
          members: [
    ...
    expect(out).toContain("Text the user selected from 2 messages earlier in this conversation")
    expect(out).not.toContain("in order")
  })
})
