...
export async function mirrorSettingsPatchToHost(
  ...
  await (deps.enqueueJob ?? enqueue)({
    ...
    // Deliberately untranslated: the queue row is written from a persistence
    // helper that has no React context, and the offline banner renders the
    // key list as data. The nine `/me/*` pages that used to build this label
    // through `useTranslations` produced the same comma-joined key list.
    label: keys.join(", "),
  })
  return keys
}
