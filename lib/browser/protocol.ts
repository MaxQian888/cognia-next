...
function formatReferenceFrame(sel: BrowserSelection): string[] {
  ...
  if (sel.parentLayout) {
    ...
    const details = [
      ...
      layout.gridTemplateColumns ? `grid-template-columns: \`${layout.gridTemplateColumns}\`` : null,
      layout.gap ? `gap: \`${layout.gap}\`` : null,
    ].filter(Boolean)
    lines.push(`- Parent: \`${layout.display}\`${details.length ? `, ${details.join(", ")}` : ""} (\`${layout.selector}\`)`)
  }
  return lines
}
...
export function formatSelectionsComment(
  ...
  selections.forEach((selection, index) => {
    const label = selection.kind === "area" ? "Area" : selection.kind === "text" ? "Text range" : "Element"
