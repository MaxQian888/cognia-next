...
;(function () {
  ...
  var refSeq = 0
  var generation = 0
  var automationCore = window.__cogniaAutomationCore || null
  ...
  function roleOf(el) {
    if (automationCore) return automationCore.implicitRole(el) || ""
  ...
  function accessibleName(el) {
    if (automationCore) return truncate(automationCore.accessibleName(el), MAX_TEXT)
  ...
  function directText(el) {
    if (automationCore) return automationCore.directText(el)
    var s = ""
  ...
  function nativeSetValue(el, value) {
    if (automationCore) return automationCore.setValue(el, value, false)
    var proto =
  ...
  function performAct(ref, action, args) {
    ...
    switch (action) {
      ...
      case "double_click": {
        var di = modifierInit(args.modifiers)
        var doubleClickInit = {
          bubbles: true,
          cancelable: true,
          ctrlKey: di.ctrlKey,
          shiftKey: di.shiftKey,
          altKey: di.altKey,
          metaKey: di.metaKey,
        }
        el.dispatchEvent(new MouseEvent("click", doubleClickInit))
        el.dispatchEvent(new MouseEvent("click", doubleClickInit))
        el.dispatchEvent(new MouseEvent("dblclick", doubleClickInit))
        break
      }
      case "hover":
   ...
   *
   * `intent` is "push" | "replace" | "traverse" — the renderer models the
   * back/forward stack itself (neither webview exposes `canGoBack`), so it has
   * to know whether the page grew its history, overwrote the current entry, or
   * merely moved inside a stack that already existed.
   */
  function signalNav(url, intent) {
    var payload = { url: String(url), intent: intent || "push" }
  ...
  var pendingNavIntent = null

  /**
   * Precedence when several history operations land inside one debounce
   * window: a push definitively grew the stack, so it wins; a traverse moved
   * inside an existing stack, so it beats a replace; a replace only overwrites.
   * Reporting the weakest of a mixed batch would desync the renderer's index.
   */
  var NAV_INTENT_RANK = { push: 3, traverse: 2, replace: 1 }

  /** Debounced URL-change report (scroll-driven replaceState can be chatty). */
  function scheduleNavReport(intent) {
    var next = NAV_INTENT_RANK[intent] ? intent : "push"
    if (pendingNavIntent === null || NAV_INTENT_RANK[next] > NAV_INTENT_RANK[pendingNavIntent]) {
      pendingNavIntent = next
    }
    if (navReportTimer) return
    navReportTimer = setTimeout(function () {
      navReportTimer = null
      var reported = pendingNavIntent || "push"
      pendingNavIntent = null
      var now = String(window.location.href)
      if (now === lastReportedUrl) return
      if (!absoluteHttpUrl(now)) return
      lastReportedUrl = now
      signalNav(now, reported)
    }, 150)
  }
  ...
  function installHistoryHook() {
    ...
    if (h && typeof h.replaceState === "function") {
      ...
      h.replaceState = function () {
        ...
        scheduleNavReport("replace")
        return r
      }
    }
    // `popstate` is the page moving inside a stack it already has (its own
    // back/forward, or ours via `browser_embed_back`). Reporting it as a push
    // would grow the renderer's stack on every Back.
    window.addEventListener("popstate", function () {
      scheduleNavReport("traverse")
    })
    // A hash change appends a new history entry, same as pushState.
      ...
      ;(function (level) {
        ...
        console[level] = function () {
          try {
            ...
            pushRing(consoleBuf, consoleEntry)
            schedulePush("console", consoleEntry)
          } catch {
  ...
  function recordNet(url, method, status, ok, durationMs) {
    ...
    pushRing(networkBuf, entry)
    schedulePush("network", entry)
  }
  ...
  // --- Push channels (ADR-0127) -------------------------------------------
  // The drain commands above stay the agent's pull path (they empty the ring
  // buffers on demand). These push a *copy* of new entries to Rust over the
  // same cancelled-navigation sentinel channel as selection / nav / loaded, so
  // the DevTools drawer can show console + network live and the agent engine
  // can invalidate its snapshot cache when the DOM changes. Pushing never
  // touches the ring buffers, so pull-after-push still returns everything.
  //
  // Sentinel navigations are one-at-a-time: a second `location.href` assignment
  // in the same task would replace the first before Rust's `on_navigation`
  // saw it, so pushes go through a small queue drained one per macrotask.
  var CONSOLE_SENTINEL = "https://cognia.invalid/__cognia_console?data="
  var NETWORK_SENTINEL = "https://cognia.invalid/__cognia_network?data="
  var SNAPSHOT_SENTINEL = "https://cognia.invalid/__cognia_snapshot?data="
  var PUSH_DEBOUNCE_MS = 250
  var SNAPSHOT_DEBOUNCE_MS = 500
  var PUSH_MAX_ENTRIES = 20
  var PUSH_MAX_CHARS = 12000
  var pushEnabled = { console: true, network: true, snapshot: true }
  var pushPending = { console: [], network: [] }
  var pushTimers = { console: null, network: null, snapshot: null }
  var sentinelQueue = []
  var sentinelDraining = false
  var snapshotSeq = 0
  var snapshotMutations = 0
  // Only the newest evaluation of this script owns the push channels. The
  // init script runs once per document in production; the jsdom harness
  // re-evaluates it against one window, and without this every earlier layer's
  // console wrapper would push the same line again.
  var pushOwner = {}
  window.__cogniaPushOwner = pushOwner

  function sendSentinel(url) {
    if (window.__cogniaPushOwner !== pushOwner) return
    // Tests override `window.__cogniaSignalPush` to capture the payload.
    if (typeof window.__cogniaSignalPush === "function") {
      window.__cogniaSignalPush(url)
      return
    }
    window.location.href = url
  }

  function drainSentinelQueue() {
    if (!sentinelQueue.length) {
      sentinelDraining = false
      return
    }
    sentinelDraining = true
    var next = sentinelQueue.shift()
    try {
      sendSentinel(next)
    } catch {
      // never let push instrumentation break the page
    }
    setTimeout(drainSentinelQueue, 30)
  }

  function enqueueSentinel(url) {
    sentinelQueue.push(url)
    if (!sentinelDraining) drainSentinelQueue()
  }

  /** Cap a batch by entry count and serialized size so the sentinel URL stays small. */
  function capBatch(entries) {
    var out = entries.slice(-PUSH_MAX_ENTRIES)
    while (out.length > 1 && JSON.stringify(out).length > PUSH_MAX_CHARS) out.shift()
    return out
  }

  function flushPush(kind) {
    pushTimers[kind] = null
    var pending = pushPending[kind]
    if (!pending.length) return
    var batch = capBatch(pending)
    pushPending[kind] = []
    var prefix = kind === "console" ? CONSOLE_SENTINEL : NETWORK_SENTINEL
    enqueueSentinel(prefix + encodeURIComponent(JSON.stringify({ entries: batch })))
  }

  function schedulePush(kind, entry) {
    if (!pushEnabled[kind]) return
    pushPending[kind].push(entry)
    if (pushTimers[kind] != null) return
    pushTimers[kind] = setTimeout(function () {
      flushPush(kind)
    }, PUSH_DEBOUNCE_MS)
  }

  /** Nodes the overlay itself paints must never count as a page change. */
  function isOverlayNode(node) {
    var el = node && node.nodeType === 1 ? node : node && node.parentElement
    while (el) {
      if (el.id && String(el.id).indexOf("__cognia") === 0) return true
      el = el.parentElement
    }
    return false
  }

  function flushSnapshotDirty() {
    pushTimers.snapshot = null
    if (!snapshotMutations) return
    snapshotSeq++
    var payload = {
      url: String(window.location.href),
      seq: snapshotSeq,
    ...
    snapshotMutations = 0
    enqueueSentinel(SNAPSHOT_SENTINEL + encodeURIComponent(JSON.stringify(payload)))
  }

  function onDomMutations(records) {
    if (!pushEnabled.snapshot) return
    var counted = 0
    ...
    if (!counted) return
    snapshotMutations += counted
    if (pushTimers.snapshot != null) return
    pushTimers.snapshot = setTimeout(flushSnapshotDirty, SNAPSHOT_DEBOUNCE_MS)
  }

  var mutationWatch = null
  function installMutationWatch() {
    if (mutationWatch || typeof MutationObserver !== "function") return
    try {
      mutationWatch = new MutationObserver(onDomMutations)
      mutationWatch.observe(document.documentElement || document, {
        childList: true,
        subtree: true,
        attributes: true,
        characterData: true,
  ...
  /** Rust / tests flip individual push channels. */
  function setPushEnabled(json) {
    try {
      var next = typeof json === "string" ? JSON.parse(json) : json
      if (next && typeof next === "object") {
        for (var k in pushEnabled) {
          if (typeof next[k] === "boolean") pushEnabled[k] = next[k]
        }
      }
    } catch {
      // ignore malformed toggles
    }
    return JSON.stringify(pushEnabled)
  }
  ...
  function showClickPointer(e) {
    ...
    var previous = document.getElementById("__cognia-click-pointer")
    if (previous && previous.parentNode) previous.parentNode.removeChild(previous)
    var pointer = document.createElement("div")
    pointer.id = "__cognia-click-pointer"
    pointer.setAttribute("data-cognia-chrome", "1")
    ...
    style.boxShadow = "0 0 0 4px rgba(239,68,68,.12)"
    style.pointerEvents = "none"
    style.zIndex = "2147483647"
    style.transition = "transform 180ms ease-out, opacity 420ms ease-out"
    ...
    ;(
      ...
      pointer.style.transform = "scale(1.7)"
  ...
  function onRecordDoubleClick(e) {
    ...
    var el = targetAt(e)
    if (!el) return
    var target = recordTarget(el)
    var removed = 0
    while (recordBuf.length && removed < 2) {
      var previous = recordBuf[recordBuf.length - 1]
      if (
        previous.act !== "click" ||
        !previous.target ||
        previous.target.selector !== target.selector
      )
        break
      recordBuf.pop()
      removed += 1
    }
    var step = { act: "double_click", target: target }
    var mods = modifiersOf(e)
    if (mods.length) step.modifiers = mods
    pushStep(step)
  }
  ...
  function onRecordHover(e) {
    ...
    if (
      ...
      return
    pushStep({ act: "hover", target: target })
  }

  function onRecordWheel(e) {
    if (!recording || active) return
    var horizontal = Math.abs(Number(e.deltaX) || 0)
    var vertical = Math.abs(Number(e.deltaY) || 0)
    if (!horizontal && !vertical) return
    var isHorizontal = horizontal > vertical
    var raw = isHorizontal ? Number(e.deltaX) : Number(e.deltaY)
    pushStep({
      act: "scroll",
      direction: isHorizontal ? (raw < 0 ? "left" : "right") : raw < 0 ? "up" : "down",
      amount: Math.max(1, Math.round(Math.abs(raw))),
    })
  }
  ...
  installNetworkHook()
  installXhrHook()
  // ADR-0127: DOM-change watcher behind the `browser://snapshot` push. Per
  // document like the load hook (a real navigation resets `window`).
  ...
  // -----------------------------------------------------------------------
  // Find-in-page. Highlights matches with the CSS Custom Highlight API when
  // available (zero DOM mutation), falling back to <mark> wrapping on older
  // engines / jsdom. Shared by the embedded webview (browser_embed_evaluate)
  // and the remote Chromium runtime (page.evaluate) — dependency-free, returns
  // a plain `{matches, index}` object (index is 0-based).
  // -----------------------------------------------------------------------
  var FIND_HL = "cognia-find"
  var FIND_HL_CURRENT = "cognia-find-current"
  var findQuery = ""
  var findMatchCase = false
  var findRanges = []
  var findMarks = []
  var findIndex = -1
  var findStyleEl = null

  function findSupportsHighlight() {
    return (
      typeof CSS !== "undefined" && CSS && CSS.highlights && typeof window.Highlight === "function"
    )
  }

  function ensureFindStyle() {
    if (findStyleEl && findStyleEl.parentNode) return
    findStyleEl = document.createElement("style")
    findStyleEl.setAttribute("data-cognia-chrome", "find")
    findStyleEl.textContent =
      "::highlight(" +
      FIND_HL +
      "){background:#fde68a;color:#111}::highlight(" +
      FIND_HL_CURRENT +
      "){background:#f59e0b;color:#111}mark[data-cognia-find]{background:#fde68a;color:#111}" +
      "mark[data-cognia-find-current]{background:#f59e0b;color:#111}"
    ;(document.head || document.documentElement).appendChild(findStyleEl)
  }
  ...
  function collectFindTextNodes() {
    ...
    while ((node = walker.nextNode())) {
      if (isFindableTextNode(node)) nodes.push(node)
    }
    return nodes
  }

  function findOccurrences(haystack, needle, matchCase) {
    ...
    var from = 0
    var at
    while ((at = hay.indexOf(ndl, from)) >= 0) {
      out.push(at)
      from = at + ndl.length
    }
    return out
  ...
  function buildFindRanges(query, matchCase) {
    var ranges = []
    var textNodes = collectFindTextNodes()
    for (var i = 0; i < textNodes.length; i++) {
      var node = textNodes[i]
      var positions = findOccurrences(node.nodeValue, query, matchCase)
      for (var p = 0; p < positions.length; p++) {
        try {
          var range = document.createRange()
          range.setStart(node, positions[p])
          range.setEnd(node, positions[p] + query.length)
          ranges.push(range)
        } catch {}
      }
    }
  ...
  function buildFindMarks(query, matchCase) {
    var marks = []
    var textNodes = collectFindTextNodes()
    for (var i = 0; i < textNodes.length; i++) {
      var node = textNodes[i]
      var text = node.nodeValue
      var positions = findOccurrences(text, query, matchCase)
      ...
      for (var p = 0; p < positions.length; p++) {
        ...
        mark.setAttribute("data-cognia-find", "1")
  ...
  function unwrapFindMarks() {
    var marks = document.querySelectorAll("mark[data-cognia-find]")
  ...
  function scrollFindIntoView(target) {
    try {
      var el =
        target.nodeType === 1
          ? target
  ...
  function findCount() {
    return findSupportsHighlight() ? findRanges.length : findMarks.length
  }

  function applyFindHighlight() {
    ensureFindStyle()
    if (findSupportsHighlight()) {
      ...
      if (findIndex >= 0 && findRanges[findIndex]) {
        var current = new window.Highlight()
        current.add(findRanges[findIndex])
        CSS.highlights.set(FIND_HL_CURRENT, current)
        scrollFindIntoView(findRanges[findIndex])
      }
    } else {
      for (var m = 0; m < findMarks.length; m++) {
        findMarks[m].removeAttribute("data-cognia-find-current")
      }
      if (findIndex >= 0 && findMarks[findIndex]) {
        findMarks[findIndex].setAttribute("data-cognia-find-current", "1")
        scrollFindIntoView(findMarks[findIndex])
      }
    }
  }
  ...
  function clearFindHighlights() {
    try {
      if (findSupportsHighlight() && CSS.highlights) {
  ...
  function find(query, options) {
    try {
      ...
      var forward = options.forward !== false
      if (!query) {
        findClear()
        return { matches: 0, index: 0 }
      }
      if (query === findQuery && matchCase === findMatchCase && findCount() > 0) {
        var n = findCount()
        findIndex = (findIndex + (forward ? 1 : -1) + n) % n
        applyFindHighlight()
        return { matches: n, index: findIndex }
      }
      clearFindHighlights()
      findQuery = query
      findMatchCase = matchCase
      if (findSupportsHighlight()) {
        findRanges = buildFindRanges(query, matchCase)
        findMarks = []
      } else {
        findMarks = buildFindMarks(query, matchCase)
        findRanges = []
      }
      var total = findCount()
      findIndex = total ? 0 : -1
      applyFindHighlight()
      return { matches: total, index: total ? findIndex : 0 }
    } catch (error) {
      return { matches: 0, index: 0, error: String(error) }
    }
  }
  ...
  // ADR-0127: Rust flips push channels (`{"console":bool,"network":bool,"snapshot":bool}`).
  window.__cogniaSetPushEnabled = setPushEnabled
  ...
  // Find-in-page (returns {matches, index}); shared by embedded + remote paths.
  window.__cogniaFind = function (query, options) {
    return find(query, options)
  }
  window.__cogniaFindClear = function () {
    return findClear()
  }
  ...
  window.__cogniaOverlay = {
    ...
    // ADR-0127 push channels (exposed for unit tests).
    flushPush: flushPush,
    flushSnapshotDirty: flushSnapshotDirty,
    onDomMutations: onDomMutations,
    setPushEnabled: setPushEnabled,
    pushSentinels: {
      console: CONSOLE_SENTINEL,
