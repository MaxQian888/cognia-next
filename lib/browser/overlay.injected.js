...
;(function () {
  ...
  var originalRequestAnimationFrame = window.requestAnimationFrame
    ...
    : function (callback) { return originalSetTimeout(callback, 16) }
  ...
  function freezePage() {
    ...
    try {
      if (document.getAnimations) {
        ...
        for (var i = 0; i < animations.length; i++) {
          ...
          if (animation.playState === "running" && !isChromeNode(animation.effect && animation.effect.target)) {
            animation.pause()
    ...
    for (var v = 0; v < videos.length; v++) {
      if (!isChromeNode(videos[v]) && !videos[v].paused && !videos[v].ended) {
        try { videos[v].pause(); pausedVideos.push(videos[v]) } catch {}
      }
    }
    if (freezeWatchdog) window.clearTimeout(freezeWatchdog)
    freezeWatchdog = originalSetTimeout(function () { unfreezePage() }, 3000)
  }
  ...
  function settleFrozenFrame() {
    return new Promise(function (resolve) {
      function fail(error) { resolve(frozenEnvelope(false, String(error))) }
      try {
        originalRequestAnimationFrame(function () {
          try {
            originalRequestAnimationFrame(function () {
              try {
                originalSetTimeout(function () { resolve(frozenEnvelope(true, null)) }, 16)
  ...
  function unfreezePage() {
    ...
    for (var i = 0; i < pausedAnimations.length; i++) {
      try { pausedAnimations[i].play() } catch {}
    }
    pausedAnimations = []
    for (var v = 0; v < pausedVideos.length; v++) {
      try { pausedVideos[v].play() } catch {}
    }
    pausedVideos = []
  ...
  function readComputedStyles(el) {
    ...
    var keys = ["display", "position", "width", "height", "margin", "padding", "gap", "color", "backgroundColor", "fontSize", "fontWeight"]
    var result = {}
  ...
  function restoreSelectionBuffer() {
    try {
      ...
      if (!raw || raw.length > MAX_SELECTION_STORAGE_BYTES) throw new Error("invalid selection buffer")
      var parsed = JSON.parse(raw)
      if (!parsed || parsed.version !== SELECTION_STORAGE_VERSION ||
          typeof parsed.savedAt !== "number" || Date.now() - parsed.savedAt > SELECTION_BUFFER_TTL_MS ||
          !Array.isArray(parsed.selections) ||
          !parsed.selections.every(isSelectionPayloadShape)) throw new Error("invalid selection buffer")
      selectionBuffer = parsed.selections.slice(-MAX_BUFFERED_SELECTIONS)
      selectionGeneration = typeof parsed.generation === "number" && isFinite(parsed.generation)
      ...
      try { sessionStorage.removeItem(SELECTION_STORAGE_KEY) } catch {}
    }
  }
  ...
  function isSelectionPayloadShape(value) {
    ...
    var rect = value.rect
    return typeof value.selector === "string" && typeof value.domPath === "string" &&
      typeof value.tagName === "string" && (value.id === null || typeof value.id === "string") &&
      (value.classes === null || typeof value.classes === "string") && rect &&
      typeof rect.x === "number" && isFinite(rect.x) && typeof rect.y === "number" && isFinite(rect.y) &&
      typeof rect.width === "number" && isFinite(rect.width) &&
      typeof rect.height === "number" && isFinite(rect.height) &&
      typeof value.outerHTML === "string" && typeof value.text === "string" &&
      typeof value.pageUrl === "string" && typeof value.pageTitle === "string"
  }

  function bufferSelection(payload) {
    selectionBuffer.push(payload)
    selectionGeneration++
    persistSelectionBuffer()
  ...
  function drainSelection() {
    try {
      ...
      if (result.length > MAX_SELECTION_DRAIN_BYTES) {
        return JSON.stringify({ ok: false, error: "selection drain exceeds byte limit", selections: [] })
      }
      selectionBuffer = []
      persistSelectionBuffer()
      return result
  ...
  function isInteractiveMarqueeTarget(el) {
    ...
    return !!(
      ...
      tag === "textarea" || tag === "summary" || el.getAttribute("role") ||
  ...
  function buildTextSelection() {
    ...
    payload.rect = roundRect(range.getBoundingClientRect ? range.getBoundingClientRect() : el.getBoundingClientRect())
  ...
  function buildPanelDom(payloads) {
    ...
    for (var i = 0; i < payloads.length; i++) {
      ...
      row.style.display = "flex"; row.style.alignItems = "center"; row.style.gap = "6px"; row.style.whiteSpace = "nowrap"
      var tag = document.createElement("span")
      tag.textContent = payload.kind === "area" ? "area" : payload.tagName
      tag.style.fontFamily = "ui-monospace, monospace"; tag.style.fontSize = "10px"
      tag.style.padding = "1px 5px"; tag.style.borderRadius = "4px"; tag.style.background = "rgba(99,102,241,0.25)"
      row.appendChild(tag)
      var ident = document.createElement("span")
      ident.style.fontWeight = "600"
      ident.textContent = payload.componentName ? "<" + payload.componentName + ">" : payload.domPath || payload.selector || (payload.kind === "area" ? "selected region" : payload.tagName)
      row.appendChild(ident)
      var size = document.createElement("span")
      size.style.color = "#9ca3af"; size.textContent = payload.rect.width + "×" + payload.rect.height
      row.appendChild(size)
      var remove = document.createElement("button")
      remove.type = "button"; remove.setAttribute("data-cognia-remove", String(i)); remove.textContent = "×"
      remove.style.marginLeft = "auto"; remove.style.cursor = "pointer"; remove.style.border = "0"
      remove.style.color = "#c7d2fe"; remove.style.background = "transparent"
      ;(function (selectionIndex) { remove.addEventListener("click", function (event) {
        event.preventDefault(); event.stopPropagation(); removeSelectionAt(selectionIndex)
      }, true) })(i)
  ...
  function positionSelection() {
    ...
    for (var b = 0; b < oldBoxes.length; b++) if (oldBoxes[b].parentNode) oldBoxes[b].parentNode.removeChild(oldBoxes[b])
    for (var i = 0; i < selectedPayloads.length; i++) {
      var item = selectedPayloads[i]
      var itemRect = selectedEls[i] && document.contains(selectedEls[i]) ? selectedEls[i].getBoundingClientRect() : {
        left: item.rect.x, top: item.rect.y, right: item.rect.x + item.rect.width,
        bottom: item.rect.y + item.rect.height, width: item.rect.width, height: item.rect.height,
      }
      var itemBox = createSelectBox(i)
      itemBox.style.display = "block"; itemBox.style.left = itemRect.left + "px"; itemBox.style.top = itemRect.top + "px"
      itemBox.style.width = itemRect.width + "px"; itemBox.style.height = itemRect.height + "px"
    }

    var panel = document.getElementById(PANEL_ID)
    if (!panel) return
    var first = selectedPayloads[0]
    var r = selectedEls[0] && document.contains(selectedEls[0]) ? selectedEls[0].getBoundingClientRect() : {
      left: first.rect.x, top: first.rect.y, right: first.rect.x + first.rect.width, bottom: first.rect.y + first.rect.height,
    }
  ...
  function onSelectionReflow() {
    ...
    var run = function () {
      ...
      if (!selectedPayloads.length) { clearSelection(); return }
      positionSelection()
    }
  ...
  function clearSelection() {
    ...
    for (var i = 0; i < boxes.length; i++) if (boxes[i].parentNode) boxes[i].parentNode.removeChild(boxes[i])
  ...
  function isCredentialish(el) {
    ...
    for (var i = 0; i < tokens.length; i++) {
      if (tokens[i] === "current-password" || tokens[i] === "new-password") return true
    }
    return false
  }
  ...
  window.__cogniaSelectionForRef = function (ref) {
    try {
      ...
      if (!el || !document.contains(el)) {
        return JSON.stringify({ ok: false, error: "Unknown or stale ref: " + String(ref), selection: null })
      }
