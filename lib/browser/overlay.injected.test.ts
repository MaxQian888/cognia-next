...
type SelectionPayload = {
  ...
  parentLayout?: { display: string; flexDirection?: string; gridTemplateColumns?: string; gap?: string; selector: string }
}
...
type OverlayApi = {
  ...
  intersects: (rect: DOMRect, area: { x: number; y: number; width: number; height: number }) => boolean
...
describe("overlay.injected buildPayload", () => {
  ...
  it("captures flex parent and main content layout", () => {
    ...
    expect(payload.parentLayout).toMatchObject({ display: "flex", flexDirection: "column", gap: "24px" })
    expect(payload.contentArea).toEqual({ selector: "body > main", left: 220, right: 1420, width: 1200, centerX: 820 })
  ...
  it("captures grid-specific parent fields only", () => {
    ...
    expect(payload.parentLayout).toMatchObject({ display: "grid", gridTemplateColumns: "1fr 2fr", gap: "8px" })
...
it("accumulates shift-clicked elements and removes only the requested panel item", () => {
    ...
    ;(window as unknown as { __cogniaSetSelectMode: (on: boolean) => void }).__cogniaSetSelectMode(true)
    document.getElementById("a")!.dispatchEvent(new MouseEvent("click", { bubbles: true, shiftKey: true }))
    document.getElementById("b")!.dispatchEvent(new MouseEvent("click", { bubbles: true, shiftKey: true }))
    expect(api.selectedElements()).toEqual([document.getElementById("a"), document.getElementById("b")])
...
it("uses strict rectangle intersection math for marquee candidates", () => {
  ...
  it("creates an element-less area payload when a drag intersects nothing", () => {
    ...
    ;(window as unknown as { __cogniaSignal: (value: unknown) => void }).__cogniaSignal = (value) => signals.push(value)
    ;(window as unknown as { __cogniaSetSelectMode: (on: boolean) => void }).__cogniaSetSelectMode(true)
    document.body.dispatchEvent(new MouseEvent("mousedown", { bubbles: true, button: 0, clientX: 10, clientY: 20 }))
    document.body.dispatchEvent(new MouseEvent("mousemove", { bubbles: true, clientX: 110, clientY: 70 }))
    document.body.dispatchEvent(new MouseEvent("mouseup", { bubbles: true, clientX: 110, clientY: 70 }))
  ...
  it("resolves a current snapshot ref and rejects an unknown ref", () => {
    ...
    const selectionForRef = (window as unknown as { __cogniaSelectionForRef: (value: string) => string }).__cogniaSelectionForRef
    expect(JSON.parse(selectionForRef(ref))).toMatchObject({ ok: true, selection: { selector: "#go" } })
...
describe("overlay.injected freeze", () => {
  ...
  it("pauses and restores only running WAAPI animations", () => {
    const running = { playState: "running", effect: { target: document.body }, pause: jest.fn(), play: jest.fn() }
    const finished = { playState: "finished", effect: { target: document.body }, pause: jest.fn(), play: jest.fn() }
    Object.defineProperty(document, "getAnimations", { configurable: true, value: () => [running, finished] })
    const api = install()
    api.freeze()
  ...
  it("preserves string timeout and interval callbacks", () => {
    ...
    const interval = window.setInterval("window.__timerStringRuns += 1" as unknown as TimerHandler, 10)
  ...
  it("acknowledges freeze only after two original frames and a settle timer", async () => {
    ...
    try {
      install()
      const pending = (window as unknown as { __cogniaSetFrozen: (on: boolean) => Promise<string> })
      ...
      ;(window as unknown as { __cogniaSetFrozen: (on: boolean) => string }).__cogniaSetFrozen(false)
    } finally {
...
describe("overlay.injected select mode", () => {
  it("signals then drains a buffered payload and auto-disables", () => {
    ...
    ;(window as unknown as { __cogniaSignal: (p: { count: number; generation: number }) => void }).__cogniaSignal = (p) => received.push(p)
