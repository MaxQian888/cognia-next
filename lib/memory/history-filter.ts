...
export type MemorySortKey = "recent" | "importance" | "accessed"

export interface MemoryFilter {
  /** Case-insensitive substring over `text` + `tags`. */
  query?: string
...
export function filterAndSortMemories(memories: Memory[], filter: MemoryFilter = {}): Memory[] {
  ...
  const allow = filter.types && filter.types.length > 0 ? new Set(filter.types) : null
  const sort = filter.sort ?? "recent"

  const filtered = memories.filter((m) => {
    if (status === "active" && m.status !== "active") return false
    if (allow && !allow.has(m.type)) return false
...
export function computeMemoryStats(memories: Memory[]): MemoryStats {
  ...
  return { total: memories.length, active, byType }
}
