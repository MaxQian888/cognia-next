 ...
 * live in `?run=` / `?kind=` / `?status=` / `?origin=` query params (read via
...
function isOrigin(value: string | null): value is ExecutionRunOrigin {
  return value === "local" || value === "gateway-api"
}
  ...
  return (
    <div className="flex h-full min-h-0 w-full flex-1 flex-col" data-bg-target="chat">
      <AgentRunsPanel
        ...
        origin={isOrigin(originParam) ? originParam : "all"}
        onOrigin={(next) => setParam("origin", next === "all" ? null : next)}
      />
    </div>
  )
}
